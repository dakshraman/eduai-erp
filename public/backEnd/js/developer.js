(function($) {
    ...
    calculateSalary = () => {
        ...
        if (basicSalary == 0) {
            ...
            var leaveDeduction = parseFloat($("#leave_deduction").val()) || 0;
            ...
            for (var i = 0; i < earningsValue.length; i++) {
                ...
                total_earnings += parseFloat(inpvalue);
            }
            ...
            $("#total_earnings").val(total_earnings.toFixed(decimal_digits));
            $("#total_deduction").val(total_deduction.toFixed(decimal_digits));
            $("#gross_salary").val(gross_salary.toFixed(decimal_digits));
            $("#final_gross_salary").val(gross_salary.toFixed(decimal_digits));
            $("#net_salary").val(net_salary.toFixed(decimal_digits));
    ...
    $(document).on('submit','#moduleForm',function(event){
   ...
   // add new row for class routine update by abuNayem
    ...
    getTotalByPrice = (row = null) => {
        if (row) {
            ...
            if (product_id > 0) {
                ...
                total = total.toFixed(decimal_digits);
    ...
    subAmount = () => {
        ...
        totalSubAmount = totalSubAmount.toFixed(decimal_digits);
        // sub total
        ...
        if ($('input[name="full_paid"]').is(":checked")) {
            ...
            $("#totalDue").val(dueAmount.toFixed(decimal_digits));
            $("#totalDueValue").val(dueAmount.toFixed(decimal_digits));
        }
    };
    ...
    paidAmount = () => {
        ...
        if (subTotal) {
            ...
            dueAmount = dueAmount.toFixed(decimal_digits);
            $("#totalDue").val(dueAmount.toFixed(decimal_digits));
            $("#totalDueValue").val(dueAmount.toFixed(decimal_digits));
        }
    ...
    $('input[name="full_paid"]').on("click", function() {
        if ($(this).is(":checked")) {
            ...
            $("#totalPaid").val(subTotal.toFixed(decimal_digits));
            $("#totalDue").val(0);
            $("#totalDueValue").val(0);
        } else {
            $("#totalPaid").val(0);
    ...
    addRowMark = () => {
        ...
        newRow +=
            "<button class='primary-btn icon-only fix-gr-bg' style='position: relative; top: 6px;' type='button' id='removeMark' >";
        newRow += "<span class='ti-trash'></span>";
        newRow += "</button>";
...
function emailCheck(value) {
    const input = value instanceof HTMLElement ? value : $(value)[0];
    input.setCustomValidity('');
    const val = input.value.trim();
    input.value = val;
    const valid = !val || input.checkValidity();
    const form = $(input).closest('form');

    input.setCustomValidity(valid ? '' : input.validationMessage);
