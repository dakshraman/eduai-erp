(function($) {
    ...
    $(document).ready(function() {
        if ($("input#total-attendance").length > 0) {
            ...
            $("#total_leave").html(attendanceArray[5]);
        }
    });
    ...
    $(document).ready(function() {
        $("#SelectDormitory").on("change", function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    var selectedRoomId = $("#selectRoomNumber").data("selected-room") || $("#selectRoomNumber").val();
                    ...
                    $.each(data, function(i, item) {
                        if (item.length) {
                            ...
                            $.each(item, function(i, room) {
                                var isSelected = String(selectedRoomId) === String(room.id);
                            ...
                            if ($.fn.niceSelect) {
                                $("#selectRoomNumber").niceSelect("update");
                            }
                        } else {
                            $("#roomNumberDiv .current").html($("#selectRoomNumber option:first").text());
                            $("#selectRoomNumber").find("option").not(":first").remove();
                            $("#roomNumberDiv ul").find("li").not(":first").remove();
                            if ($.fn.niceSelect) {
                                $("#selectRoomNumber").niceSelect("update");
        ...
        if ($("#SelectDormitory").val() && $("#selectRoomNumber").data("selected-room") && $("#selectRoomNumber option").length <= 1) {
            $("#SelectDormitory").trigger("change");
        }
    });
...
// Promote Student
                ...
                success: function(data) {
                    ...
                    $.each(data, function(i, item) {
                        if (item.length) {
                            ...
                            $("#select_section").val(firstSectionId);
                        } else {
    ...
    $(document).ready(function() {
        $("#select_section").on("change", function() {
            ...
            var branch_id = $("#branch_id").val() || $("#branchChange").val() || "";
            var formData = {
                section: $(this).val(),
                class: select_class,
                branch_id: branch_id,
            ...
            // Clear both the <select> and niceSelect <ul> immediately
            $("#select_subject").find("option").not(":first").remove();
            $("#select_subject_div ul").find("li").not(":first").remove();
            $("#select_subject_div .current").text(window.jsLang('select_subject') + ' *');
            ...
            $.ajax({
                ...
                success: function(data) {
                    // Clear again inside success to guard against concurrent requests
                    $("#select_subject").find("option").not(":first").remove();
                    $("#select_subject_div ul").find("li").not(":first").remove();
                    ...
                    $.each(data, function(i, item) {
                        if (item.length) {
                            $.each(item, function(i, subject) {
                                var type = subject.subject_type == "T" ? "Theory" : "Practical";
                                var label = subject.subject_name + " (" + type + ")";
                                $("#select_subject").append(
                                    $("<option>", {
                                        value: subject.id,
                                        text: label,
                                ...
                                $("#select_subject_div ul").append(
                                    "<li data-value='" + subject.id + "' class='option'>" + label + "</li>"
                                );
                            });
                        }
                    });
