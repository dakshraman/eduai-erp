...
class SmStudentAdmissionRequest extends FormRequest
{
    ...
    public function rules(Request $request): array
    {
        ...
        $rules = [
            ...
            }), 'nullable', 'date', 'before_or_equal:today', 'after:1900-01-01'],
            ...
            'admission_date' => [Rule::requiredIf(function () use ($field): bool {
            ...
            'document_file_1' => [
                Rule::requiredIf(function () use ($field): bool {
                    return in_array('document_file_1', $field) && is_show('document_file_1');
                }),
                'nullable', 'max:'.$maxFileSize, ],
            'document_file_2' => [
                Rule::requiredIf(function () use ($field): bool {
                    return in_array('document_file_2', $field) && is_show('document_file_2');
                }), 'nullable', 'max:'.$maxFileSize, ],
            'document_file_3' => [
                Rule::requiredIf(function () use ($field): bool {
                    return in_array('document_file_3', $field) && is_show('document_file_3');
                }), 'nullable', 'max:'.$maxFileSize,
            ],
            'document_file_4' => [
                Rule::requiredIf(function () use ($field): bool {
                    return in_array('document_file_4', $field) && is_show('document_file_4');
                }), 'nullable', 'max:'.$maxFileSize, ],
        ];
        ...
        if ($user_role_id !== 2 || $user_role_id !== GlobalVariable::isAlumni()) {
            $rules += [
                ...
                }), 'nullable', 'regex:/^(?!-)[+0-9]{6,15}$/', Rule::unique('users', 'phone_number')->where(function ($query) use ($student) {
        ...
        if (moduleStatusCheck('Branch')) {
            if ($request->has('branch_id')) {
                $rules['branch_id'] = [Rule::requiredIf(function () use ($field): bool {
                    return in_array('branch_id', $field);
                })];
            }
        }
