...
class staffRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $rules =
            [
                'staff_no' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('staff_no', $field);
                }), 'integer', Rule::unique('sm_staffs', 'staff_no')->where('school_id', $school_id)->ignore($id)],
                'role_id' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('role_id', $field);
                })],
                'department_id' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('department_id', $field);
                })],
                'designation_id' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('designation_id', $field);
                })],
                'first_name' => [Rule::requiredIf(function () use ($field): bool {
                ...
                'email' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('email', $field);
                }), 'nullable', 'email', Rule::unique('users', 'email')->ignore($staff ? $staff->user_id : null)],
                'gender_id' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('gender_id', $field);
                })],
                'date_of_birth' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('date_of_birth', $field);
                }), 'nullable', 'date'],
                'date_of_joining' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('date_of_joining', $field);
                }), 'date'],
                'mobile' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('mobile', $field);
                }), 'nullable', 'numeric', 'min:11', Rule::unique('users', 'phone_number')->ignore($staff ? $staff->user_id : null)],
                'marital_status' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('marital_status', $field);
                })],
                'emergency_mobile' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('emergency_mobile', $field);
                })],
                'driving_license' => [Rule::requiredIf(function () use ($field): bool {
                ...
                'qualification' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('qualification', $field);
                }), 'max:255'],
                'experience' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('experience', $field);
                }), 'max:255'],
                'epf_no' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('epf_no', $field);
                }), 'max:255'],
                'basic_salary' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('basic_salary', $field);
                }), 'max:100'],
                'contract_type' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('contract_type', $field);
                })],
                'location' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('location', $field);
                }), 'max:255'],
                'bank_account_name' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('bank_account_name', $field);
                }), 'max:255'],
                'bank_account_no' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('bank_account_no', $field);
                }), 'max:255'],
                'bank_brach' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('bank_brach', $field);
                }), 'max:255'],
                'facebook_url' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('facebook_url', $field);
                })],
                'twiteer_url' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('twiteer_url', $field);
                })],
                'linkedin_url' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('linkedin_url', $field);
                })],
                'instragram_url' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('instragram_url', $field);
                })],
                'resume' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('resume', $field);
                }), 'sometimes', 'nullable', 'mimes:pdf,doc,docx', 'max:'.$maxFileSize],
                'joining_letter' => [Rule::requiredIf(function () use ($field): bool {
                    return in_array('joining_letter', $field);
                }), 'sometimes', 'nullable', 'mimes:pdf,doc,docx', 'max:'.$maxFileSize],
                'other_document' => [Rule::requiredIf(function () use ($field): bool {
        ...
        return $rules + $this->generateValidateRules('staff_registration');
    }
