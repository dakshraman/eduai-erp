...
class SmFeesController extends Controller
{
    public function childrenFees($id)
    {
        ...
        */
        ...
        return view('backEnd.parentPanel.childrenFees', ['student' => $student, 'records' => $records, 'is_paystack' => $is_paystack, 'is_stripe' => $is_stripe, 'razorpay_info' => $razorpay_info, 'is_RazorPay' => $is_RazorPay, 'fees_assigneds' => $fees_assigneds, 'fees_discounts' => $fees_discounts, 'applied_discount' => $applied_discount, 'stripe_info' => $stripe_info, 'data' => $data, 'payment_gateway' => $payment_gateway]);
        /*
    ...
    public function childBankSlipStore(Request $request)
    {
        ...
        */
        ...
        $smBankPaymentSlip->academic_id = getAcademicId();
    ...
    public function feesGenerateModalChildView($id)
    {
        ...
        return view('backEnd.feesCollection.view_bank_payment', ['fees_payment' => $fees_payment]);
    }
    ...
    public function feesGenerateModalChildEdit(Request $request, $amount, $student_id, $type, $id)
    {
        ...
        */
        ...
        foreach ($discounts as $discount) {
            $fees_payment = SmFeesPayment::where('active_status', 1)->select('fees_discount_id')->where('fees_discount_id', $discount->id)->where('school_id', Auth::user()->school_id)->first();
        ...
        return view('backEnd.feesCollection.fees_generate_modal_child', ['amount' => $amount, 'discounts' => $discounts, 'fees_type_id' => $fees_type_id, 'student_id' => $student_id, 'applied_discount' => $applied_discount, 'fees_payment' => $fees_payment]);
