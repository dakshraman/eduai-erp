...
use App\Models\SmBankPaymentSlip;
use App\Models\SmBaseGroup;
use App\Models\SmBaseSetup;
use App\Models\SmBook;
use App\Models\SmBookCategory;
use App\Models\SmBookIssue;
use App\Models\SmChartOfAccount;
use App\Models\SmClass;
use App\Models\SmClassOptionalSubject;
use App\Models\SmClassRoom;
use App\Models\SmClassSection;
use App\Models\SmClassTime;
use App\Models\SmComplaint;
use App\Models\SmContentType;
use App\Models\SmCountry;
use App\Models\SmCurrency;
...
use App\Models\SmRolePermission;
...
use App\Models\SmStaffAttendence;
...
class SmApiController extends Controller
{
    ...
    public function AttendanceSync(Request $request)
    {
        ...
        try {
            ...
            $attendance_setting = (InfixBioSetting::where("school_id", app("school")->id ?? 1)->first() ?: new InfixBioSetting());
            ...
            foreach ($lastest_data as $device_log) {
                ...
                if ($user_table) {
                    if ($user_table->role_id === 2) {
                        ...
                        if ($sm_students) {
                            DB::table('device_log')->where('userid', $device_log->userid)->update(['role_id' => $sm_students->role_id, 'class_id' => $sm_students->class_id, 'section_id' => $sm_students->section_id, 'profile_id' => $sm_students->id]);
                        }
                        ...
                        if ($sm_staff) {
                            DB::table('device_log')->where('userid', $device_log->userid)->update(['role_id' => $sm_staff->role_id, 'profile_id' => $sm_staff->id]);
                        }
            ...
            return DB::table('device_log')->select('id', 'userid', 'terminalid', 'name', 'checktime', 'cloud_upload', 'area_id', 'device_ip')->where('cloud_upload', 1)->get();
        } catch (\Exception $exception) {
    ...
    public function checkColumnAvailable(Request $request): ?array
    {
        if (! Schema::hasColumn('sm_general_settings', 'api_url')) {
            Schema::table('sm_general_settings', function ($table): void {
    ...
    public function UpdateStaffApi(Request $request)
    {
        ...
        if (! empty($request->field_name)) {
            ...
            if ($request_string === 'email') {
                ...
                $user->save();
            } elseif ($request_string === 'first_name') {
    ...
    public function UpdateStudentApi(Request $request)
    {
        ...
        if (! empty($request->field_name)) {
            ...
            if ($request_string === 'first_name') {
                ...
                $full_name = $data->first_name.' '.$request->$request_string;
                $data->full_name = $full_name;
            } elseif ($request_string === 'student_photo') {
                $file = $request->file('student_photo');
                $student_photo = fileUpload($file, 'public/uploads/student/');
                $data->student_photo = $student_photo;
            } elseif (in_array($request_string, ['document_file_1', 'document_file_2', 'document_file_3', 'document_file_4'])) {
                $file = $request->file($request_string);
                $file_path = fileUpload($file, 'public/uploads/student/document/');
                $data->$request_string = $file_path;
            }
    ...
    public function storeRoom(Request $request)
    {
        ...
        $smRoomList->name = $request->room_number;
        $smRoomList->dormitory_id = $request->dormitory;
        $smRoomList->room_type_id = $request->room_type;
    ...
    public function saas_storeRoom(Request $request)
    {
        ...
        $smRoomList->cost_per_bed = $request->cost_per_bed;
        $smRoomList->description = $request->description;
        $smRoomList->school_id = $request->school_id;
        $smRoomList->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
    ...
    {
        $key_id = 'room_id';
        tableList::getTableList($key_id, $id);
        try {
            $delete_query = SmRoomList::destroy($id);
            if (ApiBaseMethod::checkUrl($request->fullUrl()) && $delete_query) {
    ...
    public function addDormitory(Request $request)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl()) && $result) {
    ...
    public function saas_addDormitory(Request $request)
    {
        ...
        $smDormitoryList->intake = $request->intake;
        $smDormitoryList->description = $request->description;
        $smDormitoryList->school_id = $request->school_id;
        $smDormitoryList->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
    ...
    {
        $tables = tableList::getTableList('dormitory_id', $id);
        try {
            $dormitory_list = SmDormitoryList::destroy($id);
            if ($dormitory_list && ApiBaseMethod::checkUrl($request->fullUrl())) {
                if ($dormitory_list) {
    ...
    public function addBook(Request $request)
    {
        ...
        $smBook->subject_id = $request->subject;
        ...
        $smBook->created_by = $request->user_id;
    ...
    public function member_role(Request $request)
    {
        ...
        $roles = InfixRole::where('active_status', '=', 1)->where(function ($q): void {
    ...
    public function feesMasterStore(Request $request)
    {
        ...
        if ($combination === 0) {
            ...
            $smFeesMaster->fees_group_id = $request->fees_group;
            $smFeesMaster->fees_type_id = $request->fees_type;
            $smFeesMaster->date = date('Y-m-d', strtotime($request->date));
            $smFeesMaster->amount = $request->fees_group !== 1 && $request->fees_group !== 2 ? $request->amount : null;
            $smFeesMaster->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
    ...
    public function feesMasterUpdate(Request $request)
    {
        ...
        if ($combination === 0) {
            ...
            $fees_master->amount = $request->fees_group !== 1 && $request->fees_group !== 2 ? $request->amount : null;
    ...
    public function NewExamSchedule(Request $request): void
    {
    ...
    public function saas_NewExamSchedule(Request $request, $school_id): void
    {
    ...
    public function convertCurrency(Request $request): string
    {
        ...
        $query = sprintf('%s_%s', $from_Currency, $to_Currency);
    ...
    public function studentFeesPayment(Request $request)
    {
        ...
        $smFeesPayment->student_id = $student_info->id;
        $smFeesPayment->fees_type_id = $request->fees_type_id;

        $smFeesPayment->discount_amount = empty($request->discount_amount) ? 0 : $request->discount_amount;
        $smFeesPayment->fine = empty($request->fine) ? 0 : $request->fine;
        $smFeesPayment->amount = (float) ($request->amount);
        $smFeesPayment->payment_date = date('Y-m-d', strtotime(date('Y-m-d')));
        $smFeesPayment->payment_mode = $request->payment_mode;
        $smFeesPayment->created_by = $request->paid_by;
    ...
    public function DownloadContent(string $file_name)
    {
    ...
    public function DownloadComplaint(string $file_name)
    {
    ...
    public function DownloadVisitor(string $file_name)
    {
    ...
    public function DownloadPostal(string $file_name)
    {
    ...
    public function DownloadDispatch(string $file_name)
    {
        $file = public_path().'/uploads/postal/'.$file_name;
        if (file_exists($file)) {
            return Response::download($file);
    ...
    {
        return SmClass::select('class_name as name')->where('id', $id)->first();
    }

    public function saas_get_class_name(Request $request, $school_id, $id)
    {

        return SmClass::select('class_name as name')->where('id', $id)->where('school_id', $school_id)->first();
    }

    public function get_section_name(Request $request, $id)
    {
        return SmSection::select('section_name as name')->where('id', $id)->first();
    }

    public function saas_get_section_name(Request $request, $school_id, $id)
    {
        return SmSection::select('section_name as name')
            ->where('id', $id)
    ...
    {
        return SmStaff::select('full_name as name')->where('id', $id)->first();
    }

    public function saas_get_teacher_name(Request $request, $school_id, $id)
    {
        return SmStaff::select('full_name as name')->where('id', $id)->where('school_id', $school_id)->first();
    }

    public function get_subject_name(Request $request, $id)
    {
        return SmSubject::select('subject_name as name')->where('id', $id)->first();
    }

    public function saas_get_subject_name(Request $request, $school_id, $id)
    {
        return SmSubject::select('subject_name as name')->where('id', $id)->where('school_id', $school_id)->first();
    }

    public function get_room_name(Request $request, $id)
    {
        return SmClassRoom::select('room_no as name')->where('id', $id)->first();
    }

    public function saas_get_room_name(Request $request, $school_id, $id)
    {
        return SmClassRoom::select('room_no as name')->where('id', $id)->where('school_id', $school_id)->first();
    }

    public function get_class_period_name(Request $request, $id)
    {
        return SmClassTime::select('period as name', 'start_time', 'end_time')->where('id', $id)->first();
    }

    public function saas_get_class_period_name(Request $request, $school_id, $id)
    {
        return SmClassTime::select('period as name', 'start_time', 'end_time')->where('id', $id)->where('school_id', $school_id)->first();
    }
    ...
    public function visitor_index(Request $request)
    {
        try {
            ...
            return view('backEnd.admin.visitor', ['visitors' => $visitors]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function saas_visitor_store(Request $request)
    {
        ...
        try {
            ...
            $smVisitor->out_time = $request->out_time;
            $smVisitor->file = $fileName;
            $smVisitor->school_id = $request->school_id;
            $result = $smVisitor->save();
    ...
    public function saas_complaint_store(Request $request)
    {
        ...
        try {
            ...
            $smComplaint->assigned = $request->assigned;
            $smComplaint->file = $fileName;
            $smComplaint->school_id = $request->school_id;
            $result = $smComplaint->save();
    ...
    public function postal_receive_index(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.admin.postal_receive', ['postal_receives' => $postal_receives]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function saas_postal_receive_store(Request $request)
    {
        ...
        try {
            ...
            $smPostalReceive->to_title = $request->to_title;
            $smPostalReceive->file = $fileName;
            $smPostalReceive->school_id = $request->school_id;
            $result = $smPostalReceive->save();
    ...
    public function postal_dispatch_index(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.admin.postal_dispatch', ['postal_dispatchs' => $postal_dispatchs]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function postal_dispatch_store(Request $request)
    {
        ...
        try {
            ...
            $smPostalDispatch->from_title = $request->from_title;
            $smPostalDispatch->reference_no = $request->reference_no;
            $smPostalDispatch->address = $request->address;
            $smPostalDispatch->date = date('Y-m-d', strtotime($request->date));
            $smPostalDispatch->note = $request->note;
            $smPostalDispatch->to_title = $request->to_title;
            $smPostalDispatch->file = $fileName;
            $result = $smPostalDispatch->save();
    ...
    public function saas_postal_dispatch_store(Request $request)
    {
        ...
        try {
            ...
            $smPostalDispatch->to_title = $request->to_title;
            $smPostalDispatch->file = $fileName;
            $smPostalDispatch->school_id = $request->school_id;
            $result = $smPostalDispatch->save();
    ...
    public function studentDetails(Request $request)
    {
        try {
            ...
            return view('backEnd.studentInformation.student_details', ['students' => $students, 'classes' => $classes, 'academic_years' => $academic_years]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function studentDetailsSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.studentInformation.student_details', ['students' => $students, 'classes' => $classes, 'class_id' => $class_id, 'name' => $name, 'roll_no' => $roll_no]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function saas_student_search_Details(Request $request, $school_id)
    {
        ...
        try {
            ...
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                return ApiBaseMethod::sendError('Validation Error.', $exception);
            }
        }
    ...
    public function studentView(Request $request, $id)
    {
        try {
            ...
            return view('backEnd.studentInformation.student_view', ['student_detail' => $student_detail, 'driver_info' => $driver_info, 'fees_assigneds' => $fees_assigneds, 'fees_discounts' => $fees_discounts, 'exams' => $exams, 'documents' => $documents, 'timelines' => $timelines, 'siblings' => $siblings, 'grades' => $grades, 'academic_year' => $academic_year]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function studentDelete(Request $request)
    {
        ...
        try {
            ...
            if ($tables === null || $tables === '' || $tables === '0') {
                ...
                return view('backEnd.studentInformation.student_details', ['student_detail' => $student_detail]);
            }
    ...
    public function studentEdit(Request $request, $id)
    {
        try {
            ...
            return view('backEnd.studentInformation.student_edit', ['student' => $student, 'classes' => $classes, 'sections' => $sections, 'religions' => $religions, 'blood_groups' => $blood_groups, 'genders' => $genders, 'route_lists' => $route_lists, 'vehicles' => $vehicles, 'dormitory_lists' => $dormitory_lists, 'categories' => $categories, 'sessions' => $sessions, 'siblings' => $siblings, 'driver_lists' => $driver_lists]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function student_type_store(Request $request)
    {
        ...
        try {
            ...
            $smStudentCategory->category_name = $request->category;
            $result = $smStudentCategory->save();
    ...
    public function saas_student_type_store(Request $request)
    {
        ...
        try {
            $smStudentCategory = new SmStudentCategory();
            $smStudentCategory->category_name = $request->category;
            $smStudentCategory->school_id = $request->school_id;
            $result = $smStudentCategory->save();
    ...
    public function student_group_store(Request $request)
    {
        ...
        try {
            ...
            $smStudentGroup->group = $request->group;
    ...
    public function saas_student_group_store(Request $request)
    {
        ...
        try {
            $smStudentGroup = new SmStudentGroup();
            $smStudentGroup->group = $request->group;
            $smStudentGroup->school_id = $request->school_id;
            $result = $smStudentGroup->save();
            ...
            try {
                ...
                foreach ($request->id as $student_id) {
                    ...
                    if ($request->result[$student_id] === 'P') {
                        $merit_list = SmTemporaryMeritlist::where(['student_id' => $student_id, 'class_id' => $request->current_class, 'section_id' => $student_details->section_id])->where('academic_id', SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->first();
    ...
    public function uploadContentList(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.teacher.uploadContentList', ['contentTypes' => $contentTypes, 'classes' => $classes, 'uploadContents' => $uploadContents]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function saveUploadContent(Request $request)
    {
        ...
        if (property_exists($request, 'available_for') && $request->available_for !== null) {
            ...
            $request->validate(
                [
                    ...
                    'upload_date' => 'required',
                    'content_file' => 'required|mimes:pdf,doc,docx,jpg,jpeg,png,mp3,mp4',
                ],
                [
        ...
        try {
            ...
            foreach ($request->available_for as $value) {
                if ($value === 'admin') {
                    $roles = InfixRole::where('id', '=', 1) /* ->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 9) */ ->where('academic_id', SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->where(function ($q): void {
                        $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
                    })->get();
    ...
    public function assignmentList(Request $request)
    {
        ...
        try {
            ...
            if (Auth()->user()->role_id === 1) {
                ...
                SmNotification::clearUnreadCache($user->id, 1);
            }
            ...
            return view('backEnd.teacher.assignmentList', ['uploadContents' => $uploadContents]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function studyMetarialList(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.teacher.studyMetarialList', ['uploadContents' => $uploadContents]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function syllabusList(Request $request)
    {
        try {
            ...
            return view('backEnd.teacher.syllabusList', ['uploadContents' => $uploadContents]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function otherDownloadList(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.teacher.otherDownloadList', ['uploadContents' => $uploadContents]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function collectFeesStudentApi(Request $request, $id)
    {
        try {
            ...
            foreach ($fees_assigneds2 as $fee_assigneds2) {
                ...
                $d[$i]['discount_amount'] = $fee_assigneds2->applied_discount;
            ...
            foreach ($fees_discounts as $fee_discount) {
                $fees_payment = SmFeesPayment::select('fees_discount_id')->where('fees_discount_id', $fee_discount->id)->first();
    ...
    public function searchFeesDue(Request $request)
    {
        try {
            ...
            return view('backEnd.feesCollection.search_fees_due', ['classes' => $classes, 'fees_masters' => $fees_masters]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function feesAssign(Request $request, $id)
    {
        ...
        try {
            ...
            return view('backEnd.feesCollection.fees_assign', ['classes' => $classes, 'categories' => $categories, 'genders' => $genders, 'fees_group_id' => $fees_group_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
        ...
        try {
            ...
            foreach ($students as $student) {
                foreach ($fees_masters as $fee_master) {
                    $assigned_student = SmFeesAssign::select('student_id')->where('student_id', $student->id)->where('fees_master_id', $fee_master->id)->first();
            ...
            return view('backEnd.feesCollection.fees_assign', ['classes' => $classes, 'categories' => $categories, 'genders' => $genders, 'students' => $students, 'fees_assign_groups' => $fees_assign_groups, 'fees_group_id' => $fees_group_id, 'pre_assigned' => $pre_assigned, 'class_id' => $class_id, 'category_id' => $category_id, 'gender_id' => $gender_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function saas_feesAssignSearch(Request $request, $school_id)
    {
        ...
        try {
            ...
            foreach ($students as $student) {
                foreach ($fees_masters as $fee_master) {
                    $assigned_student = SmFeesAssign::select('student_id')->where('student_id', $student->id)->where('fees_master_id', $fee_master->id)->where('school_id', $school_id)->first();
    ...
    public function fees_type_store(Request $request)
    {
        ...
        try {
            ...
            $smFeesType->name = $request->name;
            $smFeesType->fees_group_id = $request->fees_group;
            $smFeesType->description = $request->description;
    ...
    public function saas_fees_type_store(Request $request)
    {
        ...
        try {
            ...
            $smFeesType->fees_group_id = $request->fees_group;
            $smFeesType->description = $request->description;
            $smFeesType->school_id = $request->school_id;
            $result = $smFeesType->save();
    ...
    public function fees_discount_store(Request $request)
    {
        ...
        try {
            ...
            $smFeesDiscount->name = $request->name;
            $smFeesDiscount->code = $request->code;
            $smFeesDiscount->type = $request->type;
            $smFeesDiscount->amount = $request->amount;
            $smFeesDiscount->description = $request->description;
    ...
    public function saas_fees_discount_store(Request $request)
    {
        ...
        try {
            ...
            $smFeesDiscount->amount = $request->amount;
            $smFeesDiscount->description = $request->description;
            $smFeesDiscount->school_id = $request->school_id;
            $result = $smFeesDiscount->save();
    ...
    public function feesDiscountAssign(Request $request, $id)
    {
        ...
        try {
            ...
            return view('backEnd.feesCollection.fees_discount_assign', ['classes' => $classes, 'categories' => $categories, 'genders' => $genders, 'fees_discount_id' => $fees_discount_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function feesDiscountAssignSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.feesCollection.fees_discount_assign', ['classes' => $classes, 'categories' => $categories, 'genders' => $genders, 'students' => $students, 'fees_discount' => $fees_discount, 'fees_discount_id' => $fees_discount_id, 'pre_assigned' => $pre_assigned, 'class_id' => $class_id, 'category_id' => $category_id, 'gender_id' => $gender_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function feesGenerateModal(Request $request, $school_id, $amount, $student_id, $type)
    {
        try {
            ...
            foreach ($discounts as $discount) {
                $fees_payment = SmFeesPayment::select('fees_discount_id')->where('fees_discount_id', $discount->id)->where('school_id', $school_id)->first();
                if (isset($fees_payment->fees_discount_id)) {
                    $applied_discount[] = $fees_payment->fees_discount_id;
                }
            }
            ...
            return view('backEnd.feesCollection.fees_generate_modal', ['amount' => $amount, 'discounts' => $discounts, 'fees_type_id' => $fees_type_id, 'student_id' => $student_id, 'applied_discount' => $applied_discount]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function searchProfitByDate(Request $request)
    {
        try {
            ...
            return view('backEnd.accounts.profit', ['total_income' => $total_income, 'total_expense' => $total_expense, 'date_from' => $date_from, 'date_to' => $date_to]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function income_store(Request $request)
    {
        ...
        try {
            ...
            $smAddIncome->name = $request->name;
            $smAddIncome->income_head_id = $request->income_head;
            $smAddIncome->date = $newformat;
            $smAddIncome->payment_method_id = $request->payment_method;
            if ($request->payment_method === '3') {
                $smAddIncome->account_id = $request->accounts;
            }

            $smAddIncome->amount = $request->amount;
            $smAddIncome->file = $fileName;
            $smAddIncome->description = $request->description;
            $result = $smAddIncome->save();
    ...
    public function saas_income_store(Request $request)
    {
        ...
        try {
            ...
            $smAddIncome->file = $fileName;
            $smAddIncome->description = $request->description;
            $smAddIncome->school_id = $request->school_id;
            $result = $smAddIncome->save();
    ...
    public function saas_payment_store(Request $request)
    {
        ...
        try {
            $smPaymentMethhod = new SmPaymentMethhod();
            $smPaymentMethhod->method = $request->method;
            $smPaymentMethhod->school_id = $request->school_id;
            $result = $smPaymentMethhod->save();
    ...
    public function payment_edit(Request $request, $id)
    {
        ...
        try {
            ...
            return view('backEnd.accounts.payment_method', ['payment_method' => $payment_method, 'payment_methods' => $payment_methods]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function staffList(Request $request)
    {
        ...
        try {
            ...
            $roles = InfixRole::where('active_status', '=', '1')->where('id', '!=', 2)->where('id', '!=', 3)->where(function ($q): void {
    ...
    public function saas_staffList(Request $request, $school_id)
    {
        ...
        try {
            ...
            return view('backEnd.humanResource.staff_list', ['staffs' => $staffs, 'roles' => $roles]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function saas_staffRoles(Request $request, $school_id)
    {
        ...
        try {
            $roles = InfixRole::where('active_status', '=', '1')
                ->where(function ($q) use ($school_id): void {
    ...
    public function staffAttendance(Request $request)
    {
        ...
        try {
            $roles = InfixRole::where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 10)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
    ...
    public function staffAttendanceSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.humanResource.staff_attendance', ['role_id' => $role_id, 'date' => $date, 'roles' => $roles, 'already_assigned_staffs' => $already_assigned_staffs, 'new_staffs' => $new_staffs, 'attendance_type' => $attendance_type]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function staffAttendanceReportSearch(Request $request)
    {
        ...
        try {
            ...
            $roles = InfixRole::where('id', '!=', 3)->where('id', '!=', 2)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
            ...
            return view('backEnd.humanResource.staff_attendance_report', ['attendances' => $attendances, 'days' => $days, 'year' => $year, 'month' => $month, 'current_day' => $current_day, 'roles' => $roles, 'role_id' => $role_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function saas_Approve_Leave_index(Request $request, $school_id)
    {
        ...
        try {
            ...
            $roles = InfixRole::where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                $data = [];
    ...
    public function Approve_Leave_store(Request $request)
    {
        ...
        try {
            ...
            $smLeaveRequest->approve_status = $request->approve_status;
    ...
    public function saas_Approve_Leave_store(Request $request)
    {
        ...
        try {
            ...
            $smLeaveRequest->approve_status = $request->approve_status;
            $smLeaveRequest->reason = $request->reason;
            $smLeaveRequest->school_id = $request->school_id;
            $smLeaveRequest->file = $fileName;
            $result = $smLeaveRequest->save();
    ...
    public function updateApproveLeave(Request $request)
    {
        ...
        try {
            ...
            $smNotification->user_id = $leave_request_data->student->id;
    ...
    public function classRoutine(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.academics.class_routine_new', ['classes' => $classes]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function classRoutineSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.academics.class_routine_new', ['classes' => $classes, 'class_times' => $class_times, 'class_id' => $class_id, 'section_id' => $section_id, 'sm_weekends' => $sm_weekends]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function assignSubjectSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.academics.assign_subject_create', ['classes' => $classes, 'assign_subjects' => $assign_subjects, 'teachers' => $teachers, 'subjects' => $subjects, 'class_id' => $class_id, 'section_id' => $section_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function assignSubjectFind(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.academics.assign_subject', ['classes' => $classes, 'assign_subjects' => $assign_subjects, 'teachers' => $teachers, 'subjects' => $subjects, 'class_id' => $class_id]);
    ...
    public function subject_store(Request $request)
    {
        ...
        try {
            ...
            $smSubject->subject_name = $request->subject_name;
            $smSubject->subject_type = $request->subject_type;
            $smSubject->subject_code = $request->subject_code;
    ...
    public function saas_subject_store(Request $request)
    {
        ...
        try {
            ...
            $smSubject->subject_type = $request->subject_type;
            $smSubject->subject_code = $request->subject_code;
            $smSubject->school_id = $request->school_id;
            $result = $smSubject->save();
    ...
    public function subject_delete(Request $request, $id)
    {
        try {
            ...
            try {
                $result = SmSubject::destroy($request->id);
                $delete_query = $result;
                if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                    if ($result) {
    ...
    public function saas_subject_delete(Request $request, $school_id, $id)
    {
        try {
            ...
            try {
                $result = SmSubject::where('school_id', $school_id)->destroy($request->id);
                $delete_query = $result;
                if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                    if ($result) {
                        return ApiBaseMethod::sendResponse(null, 'Subject has been deleted successfully');
    ...
    public function class_store(Request $request)
    {
        ...
        try {
            $smClass = new SmClass();
            $smClass->class_name = $request->name;
    ...
    public function saas_class_store(Request $request)
    {
        ...
        try {
            $smClass = new SmClass();
            $smClass->class_name = $request->name;
            $smClass->school_id = $request->school_id;
            $smClass->created_at = YearCheck::getYear().'-'.date('m-d h:i:s');
            $smClass->save();
            $smClass->toArray();
            try {
    ...
    public function class_edit(Request $request, $id)
    {
        ...
        try {
            ...
            return view('backEnd.academics.class', ['classById' => $classById, 'classes' => $classes, 'sections' => $sections, 'sectionId' => $sectionId]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function saas_Section_store(Request $request)
    {
        ...
        try {
            $smSection = new SmSection();
            $smSection->section_name = $request->name;
            $smSection->school_id = $request->school_id;
            $smSection->created_at = YearCheck::getYear().'-'.date('m-d h:i:s');
            $result = $smSection->save();
    ...
    public function saas_Section_delete(Request $request, $school_id, $id)
    {
        ...
        try {
            ...
            $tables = tableList::getTableList($id, $request->id);
            try {
                $delete_query = SmSection::where('school_id', $school_id)->destroy($request->id);
                $section = $delete_query;
                if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                    if ($section) {
                        return ApiBaseMethod::sendResponse(null, 'Section has been deleted successfully');
    ...
    public function class_Routine(Request $request, $id = null)
    {
        ...
        SmWeekend::where('school_id', $student_detail->school_id)->orderBy('order', 'ASC')->where('active_status', 1)->get();
        SmClassTime::where('type', 'class')->where('academic_id', SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->get();
    ...
    public function saas_class_Routine(Request $request, $school_id, $id = null)
    {
        ...
        return view('backEnd.studentPanel.class_routine', ['class_times' => $class_times, 'class_id' => $class_id, 'section_id' => $section_id, 'sm_weekends' => $sm_weekends]);
    }
    ...
    public function saveNoticeData(Request $request)
    {
        ...
        try {
            $roles_array = empty($request->role) ? '' : implode(',', $request->role);
            $user = Auth()->user();

            $login_id = $user ? $user->id : $request->login_id;

            $smNoticeBoard = new SmNoticeBoard();
            if (property_exists($request, 'is_published') && $request->is_published !== null) {
                $smNoticeBoard->is_published = $request->is_published;
            }

            $smNoticeBoard->notice_title = $request->notice_title;
            $smNoticeBoard->notice_message = $request->notice_message;
            $smNoticeBoard->notice_date = date('Y-m-d', strtotime($request->notice_date));
            $smNoticeBoard->publish_on = date('Y-m-d', strtotime($request->publish_on));
            $smNoticeBoard->inform_to = $roles_array;
            $smNoticeBoard->created_by = $login_id;
            $results = $smNoticeBoard->save();
    ...
    public function saas_saveNoticeData(Request $request)
    {
        ...
        try {
            ...
            $smNoticeBoard->inform_to = $roles_array;
            $smNoticeBoard->created_by = $login_id;
            $smNoticeBoard->school_id = $request->school_id;
            $results = $smNoticeBoard->save();
    ...
    {
        try {
            $roles = InfixRole::select('*')->where('id', '!=', 1)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
    ...
    public function saveIssueBookData(Request $request)
    {
        ...
        try {
            ...
            $user_id = $user ? $user->id : $request->login_id;
            $smBookIssue = new SmBookIssue();
            $smBookIssue->book_id = $request->book_id;
            $smBookIssue->member_id = $request->member_id;
            $smBookIssue->given_date = date('Y-m-d');
            $smBookIssue->due_date = date('Y-m-d', strtotime($request->due_date));
            $smBookIssue->issue_status = 'I';
            $smBookIssue->created_by = $user_id;
    ...
    public function saas_saveIssueBookData(Request $request)
    {
        ...
        try {
            ...
            $smBookIssue->issue_status = 'I';
            $smBookIssue->created_by = $user_id;
            $smBookIssue->school_id = $request->school_id;
            $results = $smBookIssue->save();
            $smBookIssue->toArray();
    ...
    public function allIssuedBook(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.library.allIssuedBook', ['books' => $books, 'subjects' => $subjects, 'issueBooks' => $issueBooks]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function searchIssuedBook(Request $request)
    {
        ...
        try {
            ...
            if (! empty($request->book_id)) {
                $query = sprintf("AND i.book_id = '%s'", $request->book_id);
            }

            if (! empty($request->book_number)) {
                $query .= sprintf("AND b.book_number = '%s'", $request->book_number);
            }

            if (! empty($request->subject_id)) {
                $query .= sprintf("AND b.subject_id = '%s'", $request->subject_id);
            }
            ...
            return view('backEnd.library.allIssuedBook', ['issueBooks' => $issueBooks, 'books' => $books, 'subjects' => $subjects, 'book_id' => $book_id, 'book_number' => $book_number, 'subject_id' => $subject_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function saas_deleteItemCategoryView(Request $request, $school_id, string $id)
    {
        try {
            $title = 'Are you sure to detete this Item category?';
            $url = url('delete-item-category/'.$id);
    ...
    {
        try {
            $tables = tableList::getTableList('item_category_id', $id);
            try {
    ...
    public function deleteItemView(Request $request, string $id)
    {
    ...
    {
        try {
            $tables = tableList::getTableList('item_id', $id);
            try {
    ...
    {
        try {
            $tables = tableList::getTableList('store_id', $id);
            try {
    ...
    public function deleteSupplierView(Request $request, string $id)
    {
    ...
    {
        try {
            $tables = tableList::getTableList('supplier_id', $id);
            try {
    ...
    public function saveItemIssueData(Request $request)
    {
        ...
        try {
            ...
            $smItemIssue->role_id = $request->role_id;
            $smItemIssue->issue_to = $issue_to;
            $smItemIssue->issue_by = $user_id;
            $smItemIssue->item_category_id = $request->item_category_id;
            $smItemIssue->item_id = $request->item_id;
            $smItemIssue->issue_date = date('Y-m-d', strtotime($request->issue_date));
            $smItemIssue->due_date = date('Y-m-d', strtotime($request->due_date));
            $smItemIssue->quantity = $request->quantity;
            $smItemIssue->issue_status = 'I';
            $smItemIssue->note = $request->description;
            ...
            if ($results) {
                ...
                $items->total_in_stock -= $request->quantity;
                $result = $items->update();
                if (ApiBaseMethod::checkUrl($request->fullUrl())) {
    ...
    public function studentTransportReportSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.transport.student_transport_report', ['classes' => $classes, 'routes' => $routes, 'vehicles' => $vehicles, 'students' => $students, 'class_id' => $class_id, 'route_id' => $route_id, 'vechile_id' => $vechile_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function studentReportSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.studentInformation.student_report', ['students' => $students, 'classes' => $classes, 'types' => $types, 'genders' => $genders, 'class_id' => $class_id, 'type_id' => $type_id, 'gender_id' => $gender_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function guardianReport(Request $request)
    {
        try {
            ...
            return view('backEnd.studentInformation.guardian_report', ['students' => $students, 'classes' => $classes]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function guardianReportSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.studentInformation.guardian_report', ['students' => $students, 'classes' => $classes, 'class_id' => $class_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function studentHistory(Request $request)
    {
        try {
            ...
            return view('backEnd.studentInformation.student_history', ['students' => $students, 'classes' => $classes, 'years' => $years]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function studentHistorySearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.studentInformation.student_history', ['students' => $students, 'classes' => $classes, 'years' => $years, 'class_id' => $class_id, 'year' => $year]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function studentLoginReport(Request $request)
    {
        try {
            ...
            return view('backEnd.studentInformation.login_info', ['students' => $students, 'classes' => $classes]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function studentLoginSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.studentInformation.login_info', ['students' => $students, 'classes' => $classes, 'class_id' => $class_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function balanceFeesSearch(Request $request)
    {
        ...
        try {
            ...
            foreach ($students as $student) {
                ...
                $total_amount = 0;
                foreach ($fees_masters as $fee_master) {
                    $fees_assign = SmFeesAssign::where('student_id', $student->id)->where('fees_master_id', $fee_master->id)->first();
                    if ($fees_assign ) {
                        $discount_amount = SmFeesPayment::where('student_id', $student->id)->where('fees_type_id', $fee_master->fees_type_id)->sum('discount_amount');

                        $balance = SmFeesPayment::where('student_id', $student->id)->where('fees_type_id', $fee_master->fees_type_id)->sum('amount');
            ...
            return view('backEnd.feesCollection.balance_fees_report', ['classes' => $classes, 'balance_students' => $balance_students, 'class_id' => $class_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function transactionReportSearch(Request $request)
    {
        try {
            ...
            return view('backEnd.feesCollection.transaction_report', ['fees_payments' => $fees_payments, 'add_incomes' => $add_incomes, 'add_expenses' => $add_expenses]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
        ...
        try {
            ...
            foreach ($students as $student) {
                ...
                foreach ($fees_assigns as $fees_assign) {
                    ...
                    foreach ($fees_masters as $fee_master) {
                        $total_collection += SmFeesPayment::where('academic_id', SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->where('student_id', $student->id)->where('fees_type_id', $fee_master->fees_type_id)->sum('amount');
                    }
                }

                foreach ($fees_assigns as $fee_assign) {
                    $fees_master = SmFeesMaster::where('academic_id', SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->where('id', $fee_assign->fees_master_id)->first();
    ...
    public function classRoutineReport(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.reports.class_routine_report', ['classes' => $classes]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function classRoutineReportSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.reports.class_routine_report', ['classes' => $classes, 'class_times' => $class_times, 'class_id' => $class_id, 'section_id' => $section_id, 'sm_weekends' => $sm_weekends]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function examRoutineReportSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.reports.exam_routine_report', ['exam_types' => $exam_types, 'exam_routines' => $exam_routines, 'exam_periods' => $exam_periods, 'exam_term_id' => $exam_term_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function teacherClassRoutineReport(Request $request)
    {
        ...
        try {
            $teachers = SmStaff::select('id', 'full_name')->where('active_status', 1)->where(function ($q): void {
                $q->where('role_id', 4)->orWhere('previous_role_id', 4);
            })->get();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            return view('backEnd.reports.teacher_class_routine_report', ['teachers' => $teachers]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function teacherClassRoutineReportSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.reports.teacher_class_routine_report', ['class_times' => $class_times, 'teacher_id' => $teacher_id, 'sm_weekends' => $sm_weekends, 'teachers' => $teachers]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function meritListReport(Request $request)
    {
        try {
            ...
            return view('backEnd.reports.merit_list_report', ['exams' => $exams, 'classes' => $classes]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
            ...
            if ($request->method() === 'POST') {
                ...
                foreach ($eligible_students as $eligible_student) {
                    foreach ($eligible_subjects as $eligible_subject) {
                        ...
                        $subject_id_strings = (empty($subject_id_strings)) ? $eligible_subject->subject_id : $subject_id_strings.','.$eligible_subject->subject_id;
                        ...
                        if ($marks_string === '') {
                            $marks_string = $getMark->total_marks === 0 ? '0' : $getMark->total_marks;
                        } else {
                    ...
                    $insert_results->student_id = $eligible_student->id;
                ...
                return view('backEnd.reports.merit_list_report', ['iid' => $iid, 'exams' => $exams, 'classes' => $classes, 'subjects' => $subjects, 'class' => $class, 'section' => $section, 'exam' => $exam, 'subjectlist' => $subjectlist, 'allresult_data' => $allresult_data, 'class_name' => $class_name, 'assign_subjects' => $assign_subjects, 'exam_name' => $exam_name, 'InputClassId' => $InputClassId, 'InputExamId' => $InputExamId, 'InputSectionId' => $InputSectionId, 'optional_subject_setup' => $optional_subject_setup]);
            }
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    ...
    public function onlineExamReport(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.reports.online_exam_report', ['exams' => $exams, 'classes' => $classes]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function onlineExamReportSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.reports.online_exam_report', ['online_exam_question' => $online_exam_question, 'students' => $students, 'present_students' => $present_students, 'total_marks' => $total_marks, 'exams' => $exams, 'classes' => $classes, 'class_id' => $class_id, 'exam_id' => $exam_id]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function markSheetReportStudent(Request $request)
    {
        try {
            ...
            return view('backEnd.reports.mark_sheet_report_student', ['exams' => $exams, 'classes' => $classes]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function markSheetReportStudentSearch(Request $request)
    {
        ...
        try {
            ...
            $student_detail = SmStudent::find($request->student);
            ...
            if ($is_result_available->count() > 0) {
                ...
                return view('backEnd.reports.mark_sheet_report_student', ['optional_subject_setup' => $optional_subject_setup, 'exam_types' => $exam_types, 'studentDetails' => $studentDetails, 'exams' => $exams, 'classes' => $classes, 'subjects' => $subjects, 'section' => $section, 'class_id' => $class_id, 'student_detail' => $student_detail, 'is_result_available' => $is_result_available, 'exam_type_id' => $exam_type_id, 'section_id' => $section_id, 'student_id' => $student_id, 'exam_details' => $exam_details, 'class_name' => $class_name, 'input' => $input, 'optional_subject' => $optional_subject]);
            }
    ...
    public function tabulationSheetReport(Request $request)
    {
        try {
            ...
            return view('backEnd.reports.tabulation_sheet_report', ['exam_types' => $exam_types, 'classes' => $classes]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function tabulationSheetReportSearch(Request $request)
    {
        ...
        try {
            ...
            if ($request->student ) {
                ...
                foreach ($subjects as $subject) {
                    $subject_list_name[] = $subject->subject->subject_name;
                }
            ...
            return view(
                ...
                ['optional_subject_setup' => $optional_subject_setup, 'exam_types' => $exam_types, 'classes' => $classes, 'marks' => $marks, 'subjects' => $subjects, 'exam_term_id' => $exam_term_id, 'class_id' => $class_id, 'section_id' => $section_id, 'class_name' => $class_name, 'section_name' => $section_name, 'students' => $students, 'student_id' => $student_id, 'tabulation_details' => $tabulation_details]
            );
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    ...
    public function progressCardReport(Request $request)
    {
        try {
            ...
            return view('backEnd.reports.progress_card_report', ['exams' => $exams, 'classes' => $classes]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function progressCardReportSearch(Request $request)
    {
        ...
        try {
            ...
            if ($is_result_available->count() > 0) {
                ...
                return view('backEnd.reports.progress_card_report', ['exams' => $exams, 'optional_subject_setup' => $optional_subject_setup, 'student_optional_subject' => $student_optional_subject, 'classes' => $classes, 'studentDetails' => $studentDetails, 'is_result_available' => $is_result_available, 'subjects' => $subjects, 'class_id' => $class_id, 'section_id' => $section_id, 'student_id' => $student_id, 'exam_types' => $exam_types, 'assinged_exam_types' => $assinged_exam_types]);
            }
    ...
    public function generalSettingsView(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.systemSettings.generalSettingsView', ['editData' => $editData, 'session' => $session]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function updateGeneralSettings(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.systemSettings.updateGeneralSettings', ['editData' => $editData, 'session_ids' => $session_ids, 'dateFormats' => $dateFormats, 'languages' => $languages, 'countries' => $countries, 'currencies' => $currencies, 'academic_years' => $academic_years, 'time_zones' => $time_zones]);
        } catch (\Exception $exception) {
            return ApiBaseMethod::sendError('Error.', $exception->getMessage());
        }
    }
    ...
    public function updateSchoolLogo(Request $request)
    {
        ...
        try {
            ...
            if ($request->file('main_school_logo') ) {
                ...
                $generalSettData = generalSetting();
                $generalSettData->logo = $main_school_logo;
                $results = $generalSettData->update();
            } elseif ($request->file('main_school_favicon') ) {
                $main_school_favicon = '';
    ...
    {
        try {
            $roles = InfixRole::where('active_status', '=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 9)->orderBy('id', 'desc')->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                return ApiBaseMethod::sendResponse($roles, null);
    ...
    public function role_edit(Request $request, $id)
    {
        try {
            ...
            $roles = InfixRole::where('active_status', '=', 1)->orderBy('id', 'desc')->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
    ...
    public function studentDashboard(Request $request, $id = null)
    {
        ...
        return view('backEnd.studentPanel.my_profile', ['driver' => $driver, 'academic_year' => $academic_year, 'student_detail' => $student_detail, 'fees_assigneds' => $fees_assigneds, 'fees_discounts' => $fees_discounts, 'exams' => $exams, 'documents' => $documents, 'timelines' => $timelines, 'siblings' => $siblings, 'grades' => $grades]);
    }
    ...
    public function studentNoticeboard(Request $request)
    {
        ...
        return view('backEnd.studentPanel.studentNoticeboard', ['allNotices' => $allNotices]);
    }
    ...
    public function studentAssignmentApi(Request $request, $id)
    {
        ...
        $uploadContents = SmTeacherUploadContent::where('content_type', 'as')
            ...
            ->where(function ($query) use ($student_detail): void {
    ...
    public function viewAllNotification(Request $request, $user_id)
    {
        try {
            ...
            SmNotification::clearUnreadCache($user_id, $user->role_id);
    ...
    public function childBankSlipStore(Request $request)
    {
        ...
        try {
            ...
            if ($request->payment_mode === 'bank' && $request->bank_id === '' && ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $smBankPaymentSlip->class_id = $request->class_id;
            $smBankPaymentSlip->section_id = $request->section_id;
            $smBankPaymentSlip->school_id = 1;
            $smBankPaymentSlip->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
    ...
    public function collectFeesChildApi(Request $request, $id)
    {
        ...
        SmFeesAssign::where('student_id', $id)->orderBy('id', 'desc')->get();

        return DB::table('sm_fees_assigns')
    ...
    public function saas_collectFeesChildApi(Request $request, $school_id, $id)
    {
        ...
        SmFeesAssign::where('student_id', $id)->orderBy('id', 'desc')->where('school_id', $school_id)->get();
    ...
    public function classRoutineApi(Request $request, $id)
    {
        ...
        SmWeekend::where('school_id', Auth::user()->school_id)->orderBy('order', 'ASC')->where('active_status', 1)->get();
        SmClassTime::where('type', 'class')->get();
    ...
    public function childInfo(Request $request, $user_id)
    {
        ...
        try {
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                ...
                if ($user) {
                    $user->document_file_1 = $user->document_file_1 ? asset($user->document_file_1) : null;
                    $user->document_file_2 = $user->document_file_2 ? asset($user->document_file_2) : null;
                    $user->document_file_3 = $user->document_file_3 ? asset($user->document_file_3) : null;
                    $user->document_file_4 = $user->document_file_4 ? asset($user->document_file_4) : null;
                    $user->student_photo = $user->student_photo ? asset($user->student_photo) : null;
                    
                    if ($user->studentDocument) {
                        foreach ($user->studentDocument as $document) {
                            $document->file = $document->file ? asset($document->file) : null;
                        }
                    }
                }
                ...
                if ($data['userDetails']) {
                    $data['userDetails']->document_file_1 = $data['userDetails']->document_file_1 ? asset($data['userDetails']->document_file_1) : null;
                    $data['userDetails']->document_file_2 = $data['userDetails']->document_file_2 ? asset($data['userDetails']->document_file_2) : null;
                    $data['userDetails']->document_file_3 = $data['userDetails']->document_file_3 ? asset($data['userDetails']->document_file_3) : null;
                    $data['userDetails']->document_file_4 = $data['userDetails']->document_file_4 ? asset($data['userDetails']->document_file_4) : null;
                    $data['userDetails']->student_photo = $data['userDetails']->student_photo ? asset($data['userDetails']->student_photo) : null;
                }
    ...
    public function myRoutine(Request $request, $id)
    {
        ...
        SmWeekend::orderBy('order', 'ASC')->where('active_status', 1)->get();
        SmClassTime::where('type', 'class')->get();
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            foreach ($weekenD as $weeken) {
                $data[$weeken->name] = DB::table('sm_class_routine_updates')
                    ...
                    ->where([
                        ['sm_class_routine_updates.teacher_id', $teacher_id], ['sm_class_routine_updates.day', $weeken->id],
                    ])->get();
            }
    ...
    public function saas_myRoutine(Request $request, $school_id, $id)
    {
        DB::table('sm_staffs')
            ...
            ->where('school_id', $school_id)->first();
        SmWeekend::where('school_id', Auth::user()->school_id)->orderBy('order', 'ASC')->where('active_status', 1)->get();
        SmClassTime::where('type', 'class')->where('school_id', $school_id)->get();
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = [];
            $weekenD = SmWeekend::where('school_id', Auth::user()->school_id)->get();
            foreach ($weekenD as $weeken) {
                $data[$weeken->name] = DB::table('sm_class_routine_updates')
                    ...
                    ->where([
                        ['sm_class_routine_updates.teacher_id', @_ID], ['sm_class_routine_updates.day', $weeken->id],
                    ])->where('sm_class_routine_updates.school_id', $school_id)->get();
            }
    ...
    public function sectionRoutine(Request $request, $id, $class, $section)
    {
        ...
        SmWeekend::where('school_id', $teacher->school_id)->orderBy('order', 'ASC')->where('active_status', 1)->get();
        SmClassTime::where('type', 'class')->get();
            ...
            foreach ($weekenD as $weeken) {
                $data[$weeken->name] = DB::table('sm_class_routine_updates')
                    ...
                    ->where([
                        ...
                        ['sm_class_routine_updates.day', $weeken->id],
                    ])->get();
            }
    ...
    public function classSection(Request $request, $id)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            foreach ($teacher_classes as $teacher_class) {
                $data[$teacher_class->class_name] = DB::table('sm_assign_subjects')
                    ->join('sm_subjects', 'sm_subjects.id', '=', 'sm_assign_subjects.subject_id')
                    ->join('sm_sections', 'sm_sections.id', '=', 'sm_assign_subjects.section_id')
                    ->select('section_name', 'subject_name')
                    ->distinct('section_id')
                    ->where([
                        ['sm_assign_subjects.class_id', $teacher_class->id],
    ...
    public function applyLeave(Request $request)
    {
        ...
        $smLeaveRequest = new SmLeaveRequest();
        $smLeaveRequest->staff_id = $request->input('teacher_id');
        $smLeaveRequest->role_id = 4;
        $smLeaveRequest->apply_date = date('Y-m-d');
        $smLeaveRequest->leave_define_id = $request->input('leave_type');
        $smLeaveRequest->type_id = $request->input('leave_type');
        $smLeaveRequest->leave_from = $request->input('leave_from');
        $smLeaveRequest->leave_to = $request->input('leave_to');
        $smLeaveRequest->approve_status = 'P';
        $smLeaveRequest->reason = $request->input('reason');
        $smLeaveRequest->file = $fileName;
    ...
    public function saas_applyLeave(Request $request)
    {
        ...
        $smLeaveRequest->approve_status = 'P';
        $smLeaveRequest->reason = $request->input('reason');
        $smLeaveRequest->school_id = $request->input('school_id');
        $smLeaveRequest->file = $fileName;
    ...
    {
        SmStaff::withoutGlobalScopes()->where('user_id', '=', $id)->where('school_id', $school_id)->first();
    ...
    public function uploadContent(Request $request)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $validator = Validator::make($input, [
                ...
                'attach_file' => 'nullable|mimes:pdf,doc,docx,jpg,jpeg,png,mp4,mp3',
        ...
        if ($request->input('available_for') === 'admin') {
            $roles = InfixRole::where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 9)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
    ...
    public function saas_uploadContent(Request $request)
    {
        ...
        $smTeacherUploadContent->description = $request->input('description');
        $smTeacherUploadContent->upload_file = $fileName;
        $smTeacherUploadContent->school_id = $request->input('school_id');
        $smTeacherUploadContent->academic_id = SmAcademicYear::API_ACADEMIC_YEAR($smTeacherUploadContent->school_id);
        $smTeacherUploadContent->created_by = $request->input('created_by');
        $smTeacherUploadContent->save();
    ...
    {
        DB::table('sm_teacher_upload_contents')->where('id', $id)->delete();
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = '';
    ...
    {
        DB::table('sm_teacher_upload_contents')->where('id', $id)->where('school_id', $school_id)->delete();
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = '';
    ...
    public function saas_pendingLeave(Request $request, $school_id)
    {
        ...
        try {
            ...
            $leave_types = SmLeaveType::where('active_status', 1)->get();
            $roles = InfixRole::where('id', '!=', 1)->where('id', '!=', 3)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
    ...
    public function apply_Leave(Request $request)
    {
        ...
        try {
            ...
            $smLeaveRequest = new SmLeaveRequest();
            $smLeaveRequest->staff_id = $request->input('staff_id');
            $smLeaveRequest->role_id = 4;
            $smLeaveRequest->apply_date = date('Y-m-d');
            $smLeaveRequest->leave_define_id = $request->input('leave_type');
            $smLeaveRequest->type_id = $request->input('leave_type');
    ...
    public function updateLeave(Request $request)
    {
        ...
        try {
            ...
            $smNotification = new SmNotification;
            $smNotification->user_id = $staff_id;
            $smNotification->role_id = $role_id;
            $smNotification->school_id = Auth::user()->school_id;
            $smNotification->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
            $smNotification->date = date('Y-m-d');
