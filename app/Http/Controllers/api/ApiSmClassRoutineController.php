...
class ApiSmClassRoutineController extends Controller
{
    public function classRoutineSearch(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['classes' => $classes, 'teachers' => $teachers, 'rooms' => $rooms, 'subjects' => $subjects, 'class_id' => $class_id, 'section_id' => $section_id, 'sm_weekends' => $sm_weekends]);
    ...
    public function dayWiseClassRoutine(Request $request)
    {
        ...
        return response()->json(['day_id' => $day_id, 'class_routines' => $class_routines, 'sm_weekends' => $sm_weekends, 'subjects' => $subjects, 'rooms' => $rooms, 'teachers' => $teachers, 'section_id' => $section_id, 'class_id' => $class_id]);
    }
    ...
    public function studentClassRoutine(Request $request, $user_id, $record_id = null)
    {
        try {
            ...
            $class_routines = SmClassRoutineUpdate::withOutGlobalScope(StatusAcademicSchoolScope::class)->with('weekend', 'classRoom', 'subject', 'teacherDetail', 'class', 'section')->where('class_id', $class_id)->where('section_id', $section_id)
                ->where('school_id', $school_id)->get()->map(function ($value): array {
                    return [
                        'id' => $value->id,
            ...
            return response()->json(['student_detail' => $student_detail, 'class_routines' => $class_routines]);
        } catch (Exception$exception) {
    ...
    public function teacherClassRoutine($user_id, $school_id = null)
    {
        try {
            ...
            $class_routines = SmClassRoutineUpdate::withoutGlobalScopes()->with('weekend', 'classRoom', 'subject', 'teacherDetail', 'class', 'section')->where('teacher_id', $teacher_id)->where('school_id', $school_id)->get()->map(function ($value): array {
                return [
                    'id' => $value->id,
            ...
            return response()->json(['staff_detail' => $staff_detail, 'class_routines' => $class_routines]);
        } catch (Exception$exception) {
    ...
    public function sectionRoutine(Request $request, $user_id, $class, $section)
    {
        try {
            ...
            $class_routines = SmClassRoutineUpdate::with('weekend', 'classRoom', 'subject', 'teacherDetail', 'class', 'section')->where('teacher_id', $teacher_id)->where('class_id', $class)->where('section_id', $section)->where('school_id', $school_id)->get()->map(function ($value): array {
                return [
                    'id' => $value->id,
                    'day' => $value->weekend ? $value->weekend->name : '',
                    'room' => $value->classRoom ? $value->classRoom->room_no : '',
