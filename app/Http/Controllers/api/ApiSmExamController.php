...
class ApiSmExamController extends Controller
{
    ...
    public function examResult_Api(Request $request, $user_id, $exam_id, $record_id)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $exam = \App\Models\SmExamType::find($exam_id);
            ...
            if ($get_results) {
                foreach ($get_results as $get_result) {
                    $result[] = [
                        ...
                        'subject_name' => @$get_result->subject->subject_name,
                        'obtained_marks' => @$get_result->total_marks,
                        'total_marks' => @subjectFullMark($get_result->exam_type_id, $get_result->subject_id, $record->class_id, $record->section_id),
                        'grade' => @$get_result->total_gpa_grade,
                    ];
                }
            }
    ...
    public function saas_examResult_Api(Request $request, $school_id, $user_id, $exam_id, $record_id)
    {
        ...
        $student_id = SmStudent::where('user_id', $user_id)->value('id');
        $record = StudentRecord::where('student_id', $student_id)->where('id', $record_id)->first();
        $exam = \App\Models\SmExamType::withOutGlobalScopes()->find($exam_id);
        ...
        if ($get_results) {
            foreach ($get_results as $get_result) {
                $result[] = [
                    'id' => $get_result->id,
                    'exam_name' => @$exam->title,
                    'subject_name' => SmSubject::withOutGlobalScopes()->where('id', @$get_result->subject_id)->first()->subject_name,
                    'obtained_marks' => @$get_result->total_marks,
                    'total_marks' => @subjectFullMark($get_result->exam_type_id, $get_result->subject_id, $record->class_id, $record->section_id),
                    'grade' => @$get_result->total_gpa_grade,
                ];
    ...
    public function saas_examResultApi(Request $request, $school_id, $user_id, $exam_id, $record_id)
    {
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            foreach ($exam_result as $row) {
                ...
                $show_result = '';
                $result = ! empty($obtained_marks) ? $obtained_marks->total_marks * 100 / $total_marks : 0;

                $show_result = $result >= $online_exam_question->percentage ? 'Pass' : 'Fail';
