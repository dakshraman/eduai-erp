...
class ApiSmAcademicYearController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $tables = [SmClass::class, SmSection::class, \App\Models\SmExamType::class];
            // $tables = ['App\Models\SmClass', 'App\Models\SmSection', 'App\Models\SmSubject', 'App\Models\SmExamSetup', 'App\Models\SmExamType', 'App\Models\SmMarksGrade', 'App\Models\SmVehicle', 'App\Models\SmClassTime', 'App\Models\SmClassSection'];
            foreach ($tables as $table) {
                $data = $table::where('created_at', 'like', '%'.$yr->year.'%')->get();
                if (! empty($data)) {
            ...
            $smAcademicYear->year = $request->year;
            $smAcademicYear->title = $request->title;
            // $academic_year->starting_date = date('Y-m-d', strtotime($request->starting_date));
            $smAcademicYear->starting_date = $request->year.'-01-01';
            // $academic_year->ending_date = date('Y-m-d', strtotime($request->ending_date));
            $smAcademicYear->ending_date = $request->year.'-12-31';
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($yr->year !== $request->year) {
                // $tables = ['App\Models\SmClass', 'App\Models\SmSection', 'App\Models\SmSubject', 'App\Models\SmExamSetup', 'App\Models\SmExamType', 'App\Models\SmMarksGrade', 'App\Models\SmVehicle', 'App\Models\SmClassTime'];
                $tables = [SmClass::class, SmSection::class];
                foreach ($tables as $table) {
                    $data = $table::where('created_at', 'like', '%'.$yr->year.'%')->get();
                    if (! empty($data)) {
                        foreach ($data as $value) {
