...
use App\Http\Resources\v2\ProfileParnetResource;
...
class AuthenticationController extends Controller
{
    ...
    public function emailVerify(Request $request)
    {
        ...
        if ($student && $emailCheck->role_id === 2) {
    ...
    {
        $data['student_detail'] = SmStudent::with(['bloodGroup' => function ($q1): void {
            $q1->select('id', 'base_setup_name as bloodgroup_name');
        }, 'religion' => function ($q2): void {
            $q2->select('id', 'base_setup_name as religion_name');
        }, 'parents' => function ($q3): void {
        ...
        }, 'route' => function ($q4): void {
            $q4->select('id', 'title');
        }, 'vehicle' => function ($q5): void {
            $q5->select('id', 'vehicle_no');
        }, 'dormitory' => function ($q6): void {
            $q6->select('id', 'dormitory_name');
        }, 'studentDocument' => function ($q7): void {
    ...
    {
        $user = auth('api')->user();
        $schoolId = $user?->school_id ?? app('school')->id;

        $data['system_settings'] = SmGeneralSettings::where('school_id', $schoolId)->first();
    ...
    public function profileParents()
    {
        ...
        $parentProfile = SmParent::withoutGlobalScope(SchoolScope::class)
            ->where('school_id', auth()->user()->school_id)
        ...
        $data['profileParents'] = new ProfileParnetResource($parentProfile);
    ...
    public function profileDocuments()
    {
        ...
        $data['profileDocuments'] = SmStudentDocument::where('school_id', auth()->user()->school_id)
            ...
            ->get()->map(function ($document): array {
                return [
                    'id' => (int) $document->id,
                    'title' => (string) $document->title,
                    'file' => $document->file ? (string) assetPath($document->file) : (string) null,
                ];
            });
    ...
    public function studentSubject(Request $request)
    {
        ...
        $anonymousResourceCollection = AssignSubjectResource::collection($assign_subject);
    ...
    public function profileDocumentsStore(Request $request)
    {
        ...
        if ($request->file('photo')  && $request->title ) {
            ...
            $smStudentDocument->academic_id = SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR();
            $smStudentDocument->save();
            $doc = [
                'id' => (int) $smStudentDocument->id,
                'title' => (string) $smStudentDocument->title,
                'file' => (string) assetPath($smStudentDocument->file),
            ];
        }
