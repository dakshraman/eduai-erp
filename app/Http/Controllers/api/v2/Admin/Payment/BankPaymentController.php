...
class BankPaymentController extends Controller
{
    public function paymentList(Request $request)
    {
        ...
        foreach ($statuses as $status) {
            $statusHint .= mb_strtoupper($status).': '.mb_strtolower($status).', ';
            $data[$status] = $paymentData[$status] ?? [];
        }

        $data['status'] = rtrim($statusHint, ', ');
    ...
    public function changePaymentStatus(Request $request)
    {
        ...
        switch ($request->change_status) {
            case 'approve':
                ...
                $total_paid_amount = $request->paid_amount ?: null;
    ...
    private function addFeesAmount($transcation_id, $total_paid_amount): bool
    {
