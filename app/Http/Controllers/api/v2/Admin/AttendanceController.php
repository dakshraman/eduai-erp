...
class AttendanceController extends Controller
{
    ...
    public function studentSearch(Request $request)
    {
        ...
        $students = StudentRecord::with([
            'studentDetail' => function ($q) use ($request): void {
                $q->with(['DateWiseAttendances' => function ($q) use ($request): void {
                    $q->withoutGlobalScope(AcademicSchoolScope::class)->where('attendance_date', $request->attendance_date);
                }]);
            }, 'studentAttendance' => function ($q): void {
            ...
            ->where('academic_id', SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())
            ->where('school_id', auth()->user()->school_id)
            ->get()->sortBy('roll_no')->map(function ($student) {
                    if (!$student->studentAttendance) {
                        $student->studentAttendance = (object)[
                            'attendance_type' => 'P'
                        ];
                    }
                    return $student;
    ...
    public function studentSearchAttend(Request $request)
    {
        ...
        $students = StudentRecord::where('class_id', $request->class)
            ...
            ->when($request->roll_no, function ($q2) use ($request): void {
    ...
    public function subjectWiseStudents(Request $request)
    {
        ...
        if ($students[0]['studentDetail']['DateSubjectWiseAttendances'] !== null) {
            $students[0]['studentDetail'];
        }
    ...
    public function studentSubjectAttendanceSearch(Request $request)
    {
        ...
        foreach ($all_attendances as $all_attendance) {
            if ($all_attendance->attendance_type === 'P') {
                ...
                $total_half += 1;
            } elseif ($all_attendance->attendance_type === 'H') {
                $total_holiday += 1;
            }
        }
    ...
    public function studentAttendanceStore(Request $request)
    {
        ...
        foreach ($request->get('record_id', []) as $key => $record) {
            ...
            $attendance = DB::table('sm_student_attendances')->where('class_id', $request->class)
    ...
    public function subjectHolidayStore(Request $request)
    {
        ...
        if ($request->purpose === 'mark') {
            ...
            foreach ($students as $student) {
                $attendance = SmSubjectAttendance::where('student_id', $student->student_id)
                    ->where('subject_id', $request->subject_id)
                    ->where('attendance_date', date('Y-m-d', strtotime($request->attendance_date)))
                    ->where('class_id', $request->class_id)->where('section_id', $request->section_id)
