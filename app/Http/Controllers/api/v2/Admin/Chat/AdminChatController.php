...
class AdminChatController extends Controller
{
    ...
    public function userList(Request $request)
    {
        ...
        if ($anonymousResourceCollection->isEmpty()) {
            $status = 200;
            $response = [
                'success' => true,
                'data' => null,
    ...
    public function sendMessage(Request $request)
    {
        ...
        $singleChatResource = new SingleChatResource($message);

        return response()->json([
            'success' => true,
            'data' => $singleChatResource,
    ...
    public function messages(Request $request)
    {
        ...
        $messages = Conversation::whereHas('fromUser', function ($u) use ($request): void {
            $u->where('id', auth()->user()->id)->orWhere('id', $request->to_user_id);
        })->whereHas('toUser', function ($us) use ($request): void {
            $us->where('id', $request->to_user_id)->orWhere('id', auth()->user()->id);
        })->get();
        $anonymousResourceCollection = SingleChatResource::collection($messages);
        return response()->json([
                'success' => true,
                'data' => $anonymousResourceCollection,
    ...
    public function deleteMessage(Request $request)
    {
        ...
        $conversation = Conversation::where('id', $request->message_id)
            ->when(auth()->user()->id, function ($q): void {
        ...
        $data = Conversation::whereNotNull('file_name')
            ...
            ->map(function ($value): array {
                return [
                    ...
                    'file' => $value->file_name ? (string) assetPath($value->file_name) : null,
