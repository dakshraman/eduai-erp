...
class GroupChatController extends Controller
{
    ...
    public function show(Group $group): void
    {
        ...
        $users->filter(function ($value, $key) use ($group): bool {
        ...
        $only_threads->sortByDesc('created_at')->take(20)->map(function ($value, $key): GroupMessageRecipient {
        ...
        GroupUser::where('user_id', auth()->id())->where('group_id', $group->id)->first()->role;
    }
    ...
    public function groupList()
    {
        ...
        foreach ($groups as $group) {
            ...
            $allGroup[] = [
                ...
                'image' => $group->photo_url ? (string) assetPath($group->photo_url) : null,
    ...
    public function sendGroupMessage(Request $request)
    {
        ...
        $thisMessage = Conversation::select('id', 'message', 'status', 'message_type', 'file_name', 'original_file_name', 'reply', 'from_id', 'to_id', 'forward', 'deleted_by_to')->find($message->id);

        $thisThread = GroupMessageRecipient::select('id', 'updated_at', 'user_id')->find($thread->id);
        ...
        $thisUser = [
            ...
            'avatar_url' => assetPath($user->avatar_url),
        ...
        $groupchatMessageResource = new GroupchatMessageResource($resourceItems);

        $response = [
            'success' => true,
            'data' => [$groupchatMessageResource],
        ...
        $members = GroupUser::where('group_id', $request->group_id)
            ...
            ->map(function ($value): array {
                return [
                    ...
                    'image' => @$value->user->avatar_url ? (string) assetPath($value->user->avatar_url) : null,
                ];
            });
    ...
    public function leaveGroup(Request $request)
    {
        ...
        return response()->json( [
            'success' => true,
            'data' => null,
            'message' => sprintf("You left '%s' group successfully", $groupName),
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $data = [
            ...
            'image' => $groupData->photo_url ? (string) assetPath($groupData->photo_url) : null,
        ];
        ...
        try {
            ...
            if (! empty($msg)) {
                $message = Conversation::create([
                    ...
                    'message' => $msg->message,
                    'file_name' => $msg->file_name ? assetPath($msg->file_name) : null,
