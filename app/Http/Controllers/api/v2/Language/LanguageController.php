...
class LanguageController extends Controller
{
    public function allList(Request $request)
    {
        $languages = SmLanguage::orderBy('id', 'ASC')
            ->get()->map(function ($language) use ($request): array {
                $active = $request->user() ? ($request->user()->language === $language->language_universal) : (bool) ($language->active_status);
                $rtl = false;
                if ($active) {
    ...
    public function myLanguages(Request $request)
    {
        ...
        $languages = SmLanguage::where('school_id', auth()->user()->school_id)
            ->get()->map(function ($language) use ($user): array {
