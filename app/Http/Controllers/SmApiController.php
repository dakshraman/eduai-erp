...
class SmApiController extends Controller
{
    ...
    public function UpdateStudentApi(Request $request)
    {
        ...
        if (!empty($request->field_name)) {
            ...
            if ($request_string == "first_name") {
                ...
                $student_photo = fileUpload($file, $student_file_destination);
    ...
    public function visitor_store(Request $request)
    {
        ...
        if ($request->file('file') != "") {
            ...
            $fileName = fileUpload($file, 'public/uploads/visitor/');
        }
    ...
    public function complaintStore(Request $request)
    {
        ...
        if ($request->file('file') != "") {
            $file = $request->file('file');
            $fileName = fileUpload($file, 'public/uploads/complaint/');
        }
    ...
    public function postal_receive_store(Request $request)
    {
        ...
        if ($request->file('file') != "") {
            $file = $request->file('file');
            $fileName = fileUpload($file, 'public/uploads/postal/');
        }
    ...
    public function studentDelete(Request $request)
    {
        ...
        $tables = \App\Support\tableList::getTableList('student_id',$request->id);
        try {
            ...
            try {
                ...
                foreach ($request->id as $student_id) {
                    ...
                    if ($request->result[$student_id] == 'P') {
                        $merit_list = \App\Models\SmTemporaryMeritlist::where(['student_id' => $student_id, 'class_id' => $request->current_class, 'section_id' => $student_details->section_id])->where('academic_id', SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())->first();
    ...
    public function saveUploadContent(Request $request)
    {
        ...
        try {
            ...
            if ($request->file('content_file') != "") {
                $file = $request->file('content_file');
                $fileName = fileUpload($file, 'public/uploads/upload_contents/');
            }
    ...
    public function income_store(Request $request)
    {
        ...
        if ($request->file('file') != "") {
            $file = $request->file('file');
            $fileName = fileUpload($file, 'public/uploads/add_income/');
        }
    ...
    public function Approve_Leave_store(Request $request)
    {
        ...
        if ($request->file('attach_file') != "") {
            ...
            $fileName = fileUpload($file, 'public/uploads/leave_request/');
        }
    ...
    {
        $tables = \App\Support\tableList::getTableList('supplier_id',$id);
    ...
    public function generalSettingsView(Request $request, $school_id)
    {
        ...
        $modules = InfixModuleManager::select('id', 'name')->get();
        ...
        $is_saas = moduleStatusCheck('Saas');
        
        $assigned_modules = [];
        $menu_permissions = [];
        
        if ($is_saas) {
            $assigned_modules = DB::table('saas_school_module_permission_assigns')
                ->where('school_id', $school_id)
                ->pluck('module_id')
                ->toArray();
                
            $menu_permissions = DB::table('saas_school_module_permission_assigns')
                ->join('sm_module_permissions', 'saas_school_module_permission_assigns.module_id', '=', 'sm_module_permissions.id')
                ->where('saas_school_module_permission_assigns.school_id', $school_id)
                ->select('sm_module_permissions.*')
                ->get();
        } else {
            $menu_permissions = DB::table('sm_module_permissions')
                ->where('active_status', 1)
                ->get();
        }
        ...
        foreach ($modules as $module) {
            if ($is_saas) {
                if (!in_array($module->id, $assigned_modules)) {
                    $editData->{$module->name} = 0;
                } else {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $editData['modules'] = $assigned_modules;
            $editData['menu_permissions'] = $menu_permissions;
    ...
    public function updateSchoolLogo(Request $request)
    {
        ...
        if ($request->file('main_school_logo') != "") {
            ...
            $main_school_logo = fileUpload($file, 'public/uploads/settings/');
            ...
            $main_school_favicon = fileUpload($file, 'public/uploads/settings/');
            $generalSettData          = generalSetting();
    ...
    public function studentDashboard(Request $request, $id = null)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = [];
            $data['student_detail'] = !empty($student_detail) ? $student_detail->toArray():[];
            $data['fees_assigneds'] = !empty($fees_assigneds) ? $fees_assigneds->toArray():[];
            $data['fees_discounts'] = !empty($fees_discounts) ? $fees_discounts->toArray():[];
            $data['exams'] = !empty($exams) ? $exams->toArray():[];
            $data['documents'] = !empty($documents) ? $documents->toArray():[];
            $data['timelines'] = !empty($timelines) ? $timelines->toArray():[];
            $data['siblings'] = !empty($siblings) ? $siblings->toArray():[];
            $data['grades'] =  !empty($grades) ?  $grades->toArray():[];
            return ApiBaseMethod::sendResponse($data, null);
        }
    ...
    public function childBankSlipStore(Request $request)
    {
        ...
        if ($request->file('slip') != "") {
            ...
            $fileName = fileUpload($file, 'public/uploads/bankSlip/');
        }
