...
class SmAcademicCalendarController extends Controller
{
    ...
    {
        $settings = calandarSettingByMenuName($menu_name);

        if(!$settings){
            $calender_setting = SmCalendarSetting::withoutGlobalScopes()->where('school_id', 1)->where('menu_name', $menu_name)->first();

            $new_calender_setting = new SmCalendarSetting();
            $new_calender_setting->school_id = auth()->user()->school_id;
            $new_calender_setting->menu_name = $menu_name;
            $new_calender_setting->status = $calender_setting ? $calender_setting->status : 1;
            $new_calender_setting->font_color = $calender_setting ? $calender_setting->font_color : '#ffffff';
            $new_calender_setting->bg_color = $calender_setting ? $calender_setting->bg_color : '#4150f7';
            $new_calender_setting->save();
            return $new_calender_setting;
        }
    ...
    public function calenderData()
    {
        ...
        $userData = auth()->user()->load([
            ...
            'parent.childrens.studentRecords',
            'staff.classes',
        ]);
        $admissionQuries = [];
        $homeworks = [];
        ...
        if ($userRoleId === 3) {
            ...
            $childrenInfos = $userData->parent->childrens ?? [];
            $childrenRecords = [
                'class_id' => $childrenInfos
                    ->flatMap(fn ($child) => $child->studentRecords)
                    ->pluck('class_id')
                    ->toArray(),

                'section_id' => $childrenInfos
                    ->flatMap(fn ($child) => $child->studentRecords)
                    ->pluck('section_id')
                    ->toArray(),
            ];

            $childrenUserids = $childrenInfos->pluck('user_id')->toArray();
