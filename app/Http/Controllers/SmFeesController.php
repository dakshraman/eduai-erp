...
class SmFeesController extends Controller
{
    ...
    public function feesStatemnt(Request $request)
    {
        try {
            ...
            $fees_masters = branchWise(SmFeesMaster::select('fees_group_id')->where('active_status', 1)->distinct('fees_group_id')->where('school_id', Auth::user()->school_id)->get());
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                $data = [];
                $data['classes'] = $classes->toArray();
                $data['fees_masters'] = $fees_masters->toArray();
    ...
    public function feesStatementSearch(Request $request)
    {
        ...
        try {
            ...
            $fees_assigneds = branchWise(SmFeesAssign::where('student_id', $request->student)->where('school_id', Auth::user()->school_id)->get());
    ...
    public function fineReportSearch(Request $request)
    {
        ...
        try {
            ...
            if ($request->date_range) {
                $fine_info = branchWise(SmFeesPayment::where('active_status', 1)->where('payment_date', '>=', $date_from)
                    ->where('payment_date', '<=', $date_to)
                    ->when($request->branch_id, function ($q) use ($request): void {
                        $q->where('branch_id', $request->branch_id);
                    })
