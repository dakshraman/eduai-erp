...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (config('app.app_sync') && $request->auto_login) {
            ...
            if (!$user) {
                $user = User::whereRaw('LOWER(email) = ?', [mb_strtolower($request->email)])
                    ->where('school_id', $school->id)
                    ->first();
            }

            if ($user) {
                if ($user->active_status && $user->access_status && Hash::check($request->password, $user->password)) {
                    $this->guard()->login($user);
                    $logged_in = Auth::check();
                }
            }
        ...
        if ($logged_in) {
            ...
            if (!Auth::user()->access_status || (!Auth::user()->active_status && Auth::user()->id != 1)) {
                $this->guard()->logout();
                Toastr::error('You are not allowed, Please contact with administrator.', 'Failed');
    ...
    public function loginCodeCanyon()
    {
        ...
        $users = User::whereIn('role_id', [1, 2, 3, 4, 5, 6, 7, 8, 10])->select('email', 'role_id')->get();
        $data = [
            ...
            'user_10' => $users->where('role_id', 10)->first(),
        ];
        if (generalSetting() && generalSetting()->active_theme === 'edulia') {
