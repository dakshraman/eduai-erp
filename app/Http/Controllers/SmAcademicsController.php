...
class SmAcademicsController extends Controller
{
    ...
    public function ajaxSelectSubject(Request $request)
    {
        ...
        $subjectQuery = SmAssignSubject::withoutGlobalScope(StatusAcademicSchoolScope::class)
            ->where('class_id', $request->class)
            ->where('section_id', $request->section)
            ->where('academic_id', $academicId)
            ->where('school_id', Auth::user()->school_id)
            ->where('active_status', 1)
            ->when($request->shift_id, function ($query, $shiftId): void {
                $query->where('shift_id', $shiftId);
            });

        if (teacherAccess()) {
            $subjectQuery->where('teacher_id', optional($staff_info)->id);
        }

        if (moduleStatusCheck('Branch')) {
            if ($request->branch_id) {
                $subjectQuery->where(function ($query) use ($request): void {
        ...
        $subjectIds = $subjectQuery->pluck('subject_id')->unique()->values();

        $subjectModel = SmSubject::withoutGlobalScope(StatusAcademicSchoolScope::class)
            ->whereIn('id', $subjectIds)
            ->where('active_status', 1)
            ->where(function ($query) use ($academicId): void {
                $query->where('academic_id', $academicId)
                    ->orWhereNull('academic_id')
                    ->orWhere('academic_id', 0);
            })
            ->where('school_id', Auth::user()->school_id);

        $students = branchWise($subjectModel)->get(['id', 'subject_name', 'subject_type'])
    ...
    public function classRoutineReportSearch(Request $request)
    {
        ...
        $classes = branchWise(SmClass::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get());
        $class_routines = branchWise(SmClassRoutine::where('class_id', $request->class)->where('section_id', $request->section)->get());
        $class_id = $request->class;
    ...
    {
        $classes = branchWise(SmClass::where('academic_id', getAcademicId())->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get());
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            return ApiBaseMethod::sendResponse($classes, null);
        }
    ...
    public function universityClassReport($request)
    {
        ...
        $students = branchWise(SmStudent::whereIn('id', $student_ids_from_record)->where('school_id', $user->school_id));
        ...
        if ($request->branch_id ) {
            $assign_subjects->where('branch_id', $request->branch_id);
        }
        ...
        if ($request->branch_id ) {
            $assign_class_teacher->where('branch_id', $request->branch_id);
        }
