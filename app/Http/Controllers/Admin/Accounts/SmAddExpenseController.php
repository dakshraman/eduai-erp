...
class SmAddExpenseController extends Controller
{
    ...
    public function store(SmExpenseRequest $smExpenseRequest)
    {
        ...
        */
        ...
        // Updated to round amount to 5 decimal places
        $smAddExpense->amount = (float) round($smExpenseRequest->amount, getDecimalDigit());
        ...
        if (paymentMethodName($smExpenseRequest->payment_method)) {
            $bank = SmBankAccount::where('id', $smExpenseRequest->accounts)
                ...
                ->first();
            $after_balance = $bank->current_balance - (float) round($smExpenseRequest->amount, getDecimalDigit());

            $smBankStatement = new SmBankStatement();
            $smBankStatement->amount = (float) round($smExpenseRequest->amount, getDecimalDigit());
    ...
    public function update(SmExpenseRequest $smExpenseRequest, $id)
    {
        ...
        */
        ...
        $add_expense->amount = (float) round($smExpenseRequest->amount, getDecimalDigit());
    ...
    public function destroy(Request $request)
    {
        ...
        */
        ...
        if ($add_expense->file && is_file($add_expense->file)) {
        ...
        if (paymentMethodName($add_expense->payment_method_id)) {
            ...
            $bank = SmBankAccount::where('id', $add_expense->account_id)
                ...
                ->first();
            if ($bank) {
                $bank->current_balance += $add_expense->amount;
                $bank->update();
            }
