...
class SmLibraryMemberController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        */
        ...
        $libraryMembers = branchWise(SmLibraryMember::with('roles', 'studentDetails', 'staffDetails', 'parentsDetails', 'memberTypes')
            ->where('active_status', '=', 1)
            ->where('school_id', $user->school_id)
            ->orderby('id', 'ASC')
            ->get());

        $roles = InfixRole::where('is_saas', 0)->where(function ($q) use ($user) {
    ...
    public function store(Request $request)
    {
        ...
        if (! empty($request->student)) {
            ...
            $student = SmStudent::where('id', $request->student)->first();
        ...
        */
        ...
        if (moduleStatusCheck('Branch')) {
            $members->branch_id = $request->branch_id;
        }
    ...
    {
        $data = $request->all();
        $student_records = branchWise(StudentRecord::with(['student', 'student.parents'])->where('un_session_id', $data['session_id'])
            ->where('un_session_id', $data['academic_id'])
            ->where('un_faculty_id', $data['faculty_id'])
            ->where('un_department_id', $data['dept_id'])
            ->where('un_semester_id', $data['semester_id'])
            ->where('un_semester_label_id', $data['semester_label'])
            ->where('un_section_id', $data['section_id'])
            ->get());

        $parents = [];
        foreach ($student_records as $stdRecord) {
            if (! empty($stdRecord) && ! empty($stdRecord->student) && ! empty($stdRecord->student->parents)) {
                $array = [
                    'id' => $stdRecord->student->parents->id,
                    'name' => $stdRecord->student->parents?->guardians_name,
                ];
                $parents[] = $array;
            }
        }
