...
class SmRoomListController extends Controller
{
    public function index(Request $request)
    {
        ...
        try {
        */
        $room_lists = branchWise(SmRoomList::with('dormitory', 'roomType')->get());
        $room_types = branchWise(SmRoomType::get());
        $dormitory_lists = branchWise(SmDormitoryList::orderby('id', 'DESC')->get());
    ...
    public function store(SmDormitoryRoomRequest $smDormitoryRoomRequest)
    {
        ...
        */
        ...
        $smRoomList->cost_per_bed = (float) round($smDormitoryRoomRequest->cost_per_bed, getDecimalDigit());
        $smRoomList->description = $smDormitoryRoomRequest->description;
        $smRoomList->school_id = Auth::user()->school_id;
        if (moduleStatusCheck('Branch')) {
            $smRoomList->branch_id = $smDormitoryRoomRequest->branch_id;
        }
    ...
    public function show(Request $request, $id)
    {
        ...
        */
        ...
        $room_lists = branchWise(SmRoomList::with('dormitory', 'roomType')->get());
        $room_types = branchWise(SmRoomType::get());
        $dormitory_lists = branchWise(SmDormitoryList::where('school_id', Auth::user()->school_id)->get());
    ...
    public function update(SmDormitoryRoomRequest $smDormitoryRoomRequest, $id)
    {
        ...
        */
        ...
        $room_list->cost_per_bed = (float) round($smDormitoryRoomRequest->cost_per_bed, getDecimalDigit());
        $room_list->description = $smDormitoryRoomRequest->description;
        if (moduleStatusCheck('Branch')) {
            $room_list->branch_id = $smDormitoryRoomRequest->branch_id;
        }
        if (moduleStatusCheck('University')) {
