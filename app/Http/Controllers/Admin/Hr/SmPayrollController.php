...
use App\Models\SmLeaveDeductionInfo;
...
class SmPayrollController extends Controller
{
    ...
    public function searchStaffPayr(Request $request)
    {
        ...
        */
        ...
        $roles = InfixRole::where('is_saas', 0)->where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where(function ($q): void {
            $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
        })->get();
    ...
    public function savePayrollData(Request $request)
    {
        ...
        try {
            ...
            $smHrPayrollGenerate->basic_salary = (float) round($request->basic_salary, getDecimalDigit());
            $smHrPayrollGenerate->total_earning = (float) round($request->total_earning, getDecimalDigit());
            $smHrPayrollGenerate->total_deduction = (float) round($request->total_deduction, getDecimalDigit());
            $smHrPayrollGenerate->gross_salary = (float) round($request->final_gross_salary, getDecimalDigit());
            $smHrPayrollGenerate->tax = (float) round($request->tax, getDecimalDigit());
            $smHrPayrollGenerate->net_salary = (float) round($request->net_salary, getDecimalDigit());
            ...
            if ($request->leave_deduction > 0) {
                ...
                $smLeaveDeductionInfo->salary_deduct = (float) round($request->leave_deduction, getDecimalDigit());
            ...
            if ($result) {
                ...
                for ($i = 0; $i < $earnings; $i++) {
                    ...
                    if (! empty($request->earningsType[$i]) && ! empty($request->earningsValue[$i])) {
                        ...
                        if ($request->earningsType[0] === 'lms_balance' && moduleStatusCheck('Lms') === true) {
                            $payable_amount = (float) round($request->earningsValue[0], getDecimalDigit());
                        ...
                        $payroll_earn_deducs->amount = (float) round($request->earningsValue[$i], getDecimalDigit());
                ...
                for ($i = 0; $i < $deductions; $i++) {
                    if (! empty($request->deductionstype[$i]) && ! empty($request->deductionsValue[$i])) {
                        ...
                        $payroll_earn_deducs->amount = (float) round($request->deductionsValue[$i], getDecimalDigit());
    ...
    public function paymentPayroll(Request $request, $id, $role_id)
    {
        ...
        try {
        */
        $chart_of_accounts = branchWise(SmChartOfAccount::where('type', 'E')
            ->where('school_id', Auth::user()->school_id)
            ->get());
    ...
    public function savePayrollPaymentData(Request $request)
    {
        ...
        $expenseHead = SmChartOfAccount::where('id', $request->expense_head_id)
            ->where('type', 'E')
            ->where('school_id', Auth::user()->school_id);

        if (moduleStatusCheck('Branch')) {
            branchWiseApplyFilter($expenseHead, 'branch_id', userBranch());
        }

        if (! $expenseHead->exists()) {
            Toastr::error('Invalid expense head', 'Failed');
        ...
        */
        ...
        $payrollPayment->amount = (float) round($request->submit_amount, getDecimalDigit());
        ...
        if ($result) {
            ...
            $smAddExpense->amount = (float) round($request->submit_amount, getDecimalDigit());
        ...
        if ($request->payment_mode === 3) {
            $bank = SmBankAccount::where('id', $request->bank_id)
                ->where('school_id', Auth::user()->school_id)
                ->first();
            $after_balance = $bank->current_balance - (float) round($request->submit_amount, getDecimalDigit());

            $smBankStatement = new SmBankStatement();
            $smBankStatement->amount = (float) round($request->submit_amount, getDecimalDigit());
        ...
        $data['roles'] = InfixRole::where('is_saas', 0)->where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where(function ($q): void {
            $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
        })->get();
    ...
    public function payrollReport(Request $request)
    {
        ...
        try {
        */
        $roles = InfixRole::select(['id', 'name'])->where('is_saas', 0)
            ->where('active_status', '=', '1')
            ->where('id', '!=', 1)
            ->where('id', '!=', 2)
            ->where('id', '!=', 3)
    ...
    public function searchPayrollReport(Request $request)
    {
        ...
        */
        ...
        $roles = InfixRole::select(['id', 'name'])->where('is_saas', 0)->where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where(function ($q): void {
            $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
        })->get();
