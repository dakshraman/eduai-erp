...
use App\Models\SmStaffAttendanceImport;
...
class SmStaffAttendanceController extends Controller
{
    ...
    public function staffAttendanceSearch(staffAttendanceSearchRequest $staffAttendanceSearchRequest)
    {
        ...
        */
        ...
        $staffs = branchWise(SmStaff::with('DateWiseStaffAttendance', 'roles')
        ...
        if (moduleStatusCheck('Branch')) {
            $staffs = $staffs
                ->where('branch_id', $staffAttendanceSearchRequest->branch_id)
                ->select(['last_name', 'first_name', 'id', 'staff_no', 'branch_id'])
                ->get();
        } else {
            $staffs = $staffs
        ...
        $branch_id = $staffAttendanceSearchRequest->branch_id ?? null;
        $date_modified = \Carbon\Carbon::createFromFormat('m/d/Y', $date)->format('Y-m-d');
        foreach ($staffs as $staff) {
            $leave_request = \App\Models\SmLeaveRequest::where('staff_id', $staff->staff_no)
                ->where('approve_status', 'A')
                ->whereDate('leave_from', '<=', $date_modified)
                ->whereDate('leave_to', '>=', $date_modified)
                ->when(moduleStatusCheck('Branch'), function ($query) use ($staffAttendanceSearchRequest): void {
                    $query->where('branch_id', $staffAttendanceSearchRequest->branch_id);
                })
                ->first();
            $staff->is_on_leave = ! empty($leave_request);
            $staff->leave_reason = $leave_request?->reason;
        }

        return view('backEnd.humanResource.staff_attendance', ['role_id' => $role_id, 'date' => $date, 'roles' => $roles, 'staffs' => $staffs, 'attendance_type' => $attendance_type, 'branch_id' => $branch_id]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    {
        $staffs = branchWise(SmStaff::where('role_id', $request->role_id)
    ...
    public function staffAttendanceReportSearch(staffAttendanceReportSearchRequest $staffAttendanceReportSearchRequest)
    {
        ...
        */
        ...
        $branch_id = $staffAttendanceReportSearchRequest->branch_id ?? null;
        $current_day = date('d');
        $user = Auth::user();
        ...
        $staffs = SmStaff::where('role_id', $role_id)
            ...
            ->when(moduleStatusCheck('Branch'), function ($query) use ($staffAttendanceReportSearchRequest): void {
                $query->where('branch_id', $staffAttendanceReportSearchRequest->branch_id);
            })
        ...
        $attendances = SmStaffAttendence::whereIn('staff_id', $staffIds)
            ...
            ->select(['id', 'attendence_type', 'staff_id', 'attendence_date', 'notes'])
            ->get()
            ->groupBy('staff_id'); // Grouping by staff ID to simplify processing in the view

        return view('backEnd.humanResource.staff_attendance_report', ['attendances' => $attendances, 'staffs' => $staffs, 'days' => $days, 'year' => $year, 'month' => $month, 'current_day' => $current_day, 'roles' => $roles, 'role_id' => $role_id, 'branch_id' => $branch_id]);
    ...
    public function staffAttendancePrint($role_id, $month, $year, $branch_id = null)
    {
        ...
        */
        ...
        return view('backEnd.humanResource.staff_attendance_print', ['attendances' => $attendances, 'days' => $days, 'year' => $year, 'month' => $month, 'current_day' => $current_day, 'roles' => $roles, 'role_id' => $role_id, 'role' => $role, 'staffs' => $staffs, 'branch' => $branch]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
