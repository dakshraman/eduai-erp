...
class SmVehicleController extends Controller
{
    public function index(Request $request)
    {
        ...
        try {
        */
        $drivers = branchWise(SmStaff::whereRole(9)->get());
        $assign_vehicles = branchWise(SmVehicle::where('school_id', Auth::user()->school_id)->get());
    ...
    public function store(SmVehicleRequest $smVehicleRequest)
    {
        try {
            ...
            if (moduleStatusCheck('Branch')) {
                $smVehicle->branch_id = $smVehicleRequest->branch_id;
            }
    ...
    public function update(SmVehicleRequest $smVehicleRequest, $id)
    {
        ...
        */
        ...
        if (moduleStatusCheck('Branch')) {
            $assign_vehicle->branch_id = $smVehicleRequest->branch_id;
        }
        if (moduleStatusCheck('University')) {
    ...
    {
        $tables = \App\Support\tableList::getTableList('vehicle_id', $id);
        /*
        try {
        */
