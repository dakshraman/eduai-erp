...
class SmAssignVehicleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        */
        ...
        $assign_vehicles = branchWise(SmAssignVehicle::with('route', 'vehicle')->where('school_id', Auth::user()->school_id)->get());
        $vehicles = branchWise(SmVehicle::where('school_id', Auth::user()->school_id)->get());
    ...
    public function store(SmAssignVehicleRequest $smAssignVehicleRequest)
    {
        ...
        */
        ...
        if (moduleStatusCheck('Branch')) {
            $smAssignVehicle->branch_id = $smAssignVehicleRequest->branch_id;
        }
        ...
        if (Schema::hasColumn('sm_assign_vehicles', 'un_academic_id')) {
            $smAssignVehicle->un_academic_id = getAcademicId();
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        try {
        */
        $routes = branchWise(SmRoute::get());
        $assign_vehicles = branchWise(SmAssignVehicle::with('route', 'vehicle')->where('school_id', Auth::user()->school_id)->get());
        $assign_vehicle = SmAssignVehicle::find($id);
        $vehiclesIds = explode(',', $assign_vehicle->vehicle_id);
        $vehicles = branchWise(SmVehicle::select('id', 'vehicle_no')->get());
    ...
    public function update(SmAssignVehicleRequest $smAssignVehicleRequest, $id)
    {
        ...
        */
        ...
        if (moduleStatusCheck('Branch')) {
            $assign_vehicle->branch_id = $smAssignVehicleRequest->branch_id;
        }
        if (moduleStatusCheck('University')) {
