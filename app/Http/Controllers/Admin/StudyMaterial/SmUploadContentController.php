...
class SmUploadContentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        */
        $user = auth()->user();
        $uploadContents = branchWise(SmTeacherUploadContent::query()->with('classes', 'sections'));
        if (teacherAccess()) {
            $uploadContents->where(function ($q) use ($user): void {
                $q->where('created_by', $user->id)->orWhere('available_for_admin', 1);
            });
        }

        $uploadContents = $uploadContents->where('school_id', $user->school_id)
        ...
        if (teacherAccess()) {
            $teacher_info = branchWise(SmStaff::where('user_id', $user->id)
                ->first());
            $classes = $teacher_info->classes;
        } else {
            $classes = branchWise(SmClass::select('sm_classes.id', 'sm_classes.class_name')->get());
        }

        return view('backEnd.teacher.uploadContentList', ['classes' => $classes, 'uploadContents' => $uploadContents]);
        /*
    ...
    public function store(Request $request)
    {
        ...
        if ($request->status !== 'lmsStudyMaterial') {
            ...
            if ($request->available_for && is_array($request->available_for)) {
                if (array_search('admin', $request->available_for, true) || $request->all_classes === 'on') {
                    $rules['class'] = 'sometimes|nullable';
                } elseif (moduleStatusCheck('University') === false && array_search('student', $request->available_for, true) && $request->all_classes !== 'on') {
            ...
            if ($request->filled('inside_study_material')) {
                ...
                if ($request->un_session_id) {
                    ...
                    if (is_null($request->un_section_id)) {
                        foreach ($sections as $section) {
                            ...
                            if (moduleStatusCheck('Branch')) {
                                $uploadContents->branch_id = $request->branch_id;
                            }
                            $uploadContents->save();
                        }
                    } else {
        ...
        foreach ($request->available_for as $value) {
            if ($value === 'admin') {
                ...
                foreach ($roles as $role) {
                    $query = SmStaff::where('role_id', $role->id)->where('school_id', Auth::user()->school_id);
                    if (moduleStatusCheck('Branch')) {
            ...
            if (($value === 'student') && ($request->status !== 'lmsStudyMaterial')) {
                if (property_exists($request, 'all_classes') && $request->all_classes !== null) {
                    $query = branchWise(StudentRecord::with('studentDetail', 'class', 'section'));
                    if (shiftEnable()) {
                        $query->with('shift');
                    }
                    if (moduleStatusCheck('Branch')) {
                    ...
                    foreach ($records as $record) {
                        ...
                        if (shiftEnable()) {
                            $data['shift'] = $record->shift->name ?? 'N/A';
                        }
                        if (moduleStatusCheck('Branch')) {
                            $data['branch'] = $record->branch->branch_name ?? 'N/A';
                        }
                    ...
                    $query = branchWise(StudentRecord::with('studentDetail', 'class', 'section')
                        ...
                        ->where('academic_id', getAcademicId()));
                    if (moduleStatusCheck('Branch')) {
                        $query->with('branch')->where('branch_id', $request->branch_id);
                    }
                    $records = $query->get();
    ...
    public function uploadContentEdit($id)
    {
        ...
        $contentTypes = branchWise(SmContentType::where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());

        if (teacherAccess()) {
            $uploadContents = branchWise(SmTeacherUploadContent::with('classes', 'sections')->where(function ($q): void {
                $q->where('created_by', Auth::user()->id)->orWhere('available_for_admin', 1);
            })->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
        } else {
            $uploadContents = branchWise(SmTeacherUploadContent::with('classes', 'sections')->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)
                ->get());
        }

        if (teacherAccess()) {
            $teacher_info = SmStaff::where('user_id', Auth::user()->id)
                ->with(['classes' => function ($query) {
                    $query->select('sm_classes.id', 'sm_classes.class_name');
                    if (moduleStatusCheck('Branch')) {
                        $query->addSelect('sm_classes.branch_id');
                    }
                }])
                ->first();
            $classes = $teacher_info?->classes ?? collect();
        } else {
    ...
    public function uploadContentView(Request $request, $id)
    {
        ...
        */
        ...
        return view('backEnd.teacher.uploadContentDetails', ['ContentDetails' => $ContentDetails]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function updateUploadContent(Request $request)
    {
        ...
        $uploadContents->available_for_admin = in_array('admin', $request->available_for) ? 1 : null;
        ...
        if (in_array('student', $request->available_for)) {
            if (property_exists($request, 'all_classes') && $request->all_classes !== null) {
                ...
                $uploadContents->section = $request->section;
                $uploadContents->shift_id = shiftEnable() ? $request->shift : null;
                $uploadContents->available_for_all_classes = 0;
            }
        } else {
            $uploadContents->class = null;
            $uploadContents->section = null;
            $uploadContents->shift_id = null;
            $uploadContents->available_for_all_classes = 0;
        }
        ...
        foreach ($request->available_for as $value) {
            if ($value === 'admin') {
                $roles = InfixRole::where('is_saas', 0)->where('id', '=', 1)->where(function ($q): void {
                    $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
                })->get();
                foreach ($roles as $role) {
                    $query = branchWise(SmStaff::where('role_id', $role->id)->where('school_id', Auth::user()->school_id));
                    if (moduleStatusCheck('Branch')) {
                        $query->where('branch_id', $request->branch_id);
                    }
                    $staffs = $query->get();
            ...
            if ($value === 'student') {
                if (property_exists($request, 'all_classes') && $request->all_classes !== null) {
                    $query = branchWise(SmStudent::select('id', 'user_id')->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id));
                    if (moduleStatusCheck('Branch')) {
                        $query->where('branch_id', $request->branch_id);
                    }
                    $students = $query->get();
                    ...
                    $student_ids = branchWise(StudentRecord::where('class_id', $request->class)
                        ...
                        ->when(moduleStatusCheck('Branch'), function ($q) use ($request) {
                    ...
                    $students_query = branchWise(SmStudent::select('id')->whereIn('id', $student_ids));
                    if (moduleStatusCheck('Branch')) {
                        $students_query->where('branch_id', $request->branch_id);
                    }
        ...
        if ($results) {
            ...
            if ($request->content_type === 'sy') {
                return redirect()->route('syllabus-list');
            }

            if ($request->content_type === 'st') {
    ...
    public function assignmentList(Request $request)
    {
        ...
        */
        ...
        if (! teacherAccess()) {
            ...
            $uploadContents = branchWise(SmTeacherUploadContent::where(function ($q): void {
                $q->where('created_by', Auth::user()->id)->orWhere('available_for_admin', 1);
    ...
    public function studyMetarialList(Request $request)
    {
        ...
        */
        if (teacherAccess()) {
            ...
            $uploadContents = branchWise(SmTeacherUploadContent::where('content_type', 'st')
                ->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)
                ->get());
        }
    ...
    public function syllabusList(Request $request)
    {
        ...
        */
        if (teacherAccess()) {
            ...
            $uploadContents = branchWise(SmTeacherUploadContent::with('classes', 'sections')
                ->where('content_type', 'sy')
                ->where('course_id', '=', null)
                ->where('chapter_id', '=', null)
                ->where('lesson_id', '=', null)
    ...
    public function deleteUploadContent(Request $request)
    {
        ...
        */ $id = $request->id;
        ...
        \Log::info('deleteUploadContent test: id=' . $id . ', Auth school=' . Auth::user()->school_id . ', content=' . ($uploadContent ? $uploadContent->school_id : 'null'));
