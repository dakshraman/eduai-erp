...
class SmStudentAjaxController extends Controller
{
    ...
    public function ajaxSiblingInfo(Request $request)
    {
        ...
        */
        ...
        $records->when($request->filled('class_id'), function ($u_query) use ($request): void {
    ...
    public function ajaxRoomDetails(Request $request)
    {
        ...
        */
        ...
        $roomQuery = SmRoomList::where('dormitory_id', '=', $request->id)->where('school_id', $school_id);

        if (moduleStatusCheck('Branch') && $request->filled('branch_id')) {
            $roomQuery = branchWiseApplyFilter($roomQuery, 'branch_id', $request->branch_id);
        }

        $room_details = $roomQuery->get();
        $rest_rooms = [];
        foreach ($room_details as $room_detail) {
            $count_room = SmStudent::where('room_id', $room_detail->id)
    ...
    public function ajaxStudentPromoteSection(Request $request)
    {
        if ($request->parent) {
            ...
            if (teacherAccess()) {
                ...
                $sectionQuery = SmClassSection::where('class_id', $request->id)
                    ->when($request->shift_id, function ($query) use ($request) {
                        $query->where('shift_id', $request->shift_id);
                    })
                    ->where('school_id', Auth::user()->school_id)
                ...
                if (moduleStatusCheck('Branch') && $request->branch_id) {
                    $sectionQuery = branchWiseApplyFilter($sectionQuery, 'branch_id', $request->branch_id);
                }

                $sectionIds = $sectionQuery->get();
            }
    ...
    public function ajaxSelectStudent(Request $request)
    {
        ...
        $students = SmStudent::with('parents')->whereIn('id', $student_ids)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get()->map(function ($student): array {
            return [
                'id' => $student->id,
        ...
        */
        ...
        if (teacherAccess()) {
            $sectionIds = SmAssignSubject::withOutGlobalScope(StatusAcademicSchoolScope::class)->where('class_id', '=', $request->id)
                ->where('teacher_id', Auth::user()->staff->id)
                ->where('school_id', Auth::user()->school_id)
                ->when($class, function ($q) use ($class): void {
