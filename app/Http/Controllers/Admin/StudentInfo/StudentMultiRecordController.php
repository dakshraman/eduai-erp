...
class StudentMultiRecordController extends Controller
{
    ...
    public function multiRecord(Request $request)
    {
        ...
        if (! empty($request->all())) {
            ...
            })->when($request->branch_id && moduleStatusCheck('Branch') && Schema::hasColumn('student_records', 'branch_id'), function ($q) use ($request): void {
                branchWiseApplyFilter($q, 'branch_id', $request->branch_id);
            })->pluck('student_id')->toArray();
            $students = branchWise(SmStudent::whereIn('id', $record_student_ids)->where('active_status', 1)->get());
        }
        ...
        return view('backEnd.studentInformation.multi_class_student', ['shift' => $shifts, 'sessions' => $sessions, 'students' => $students, 'classes' => $classes, 'data' => $data, 'academic_year' => $academic_year, 'class_id' => $class_id, 'section_id' => $section_id, 'shift_id' => $shift_id, 'student_id' => $student_id, 'un_session_id' => $un_session_id, 'un_faculty_id' => $un_faculty_id, 'un_department_id' => $un_department_id, 'un_academic_id' => $un_academic_id, 'un_semester_id' => $un_semester_id, 'un_semester_label_id' => $un_semester_label_id, 'un_section_id' => $un_section_id, 'branch_id' => $branch_id]);
    }
    ...
    public function multiRecordStore(Request $request)
    {
            ...
            if ($request->new_record) {
                foreach ($request->new_record as $row_id => $newRecord) {
                    $is_default = ((string) $row_id === (string) $default_id) ? 1 : null;
            ...
            if ($request->old_record) {
                foreach ($request->old_record as $record_id => $oldRecord) {
                    $is_default = (string) $record_id === (string) $default_id ? 1 : null;
            ...
            return response()->json([
                'status' => false,
                'message' => $validation ?: __('student.Record info updated Failed'),
    ...
    public function insertStudentRecord($request, $pre_record = null): void
    {
        ...
        if (moduleStatusCheck('Branch') && Schema::hasColumn('student_records', 'branch_id')) {
            $studentRecord->branch_id = $request->branch_id ?: SmStudent::where('school_id', Auth::user()->school_id)
    ...
    {
        $studentRecords = branchWise(StudentRecord::with([
    ...
    private function checkExitRecord(Request $request)
    {
        ...
        $exit = StudentRecord::where('class_id', $request->class)
            ...
            ->where('academic_id', $request->session ?? getAcademicId())
            ->when($request->branch_id && moduleStatusCheck('Branch') && Schema::hasColumn('student_records', 'branch_id'), function ($q) use ($request): void {
                branchWiseApplyFilter($q, 'branch_id', $request->branch_id);
            })
            ->where('school_id', auth()->user()->school_id)
