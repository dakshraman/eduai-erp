...
class SmStudentAdmissionController extends Controller
{
    ...
    public function store(SmStudentAdmissionRequest $smStudentAdmissionRequest)
    {
        ...
        if ($smStudentAdmissionRequest->filled('phone_number') || $smStudentAdmissionRequest->filled('email_address')) {
            ...
            if ($user && $user->role_id === 2) {
                if (moduleStatusCheck('University')) {
                    ...
                    $studentRecord = StudentRecord::where('branch_id', $smStudentAdmissionRequest->branch_id)
        ...
        try {
            ...
            if (moduleStatusCheck('Branch')) {
                $user_stu->branch_id = $smStudentAdmissionRequest->branch_id;
            }
            $user_stu->save();
            $user_stu->toArray();
            ...
            if (! $smStudentAdmissionRequest->parent_id) {
                ...
                if ($smStudentAdmissionRequest->filled('guardians_phone') || $smStudentAdmissionRequest->filled('guardians_email')) {
                    ...
                    if (! $staff) {
                        ...
                        if (moduleStatusCheck('Branch')) {
                            $user_parent->branch_id = $smStudentAdmissionRequest->branch_id;
                        }
                        $user_parent->save();
                        $user_parent->toArray();
                    }
                ...
                if ($parentInfo && ! $smStudentAdmissionRequest->staff_parent) {
                    ...
                    if (moduleStatusCheck('Branch')) {
                        $parent->branch_id = $smStudentAdmissionRequest->branch_id;
                    }
                    $parent->save();
                    $parent->toArray();
                    $hasParent = $parent->id;
            ...
            if (moduleStatusCheck('Branch')) {
                $smStudent->branch_id = $smStudentAdmissionRequest->branch_id;
            }
            ...
            $this->fillStudentDocuments($smStudent, $smStudentAdmissionRequest, $destination);
            ...
            foreach ($existingLeaveDefines as $existingLeaveDefine) {
                if (! isset($existingTypes[$existingLeaveDefine->type_id])) {
                    ...
                    if (moduleStatusCheck('Branch')) {
                        $leaveDefineInstance->branch_id = $smStudentAdmissionRequest->branch_id;
                    }
    ...
    {
        $parentInfo = $request->fathers_name || $request->fathers_phone || $request->fathers_occupation || $request->mothers_name || $request->mothers_phone || $request->mothers_occupation || $request->guardians_name || $request->guardians_email || $request->guardians_phone || $request->guardians_occupation || $request->relation;
        ...
        try {
            ...
            $sibling_id = (int)$request->sibling_id;
            if (($sibling_id === 0 || $sibling_id === 1) && ! $request->parent_id) {
            ...
            } elseif ($sibling_id === 0 && $request->parent_id) {
                User::destroy($student_detail->parents->user_id);
            } elseif (($sibling_id === 2 || $sibling_id === 1) && $request->parent_id) {
            } elseif ($sibling_id === 2 && ! $request->parent_id) {
                $username = $request->guardians_phone ?: $request->guardians_email;
                $phone_number = $request->guardians_phone;
                if ($request->guardians_phone || $request->guardians_email) {
            ...
            if ($sibling_id === 0 && $request->parent_id) {
                SmParent::destroy($student_detail->parent_id);
            } elseif (($sibling_id === 2 || $sibling_id === 1) && $request->parent_id) {
            } elseif ($parentInfo || $student_detail->parent_id) {
                if (($sibling_id === 0 || $sibling_id === 1) && ! $request->parent_id) {
                    // when find parent
                    $parent = $student_detail->parent_id ? SmParent::find($student_detail->parent_id) : new SmParent();
                } elseif ($sibling_id === 2 && ! $request->parent_id) {
                    $parent = new SmParent();
                }
            ...
            if (($sibling_id === 0 || $sibling_id === 1) && $request->parent_id== '') {
                ...
                $student->parent_id = $request->parent_id;
            } elseif ($sibling_id === 2 && $request->parent_id== '' && $parent !== null) {
            ...
            $this->fillStudentDocuments($student, $request, $destination, true);
            if ($request->customF) {
    ...
    public function update(SmStudentAdmissionRequest $smStudentAdmissionRequest)
    {
        ...
        $updateResult = $this->updateStudentInfo($smStudentAdmissionRequest, $studentRecord);
        if ($updateResult instanceof \Illuminate\Http\RedirectResponse) {
            return $updateResult;
        }
    ...
    public function studentDetails(Request $request)
    {
        ...
        */
        ...
        if (generalSetting()->student_grid_view === 1) {
            $user = auth()->user();
            $records = StudentRecord::query()
                ->where('is_promote', 0)
                ->when(moduleStatusCheck('University'), function ($query) use ($academic_year) {
                    $query->where('un_academic_id', $academic_year);
                }, function ($query) use ($academic_year) {
                    $query->where('academic_id', $academic_year);
                })
            ...
            $all_students = SmStudent::with([
                'studentRecords' => function ($query) use ($academic_year): void {
                    if (moduleStatusCheck('University')) {
                        $query->where('un_academic_id', $academic_year);
                    } else {
                        $query->where('academic_id', $academic_year);
            ...
            if ($user->role_id === 4 && ! userPermission('student.show-all-student')) {
                ...
                $all_students->whereHas('studentRecords', function ($q) use ($assignClassTeacher) {
        ...
        return view('backEnd.studentInformation.student_details', compact('classes', 'sessions', 'academic_year', 'branch_id', 'class_id', 'section_id', 'shift_id', 'name', 'roll_no'));
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function insertStudentRecord($request, $pre_record = null): void
    {
        ...
        if (moduleStatusCheck('Branch')) {
            $studentRecord->branch_id = $request->branch_id ?: userBranch();
        }
    ...
    public function recordEdit($student_id, $record_id)
    {
        ...
        $data['sessions'] = SmAcademicYear::get(['id', 'year', 'title']);
        $data['student_records'] = branchWise(StudentRecord::where('student_id', $student_id)->get());
        $data['student_detail'] = SmStudent::where('id', $student_id)->first();
        $data['classes'] = branchWise(SmClass::get());
        $data['siblings'] = branchWise(SmStudent::where('parent_id', $data['student_detail']->parent_id)->where('id', '!=', $student_id)->status()->withoutGlobalScope(StatusAcademicSchoolScope::class)->get());
        if (moduleStatusCheck('University')) {
    ...
    public function studentBulkStore(Request $request)
    {
        ...
        if ($step === 'upload') {
            ...
            if (moduleStatusCheck('University')) {
                return view('backEnd.partials.student-import._map', ['filteredHeaders' => $filteredHeaders, 'expectedHeaders' => $expectedHeaders, 'file' => $file, 'session_id' => $session_id, 'faculty_id' => $faculty_id, 'dept_id' => $dept_id, 'academic_id' => $academic_id, 'semester_id' => $semester_id, 'semester_label_id' => $semester_label_id, 'section_id' => $section_id, 'branch_id' => $branch_id, 'branch' => $branch]);
            }

            return view('backEnd.partials.student-import._map', ['filteredHeaders' => $filteredHeaders, 'expectedHeaders' => $expectedHeaders, 'file' => $file, 'session' => $session, 'class' => $class, 'section' => $section, 'shift' => $shift, 'branch_id' => $branch_id, 'branch' => $branch]);
        ...
        if ($step === 'map') {
            ...
            return view('backEnd.partials.student-import._import', ['expectedHeaders' => $expectedHeaders, 'allData' => $allData, 'mappedHeaders' => $mappedHeaders, 'url' => $url, 'requiredHeaders' => $requiredHeaders, 'session' => $session, 'class' => $class, 'section' => $section, 'shift' => $shift, 'branch_id' => $branch_id, 'branch' => $branch]);
            ...
            try {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    if (isSubscriptionEnabled()) {
                        ...
                        if (SmPackagePlan::staff_limit() <= $active_student && saasDomain() !== 'school') {
                            DB::rollBack();
                    ...
                    if (! empty($guardian_email) || ! empty($request->guardian_phone[$i])) {
                        ...
                        if (moduleStatusCheck('Branch')) {
                            $guardianUser->branch_id = $branch_id;
                        }
                        $guardianUser->save();
    ...
    //         $shcool_details = SmGeneralSettingss::where('school_id', auth()->user()->school_id)->first();
    ...
    {
        $request->validate([
            'student_grid_view' => 'required|boolean',
        ]);

        try {
            $setting = SmGeneralSettings::first();
            if ($setting) {
                $setting->student_grid_view = $request->student_grid_view;
                $setting->save();
            }
    ...
    public function studentGrid(Request $request)
    {
        ...
        if (moduleStatusCheck('University')) {
            $records
                ->when($request->filled('un_academic_id'), fn ($q) => $q->where('un_academic_id', $request->un_academic_id))
                ->when($request->filled('un_faculty_id'), fn ($q) => $q->where('un_faculty_id', $request->un_faculty_id))
                ->when($request->filled('un_department_id'), fn ($q) => $q->where('un_department_id', $request->un_department_id))
                ->when($request->filled('un_session_id'), fn ($q) => $q->where('un_session_id', $request->un_session_id))
                ->when($request->filled('un_semester_label_id'), fn ($q) => $q->where('un_semester_label_id', $request->un_semester_label_id));
        } else {
            $records
                ->when(! $request->academic_year, fn ($q) => $q->where('academic_id', getAcademicId()), fn ($q) => $q->where('academic_id', $request->academic_year))
        ...
        if (generalSetting()->multiple_roll && $request->roll_no) {
            $records->where('roll_no', 'like', '%'.$request->roll_no.'%');
        }

        if ($request->quick_search) {
            $keyword = $request->quick_search;
            $records->whereHas('student', function ($q) use ($keyword) {
        ...
        $student_ids = $records->whereHas('student')->pluck('student_id')->unique()->values();
        ...
        if (! generalSetting()->multiple_roll && $request->roll_no) {
            $all_students->where('roll_no', 'like', '%'.$request->roll_no.'%');
        }

        if ($user->role_id === 4 && ! userPermission('student.show-all-student')) {
            $classTeacherIds = SmClassTeacher::where('teacher_id', $user->staff->id)
                ->pluck('assign_class_teacher_id');
            $assignClassTeacher = SmAssignClassTeacher::whereIn('id', $classTeacherIds)->get();
            $all_students->whereHas('studentRecords', function ($q) use ($assignClassTeacher) {
                $q->whereIn('class_id', $assignClassTeacher->pluck('class_id'))
                    ->whereIn('section_id', $assignClassTeacher->pluck('section_id'));
            });
        ...
        if ($request->ajax()) {
            return view('backEnd.studentInformation.partial.student-grid', compact('all_students'))->render();
        }
    ...
    private function fillStudentDocuments(SmStudent $student, Request $request, string $destination, bool $isUpdate = false): void
    {
        for ($i = 1; $i <= 4; $i++) {
            $titleField = "document_title_{$i}";
            $fileField = "document_file_{$i}";

            if (! is_show($fileField)) {
                continue;
            }

            $student->{$titleField} = $request->input($titleField);
            $student->{$fileField} = $isUpdate
                ? fileUpdate($student->{$fileField}, $request->file($fileField), $destination)
                : fileUpload($request->file($fileField), $destination);
        }
    }
