...
use App\Models\SmAdmissionQuery;
...
class SmAdmissionQueryController extends Controller
{
    public function index()
    {
        ...
        try {
        */
        $classes = branchWise(SmClass::query())->get();
        $references = branchWise(SmSetupAdmin::where('type', 4)->get());
        $sources = branchWise(SmSetupAdmin::where('type', 3)->get());
    ...
    public function store(SmAdmissionQueryRequest $smAdmissionQueryRequest)
    {
        ...
        */
        ...
        if (moduleStatusCheck('Branch')) {
            $smAdmissionQuery->branch_id = $smAdmissionQueryRequest->branch_id;
        }
    ...
    public function edit($id)
    {
        ...
        */
        $data = [];
        $admission_query = branchWise(SmAdmissionQuery::find($id));
    ...
    public function update(SmAdmissionQueryRequest $smAdmissionQueryRequest)
    {
        ...
        */
        ...
        if (moduleStatusCheck('Branch')) {
            $admission_query->branch_id = $smAdmissionQueryRequest->branch_id;
        }
    ...
    public function addQuery($id)
    {
        ...
        */
        ...
        $follow_up_lists = branchWise(SmAdmissionQueryFollowup::where('academic_id', getAcademicId())->where('admission_query_id', $id)->orderby('id', 'DESC')->get());
        $classes = branchWise(SmClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
        $references = branchWise(SmSetupAdmin::where('type', 4)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get());
        $sources = branchWise(SmSetupAdmin::where('type', 3)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get());
    ...
    public function admissionQuerySearch(SmAdmissionQuerySearchRequest $smAdmissionQuerySearchRequest)
    {
        ...
        */
        ...
        $requestData['branch_id'] = $smAdmissionQuerySearchRequest->branch_id;
        ...
        $branch_id = $smAdmissionQuerySearchRequest->branch_id;
        $classes = branchWise(SmClass::get());
        $references = branchWise(SmSetupAdmin::where('type', 4)->get());
        $sources = branchWise(SmSetupAdmin::where('type', 3)->get());

        return view('backEnd.admin.admission_query', ['requestData' => $requestData, 'references' => $references, 'classes' => $classes, 'sources' => $sources, 'date_from' => $date_from, 'date_to' => $date_to, 'source_id' => $source_id, 'status_id' => $status_id, 'branch_id' => $branch_id]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function admissionQueryDatatable(Request $request)
    {
        ...
        */
        if ($request->ajax()) {
            ...
            $admission_queries = branchWise(SmAdmissionQuery::query());
            if (moduleStatusCheck('Branch') && userBranch() !== 0) {
                $admission_queries->where('branch_id', userBranch());
            }
            if (moduleStatusCheck('Branch')) {
                $admission_queries->with('sourceSetup', 'className', 'user', 'referenceSetup', 'branch')->orderBy('id', 'DESC');
            } else {
            ...
            if (moduleStatusCheck('Branch')) {
                if ($request->branch_id) {
                    $admission_queries->where('branch_id', $request->branch_id);
                }
            }
            ...
            return DataTables::of($admission_queries)
                ...
                ->addColumn('action', function ($row): string {
                    $action = '<a class="primary-btn small fix-gr-bg icon-only ml-10  l-h-30" href="'.route('add_query', [$row->id]).'">
                                        <i class="fa fa-plus"></i>
                                    </a>';

                    if (userPermission('admission_query_edit')) {
                        $action .= '<a class="primary-btn small fix-gr-bg icon-only ml-10  l-h-30" href="'.route('admission_query_edit', [$row->id]).'">
