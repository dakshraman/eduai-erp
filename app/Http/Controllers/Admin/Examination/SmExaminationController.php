...
class SmExaminationController extends Controller
{
    ...
    public function resultsArchiveView()
    {
        ...
        $exam_types = branchWise(SmExamType::where('school_id', Auth::user()->school_id)->get());
        $classes = branchWise(SmClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
    ...
    public function exam_type()
    {
        ...
        if (teacherAccess()) {
            $teacher_info = SmStaff::where('user_id', Auth::user()->id)->first();
            $classes = branchWise(SmAssignSubject::where('teacher_id', $teacher_info->id)->join('sm_classes', 'sm_classes.id', 'sm_assign_subjects.class_id')
                ->where('sm_assign_subjects.academic_id', getAcademicId())
                ->where('sm_assign_subjects.active_status', 1)
                ->where('sm_assign_subjects.school_id', Auth::user()->school_id)
                ->select('sm_classes.id', 'class_name')
        ...
        $exams_types = branchWise(SmExamType::where('active_status', 1)
            ->where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)
            ->get());
    ...
    public function exam_type_update(ExamTypeStoreRequest $request)
    {
        DB::beginTransaction();
        try {
            if (checkAdmin()) {
            ...
            if (moduleStatusCheck('Branch')) {
                $update_exame_type->branch_id = $request->branch_id ?: userBranch();
            }
    ...
    public function exam_type_delete(Request $request, $id)
    {
        ...
        $type = \App\Support\tableList::getTableList($id_key, $id);
        $term = \App\Support\tableList::getTableList($term_key, $id);
        $tables = $type.''.$term;
    ...
    public function examAttendance()
    {
        ...
        $subjects = branchWise(SmSubject::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
    ...
    public function examAttendanceAeportSearch(SmExamAttendanceSearchRequest $request)
    {
        ...
        if (moduleStatusCheck('University')) {
            ...
            return view('backEnd.examination.exam_attendance', compact(
                ...
                'un_section',
                'subjectName',
                'un_session_id', 'un_faculty_id', 'un_department_id', 'un_academic_id', 'un_semester_id', 'un_semester_label_id', 'un_section_id', 'branch_id'
            ))->with($data);
        ...
        if (! empty($request->shift)) {
            $exam_attendance->where('shift_id', $request->shift);
        }
        ...
        $exams = branchWise(SmExamType::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
        if (teacherAccess()) {
            $teacher_info = SmStaff::where('user_id', Auth::user()->id)->first();
            $classes = branchWise(SmAssignSubject::where('teacher_id', $teacher_info->id)->join('sm_classes', 'sm_classes.id', 'sm_assign_subjects.class_id')
                ->where('sm_assign_subjects.academic_id', getAcademicId())
        ...
        return view('backEnd.examination.exam_attendance', compact('exam_id', 'class_id', 'section_id', 'shift_id', 'subject_id', 'exams', 'classes', 'subjects', 'exam_attendance_childs', 'branch_id'));
    ...
    public function examAttendanceSearch(Request $request)
    {
        ...
        if ($request->shift !== null) {
            $exam_schedules->where('shift_id', $request->shift);
        }
        ...
        return view('backEnd.examination.exam_attendance_create', compact('exams', 'classes', 'subjects', 'students', 'exam_id', 'subject_id', 'class_id', 'section_id', 'shift_id', 'exam_attendance_childs', 'search_info'));
    ...
    public function universitySendSms($request)
    {
        $request->validate([
    ...
    public function sendMarksBySmsStore(Request $request)
    {
        ...
        $students = branchWise(StudentRecord::where('class_id', $request->class)
            ...
            ->when($request->branch_id, function ($query, $shift) {
                return $query->where('branch_id', $shift);
            })
            ->with('student')
            ->get());
    ...
    public function meritListReportSearch(MeritListReportRequest $request)
    {
        ...
        try {
            ...
            if ($request->method() === 'POST') {
                ...
                $InputShiftId = shiftEnable() ? $request->shift : '';
                ...
                $is_data = DB::table('sm_mark_stores')
                    ->where('class_id', $InputClassId)
                    ->where('section_id', $InputSectionId)
                    ->where('exam_term_id', $InputExamId)
                    ->when(shiftEnable() && ! empty($InputShiftId), function ($query) use ($InputShiftId) {
                        return $query->where('shift_id', $InputShiftId);
                    })
                    ->first();
                if (empty($is_data)) {
                    Toastr::error('Your result is not found!', 'Failed');
                ...
                $examSubjects = SmExam::where([
                    ['exam_type_id', $InputExamId],
                    ['section_id', $InputSectionId],
                    ['class_id', $InputClassId],
                ])
                    ->when(shiftEnable(), function ($query) use ($InputShiftId) {
                        return $query->where('shift_id', $InputShiftId);
                    })
                    ->where('school_id', Auth::user()->school_id)
                    ->where('academic_id', getAcademicId())
                ...
                foreach ($eligible_students as $SingleStudent) {
                    foreach ($eligible_subjects as $subject) {
                        ...
                        $subject_total_mark += subjectFullMark($InputExamId, $subject->subject_id, $InputClassId, $InputSectionId, $InputShiftId);

                        $subject_total_mark = SmExam::where('class_id', $InputClassId)->where('section_id', $InputSectionId)
                            ->when(shiftEnable(), function ($query) use ($InputShiftId) {
                                $query->where('shift_id', $InputShiftId);
                            })
                            ->where('exam_type_id', $InputExamId)
                    ...
                    $insert_results->class_id = $InputClassId;
                    $insert_results->exam_id = $InputExamId;
                    $insert_results->shift_id = $InputShiftId;
                    $insert_results->created_at = YearCheck::getYear().'-'.date('m-d h:i:s');
                    $insert_results->school_id = Auth::user()->school_id;
                    $insert_results->academic_id = getAcademicId();
                    $insert_results->save();
                ...
                if (shiftEnable() && $InputShiftId) {
                    $shift = Shift::find($InputShiftId);
                }
                ...
                $baseQuery = SmTemporaryMeritlist::where('exam_id', $InputExamId)
                    ->where('academic_id', getAcademicId())
                    ->where('school_id', Auth::user()->school_id)
                ...
                if ($meritListSettings === 'total_grade') {
                    $allresult_data = (clone $baseQuery)
                ...
                if (empty($optional_subject_setup)) {
                    return view('backEnd.reports.merit_list_report_normal', compact('iid', 'exams', 'classes', 'subjects', 'class', 'section', 'exam', 'subjectlist', 'allresult_data', 'class_name', 'assign_subjects', 'exam_name', 'InputClassId', 'InputExamId', 'InputSectionId', 'InputShiftId', 'optional_subject_setup', 'failgpaname', 'subject_total_mark', 'exam_content', 'shift'));
                }

                return view('backEnd.reports.merit_list_report', compact('iid', 'exams', 'classes', 'subjects', 'class', 'section', 'exam', 'subjectlist', 'allresult_data', 'class_name', 'assign_subjects', 'exam_name', 'InputClassId', 'InputExamId', 'InputSectionId', 'InputShiftId', 'optional_subject_setup', 'failgpaname', 'failgpa', 'maxGpa', 'exam_content', 'shift'));
    ...
    public function meritListPrint($exam_id, $class_id, $section_id, $shift_id = null)
    {
        set_time_limit(2700);
        ...
        $examSubjects = SmExam::where([['section_id', $InputSectionId], ['class_id', $InputClassId], ['class_id', $InputClassId]])
            ->when(shiftEnable() && ! empty($InputShiftId), function ($query) use ($InputShiftId) {
                return $query->where('shift_id', $InputShiftId);
            })
            ->where('school_id', Auth::user()->school_id)
        ...
        $subject_total_mark = subjectFullMark($InputExamId, $subject, $class->id, $section->id, $InputShiftId);
        ...
        // $allresult_data = SmTemporaryMeritlist::orderBy('merit_order', 'asc')->where('academic_id', getAcademicId())->where('school_id',Auth::user()->school_id)->get();
        $subjectlist = $allresult_dat ? explode(',', $allresult_dat->subjects_string) : [];

        return view('backEnd.reports.merit_list_report_print', compact('exams', 'classes', 'shift', 'subjects', 'class', 'section', 'exam', 'subjectlist', 'allresult_data', 'class_name', 'assign_subjects', 'exam_name', 'optional_subject_setup', 'subject_total_mark', 'failgpa', 'maxGpa', 'failgpaname', 'InputClassId', 'InputExamId', 'InputSectionId', 'exam_content'));
    ...
    public function markSheetReportStudent(Request $request)
    {
        ...
        $exams = branchWise(SmExamType::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
        $classes = branchWise(SmClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
        $branch_id = moduleStatusCheck('Branch') ? ($request->branch_id ?? userBranch()) : null;

        return view('backEnd.reports.mark_sheet_report_student', compact('exams', 'classes', 'branch_id'));
    ...
    public function markSheetReportStudentSearch(MarkSheetReportRequest $request)
    {
        ...
        $input['shift_id'] = shiftEnable() ? $request->shift : '';
        $input['student_id'] = $request->student;
        $branch_id = moduleStatusCheck('Branch') ? $request->branch_id : null;
        $user = Auth::user();
        if (moduleStatusCheck('University')) {
            ...
            $student_detail = universityFilter($StudentRecord, $request)
                ...
                ->with(['student', 'unDepartment', 'unAcademic', 'unSemester', 'unSemesterLabel', 'unFaculty', 'unSection'])
                ->first();
            ...
            $request_data = $request->except(['_token', 'url']);
        ...
        if ($exam_content) {
            $total_class_day = SmStudentAttendance::whereBetween('attendance_date', [$exam_content->start_date, $exam_content->end_date])
                ...
                ->when(shiftEnable(), function ($query) use ($input) {
                    return $query->where('shift_id', $input['shift_id']);
                })
                ->where('academic_id', getAcademicId())
                ->where('school_id', $user->school_id)
        ...
        $examSubjects = SmExam::where('exam_type_id', $request->exam)
            ->where('section_id', $request->section)
            ->when(! empty($request->shift) && shiftEnable(), function ($query) use ($request) {
        ...
        $mark_sheet = SmResultStore::where('class_id', $request->class)
            ->where('exam_type_id', $request->exam)->where('section_id', $request->section)
        ...
        $is_result_available = SmResultStore::where('class_id', $request->class)
            ->where('section_id', $request->section)
            ->where('student_id', $request->student)
            ->where('created_at', 'LIKE', '%'.YearCheck::getYear().'%')
            ->where('school_id', $user->school_id)
        ...
        return view('backEnd.reports.mark_sheet_report_student', compact(
            ...
            'failgpaname',
            'exam_type_id',
            'section_id', 'student_id', 'branch_id', 'exam_content', 'total_class_days', 'student_attendance', 'optional_subject_setup'));
    ...
    public function percentMarkSheetReport(PercentMarkSheetReportRequest $request)
    {
        ...
        $branch_id = $request->filled('branch_id') ? $request->branch_id : (moduleStatusCheck('Branch') ? userBranch() : null);
        $exams = SmExamType::query()
        ...
        if (shiftEnable() && $request->shift) {
            $shiftInfo = Shift::find($request->shift);
        }
        $data = [];
        if (moduleStatusCheck('University')) {
        ...
        if ($exam_rule) {
            ...
            return view('backEnd.examination.report.marksheetReport', compact('exams', 'classes', 'mark_sheet', 'examInfo', 'subjectInfo', 'classInfo', 'sectionInfo', 'shiftInfo', 'pass_mark', 'exam_rule', 'data', 'exam_id', 'class_id', 'section_id', 'shift_id', 'subject_id', 'branch_id'));
        }
        $mark_sheet = SmResultStore::query();
        $mark_sheet->where('exam_type_id', $request->exam_type);
    ...
    public function percentMarksheetPrint(Request $request)
    {
        ...
        return view('backEnd.examination.report.marksheetReportPrint', compact('mark_sheet', 'examInfo', 'subjectInfo', 'classInfo', 'sectionInfo', 'pass_mark', 'exam_rule', 'data', 'shiftInfo'));

    }

    public function unmarksheetPrint(Request $request)
    {
        try {
            $user = Auth::user();
            $exam_type = $request->exam_type;
            ...
            return view('university::exam.marksheetReportPrint', compact(
                'mark_sheet',
                'request_data',
                'student_attendance',
                'total_class_days',
