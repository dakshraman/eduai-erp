...
class SmExamFormatSettingsController extends Controller
{
    ...
    public function examReportPositionStore(Request $request)
    {
        ...
        if (! empty($studenInfos)) {
            ...
            $last_position = 1;
            $last_gpa = null;
            $last_total_mark = null;

            foreach ($allStudentMarks as $index => $allStudentMark) {
                if ($allStudentMark->gpa === $last_gpa && $allStudentMark->total_mark === $last_total_mark) {
                    // Same GPA and total mark, use same position
                    $current_position = $last_position;
                } else {
                    // New GPA/marks, update position to current index + 1
                ...
                $new_position = ExamMeritPosition::where('id', $allStudentMark->id)->first();
                $new_position->position = $current_position;
                $new_position->save();

                // Prepare for next iteration
                $last_gpa = $allStudentMark->gpa;
                $last_total_mark = $allStudentMark->total_mark;
                $position++;
            }
        }
        ...
        try {
            ...
            if (count($studenInfos) > 0) {
                ...
                foreach ($students as $student) {
                    ...
                    $totalMark = 0;
                    $subjectGpa = 0;
                    $single_gpa_summation = 0;
                    $total_gpa = 0;
                    $total_subject_avarage_gpa = 0;
                    foreach ($allMarks as $allMark) {
                        ...
                        $subject_avarage_mark = round($subjectMark / $examTerm, 2);
                        $subject_gpa = markGpa($subject_avarage_mark)->gpa;
                        $subjectGpa += markGpa($subjectMark / count($allMark))->gpa;
                        $total_subject_avarage_gpa += $subject_gpa;

                    }

                    $gpa = $total_subject_avarage_gpa > 0 && count($allMarks) > 0 ? $total_subject_avarage_gpa / count($allMarks) : 0;
                    $gpaData = $gpa > $max_gpa ? $max_gpa : $gpa;
                    $data = new AllExamWisePosition();
                    $data->class_id = $class;
                ...
                // First, fetch and sort the students based on GPA and total marks
                ...
                foreach ($allStudentMarks as $index => $allStudentMark) {
                    // If GPA and total marks are the same as the previous student, keep the same position
                    if ($previousGpa === $allStudentMark->gpa) {
                        if ($previousTotalMarks === $allStudentMark->total_mark) {
                            $student_position = $position - 1;
                        } else {
                            $student_position = $position;
                        }
                    } else {
                        $student_position = $position; // New position for this student
                    }
                    AllExamWisePosition::where('id', $allStudentMark->id)->update([
                        'position' => $student_position,
                    ]);

                    // Update the previous GPA and total marks for comparison
                    $previousGpa = $allStudentMark->gpa;
                    $previousTotalMarks = $allStudentMark->total_mark;
                    $position++; // Move to the next position for the next student
                }
...
//                return $studentMarkQuery->orderBy('position', 'ASC')->get()->toArray();
            }
