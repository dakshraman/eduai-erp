...
class SmExamController extends Controller
{
    ...
    public function index()
    {
        /*
        try {
        */ $exam_query = branchWise(SmExam::query());
        ...
        if (teacherAccess()) {
            ...
            if (moduleStatusCheck('University')) {
                ...
                $subjectColumns = ['subject_id', 'class_id', 'section_id'];
                if ($this->canUseBranchColumn('sm_assign_subjects')) {
                    $subjectColumns[] = 'branch_id';
                }
                $subjects = branchWise(SmAssignSubject::where('teacher_id', $teacher_info->id)->where('school_id', auth()->user()->school_id)->get($subjectColumns));
                if ($subjects) {
                    $c_id = [];
                    $se_id = [];
                    $su_id = [];
                    ...
                    $subjectAssignedExams = branchWise(SmExam::whereIn('class_id', $c_id)->whereIn('section_id', $se_id)->whereIn('subject_id', $su_id)->get());
        ...
        $exams_types = branchWise(SmExamType::where('school_id', Auth::user()->school_id)
        ...
        $teachers = branchWise(SmStaff::where('role_id', 4)->where('active_status', 1)
            ->where('school_id', Auth::user()->school_id));
        $teachers = $teachers->get($this->staffSelectColumns());
        $rooms = branchWise(SmClassRoom::where('active_status', 1)
            ->where('school_id', Auth::user()->school_id)
            ->get());

        return view('backEnd.examination.exam', ['exams' => $exams, 'classes' => $classes, 'subjects' => $subjects, 'exams_types' => $exams_types, 'sections' => $sections, 'teachers' => $teachers, 'rooms' => $rooms]);
        /*
    ...
    public function exam_setup($id)
    {
        ...
        try {
        */
        $exams = branchWise(SmExam::with('GetExamTitle', 'class', 'section', 'subject')->get());
        ...
        return view('backEnd.examination.exam', ['exams' => $exams, 'classes' => $classes, 'subjects' => $subjects, 'exams_types' => $exams_types, 'sections' => $sections, 'selected_exam_type_id' => $selected_exam_type_id, 'teachers' => $teachers, 'rooms' => $rooms]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function exam_reset()
    {
        ...
        */
        ...
        $exams_types = branchWise(SmExamType::get());
        SmExamType::query()->truncate();
        $exam_mark_stores = branchWise(SmMarkStore::get());
        SmMarkStore::query()->truncate();
        $exam_results_stores = branchWise(SmResultStore::where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)
            ->get());
        ...
        return view('backEnd.examination.exam', ['exams' => $exams, 'classes' => $classes, 'subjects' => $subjects, 'exams_types' => $exams_types, 'sections' => $sections]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function store(Request $request)
    {
        ...
        if ($request->exam_system === 'single') {
            ...
            if (moduleStatusCheck('University')) {
                $rules = array_merge($rules, [
                    ...
                    'end_time' => 'required',
                    'room' => 'required',
                    'exam_title' => 'required|array',
                    'exam_mark' => 'required|array',
                    'exam_mark.*' => 'numeric|min:0',
                ]);
            } else {
                $rules = array_merge($rules, [
                    'exams_type' => 'required',
        ...
        $branchId = $this->shouldUseBranchInput() ? $request->branch_id : null;
        ...
        try {
            if ($request->exam_system === 'single') {
                if (moduleStatusCheck('University')) {
                    ...
                    foreach ($all_sections as $all_section) {
                        $checkExitExam = SmExam::where([
                            ...
                            'un_section_id' => $all_section->un_section_id,
                        ...
                        $exam->un_section_id = $all_section->un_section_id;
                        ...
                        $this->setBranchId($exam, 'sm_exams', $branchId);
                        $exam->save();
                        $exam->toArray();
                        ...
                        for ($i = 0; $i < $length; $i++) {
                            ...
                            $newSetupExam->un_section_id = $all_section->un_section_id;
                            ...
                            $this->setBranchId($newSetupExam, 'sm_exam_setups', $branchId);
                        ...
                        $exam_routine->un_section_id = $all_section->un_section_id;
                        ...
                        $this->setBranchId($exam_routine, 'sm_exam_schedules', $branchId);
                        $exam_routine->save();
                ...
                foreach ($sections as $section) {
                    $checkExitExam = SmExam::where('exam_type_id', $request->exams_type)
                    ...
                    $exam->shift_id = shiftEnable() ? $request->shift : '';
                    ...
                    $this->setBranchId($exam, 'sm_exams', $request->branch_id);
                    $exam->save();
                    $exam->toArray();
                    $length = count($request->exam_title);
                    for ($i = 0; $i < $length; $i++) {
                        ...
                        $newSetupExam->shift_id = shiftEnable() ? $request->shift : '';
                    ...
                    $is_exist = SmExamSchedule::where('exam_term_id', $exam->exam_type_id)
                        ->where('exam_id', $exam->id)
                        ->where('subject_id', $request->subject_id)
                        ->where('date', date('Y-m-d', strtotime($request->date)))
                        ->where('start_time', date('H:i:s', strtotime($request->start_time)))
                        ->where('end_time', date('H:i:s', strtotime($request->end_time)))
                        ->where('room_id', $request->room)
                        ->where('class_id', $exam->class_id)
                        ->where('section_id', $exam->section_id)
                        ->when(shiftEnable(), function ($query) use ($exam) {
                            $query->where('shift_id', $exam->shift_id);
                        })
                        ->where('school_id', Auth::user()->school_id)
                        ->first();
                    ...
                    $exam_routine->shift_id = shiftEnable() ? $exam->shift_id : '';
        ...
        try {
            ...
            $sections = branchWise(SmClassSection::where('class_id', $request->class_ids)
                ->when(shiftEnable(), function ($query) use ($request) {
                    $query->where('shift_id', $request->shift);
                })
                ->get());
            ...
            if (moduleStatusCheck('University')) {
                ...
                foreach ($request->exams_types as $exam_type_id) {
                    foreach ($sections as $section) {
                        $subject_for_sections = branchWise(SmAssignSubject::where('class_id', $request->class_ids)
                            ->where('section_id', $section->section_id)
                            ->when(shiftEnable(), function ($query) use ($section) {
                                $query->where('shift_id', $section->shift_id);
                            })
                            ->get());
                        $eligible_subjects = [];
                        ...
                        foreach ($request->subjects_ids as $subject_id) {
                            if (in_array($subject_id, $eligible_subjects)) {
                                $checkExitExam = SmExam::where('exam_type_id', $exam_type_id)
                                    ->where('class_id', $request->class_ids)
                                    ->where('section_id', $section->section_id)
                                ...
                                $exam->shift_id = shiftEnable() ? $section->shift_id : '';
                                ...
                                for ($i = 0; $i < $length; $i++) {
                                    ...
                                    $newSetupExam->shift_id = shiftEnable() ? $section->shift_id : '';
    ...
    public function show($id)
    {
        ...
        */
        ...
        $subjects = branchWise(SmAssignSubject::where('class_id', $exam->class_id)->where('section_id', $exam->section_id)->get());
        $sections = branchWise(SmClassSection::where('class_id', $exam->class_id)->get());
        $exams = branchWise(SmExam::get());
        ...
        return view('backEnd.examination.examEdit', ['exam' => $exam, 'exams' => $exams, 'classes' => $classes, 'subjects' => $subjects, 'sections' => $sections, 'exams_types' => $exams_types])->with($data);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function examSetup($id)
    {
        ...
        */
        ...
        $sections = branchWise(SmSection::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());

        return view('backEnd.examination.exam_setup', ['exam' => $exam, 'exams' => $exams, 'classes' => $classes, 'subjects' => $subjects, 'sections' => $sections]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
        ...
        */
        ...
        if ($total_exam_mark === $totalMark) {
            ...
            for ($i = 0; $i < $length; $i++) {
                ...
                $newSetupExam->school_id = Auth::user()->school_id;
                $newSetupExam->academic_id = getAcademicId();
                $this->setBranchId($newSetupExam, 'sm_exam_setups', $request->branch_id);
                $result = $newSetupExam->save();
                if ($result) {
                    Toastr::success('Operation successful', 'Success');
    ...
    public function getClassSubjects(Request $request)
    {
        ...
        if ($request->globalType) {
            ...
            $subjects = branchWise(SmAssignSubject::where('class_id', $request->id)
                ->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)
                ->get());
    ...
    public function examView(Request $request)
    {
        ...
        $html = view($view, ['exams_types' => $exams_types, 'classes' => $classes, 'teachers' => $teachers, 'rooms' => $rooms])->render();
    ...
    {/*
        ...
        return view('backEnd.examination.report.marksheetReport', ['exams' => $exams, 'classes' => $classes, 'branch_id' => $branch_id]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    private function staffSelectColumns(): array
    {
        $columns = ['id', 'user_id', 'full_name'];

        if ($this->canUseBranchColumn('sm_staffs')) {
            $columns[] = 'branch_id';
        }
    ...
    private function setBranchId($model, string $table, $branchId): void
    {
        if ($this->canUseBranchColumn($table)) {
            $model->branch_id = $branchId;
        }
    }

    private function canUseBranchColumn(string $table): bool
    {
        return moduleStatusCheck('Branch') && Schema::hasColumn($table, 'branch_id');
    }

    private function shouldUseBranchInput(): bool
    {
        return moduleStatusCheck('Branch') && Schema::hasTable('branches');
    }
}
