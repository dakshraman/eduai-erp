...
class SmExamRoutineController extends Controller
{
    ...
    public function examSchedule()
    {
        ...
        try {
        */
        $exam_types = branchWise(SmExamType::where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
        if (teacherAccess()) {
    ...
    public function addExamRoutineModal($subject_id, $exam_period_id, $class_id, $section_id, $exam_term_id, $section_id_all)
    {
        ...
        try {
        */
        $rooms = branchWise(SmClassRoom::where('active_status', 1)
            ->where('school_id', Auth::user()->school_id)
            ->get());
    ...
    public function checkExamRoutineDate(Request $request)
    {
        ...
        */
        if ($request->assigned_id== '') {
            $check_date = branchWise(SmExamSchedule::where('class_id', $request->class_id)->where('section_id', $request->section_id)->where('exam_term_id', $request->exam_term_id)->where('date', date('Y-m-d', strtotime($request->date)))->where('exam_period_id', $request->exam_period_id)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
        } else {
            $check_date = branchWise(SmExamSchedule::where('id', '!=', $request->assigned_id)->where('class_id', $request->class_id)->where('section_id', $request->section_id)->where('exam_term_id', $request->exam_term_id)->where('date', date('Y-m-d', strtotime($request->date)))->where('exam_period_id', $request->exam_period_id)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
        }
    ...
    public function examScheduleReportSearch(SmExamScheduleSearchRequest $smExamScheduleSearchRequest)
    {
        ...
        */
        if (moduleStatusCheck('University')) {
            ...
            $SmExamSchedule = branchWise(SmExamSchedule::query());
            ...
            $un_session_id = $smExamScheduleSearchRequest->un_session_id;
            $un_faculty_id = $smExamScheduleSearchRequest->un_faculty_id;
            $un_department_id = $smExamScheduleSearchRequest->un_department_id;
            $un_academic_id = $smExamScheduleSearchRequest->un_academic_id;
            $un_semester_id = $smExamScheduleSearchRequest->un_semester_id;
            $un_semester_label_id = $smExamScheduleSearchRequest->un_semester_label_id;
            $un_section_id = $smExamScheduleSearchRequest->un_section_id;

            return view('backEnd.examination.exam_schedule', ['exam_schedules' => $exam_schedules, 'un_session' => $un_session, 'un_faculty' => $un_faculty, 'un_department' => $un_department, 'un_academic' => $un_academic, 'un_semester' => $un_semester, 'un_semester_label' => $un_semester_label, 'un_section' => $un_section, 'examName' => $examName, 'un_session_id' => $un_session_id, 'un_faculty_id' => $un_faculty_id, 'un_department_id' => $un_department_id, 'un_academic_id' => $un_academic_id, 'un_semester_id' => $un_semester_id, 'un_semester_label_id' => $un_semester_label_id, 'un_section_id' => $un_section_id])->with($data);
        }

        $assign_subjects = branchWise(SmAssignSubject::query());
        ...
        if (! empty($smExamScheduleSearchRequest->branch_id)) {
            $assign_subjects->where('branch_id', $smExamScheduleSearchRequest->branch_id);
        }
        ...
        $exams = branchWise(SmExam::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
        ...
        $branch_id = $smExamScheduleSearchRequest->branch_id ?? '';

        $exam_types = branchWise(SmExamType::where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());

        $exam_schedules = branchWise(SmExamSchedule::query());
        if (! empty($smExamScheduleSearchRequest->section)) {
        ...
        if (! empty($smExamScheduleSearchRequest->branch_id)) {
            $exam_schedules->where('branch_id', $smExamScheduleSearchRequest->branch_id);
        }
    ...
    public function examRoutineReport(Request $request)
    {
        ...
        */
        $auth = Auth::user();
        $exam_types = branchWise(SmExamType::select(['id', 'title'])->where('school_id', $auth->school_id)->where('academic_id', getAcademicId())->get());
        $classes = branchWise(SmClass::select(['id', 'class_name'])->where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', $auth->school_id)->get());
    ...
    public function universityExamRoutineReportSearch($request)
    {
        ...
        */
        ...
        $exam_types = branchWise(SmExamType::where('school_id', $schoolId)
            ->where('academic_id', $academicId));
        if (moduleStatusCheck('Branch')) {
            $exam_types = $exam_types->get(['id', 'title', 'branch_id']);
        } else {
            $exam_types = $exam_types->get(['id', 'title']);
        }
        ...
        $exam_schedules = branchWise(SmExamSchedule::where('school_id', $schoolId)
        ...
        if (moduleStatusCheck('Branch')) {
            $exam_schedules = $exam_schedules->select(['id', 'date', 'start_time', 'end_time', 'un_section_id', 'un_semester_label_id', 'exam_term_id', 'subject_id', 'teacher_id', 'room_id', 'branch_id'])
                ->get();
        } else {
            $exam_schedules = $exam_schedules->select(['id', 'date', 'start_time', 'end_time', 'un_section_id', 'un_semester_label_id', 'exam_term_id', 'subject_id', 'teacher_id', 'room_id'])
                ->get();
        }

        return view('backEnd.reports.exam_routine_report', ['exam_schedules' => $exam_schedules, 'class_id' => $class_id, 'section_id' => $section_id, 'exam_type_id' => $exam_id, 'examName' => $examName, 'exam_types' => $exam_types, 'branch_id' => $branch_id]);

        $examName = SmExamType::where('id', $request->exam)->first()->title;

        return view('backEnd.reports.exam_routine_report', compact('exam_schedules', 'class_id', 'section_id', 'exam_id', 'exam_type_id', 'examName', 'exam_types', 'branch_id'));
        /*
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function examRoutineReportSearch(Request $request)
    {
        ...
        $academicId = getAcademicId();
        $auth = Auth::user();
        $classes = branchWise(SmClass::select()->get());
        $exams = branchWise(SmExam::get());
        ...
        $search_current_branch = null;

        $exam_types = branchWise(SmExamType::where('school_id', $auth->school_id)
            ->where('academic_id', $academicId));
        if (moduleStatusCheck('Branch')) {
            $exam_types = $exam_types->get(['id', 'title', 'branch_id']);
        } else {
        ...
        $exam_schedules = branchWise(SmExamSchedule::where('school_id', $auth->school_id)
            ->when($exam_id, fn ($query) => $query->where('exam_term_id', $exam_id))
        ...
        if (moduleStatusCheck('Branch')) {
            $exam_schedules = $exam_schedules->select(['id', 'date', 'class_id', 'section_id', 'shift_id', 'start_time', 'end_time',  'exam_term_id', 'subject_id', 'teacher_id', 'room_id', 'branch_id'])
                ->get();
        } else {
            $exam_schedules = $exam_schedules->select(['id', 'date', 'class_id', 'section_id', 'shift_id', 'start_time', 'end_time',  'exam_term_id', 'subject_id', 'teacher_id', 'room_id'])
                ->get();
        }
        ...
        return view('backEnd.reports.exam_routine_report', compact('classes', 'exams', 'exam_schedules', 'class_id', 'section_id', 'shift_id', 'exam_id', 'exam_types', 'exam_type_id', 'examName', 'search_current_class', 'search_current_section', 'search_current_branch', 'branch_id'));
        /*
        } catch (\Exception $e) {
    ...
    public function examScheduleSearch(SmExamScheduleSearchRequest $smExamScheduleSearchRequest)
    {
        ...
        */
        if (moduleStatusCheck('University')) {
            ...
            $SmExam = branchWise(SmExam::query());
            ...
            $teachers = branchWise(SmStaff::where('role_id', 4)->where('active_status', 1)
                ->where('school_id', auth()->user()->school_id));
            if (moduleStatusCheck('Branch')) {
                $teachers = $teachers->get(['id', 'user_id', 'full_name', 'branch_id']);
            } else {
                $teachers = $teachers->get(['id', 'user_id', 'full_name']);
            }

            $rooms = branchWise(SmClassRoom::where('active_status', 1)->where('school_id', auth()->user()->school_id));

            if (moduleStatusCheck('Branch')) {
                $rooms = $rooms->get(['id', 'room_no', 'branch_id']);
            } else {
                $rooms = $rooms->get(['id', 'room_no']);
            }
        ...
        if (moduleStatusCheck('Branch') && $smExamScheduleSearchRequest->branch_id !== null) {
            $assign_subjects = branchWiseApplyFilter($assign_subjects, 'branch_id', $smExamScheduleSearchRequest->branch_id);
            $subject_ids = branchWiseApplyFilter($subject_ids, 'branch_id', $smExamScheduleSearchRequest->branch_id);
        }
        ...
        $branch_id = $smExamScheduleSearchRequest->branch_id ?? null;
        $exam_type_id = $smExamScheduleSearchRequest->exam_type;
        $exam_types = branchWise(SmExamType::where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)
            ->get());

        $exam_schedule = branchWise(SmExamSchedule::query());
        ...
        if ($smExamScheduleSearchRequest->branch_id) {
            $exam_schedule->where('branch_id', $smExamScheduleSearchRequest->branch_id);
        }
        ...
        $subjects = branchWise(SmSubject::whereIn('id', $subject_ids)
            ->where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id));

        if (moduleStatusCheck('Branch')) {
            $subjects = $subjects->get(['id', 'subject_name', 'branch_id']);
        } else {
            $subjects = $subjects->get(['id', 'subject_name']);
        }
        ...
        $search_current_branch='';
        if (moduleStatusCheck('Branch')) {
            $search_current_branch = Branch::find($smExamScheduleSearchRequest->branch_id) ?? null;
        }
        ...
        if (moduleStatusCheck('Branch') && $branch_id) {
            $sectionIds = branchWiseApplyFilter($sectionIds, 'branch_id', $branch_id);
        }
        $sectionIds = $sectionIds->get();

        $sections = [];
        foreach ($sectionIds as $secId) {
            $sec = SmSection::where('id', $secId->section_id)->withoutGlobalScope(StatusAcademicSchoolScope::class)->first();
            if ($sec) {
                $sections[] = $sec;
            }
        }

        return view('backEnd.examination.exam_schedule_new_update', ['classes' => $classes, 'subjects' => $subjects, 'exam_schedule' => $exam_schedule, 'class_id' => $class_id, 'section_id' => $section_id, 'exam_type_id' => $exam_type_id, 'exam_types' => $exam_types, 'teachers' => $teachers, 'rooms' => $rooms, 'examName' => $examName, 'search_current_class' => $search_current_class, 'search_current_section' => $search_current_section, 'shift_id' => $shift_id, 'class_section' => $class_section, 'branch_id' => $branch_id, 'search_current_branch' => $search_current_branch, 'sections' => $sections]);
        /*
        } catch (Exception $exception) {
        ...
        */
        if (moduleStatusCheck('University')) {
            ...
            foreach ($request->routine as $routine_data) {
                ...
                if (moduleStatusCheck('Branch')) {
                    $exam_routine->branch_id = $request->branch_id ?? null;
                }
                $exam_routine->save();
            }
        ...
        $exam_schedule = SmExamSchedule::query()
            ...
            ->when(moduleStatusCheck('Branch') && $request->branch_id !== null, function ($query, $branch_id) {
        ...
        foreach ($request->routine as $routine_data) {
            ...
            $data['branch_id'] = $exam_routine->branch_id;
    ...
    public function examRoutineView($class_id, $section_id, $exam_term_id, $shift_id = null, $branch_id = null)
    {
        ...
        */
        ...
        $classes = branchWise(SmClass::where('active_status', 1)
            ...
            ->where('school_id', Auth::user()->school_id)
            ->get());
        $exams = branchWise(SmExam::where('active_status', 1)
            ->where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)
            ->get());
        ...
        $exam_types = branchWise(SmExamType::where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)
            ->get());
        $exam_periods = branchWise(SmClassTime::where('type', 'exam')
            ->where('academic_id', getAcademicId())
            ->where('school_id', Auth::user()->school_id)
            ->get());
        $rooms = branchWise(SmClassRoom::where('active_status', 1)
        ...
        $search_current_branch='';
        if (moduleStatusCheck('Branch')) {
            $search_current_branch = Branch::find($branch_id) ?? null;
        }

        if ($section_id === 0) {
            $exam_schedule = branchWise(SmExamSchedule::where('class_id', $class_id)->where('exam_term_id', $exam_type_id)->get());
        } else {
            $exam_schedule = branchWise(SmExamSchedule::where('class_id', $class_id)->where('section_id', $section_id)
    ...
    public function universityExamRoutinePrint($un_semester_label_id, $un_section_id, $exam_term_id, $shift_id = null, $branch_id = null)
    {
        /*
        try {
        */
        ...
        $shift_name = (shiftEnable() && $shift_id !== 0 && $shift_id !== null)
                        ...
                        : 'All Shifts';
        $branch_name = (moduleStatusCheck('Branch') && $branch_id !== 0 && $branch_id !== null)
                        ? (Branch::find($branch_id)->name ?? '')
                        : '';
        $exam_schedules = branchWise(SmExamSchedule::where('school_id', Auth::user()->school_id)
            ...
            ->when(moduleStatusCheck('Branch') && $branch_id !== 0 && $branch_id !== null, function ($query) use ($branch_id) {
    ...
    public function examRoutinePrint($class_id, $section_id, $exam_term_id, $shift_id = null, $branch_id = null)
    {
        if (moduleStatusCheck('University')) {
            return $this->universityExamRoutinePrint($class_id, $section_id, $exam_term_id, $shift_id, $branch_id);
        }
