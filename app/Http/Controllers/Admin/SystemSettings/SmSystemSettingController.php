...
use App\Models\SmFrontendPersmission;
use App\Models\SmGeneralSettings;
use App\Models\SmHomePageSetting;
...
class SmSystemSettingController extends Controller
{
    ...
    public function sendTestMail(Request $request)
    {
        ...
        $reciver_email = $request->test_email;
    ...
    public function ajaxSubjectDropdown(Request $request)
    {
        ...
        $allSubjects = SmAssignSubject::where([['section_id', '=', $request->id], ['class_id', $class_id]])->get()->unique('subject_id');
    ...
    public function languageAdd(Request $request)
    {
        ...
        if (!empty($language_details)) {
            ...
            Cache::forget('system_language_' . auth()->user()->school_id);
            Toastr::success('Operation successful', 'Success');
    ...
    {
        $sms_dbs = SmBackup::where('academic_id', getAcademicId())->where('storage_type', 'local')->orderBy('id', 'DESC')->whereNull('lang_type')->get();
        if (moduleStatusCheck('GoogleDrive')) {
            $google_backups = SmBackup::where('academic_id', getAcademicId())->where('storage_type', 'google')->orderBy('id', 'DESC')->get();
            $googleDriveSetting = GoogleDriveSetting::where('school_id', auth()->user()->school_id)->first();
        } else {
            $google_backups = [];
            $googleDriveSetting = null;
        }

        return view('backEnd.systemSettings.backupSettings', compact('sms_dbs', 'google_backups', 'googleDriveSetting'));
    }
    ...
    public function getfilesBackup(int $id)
    {
        set_time_limit(1600);
        /*
        try {
        */
        ...
        foreach ($files as $name => $file) {
            if (!$file->isDir()) {
                ...
                $relativePath = mb_substr($filePath, mb_strlen($path) + 1);
    ...
    public function getDatabaseBackup()
    {
        ...
        try {
        */
        $db_export = Shuttle_Dumper::create([
            'host' => env('DB_HOST'),
    ...
    public function updateGeneralSettings(Request $request)
    {
        ...
        $user = Auth::user();
        $editData = SmGeneralSettings::where('school_id', $user->school_id)->first();
        $session_ids = SmAcademicYear::where('school_id', $user->school_id)->where('active_status', 1)->get();
        $dateFormats = SmDateFormat::where('active_status', 1)->get();
        $languages = SmLanguage::where('school_id', 1)->get();
        $countries = SmCountry::select('currency')->distinct('currency')->get();
        $currencies = SmCurrency::where('school_id', 1)->get();
        $academic_years = SmAcademicYear::where('school_id', $user->school_id)->get();
        $time_zones = SmTimeZone::all();
        $weekends = SmWeekend::where('school_id', $user->school_id)->get();
    ...
    public function updateGeneralSettingsData(SmGeneralSettingsRequest $request)
    {
        ...
        */
        ...
        $generalSettData->carry_forword_due_day = $request->carry_forword_due_day;
        ...
        // OPTIMIZED: Invalidate the general settings cache after saving.
        // This ensures generalSetting() returns fresh data on the next request.
        \App\Support\CacheInvalidationHelper::clearGeneralSettings(Auth::user()->school_id);

        // Note: Artisan::call("optimize:clear") removed — not needed on shared hosting
        // and caused errors when artisan is unavailable. Cache invalidation above is sufficient.
    ...
    public function updateEmailSettingsData(SmEmailSettingsRequest $request)
    {
        ...
        if ($request->engine_type === 'smtp') {
            ...
            if ($request->boolean('active_status')) {
    ...
    public function paymentMethodSettings()
    {
        ...
        if (moduleStatusCheck('SslCommerz') === false) {
            $paymeny_gateway->where('method', '!=', 'SslCommerz');
        }

             if(moduleStatusCheck('MomoPay') == false){
                $paymeny_gateway->where('method','!=','MomoPay');
            }
        ...
        if (moduleStatusCheck('SslCommerz') === false) {
            $paymeny_gateway_settings->where('gateway_name', '!=', 'SslCommerz');
        }
    ...
    public function bankStatus(Request $request)
    {
        ...
        $active_bank = SmBankAccount::where('id', $request->account_id)
            ->where('school_id', Auth::user()->school_id)
            ->update(['active_status' => $request->account_status]);
    ...
    public function setFCMkey(Request $request)
    {
        ...
        */
        ...
        if (! is_array($data) || empty($data['private_key']) || empty($data['client_email']) || empty($data['project_id'])) {
            ...
            return redirect()->back()->withErrors(['fcm_json' => 'The Firebase service-account JSON is missing required credentials.']);
        }

        $private_key = trim(str_replace(['\\r\\n', '\\n', '\\r'], "\n", $data['private_key']));
        if (@openssl_pkey_get_private($private_key) === false) {
            Storage::delete($path);

            return redirect()->back()->withErrors(['fcm_json' => 'The Firebase private key is invalid. Upload a fresh service-account JSON file.']);
        }
    ...
    public function setFCMSMSkey(Request $request)
    {
        ...
        */
        ...
        if (! is_array($data) || empty($data['private_key']) || empty($data['client_email']) || empty($data['project_id'])) {
            ...
            return redirect()->back()->withErrors(['fcm_json' => 'The Firebase SMS service-account JSON is missing required credentials.']);
        }
        ...
        if (@openssl_pkey_get_private($private_key) === false) {
            ...
            return redirect()->back()->withErrors(['fcm_json' => 'The Firebase SMS private key is invalid. Upload a fresh service-account JSON file.']);
        }
