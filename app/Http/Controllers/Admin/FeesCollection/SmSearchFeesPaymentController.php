...
class SmSearchFeesPaymentController extends Controller
{
    public function index(Request $request)
    {
        ...
        */
        if (auth()->user()->role_id === 1 || auth()->user()->role_id === 5) {
            $fees_payments = branchWise(SmFeesPayment::with('recordDetail', 'installmentPayment', 'recordDetail.studentDetail', 'recordDetail.class', 'recordDetail.section')
    ...
    public function search(SmFeesCollectSearchRequest $smFeesCollectSearchRequest)
    {
        ...
        */
        ...
        $search['shift'] = $smFeesCollectSearchRequest->shift;
        ...
        return view('backEnd.feesCollection.search_fees_payment', ['classes' => $classes, 'invoice_setting' => $invoice_setting, 'un_session_id' => $un_session_id, 'un_faculty_id' => $un_faculty_id, 'un_department_id' => $un_department_id, 'un_academic_id' => $un_academic_id, 'un_semester_id' => $un_semester_id, 'un_semester_label_id' => $un_semester_label_id, 'un_section_id' => $un_section_id, 'search' => $search, 'branch_id' => $branch_id])->with($search);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function updateFeesPayment(Request $request)
    {
        ...
        */
        ...
        if ($result) {
            $payment = SmFeesPayment::find($request->fees_payment_id);
            $payment->amount = (float) round($request->amount, getDecimalDigit());
            $payment->payment_mode = $request->payment_mode;
            $payment->bank_id = $request->payment_mode === 'bank' ? $request->bank_id : null;
            $payment->save();
        } else {
