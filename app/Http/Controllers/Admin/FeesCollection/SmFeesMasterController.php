...
class SmFeesMasterController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $fees_masters = branchWise(SmFeesMaster::with('feesTypes', 'feesGroups')->get());
        $already_assigned = [];
        ...
        $fees_types = branchWise(SmFeesType::get());

        if (moduleStatusCheck('University')) {

            return view('university::fees.fees_master', ['fees_groups' => $fees_groups, 'fees_types' => $fees_types, 'fees_masters' => $fees_masters, 'already_assigned' => $already_assigned]);
        }

        if (directFees()) {
            $classes = branchWise(SmClass::get());

            return view('backEnd.feesCollection.directFees.fees_master', ['fees_groups' => $fees_groups, 'fees_types' => $fees_types, 'fees_masters' => $fees_masters, 'already_assigned' => $already_assigned, 'classes' => $classes]);
        }
    ...
    public function store(SmFeesMasterRequest $smFeesMasterRequest)
    {
        /*
                try {
                */
        $feesTypeId = $smFeesMasterRequest->fees_type;
        $feesGroupId = $smFeesMasterRequest->fees_group;
        if (moduleStatusCheck('University')) {
            $fees_group = new SmFeesGroup();
            $fees_group->name = $smFeesMasterRequest->name;
            $fees_group->description = $smFeesMasterRequest->description;
            $fees_group->school_id = Auth::user()->school_id;
            $fees_group->un_academic_id = getAcademicId();
            if (moduleStatusCheck('Branch')) {
                $fees_group->branch_id = $smFeesMasterRequest->branch_id;
            }
            $fees_group->save();
            ...
            $fees_type->name = $smFeesMasterRequest->name;
            $fees_type->fees_group_id = $feesGroupId;
            $fees_type->description = $smFeesMasterRequest->description;
            $fees_type->school_id = Auth::user()->school_id;
            $fees_type->un_academic_id = getAcademicId();
            if (moduleStatusCheck('Branch')) {
                $fees_type->branch_id = $smFeesMasterRequest->branch_id;
            }
            $fees_type->save();
        ...
        if ($combination === 0) {
            ...
            $smFeesMaster->fees_type_id = $feesTypeId;
            $smFeesMaster->date = date('Y-m-d', strtotime($smFeesMasterRequest->date));
            $smFeesMaster->school_id = Auth::user()->school_id;

            if (moduleStatusCheck('University')) {
                $smFeesMaster->fees_group_id = $feesGroupId;
                $smFeesMaster->un_academic_id = getAcademicId();
            } elseif (directFees()) {
                $smFeesMaster->fees_group_id = $fees_type->fees_group_id;
                $smFeesMaster->academic_id = getAcademicId();
                $smFeesMaster->class_id = $smFeesMasterRequest->class;
                if ($smFeesMasterRequest->section_id !== 'all_section') {
                    $smFeesMaster->section_id = $smFeesMasterRequest->section_id;
                }
            } else {
                $smFeesMaster->fees_group_id = $fees_type->fees_group_id;
                $smFeesMaster->academic_id = getAcademicId();
            ...
            if (moduleStatusCheck('Branch')) {
                $smFeesMaster->branch_id = $smFeesMasterRequest->branch_id;
            }
            $smFeesMaster->amount = (float) round($smFeesMasterRequest->amount, getDecimalDigit());
            $smFeesMaster->save();
            if (directFees()) {
                $this->installmentCreate($smFeesMaster->id, $smFeesMasterRequest);
    ...
    public function show($id)
    {
        ...
        */
        ...
        $fees_masters = $fees_masters->groupBy('fees_group_id');
        if (moduleStatusCheck('University')) {
            return view('university::fees.fees_master', ['fees_groups' => $fees_groups, 'fees_types' => $fees_types, 'fees_master' => $fees_master, 'fees_masters' => $fees_masters, 'already_assigned' => $already_assigned]);
        }

        if (directFees()) {
            $classes = SmClass::get();

            return view('backEnd.feesCollection.directFees.fees_master', ['fees_master' => $fees_master, 'fees_groups' => $fees_groups, 'fees_types' => $fees_types, 'fees_masters' => $fees_masters, 'already_assigned' => $already_assigned, 'classes' => $classes]);
        }
    ...
    public function update(SmFeesMasterRequest $smFeesMasterRequest, $id)
    {
        /*
        try {
        */
        $fees_type = SmFeesType::find($smFeesMasterRequest->fees_type);
        $fees_master = SmFeesMaster::find($smFeesMasterRequest->id);

        $fees_master->fees_type_id = $smFeesMasterRequest->fees_type;
        $fees_master->date = date('Y-m-d', strtotime($smFeesMasterRequest->date));
        $fees_master->amount = (float) round($smFeesMasterRequest->amount, getDecimalDigit());
        $fees_master->fees_group_id = $fees_type->fees_group_id;
        if (moduleStatusCheck('Branch')) {
            $fees_master->branch_id = $smFeesMasterRequest->branch_id;
        }
        $fees_master->save();
        if (moduleStatusCheck('University')) {
            $feesGroup = SmFeesGroup::find($smFeesMasterRequest->fees_group_id);
            ...
            $feesGroup->name = $smFeesMasterRequest->name;
            $feesGroup->save();
        }
        ...
        if (directFees()) {
            ...
            $feesGroup->name = $smFeesMasterRequest->name;
            if (moduleStatusCheck('Branch')) {
                $feesGroup->branch_id = $smFeesMasterRequest->branch_id;
            }
            $feesGroup->save();
            $this->installmentCreate($fees_master->id, $smFeesMasterRequest);
        ...
        */
        ...
        if (directFees()) {
            ...
            if ($master) {
                ...
                if (count($master_installments) > 0) {
                    foreach ($master_installments as $master_installment) {
                        $assignrd_inst = DirectFeesInstallmentAssign::where('fees_installment_id', $master_installment->id)->first();
                        ...
                        DirectFeesInstallmentAssign::where('fees_installment_id', $master_installment->id)->delete();
    ...
    public function feesAssign(Request $request, $id)
    {
        ...
        */
        $fees_group_id = $id;
        $classes = branchWise(SmClass::get());
        $groups = branchWise(SmStudentGroup::where('active_status', '=', '1')->where('school_id', Auth::user()->school_id)->get());
        $categories = branchWise(SmStudentCategory::where('school_id', Auth::user()->school_id)->where('academic_id', getAcademicId())->get());
    ...
    public function feesAssignSearch(Request $request)
    {
        ...
        */
        ...
        $requestData['branch_id'] = $request->branch_id ?? null;
        ...
        $fees_assign_groups = branchWise(SmFeesMaster::where('fees_group_id', $request->fees_group_id)->where('school_id', Auth::user()->school_id)->get());

        if (moduleStatusCheck('University')) {
            $interface = App::make(UnCommonRepositoryInterface::class);
            $search_info = $interface->oldValueSelected($request);

            return view('university::fees.fees_assign', ['classes' => $classes, 'categories' => $categories, 'groups' => $groups, 'students' => $students, 'student_count' => $student_count, 'fees_assign_groups' => $fees_assign_groups, 'fees_group_id' => $fees_group_id, 'pre_assigned' => $pre_assigned, 'class_id' => $class_id, 'category_id' => $category_id, 'group_id' => $group_id, 'assigned_value' => $assigned_value, 'section_id' => $section_id, 'requestData' => $requestData, 'student_ids' => $student_ids])->with($search_info);
        }

        return view('backEnd.feesCollection.fees_assign', ['classes' => $classes, 'categories' => $categories, 'groups' => $groups, 'students' => $students, 'student_count' => $student_count, 'fees_assign_groups' => $fees_assign_groups, 'fees_group_id' => $fees_group_id, 'pre_assigned' => $pre_assigned, 'class_id' => $class_id, 'category_id' => $category_id, 'group_id' => $group_id, 'assigned_value' => $assigned_value, 'section_id' => $section_id, 'requestData' => $requestData, 'student_ids' => $student_ids]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function feesAssignStoreOld(Request $request)
    {
        ...
        try {
        */
        $fees_masters = branchWise(SmFeesMaster::where('fees_group_id', $request->fees_group_id)
            ->where('school_id', Auth::user()->school_id)
            ->get());
        ...
        */
        if ($request->ajax()) {
            ...
            $students->with('studentDetail.gender', 'studentDetail.parents', 'studentDetail.category', 'class', 'section', 'studentDetail')
                ->whereHas('studentDetail', function ($query): void {
            ...
            return DataTables::of($students)
                ->addIndexColumn()
                ->addColumn('input_checkbox', function ($row) use ($pre_assigned): string {
                    return '<input type="checkbox" id="student.'.$row->id.'" class="common-checkbox" name="data['.$row->id.'][checked]" value="1"'.(in_array($row->id, $pre_assigned) ? 'checked' : '').'>
                            ...
                            <input type="hidden" name="data['.$row->id.'][shift_id]" value="'.@$row->shift->id.'">
                ...
                ->editColumn('class_section_name', function ($row): string {
                    if (shiftEnable()) {
                        return @$row->class->class_name.'('.@$row->section->section_name.')'.'['.@$row->shift->shift_name.']';
                    }

                    return @$row->class->class_name.'('.@$row->section->section_name.')';

                })
                ->editColumn('parents_name', function ($row) {
                    return $row->studentDetail->parents  ? $row->studentDetail->parents->fathers_name : '';
                })
                ->editColumn('category_name', function ($row) {
                    return @$row->studentDetail->category->category_name;
                })
        ...
        if (moduleStatusCheck('University')) {
            ...
            if ($request->fees_assign_all === 1) {
                $filters = [
                    ...
                    'shift' => $request->fees_assign_shift,
                ...
                if (moduleStatusCheck('Branch')) {
                    $filters = [
                        'branch_id' => $request->fees_assign_branch,
                    ];
                }

                $students = $this->studentIds(new Request($filters));
                $this->feesStoreStudentRecord(null, $students, $fees_masters, $request, true);
            } else {
                $students = branchWise(SmStudent::with(['feesAssign', 'feesPayment', 'feesAssignDiscount', 'forwardBalance', 'user', 'parents', 'parents.parent_user'])
    ...
    public function feesUnassignAll(Request $request)
    {
        ...
        */
        ...
        if (shiftEnable()) {
            $filters['shift'] = $request->fees_assign_shift;
        }
        if (moduleStatusCheck('Branch')) {
            $filters = [
                'branch_id' => $request->fees_assign_branch,
        ...
        $studentRecords = $this->studentIds(new Request($filters));
    ...
    private function studentIds($request)
    {
        ...
        if ($request->category || $request->group) {
            $sm_student_ids = SmStudent::when($request->category, function ($query) use ($request): void {
                $query->where('student_category_id', $request->category);
            })
                ->when($request->group, function ($query) use ($request): void {
        ...
        $students = branchWise(StudentRecord::query());
        if (moduleStatusCheck('University')) {
            $students = universityFilter($students, $request)->where('is_promote', 0);
        } else {
            if ($request->class ) {
        ...
        return $students->when($request->category || $request->group, function ($query) use ($sm_student_ids): void {
            $query->whereIn('student_id', $sm_student_ids);
        })
            ->with('studentDetail.gender', 'studentDetail.parents', 'studentDetail.category', 'class', 'section', 'shift', 'studentDetail')
            ->whereHas('studentDetail', function ($query): void {
                $query->where('active_status', 1);
            })->get();
    }

    private function feesStoreStudentRecord($student, $studentRecords, $fees_masters, Request $request, $studentFromRecord = null): void
    {
        foreach ($studentRecords as $studentRecord) {
            ...
            foreach ($fees_masters as $fee_master) {
                $payment_info = $student->feesPayment->where('active_status', 1)
                    ->where('fees_type_id', $fee_master->fees_type_id)
                    ->where('record_id', $studentRecordId) // record
                    ->count();
                // delete assign fees if no payment
                if (! $payment_info && $student->feesAssign->where('fees_master_id', $fee_master->id)
                    ->where('record_id', $studentRecordId)->count()) {
                    $student->feesAssign()->where('fees_master_id', $fee_master->id)->where('record_id', $studentRecordId)
                        ->delete();
                }

                if (! $studentFromRecord && ! gbv($studentRecord, 'checked')) {
                    continue;
                }

                $assign_fees = $student->feesAssign()->where('fees_master_id', $fee_master->id)->where('record_id', $studentRecordId)->first(); // record
                ...
                if ($forward) {
                    ...
                    if ($forwardAmount) {
                        ...
                        $fees_payment->fees_type_id = $fee_master->fees_type_id;
                        ...
                        if ($forwardAmount > 0 && $fee_master->amount < $forwardAmount) {
                            $fees_payment->amount = $fee_master->amount;
                            $extra_forword = $forwardAmount - $fee_master->amount;
                        } else {
