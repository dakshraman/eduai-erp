...
class SmFeesController extends Controller
{
    ...
    public function feesPaymentStore(Request $request)
    {
        ...
        try {
            ...
            $smFeesPayment->discount_amount = empty($request->applied_amount) ? 0 : (float) round($request->applied_amount, getDecimalDigit());
            $smFeesPayment->fine = empty($request->fine) ? 0 : (float) round($request->fine, getDecimalDigit());
            $smFeesPayment->assign_id = $request->assign_id;
            $smFeesPayment->amount = empty($request->amount) ? 0 : (float) round($request->amount, getDecimalDigit());
            $smFeesPayment->assign_id = $request->assign_id;
    ...
    */
    ...
    public function feesDueSearch(Request $request)
    {
        ...
        */
        ...
        return view('backEnd.feesCollection.search_fees_due', ['classes' => $classes, 'fees_masters' => $fees_masters, 'fees_dues' => $fees_dues, 'class_id' => $class_id, 'section_id' => $section_id, 'shift_id' => $shift_id, 'fees_group_id' => $fees_group_id]);
    ...
    public function feesStatementSearch(Request $request)
    {
        ...
        */
        ...
        $student = SmStudent::with('class', 'section')->find($request->student);
        ...
        return view('backEnd.feesCollection.fees_statment', ['classes' => $classes, 'fees_masters' => $fees_masters, 'fees_assigneds' => $fees_assigneds, 'fees_discounts' => $fees_discounts, 'applied_discount' => $applied_discount, 'student' => $student, 'shift_id' => $shift_id, 'class_id' => $class_id, 'section_id' => $section_id, 'student_id' => $student_id]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function feesPaymentInvoicePrint($id, $s_id)
    {
        ...
        */
        ...
        if (!$invoiceSettings) {
            $invoiceSettings = InvoiceSetting::where('school_id', Auth::user()->school_id)->first();
        }
        if (!$invoiceSettings) {
            $invoiceSettings = new InvoiceSetting();
            $invoiceSettings->per_th = 1;
            $invoiceSettings->prefix = 'INV-';
            $invoiceSettings->signature_p = 0;
            $invoiceSettings->signature_c = 0;
            $invoiceSettings->signature_o = 0;
        }
    ...
    public function fineReportSearch(SmFineReportSearchRequest $request)
    {
        ...
        */
        ...
        if ($request->class && $request->section && $request->shift) {
            ...
            $main_students = StudentRecord::where('class_id', $request->class)
        ...
        } elseif ($request->class && $request->section) {
        ...
        $students = SmStudent::whereIn('id', $main_students)->pluck('id');
        $fine_info = SmFeesPayment::where('active_status', 1)
        ...
        return view('backEnd.accounts.fine_report', compact('classes', 'fine_info', 'class_id', 'section_id', 'shift_id'));
        /*
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
