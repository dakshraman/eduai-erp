...
class SmHomeworkController extends Controller
{
    ...
    public function searchHomework(Request $request)
    {
        ...
        */
        ...
        return view('backEnd.homework.homeworkList', ['classes' => $classes, 'un_session_id' => $un_session_id, 'un_faculty_id' => $un_faculty_id, 'un_department_id' => $un_department_id, 'un_academic_id' => $un_academic_id, 'un_semester_id' => $un_semester_id, 'un_semester_label_id' => $un_semester_label_id, 'un_section_id' => $un_section_id, 'un_subject_id' => $un_subject_id, 'branch_id' => $branch_id])->with($data);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function addHomework()
    {
        ...
        */
        if (teacherAccess()) {
            $teacher_info = SmStaff::where('user_id', Auth::user()->id)->first();
            $classes = $teacher_info ? $teacher_info->classes : collect();
        } else {
            $classes = branchWise(SmClass::get());
        }
        ...
        */
        ...
        if (moduleStatusCheck('University')) {
            ...
            if (is_null($smHomeworkRequest->section_id)) {
                foreach ($sections as $section) {
                    ...
                    if (moduleStatusCheck('Branch')) {
                        $homeworks->branch_id = $smHomeworkRequest->branch_id;
                    }
                    $homeworks->save();
                }
            } else {
            ...
            if ($smHomeworkRequest->status === 'lmsHomework') {
                ...
                $student_ids = branchWise(StudentRecord::when($smHomeworkRequest->class, function ($query) use ($smHomeworkRequest): void {
                ...
                $classes = branchWise(SmClassSection::when($smHomeworkRequest->class_id, function ($query) use ($smHomeworkRequest): void {
                    $query->where('class_id', $smHomeworkRequest->class_id);
                })
        ...
        if (moduleStatusCheck('University')) {
            $records = branchWise(StudentRecord::where('un_semester_label_id', $smHomeworkRequest->un_semester_lable_id)->pluck('student_id')->unique());
            $student_ids = [];
    ...
    public function downloadHomeworkData($id, $student_id)
    {
        ...
        */
        ...
        $zip_file_url = assetPath('public/uploads/homeworkcontent/'.$zip_file_name);
    ...
    public function unEvaluationHomework($sem_label_id, $homework_id)
    {
        ...
        */
        ...
        $student_records = branchWise(StudentRecord::where('un_semester_label_id', $sem_label_id)->distinct('student_id')->get('student_id'));
        $student_ids = [];
    ...
    public function evaluationHomework(Request $request, $class_id, $section_id, $homework_id)
    {
        ...
        */
        ...
        $students = branchWise(SmStudent::where('active_status', 1)->whereIn('id', $student_ids)->where('school_id', auth()->user()->school_id)->get());
    ...
    public function saveHomeworkEvaluationData(Request $request)
    {
        ...
        */
        ...
        /*
                } catch (Exception $exception) {ed', 'Failed');
    ...
    public function evaluationReport(Request $request)
    {
        ...
        */
        ...
        return view('backEnd.reports.evaluation', ['classes' => $classes, 'branch_id' => $branch_id]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function searchEvaluation(Request $request)
    {
        ...
        */
        ...
        if ($branch_id !== null) {
            branchWiseApplyFilter($homeworkLists, 'branch_id', $branch_id);
        }
        ...
        $subjects = branchWise(SmAssignSubject::when($class_id, function ($q) use ($class_id): void {
            ...
            branchWiseApplyFilter($q, 'branch_id', $branch_id);
        })->get());

        return view('backEnd.reports.evaluation', ['homeworkLists' => $homeworkLists, 'classes' => $classes, 'class_id' => $class_id, 'shift_id' => $shift_id, 'section_id' => $section_id, 'subject_id' => $subject_id, 'smClass' => $smClass, 'subjects' => $subjects, 'homeworkPercentageData' => $homeworkPercentageData, 'branch_id' => $branch_id]);
    ...
    public function searchEvaluationData(Request $request)
    {
        ...
        $builder = branchWise(SmHomework::query()->with('subjects', 'sections', 'classes', 'classes.classSections')->withCount('homeworkCompleted'));
        $user = Auth::user();
        if ($request->class_id !== null) {
    ...
    public function homeworkEdit($id)
    {
        ...
        */
        ...
        $sections = branchWise(SmClassSection::where('class_id', '=', $homeworkList->class_id)->get());

        $subjects = branchWise(SmAssignSubject::where('class_id', $homeworkList->class_id)
        ...
        $data['branchActive'] = moduleStatusCheck('Branch') && class_exists(\Modules\Branch\Entities\Branch::class);
        if (moduleStatusCheck('University')) {
            $interface = App::make(UnCommonRepositoryInterface::class);
    ...
    public function homeworkDelete($id)
    {
        ...
        try {
        */
        $tables = \App\Support\tableList::getTableList('homework_id', $id);
    ...
    public function homeworkReport()
    {
        ...
        */
        ...
        return view('backEnd.homework.homework_report', ['classes' => $classes, 'branchActive' => $branchActive]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function universityHomeworkSearch($request)
    {
        ...
        */
        ...
        $homeworks = branchWise(SmHomework::when($request->un_session_id, function ($query) use ($request): void {
            ...
            ->when($branchActive && $request->branch_id, function ($query) use ($request): void {
        ...
        foreach ($homeworks as $homework) {
            ...
            foreach ($homework->class->students as $record) {
                ...
                $data[] = [
                    'branch' => $branchActive && $homework->branch ? $homework->branch->branch_name : '',
        ...
        return view('backEnd.homework.homework_report', ['data' => $data, 'un_session_id' => $un_session_id, 'un_faculty_id' => $un_faculty_id, 'un_department_id' => $un_department_id, 'un_academic_id' => $un_academic_id, 'un_semester_id' => $un_semester_id, 'un_semester_label_id' => $un_semester_label_id, 'un_section_id' => $un_section_id, 'un_subject_id' => $un_subject_id, 'branch_id' => $branch_id, 'branchActive' => $branchActive]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function homeworkReportSearch(Request $request)
    {
        ...
        */
        ...
        $homeworks = branchWise(SmHomework::where('class_id', $request->class_id)
            ...
            })->when($branchActive && $request->branch_id, function ($query) use ($request) {
                $query->where('branch_id', $request->branch_id);
            }));
        ...
        foreach ($homeworks as $hw) {
            ...
            foreach ($hw->class->records as $record) {
                ...
                $submission = $hw_contents->where('student_id', $record->student_id)->first();
                $data[] = [
                    'branch' => $branchActive && $hw->branch ? $hw->branch->branch_name : '',
                    'student' => $record->studentDetail ? $record->studentDetail->full_name : '',
                    'student_id' => $record->studentDetail ? $record->studentDetail->id : '',
        ...
        return view('backEnd.homework.homework_report', compact('classes', 'data', 'class_id', 'section_id', 'shift_id', 'subject_id', 'branch_id', 'branchActive'));
        /*
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
