...
class SmClassController extends Controller
{
    ...
    public function index(Request $request)
    {
        /*
         try {
         */
        $sections = branchWise(SmSection::query());
        ...
        $classes = branchWise(SmClass::with('groupclassSections')->withCount('records')->get());
            ...
            if (shiftEnable()) {
                foreach ($request->shift as $shift) {
                    foreach ($request->section as $section) {
                        ...
                        if (moduleStatusCheck('Branch')) {
                            $smClassSection->branch_id = $request->branch_id;
                        }
                        $smClassSection->save();
                    }
                }
    ...
    public function edit(Request $request, $id)
    {
        try {
            ...
            $sectionByNames = branchWise(SmClassSection::select('section_id', 'shift_id')->where('class_id', '=', $classById->id)->get());
            $sectionId = [];
            $shiftId = [];
            ...
            $sections = branchWise(SmSection::where('active_status', '=', 1)->where('created_at', 'LIKE', '%' . $this->date . '%')->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());

            $classes = branchWise(SmClass::where('active_status', '=', 1)->orderBy('id', 'desc')->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->withCount('records')->get());
    ...
    public function update(ClassRequest $classRequest)
    {
        ...
        try {
            ...
            if (moduleStatusCheck('Branch')) {
                $class->branch_id = $classRequest->branch_id;
            }
            $class->save();
            $class->toArray();
                ...
                if (shiftEnable()) {
                    foreach ($classRequest->shift as $shift) {
                        foreach ($classRequest->section as $section) {
                            ...
                            $smClassSection->academic_id = getAcademicId();
                            if (moduleStatusCheck('Branch')) {
                                $smClassSection->branch_id = $classRequest->branch_id;
                            }
                            $smClassSection->save();
                        }
                    }
