...
use App\Models\SmClassRoutineUpdate;
...
class SmClassRoutineNewController extends Controller
{
    ...
    public function classRoutine(Request $request)
    {
        ...
        try {
            ...
            $class_routines = branchWise(SmClassRoutineUpdate::where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
    ...
    public function classRoutineSearch(Request $request)
    {
        ...
        try {
            ...
            $sm_weekends = SmWeekend::with('classRoutine')
            ...
            $subjectQuery = SmAssignSubject::where('class_id', $class_id)
                ->where('section_id', $section_id)
                ->where('school_id', $school_id);

            if ($shift_id) {
                $subjectQuery->where('shift_id', $shift_id);
            }

            if ($branch_id) {
                branchWiseApplyFilter($subjectQuery, 'branch_id', $branch_id);
            } else {
                $subjectQuery = branchWise($subjectQuery);
            }
            ...
            $teachers = branchWise(SmStaff::where('role_id', 4)
                ->where('school_id', $school_id)
                ->get());
            ...
            return view('backEnd.academics.class_routine_new', compact(
                'classes',
                'teachers',
                'rooms',
                'subjects',
    ...
    public function dayWiseClassRoutine(Request $request)
    {
        ...
        $class_routines = SmClassRoutineUpdate::where('day', $day_id)->where('class_id', $class_id)
            ->where('section_id', $section_id)
            ...
            ->orderBy('start_time', 'ASC')->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id);

        if (moduleStatusCheck('Branch') && ! $branch_id) {
            $class_routines = branchWise($class_routines);
        }

        $class_routines = $class_routines->get();
        ...
        $stds = SmStudent::where('class_id', $class_id)
            ->where('section_id', $section_id)
            ->when(moduleStatusCheck('Branch') && $branch_id, function ($query) use ($branch_id) {
                return branchWiseApplyFilter($query, 'branch_id', $branch_id);
            })
            ->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->count();
        $rooms = branchWise(SmClassRoom::where('active_status', 1)->where('capacity', '>=', $stds)
            ->where('school_id', Auth::user()->school_id)
            ->get());
        $teachers = branchWise(SmStaff::where('role_id', 4)->where('school_id', Auth::user()->school_id)->get());
        $sm_weekends = SmWeekend::where('school_id', Auth::user()->school_id)
            ->orderBy('order', 'ASC')
            ->where('active_status', 1)
            ->get();

        return view('backEnd.academics.classRoutine.form', compact('day_id', 'class_routines', 'sm_weekends', 'subjects', 'rooms', 'teachers', 'section_id', 'class_id', 'shift_id', 'branch_id'));
    }
    ...
    public function addNewClassRoutineStore(Request $request)
    {
        try {
            ...
            SmClassRoutineUpdate::where('day', $request->day)->where('class_id', $request->class_id)
                ...
                ->when(moduleStatusCheck('Branch'), function ($query) use ($request) {
            ...
            foreach ($request->routine as $key => $routine_data) {
                ...
                $days = gv($routine_data, 'day_ids') === null ? [$request->day] : gv($routine_data, 'day_ids', []);
                ...
                foreach ($days as $day) {
                    ...
                    $class_routine->shift_id = shiftEnable() ? $request->shift_id : '';
                    if (moduleStatusCheck('Branch')) {
                        $class_routine->branch_id = $request->branch_id;
                    }
    ...
    public function classRoutinePrint($class, $section, $shift = null, $branch_id = null)
    {
        $print = request()->print;
        $school_id = Auth::user()->school_id;
        $academic_id = getAcademicId();
        ...
        // Fetch class times
        $class_times = SmClassTime::where('type', 'class')
            ->where('academic_id', $academic_id)
            ->where('school_id', $school_id)
            ->get();
        ...
        // Fetch class routine based on shift presence
        $sm_weekends = SmWeekend::with(['classRoutine' => function ($q) use ($class_id, $section_id, $shift_id, $branch_id) {
            $q->where('class_id', $class_id)
                ->where('section_id', $section_id);

            if (! is_null($shift_id) && $shift_id !== 'null') {
                $q->where('shift_id', $shift_id);
            }

            if (moduleStatusCheck('Branch')) {
                if (! is_null($branch_id) && $branch_id !== 'null') {
        ...
        return view('backEnd.academics.class_routine_print', [
            ...
            'shift' => (! is_null($shift_id) && $shift_id !== 'null') ? Shift::find($shift_id) : null,
            'branch' => (moduleStatusCheck('Branch') && ! is_null($branch_id) && $branch_id !== 'null') ? Branch::find($branch_id) : null,
            'print' => $print,
        ]);
    }
    ...
    public function classRoutineReport(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.reports.class_routine_report', compact('classes', 'branch_id'));
        } catch (Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function classRoutineReportSearch(Request $request)
    {
        ...
        try {
            ...
            $data = [];
            $class_times = SmClassTime::where('type', 'class')->where('academic_id', $academic_id)->where('school_id', $user->school_id)->get();
            $sm_weekends = SmWeekend::with('classRoutine', 'classRoutine.subject', 'classRoutine.classRoom')->where('school_id', $user->school_id)->orderBy('order', 'ASC')->where('active_status', 1)->get();

            if (moduleStatusCheck('University')) {
                $routineQuery = SmClassRoutineUpdate::where('un_semester_label_id', $request->un_semester_label_id)
                    ->when(moduleStatusCheck('Branch') && $branch_id, function ($query) use ($branch_id) {
                        return branchWiseApplyFilter($query, 'branch_id', $branch_id);
                    });

                if (moduleStatusCheck('Branch') && ! $branch_id) {
                    $routineQuery = branchWise($routineQuery);
                }

                $sm_routine_updates = $routineQuery->get();
                ...
                $routineQuery = SmClassRoutineUpdate::where('class_id', $request->class)->where('section_id', $request->section)
            ...
            return view('backEnd.reports.class_routine_report', array_merge($data, $selected));
        } catch (Exception $e) {
