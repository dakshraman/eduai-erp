...
class SmItemSellController extends Controller
{
    ...
    public function itemSell(Request $request)
    {
        ...
        */
        ...
        $roles = InfixRole::where('is_saas', 0)->when((generalSetting()->with_guardian != 1), function ($query): void {
            $query->where('id', '!=', 3);
    ...
    public function saveItemSellData(SmItemSellRequest $smItemSellRequest)
    {
        ...
        */
        ...
        $total_paid = (float) round($total_paid, getDecimalDigit());
        $subTotalValue = (float) round($smItemSellRequest->subTotalValue, getDecimalDigit());
        $totalDueValue = (float) round($smItemSellRequest->totalDueValue, getDecimalDigit());
        $paid_status = '';
        if ($totalDueValue == 0) {
            $paid_status = 'P';
        ...
        if (@$smItemSellRequest->subTotalValue) {
            $smItemSell->grand_total = $subTotalValue;
        }
        ...
        $smItemSell->total_due = $totalDueValue;
        ...
        if ($results) {
            ...
            for ($i = 0; $i < $item_ids; $i++) {
                if (! empty($smItemSellRequest->item_id[$i])) {
                    ...
                    $itemSellChild->sell_price = (float) round($smItemSellRequest->unit_price[$i], getDecimalDigit());
                    $itemSellChild->quantity = $smItemSellRequest->quantity[$i];
                    $itemSellChild->sub_total = (float) round($smItemSellRequest->totalValue[$i], getDecimalDigit());
    ...
    public function editItemSell(Request $request, $id)
    {
        ...
        */
        ...
        $roles = InfixRole::where('is_saas', 0)->where(function ($q): void {
        ...
        $students = $allStudentsSameClassSection;

        $paymentMethhods = SmPaymentMethhod::get();

        return view('backEnd.inventory.editItemSell', ['editData' => $editData, 'editDataChildren' => $editDataChildren, 'roles' => $roles, 'items' => $items, 'paymentMethhods' => $paymentMethhods, 'classes' => $classes, 'sections' => $sections, 'studentClassSection' => $studentClassSection, 'allStudentsSameClassSection' => $allStudentsSameClassSection, 'staffsByRole' => $staffsByRole, 'sell_heads' => $sell_heads, 'account_id' => $account_id, 'students' => $students]);
        /*
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function UpdateItemSellData(SmItemSellRequest $smItemSellRequest)
    {
        ...
        */
        ...
        $itemSells->grand_total = $subTotalValue;
        ...
        $itemSells->total_due = $totalDueValue;
    ...
    public function updateItemReceiveData(Request $request, $id)
    {
        ...
        */
        ...
        $total_paid = (float) round($total_paid, getDecimalDigit());
        $subTotalValue = (float) round($request->subTotalValue, getDecimalDigit());
        $totalDueValue = (float) round($request->totalDueValue, getDecimalDigit());
        ...
        $itemReceives->grand_total = $subTotalValue;
        ...
        $itemReceives->total_due = $totalDueValue;
        ...
        if ($itemReceiveChildren) {
            ...
            for ($i = 0; $i < $item_ids; $i++) {
                if (! empty($request->item_id[$i])) {
                    ...
                    $itemReceivedChild->unit_price = (float) round($request->unit_price[$i], getDecimalDigit());
                    $itemReceivedChild->quantity = $request->quantity[$i];
                    $itemReceivedChild->sub_total = (float) round($request->totalValue[$i], getDecimalDigit());
    ...
    public function saveItemSellPayment(Request $request)
    {
        ...
        */
        ...
        $smInventoryPayment->amount = (float) round($request->amount, getDecimalDigit());
        ...
        if (isset($itemPaymentDue)) {
            ...
            $updated_total_due = $total_due - (float) round($request->amount, getDecimalDigit());
            $updated_total_paid = $total_paid + (float) round($request->amount, getDecimalDigit());
        ...
        $smAddIncome->amount = (float) round($request->amount, getDecimalDigit());
        ...
        if (paymentMethodName($request->payment_method)) {
            $bank = SmBankAccount::where('id', $request->bank_id)
                ->where('school_id', Auth::user()->school_id)
                ->first();
            $after_balance = $bank->current_balance + (float) round($request->amount, getDecimalDigit());

            $smBankStatement = new SmBankStatement();
            $smBankStatement->amount = (float) round($request->amount, getDecimalDigit());
            $smBankStatement->after_balance = (float) round($after_balance, getDecimalDigit());
            $smBankStatement->type = 1;
            $smBankStatement->details = 'Item Sell Payment';
