...
class UserForumController extends Controller
{
    public function index()
    {
        ...
        $filtered_forum_items = $forums->filter(function ($item) use ($role_id, $school_id): bool {
        ...
        return view('backEnd.userForum.title_index', ['forum_categories' => $forum_categories, 'forum_items' => $forum_items, 'forum_titles' => $forum_titles]);
    }
    ...
    public function userTitleSearch(Request $request)
    {
        ...
        $query = ForumTitle::with(['forumCategory', 'forumCategory.schools'])
            ->orderBy('id', 'desc')
            ->when($request->forum_category_id, function ($query) use ($request): void {
                $query->whereHas('forumCategory', function ($q) use ($request): void {
                    $q->where('id', $request->forum_category_id);
                });
            })
            ->when($request->forum_title_id, function ($query) use ($request): void {
                $query->where('id', $request->forum_title_id);
            });

        $forum_items = $query->get()->filter(function ($item) use ($role_id, $school_id): bool {
            $available_for = json_decode($item->available_for, true);
            $school_ids = $item->forumCategory->schools->pluck('id')->toArray();
    ...
    public function forumTopicIndex($id)
    {
        ...
        $filtered_forum_items = $forums->filter(function ($item) use ($school_id, $user): bool {
            // $available_for                  = json_decode($item->available_for, true);
        ...
        $roles = InfixRole::where('is_saas', 0)
            ...
            ->where(function ($query) use ($school_id): void {
        ...
        return view('backEnd.userForum.topic_index', ['forum_topics' => $forum_topics, 'forum_categories' => $forum_categories, 'forum_topic_items' => $forum_topic_items, 'roles' => $roles, 'forum_title' => $forum_title, 'forum_titles' => $forum_titles]);
    }
    ...
    public function userTopicSearch(Request $request)
    {
        ...
        $forum_topic_items = ForumTopic::with(['forumTitle', 'forumTitle.forumCategory.schools'])
            ...
            ->when($request->forum_topic_id, function ($query) use ($request): void {
        ...
        $forum_titles = ForumTitle::with(['forumCategory', 'forumCategory.schools'])
            ->orderBy('id', 'desc')
            ->get()
            ->filter(function ($item) use ($role_id, $school_id): bool {
                $available_for = json_decode($item->available_for, true);
                $school_ids = $item->forumCategory->schools->pluck('id')->toArray();
        ...
        return view('backEnd.userForum.topic_index', ['forum_title' => $forum_title, 'forum_categories' => $forum_categories, 'forum_topic_items' => $forum_topic_items, 'roles' => $roles, 'forum_titles' => $forum_titles, 'forum_topics' => $forum_topics]);
    }
    ...
    public function forumTopicStore(Request $request)
    {
        ...
        try {
            ...
            $forumTopic->forum_title_id = $request->forum_title_id;
            $forumTopic->title = $request->title;
            $forumTopic->description = $request->description;
            // $forum->available_for  = json_encode($request->role);
            $forumTopic->created_by = $user->id;
            $forumTopic->save();
    ...
    public function forumTopicView($id)
    {
        ...
        return view('backEnd.userForum.topic_view', ['forum' => $forum, 'recent_forum' => $recent_forum]);
    }
    ...
    public function userVote(Request $request)
    {
        ...
        $forumUserVote = new ForumUserVote();
        $forumUserVote->user_id = $user->id;
        $forumUserVote->forum_topic_id = $forumId;
        $forumUserVote->vote = $voteType;
        $forumUserVote->save();
    ...
    public function userMyTopics()
    {
        ...
        $filtered_forum_items = $forums_title->filter(function ($item) use ($role_id, $school_id): bool {
            $available_for = json_decode($item->available_for, true);
            $school_ids = $item->forumCategory->schools->pluck('id')->toArray();
        ...
        return view('backEnd.userForum.user_forum_my_topic', ['forums' => $forums, 'forum_categories' => $forum_categories, 'forum_titles' => $forum_titles, 'forum_topics' => $forum_topics, 'roles' => $roles]);
    }
    ...
    public function userMyTopicSearch(Request $request)
    {
        ...
        $forums = ForumTopic::with(['forumTitle', 'forumTitle.forumCategory.schools'])
            ->where('created_by', $user->id)
            ->orderBy('id', 'desc')
            ->when($request->news_category_id, function ($query) use ($request): void {
                $query->whereHas('forumTitle.forumCategory', function ($q) use ($request): void {
                    $q->where('id', $request->news_category_id);
                });
            })
            ->when($request->forum_title_id, function ($query) use ($request): void {
                $query->whereHas('forumTitle', function ($q) use ($request): void {
    ...
    public function userCommentVote(Request $request)
    {
        ...
        $commentVote->forum_comment_id = $commentId;
        $commentVote->vote = $voteType;
        $commentVote->save();
    ...
    public function userReplyVote(Request $request)
    {
        ...
        $replyVote->user_id = $user->id;
        $replyVote->forum_comment_reply_id = $replyId;
        $replyVote->vote = $voteType;
        $replyVote->save();
