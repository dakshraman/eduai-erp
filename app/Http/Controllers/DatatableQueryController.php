...
{
    public function studentDetailsData(Request $request)
    {
        ...
        $user = auth()->user();
        $academicYearId = $request->academic_year ?: getAcademicId();
        ...
        $records->where('is_promote', 0)
            ...
            ->when(moduleStatusCheck('Branch'), function ($query) use ($branch_id): void {
                $query->where(function ($query) use ($branch_id): void {
                    $query->where('branch_id', $branch_id ?: userBranch())
        ...
        if (moduleStatusCheck('University')) {
            ...
            $records
                ->when($request->filled('un_academic_id'), function ($query) use ($request) {
                    $query->where('un_academic_id', $request->un_academic_id);
                })
                ->when($request->filled('un_faculty_id'), function ($query) use ($request) {
                    $query->where('un_faculty_id', $request->un_faculty_id);
                })
                ->when($request->filled('un_department_id'), function ($query) use ($request) {
                    $query->where('un_department_id', $request->un_department_id);
                })
                ->when($request->filled('un_session_id'), function ($query) use ($request) {
                    $query->where('un_session_id', $request->un_session_id);
                })
                ->when($request->filled('un_semester_label_id'), function ($query) use ($request) {
        ...
        $all_students = SmStudent::with([
            'studentRecords' => function ($query) use ($academicYearId): void {
                $query->where('academic_id', $academicYearId);
            },
        ...
        if ($user->role_id === 4 && ! userPermission('student.show-all-student')) {
            ...
            $classTeacherIds = SmClassTeacher::where('teacher_id', $user->staff->id)
                ->pluck('assign_class_teacher_id');

            $assignClassTeacher = branchWise(SmAssignClassTeacher::whereIn('id', $classTeacherIds)->get());

            $all_students->whereHas('studentRecords', function ($query) use ($assignClassTeacher) {
                $query->whereIn('class_id', $assignClassTeacher->pluck('class_id'))
                    ->whereIn('section_id', $assignClassTeacher->pluck('section_id'));
            });
        }
        $all_students = $all_students->paginate(12);

        if (request()->ajax()) {
            return view(
                'backEnd.studentInformation.partial.student-grid',
                compact('all_students')
            )->render();
        }

        return view('backEnd.studentInformation.student_details', ['classes' => $classes, 'class_id' => $class_id, 'name' => $name, 'roll_no' => $roll_no, 'sessions' => $sessions, 'section_id' => $section_id, 'academic_year' => $academic_year, 'shift_id' => $shift_id, 'all_students' => $all_students, 'branch_id' => $branch_id]);
    }
    ...
    public function studentDetailsDatatable(Request $request)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            return view('backEnd.studentInformation.student_details', ['classes' => $classes, 'class_id' => $class_id, 'name' => $name, 'roll_no' => $roll_no, 'sessions' => $sessions, 'section_id' => $section_id, 'academic_year' => $academic_year, 'shift_id' => $shift_id, 'branch_id' => $branch_id]);
        }
        ...
        if ($request->ajax()) {
            ...
            $records->where('is_promote', 0)->where('school_id', $user->school_id)
                ->when(moduleStatusCheck('Branch'), function ($query) use ($request): void {
                    $query->where(function ($query) use ($request): void {
                        $query->where('branch_id', $request->branch_id ?: userBranch())
                            ->orWhereNull('branch_id')
                            ->orWhere('branch_id', 0);
                    });
                });
            if (moduleStatusCheck('University')) {
                ...
                $records->where('academic_id', $academicYearId)->when($request->section, function ($query) use ($request): void {
                    $query->where('section_id', $request->section);
                })->when($request->shift_id, function ($query) use ($request) {
                    $query->where('shift_id', $request->shift_id);
            ...
            $student_records = $records->whereHas('student')
                ->pluck('student_id')
                ->unique()
                ->values();
            $studentRecordRelations = moduleStatusCheck('University')
                ? ['studentRecords', 'studentRecords.class', 'studentRecords.section', 'studentRecords.shift']
                : [
                    'studentRecords' => function ($query) use ($academicYearId): void {
                        $query->where('academic_id', $academicYearId);
                    },
            ...
            $all_students = SmStudent::with($studentRecordRelations)->whereIn('id', $student_records)
                ->where('active_status', 1)
            ...
            if ($user->role_id === 4 && ! userPermission('student.show-all-student')) {
                ...
                $assignClassTeacher = branchWise(SmAssignClassTeacher::whereIn('id', $classTeacher)->get());
    ...
    public function getStaffList(Request $request)
    {
        ...
        if ($user->role_id === 1) {
            $staffs = branchWise(SmStaff::query()
                ->withOutGlobalScope(ActiveStatusSchoolScope::class)
                ->where('sm_staffs.school_id', $user->school_id)
                ->where('sm_staffs.is_saas', 0)
        ...
        if (moduleStatusCheck('Branch')) {
            $staffs->with('branch:id,branch_name');
            if ($request->branch_id) {
                $staffs->where('sm_staffs.branch_id', $request->branch_id);
            }
        }
        ...
        if ($request->staff_no) {
            $staffs->where('sm_staffs.staff_no', $request->staff_no);
        }

        if ($request->staff_name) {
            $staffs->where('sm_staffs.full_name', 'like', '%'.$request->staff_name.'%');
        }

        if ($user->role_id !== 1) {
            $staffs->where('sm_staffs.role_id', '!=', 1);
        }
        ...
        if ($request->has('order')) {
            ...
            if(moduleStatusCheck('Branch')) {
                ...
                if ($orderColumn === 'branch.branch_name') {
                    $staffs = $staffs
                        ->leftJoin('branches', 'sm_staffs.branch_id', '=', 'branches.id')
                        ->orderBy('branches.branch_name', $orderDir)
                        ->select('sm_staffs.*'); // avoid ambiguous columns
                }
            }
        }

        $dataTable = DataTables::of($staffs)
            ->addIndexColumn();
        ...
        $dataTable->addColumn('switch', function ($row): string {
            if (Auth::user()->id !== $row->user_id || Auth::user()->role_id !== 1) {
                return '<label class="switch_toggle">
                    <input type="checkbox" id="'.$row->id.'" value="'.$row->id.'"
                        class="switch-input-staff hr_'.$row->id.'"
        ...
        $dataTable->addColumn('action', function ($row): string {
            return '<div class="dropdown CRM_dropdown">
                ...
                (userPermission('viewStaff')
                    ? '<a class="dropdown-item" target="_blank" href="'.route('viewStaff', [$row->id]).'">'
                    .app('translator')->get('common.view').'</a>'
                    : '').

                (userPermission('editStaff')
                    ? '<a class="dropdown-item" href="'.route('editStaff', [$row->id]).'">'
                    .app('translator')->get('common.edit').'</a>'
                    : '').

                (userPermission('deleteStaff') && $row->user_id !== Auth::id()
                    ? '<a onclick="deleteStaff('.$row->id.');" class="dropdown-item" href="#"
                        data-toggle="modal" data-target="#deleteStudentModal" data-id="'.$row->id.'">'
                    .app('translator')->get('common.delete').'</a>'
                    : '').

                '</div></div>';
        });
        $dataTable->rawColumns(['action', 'switch']);
    ...
    {
        $bank_slips = branchWise(SmBankPaymentSlip::query());
        if (moduleStatusCheck('University')) {
            ...
            if ($request->shift ) {
                $bank_slips->where('shift_id', $request->shift);
            }

            if (moduleStatusCheck('Branch') && $request->branch_id ) {
                $bank_slips->where('branch_id', $request->branch_id);
            }
    ...
    public function assignmentList()
    {
        ...
        if (! teacherAccess()) {
            $uploadContents = branchWise(SmTeacherUploadContent::where('content_type', 'as')
                ->where('school_id', $user->school_id)
                ->whereNullLms());
        } else {
            $uploadContents = branchWise(SmTeacherUploadContent::where(function ($q) {
        ...
        if (moduleStatusCheck('University')) {
            $uploadContents = $uploadContents->where('un_academic_id', getAcademicId())
                ->with([
                    'semesterLabel:id,name',
                    'unAcademic:id,name',
                    'unDepartment:id,name',
                    'unSection:id,section_name',
                ])
                ->select([
                    'id',
                    'content_title',
        ...
        } elseif (moduleStatusCheck('Branch')) {
            $uploadContents = $uploadContents->where('academic_id', getAcademicId())
                ->with([
        ...
        return DataTables::of($uploadContents)
            ...
            ->addColumn('type', function ($row) {
                return __('study.assignment');
            })
            ->addColumn('avaiable', function ($row): string {
                $avaiable = '';

                if ($row->available_for_admin === 1) {
                    $avaiable .= __('study.all_admins').', ';
                }

                if ($row->available_for_all_classes === 1) {
                    $avaiable .= __('study.all_classes_student').', ';
                }

                if (moduleStatusCheck('University')) {
                    $avaiable .= __('study.all_students_of').' '.
                        optional($row->semesterLabel)->name.' ('.
                        optional($row->unSection)->section_name.'-'.
                        optional($row->unDepartment)->name.')';
                } else {
                    if ($row->classes && $row->sections) {
                        if (shiftEnable()) {
                            $avaiable .= __('study.all_students_of').' '.
                                optional($row->classes)->class_name.' → '.
                                optional($row->sections)->section_name.' → '.
                    ...
                    if ($row->classes && ! $row->section) {
                        $avaiable .= __('study.all_students_of').' '.
                            optional($row->classes)->class_name.' → '.
                            __('study.all_sections').', ';
                    }
                }

                return trim($avaiable, ', ');
            })
            ->addColumn('class_sections', function ($row) {
                if (moduleStatusCheck('University')) {
                    $semLabel = optional($row->semesterLabel)->name;
                    $academic = optional($row->unAcademic)->name;

                    return $semLabel.' ('.$academic.')';
                }

                $className = optional($row->classes)->class_name;
                $sectionName = optional($row->sections)->section_name;
                $shiftName = optional($row->shift)->name;

                if ($className && $sectionName) {
                    if (shiftEnable()) {
                        return "$className ($sectionName) - $shiftName";
                    }

                    return "$className ($sectionName)";
                }

                if ($className && ! $row->section) {
                    return "$className (".__('common.all_sections').')';
                }
            ...
            ->addColumn('action', function ($row): string {
                ...
                if (userPermission('assignment-list-delete')) {
                    if (Config::get('app.app_sync')) {
                        $btn .= '<span data-toggle="tooltip" title="Disabled For Demo ">
                                    <a class="dropdown-item" href="#">'.__('common.disable').'</a>
                                </span>';
                    } else {
                ...
                if (userPermission('assignment-list-download') && $row->upload_file) {
                    $btn .= '<a class="dropdown-item" href="'.url($row->upload_file).'" download>'
                        .__('common.download').' <span class="pl ti-download"></span></a>';
                }
    ...
    public function syllabusList()
    {
        ...
        if (! teacherAccess()) {
            $uploadContents = branchWise(SmTeacherUploadContent::where('content_type', 'sy')
                ->where('school_id', $user->school_id)
                ->whereNullLms());
        } else {
            $uploadContents = branchWise(SmTeacherUploadContent::where(function ($q) {
        ...
        return DataTables::of($uploadContents)
            ->addIndexColumn()
            ->addColumn('branch', function ($row) use ($branchModuleAvailable) {
    ...
    {
        if ($request->ajax()) {
            $relations = ['complaintType', 'complaintSource'];
            if (moduleStatusCheck('Branch')) {
                $relations[] = 'branch';
            }
            $complaints = branchWise(SmComplaint::with($relations));
    ...
    public function unAssignStudentList(Request $request)
    {
        ...
        if ($request->ajax()) {
            $all_students = branchWise(SmStudent::with(['parents' => function ($query) {
        ...
        if ($request->ajax()) {
            ...
            if (moduleStatusCheck('University')) {
                $records->when($request->filled('un_academic_id'), function ($u_query) use ($request): void {
                    ...
                    ->when($request->filled('un_semester_label_id'), function ($query) use ($request): void {
                        $query->where('un_semester_label_id', $request->un_semester_label_id);
                    })->when(moduleStatusCheck('Branch') && $request->filled('branch_id'), function ($query) use ($request): void {
            ...
            $all_students = branchWise(SmStudent::with('studentRecords', 'studentRecords.class', 'studentRecords.section')->whereIn('id', $student_records)
    ...
    public function uploadContentListDatatable(Request $request)
    {
        ...
        $uploadContents = branchWise(SmTeacherUploadContent::query()
            ->where('school_id', $user->school_id)
            ->whereNullLms());

        if (teacherAccess()) {
            $uploadContents->where(function ($query): void {
        ...
        if ($request->has('order')) {
            ...
            if (! empty($orderColumn)) {
                $uploadContents->orderBy($orderColumn, $orderDirection);
            }
        } else {
            $uploadContents->latest();
        }

        // Eager loading based on module
        if (moduleStatusCheck('University')) {
            $uploadContents->where('un_academic_id', getAcademicId())
                ->with([
                    'semesterLabel:id,name',
                    'unAcademic:id,name',
                    'unDepartment:id,name',
            ...
            $uploadContents->where('academic_id', getAcademicId())
                ->with(['classes', 'sections', 'shift'])
                ->latest();
        }
            ...
            ->addColumn('avaiable', function ($row): string {
                ...
                if (moduleStatusCheck('University')) {
                    if ($row->un_semester_label_id && $row->un_section_id) {
                        $sem = optional($row->semesterLabel)->name;
                        $sec = optional($row->unSection)->section_name;
                        $dep = optional($row->unDepartment)->name;
                        $avaiable .= __('study.all_students_of').' '.$sem.' ('.$sec.'-'.$dep.')';
                    }
                } else {
                    if ($row->classes && $row->sections) {
                        $avaiable .= __('study.all_students_of').' '.optional($row->classes)->class_name.' → '.optional($row->sections)->section_name.', ';
                    } elseif ($row->classes && ! $row->section) {
                        $avaiable .= __('study.all_students_of').' '.optional($row->classes)->class_name.' → '.__('study.all_sections').', ';
                    }
                }

                return rtrim($avaiable, ', ');
            })
            ->addColumn('class_sections', function ($row) {
                if ($row->available_for_all_classes) {
                    return __('study.all_classes');
                }
                ...
                if ($className && $sectionName && $shiftName && shiftEnable()) {
                    return "$className ($sectionName) [$shiftName]";
                }
                if ($className && $sectionName) {
                    return "$className ($sectionName)";
    ...
    public function otherDownloadList()
    {
        ...
        if (! teacherAccess()) {
            $uploadContents = branchWise(SmTeacherUploadContent::where('content_type', 'ot')
                ->where('school_id', $user->school_id)
                ->whereNullLms());
        } else {
            $uploadContents = branchWise(SmTeacherUploadContent::where(function ($q) {
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($fees_payments)
                ...
                ->addColumn('class_sec', function ($row): string {
                    ...
                    if (shiftEnable()) {
                        return $row->recordDetail->class->class_name.'('.$row->recordDetail->section->section_name.')'.'['.$row->recordDetail->shift->shift_name.']';
                    }
    ...
    public function ajaxIncomeList(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::eloquent($all_incomes)
                ...
                ->addColumn('amount', fn ($row) => currency_format($row->amount))
    ...
    public function homeworkListAjax(Request $request)
    {
        ...
        if ($request->ajax()) {
            $all_homeworks = branchWise(SmHomework::query());
            ...
            if (moduleStatusCheck('Branch')){
                $all_homeworks->when($request->branch_id, function ($query) use ($request): void {
                    $query->where('branch_id', $request->branch_id);
                });
            }
    ...
    public function bookListAjax(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (moduleStatusCheck('Branch')) {
                $books = $books->leftJoin('branches', 'sm_books.branch_id', '=', 'branches.id')
                    ->addSelect('branches.branch_name');

                $books = branchWiseBook($books);
            }
            ...
            return DataTables::of($books)
                ->filter(function ($query) use ($request) {
                    if ($request->has('search') && $request->input('search.value') ) {
                        ...
                        $query->where(function ($q) use ($search) {
                            ...
                            if (moduleStatusCheck('Branch')) {
                                $q->orWhere('branches.branch_name', 'like', "%{$search}%");
                            }
                        });
                    }
                })
                ...
                ->editColumn('book_price', function ($books) {
                    return currency_format($books->book_price);
                })
                ->addColumn('action', function ($row): string {
                    return '<div class="dropdown CRM_dropdown">
                                <button type="button" class="btn dropdown-toggle" data-toggle="dropdown">'.app('translator')->get('common.select').'</button>
    ...
    public function allIssuedBookAjax(Request $request)
    {
        if ($request->ajax()) {
            ...
            $branchActive = moduleStatusCheck('Branch');

            $query = SmBookIssue::query()
                ->select('sm_book_issues.*');

            if ($branchActive) {
                $query->leftJoin('branches', 'sm_book_issues.branch_id', '=', 'branches.id')
                    ->addSelect('branches.branch_name');
            }
            ...
            $query->with([
                ...
                ->whereHas('books', function ($q) use ($book_id, $book_number, $subject_id) {
                    if ($book_id) {
                        $q->where('id', $book_id);
                    }
                    if ($book_number) {
                        $q->where('book_number', $book_number);
                    }
                    if ($subject_id) {
                        $q->where('book_subject_id', $subject_id);
                    }
                });
            if ($branchActive) {
                $user = auth()->user();
                ...
                if ($user) {
                    if (userBranch() && userBranch() !== 0) {
                        $query->where('sm_book_issues.branch_id', userBranch());
                    } elseif ($user->role_id !== 1) {
                        $query->where('sm_book_issues.branch_id', $user->branch_id);
                    }
                }
            }
            ...
            return DataTables::of($query)
                ...
                ->addColumn('issue_status', function ($row) {
                    ...
                    if ($row->issue_status === 'I') {
                        return $dueDate < $now
                            ? '<button class="primary-btn small bg-danger text-white border-0">'.__('library.expired').'</button>'
                            : '<button class="primary-btn small bg-success text-white border-0">'.__('library.issued').'</button>';
                    }

                    return '<button class="primary-btn small bg-success text-white border-0">'.__('library.returned').'</button>';
                })

                ->addColumn('given_date', fn ($row) => dateConvert($row->given_date))
                ->addColumn('due_date', fn ($row) => dateConvert($row->due_date))
    ...
    public function itemsListAjax(Request $request)
    {
        if ($request->ajax()) {
            ...
            $items = branchWise(SmItem::with([
                'category:id,category_name',
                ...(moduleStatusCheck('Branch') ? ['branch:id,branch_name'] : []),
            ])
                ->where('school_id', Auth::user()->school_id));
            ...
            return DataTables::of($items)
                ...
                ->addColumn('branch', function ($row) {
                    return moduleStatusCheck('Branch') && $row->branch
                ...
                ->addColumn('action', function ($row): string {
                    ...
                    <button type="button" class="btn dropdown-toggle" data-toggle="dropdown">'
                        ...
                        '</button>
                    <div class="dropdown-menu dropdown-menu-right">'
                        .(userPermission('item-list-edit')
                            ? '<a class="dropdown-item" href="'.route('item-list-edit', $row->id).'">'.__('common.edit').'</a>'
                            : '')
                        .(userPermission('delete-item-view')
                            ? '<a class="dropdown-item deleteUrl" data-modal-size="modal-md"
                            href="'.route('delete-item-view', $row->id).'">'.__('common.delete').'</a>'
                            : '')
                        .'</div></div>';
                })
                ->rawColumns(['action'])
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($allItemReceiveLists)
                ...
                ->addColumn('grand_total', function ($row): string {
                    return currency_format((float) $row->grand_total);
                })
                ->addColumn('total_paid', function ($row): string {
                    return currency_format((float) $row->total_paid);
                })
                ->addColumn('total_due', function ($row): string {
                    return currency_format((float) $row->total_due);
                })
                ->addColumn('status', function ($row): string {
    ...
    public function studentTransportReportAjax(Request $request)
    {
        if ($request->ajax()) {
            ...
            $students = branchWise(SmStudent::with(['studentRecord' => function ($query) use ($request) {
                ...
                ->when(moduleStatusCheck('Branch') && ! empty($request->branch_id), function ($query) use ($request) {
                    $query->where('branch_id', $request->branch_id);
                })
