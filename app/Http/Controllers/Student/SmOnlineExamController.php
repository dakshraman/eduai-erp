...
use App\Models\SmQuestionBankMuOption;
use App\Models\SmStudent;
use App\Models\SmStudentTakeOnlineExam;
use App\Models\SmStudentTakeOnlineExamQuestion;
use App\Models\SmStudentTakeOnlnExQuesOption;
...
class SmOnlineExamController extends Controller
{
    ...
    public function student_online_exam(Request $request)
    {
        ...
        if ($online_exam_info->auto_mark === 1) {
            ...
            foreach ($question_types as $question_type) {
                $question_types_array[] = $question_type->type;
            }
        ...
        $smNotification->user_id = $teacher_info->teacher->user_id;
        $smNotification->role_id = $teacher_info->teacher->role_id;
        $smNotification->date = date('Y-m-d');
        $smNotification->message = Auth::user()->student->full_name.' Submit answer for '.$online_exam_info->title;
    ...
    public function takeOnlineExam($id)
    {
        ...
        return view('backEnd.studentPanel.take_online_exam', ['online_exam' => $online_exam, 'assigned_questions' => $assigned_questions]);

    }

    public function autoMarking($online_exam_id, $student_id, $question_id, $user_choose, $ajax_post): void
    {

        SmOnlineExam::find($online_exam_id);
        ...
        } elseif ($question_info->answer_type === 'radio') {
            $question_answer = OnlineExamStudentAnswerMarking::where('online_exam_id', $online_exam_id)->where('student_id', $student_id)->where('question_id', $question_id)->first();
        } else {
        ...
        if ($question_info->type === 'M') {
            ...
            $user_answers = is_array($user_choose) ? array_map('intval', $user_choose) : [(int) $user_choose];
        ...
        if ($question_info->type === 'MI') {
            if ($question_info->answer_type === 'radio') {
                ...
                OnlineExamStudentAnswerMarking::where('user_answer', '=', '')->delete();
                ...
                foreach ($user_answers as $user_answer) {
                    $student_given_answer[] = (int) $user_answer->user_answer;
                }

                $currect_answers = SmQuestionBankMuOption::where('question_bank_id', $question_info->id)->where('status', 1)->get();
                $question_currect_answer = [];
                foreach ($currect_answers as $currect_answer) {
                    $question_currect_answer[] = $currect_answer->id;
                }

                sort($student_given_answer);
                sort($question_currect_answer);

                $answer_marking = OnlineExamStudentAnswerMarking::where('online_exam_id', $online_exam_id)
                    ->where('student_id', $student_id)
                    ->where('question_id', $question_id)
                    ->first();
                ...
                if ($student_given_answer === $question_currect_answer) {
                    ...
                    OnlineExamStudentAnswerMarking::where('online_exam_id', $online_exam_id)
                        ->where('student_id', $student_id)
                        ->where('question_id', $question_id)
                        ->update(['obtain_marks' => 0, 'answer_status' => 0]);
                }
                $answer_marking->save();
            }
        }
    ...
    public function universityStudentOnlineExamSubmit($request)
    {
        $student = Auth::user()->student;
        ...
        try {
            ...
            $smStudentTakeOnlineExam->online_exam_id = $request->online_exam_id;
            $smStudentTakeOnlineExam->student_id = $student->id;
            $smStudentTakeOnlineExam->record_id = $record_id ?? null;
            $smStudentTakeOnlineExam->status = 1;
            $smStudentTakeOnlineExam->student_done = 1;
            $smStudentTakeOnlineExam->school_id = Auth::user()->school_id;
            $smStudentTakeOnlineExam->un_academic_id = getAcademicId();
            $smStudentTakeOnlineExam->save();
            $smStudentTakeOnlineExam->toArray();
            $question_types = [];
            foreach ($request->question_ids as $key => $question_id) {
                ...
                $exam_question->take_online_exam_id = $smStudentTakeOnlineExam->id;
                ...
                if ($question_bank->type === 'T') {
                    ...
                    foreach ($question_options as $question_option) {
                        ...
                        $exam_question_option->status = $request->$options ?? 0;
                ...
                if ($online_take_exam_mark) {
                    ...
                    if (isset($request->marks)) {
                        foreach ($request->marks as $mark) {
                            ...
                            $total_marks += $question_marks->marks;
                        }
                    }
    ...
    public function studentOnlineExamSubmit(Request $request)
    {
        ...
        try {
            ...
            $smStudentTakeOnlineExam->status = 1;
            $smStudentTakeOnlineExam->school_id = Auth::user()->school_id;
            $smStudentTakeOnlineExam->academic_id = getAcademicId();
            $smStudentTakeOnlineExam->student_done = 1;
            $smStudentTakeOnlineExam->save();
            ...
            if (! in_array('F', $question_types) && $online_exam_info->auto_mark === 1) {
                ...
                if ($online_take_exam_mark) {
                    ...
                    $total_marks = 0.00;
                    foreach ($obtain_marks as $obtain_mark) {
                        $total_marks += $obtain_mark->obtain_marks;
                    }
    ...
    public function studentAnswerScript($exam_id, $s_id)
    {
        ...
        foreach ($assigneds as $assigned) {
            $assigned_question_list[] = $assigned->question_bank_id;
        }

        $answered_question_list = [];
        $answered = $answered ?? null;
        $answereds = OnlineExamStudentAnswerMarking::where('online_exam_id', $exam_id)->where('student_id', $s_id)->select('question_id')->distinct()->get();
        $answered_question_list[] = @$answered?->question_id;
        $array_diff = array_diff($assigned_question_list, $answered_question_list);
        $skipped_questions = SmOnlineExamQuestionAssign::whereIn('question_bank_id', [$array_diff])->get();

        return view('backEnd.examination.online_answer_view_script_modal', ['take_online_exam' => $take_online_exam, 's_id' => $s_id, 'skipped_questions' => $skipped_questions]);
        $skipped_questions = [];
        if ($array_diff !== null) {
            $skipped_questions = SmOnlineExamQuestionAssign::whereIn('question_bank_id', [$array_diff])->get();
        }
