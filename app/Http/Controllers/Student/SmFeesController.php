...
class SmFeesController extends Controller
{
    ...
    public function studentFees()
    {
        ...
        return view('backEnd.studentPanel.fees_pay', ['student' => $student, 'fees_discounts' => $fees_discounts, 'applied_discount' => $applied_discount, 'payment_gateway' => $payment_gateway, 'paystack_info' => $paystack_info, 'data' => $data, 'records' => $records, 'gateway' => $gateway, 'method' => $method, 'invoice_settings' => $invoice_settings, 'razorpay_info' => $razorpay_info]);
    ...
    public function handleGatewayCallback()
    {
        ...
        $smFeesPayment->student_id = Session::get('student_id');
        $smFeesPayment->discount_amount = 0;
        $smFeesPayment->fine = 0;
        $smFeesPayment->amount = $amount;
        $smFeesPayment->assign_id = Session::get('assign_id');
        $smFeesPayment->payment_date = date('Y-m-d', strtotime(date('Y-m-d')));
        $smFeesPayment->payment_mode = 'PS';
        $smFeesPayment->record_id = Session::get('record_id');
        ...
        if (moduleStatusCheck('University')) {
            ...
            if ($installment) {
                ...
                $smFeesPayment->un_fees_installment_id = $installment->installment_id;
                $smFeesPayment->un_semester_label_id = $installment->un_semester_label_id;
            ...
            $smFeesPayment->fees_type_id = Session::get('fees_type_id');
        ...
        $smAddIncome->payment_method_id = 5;
        ...
        if (! moduleStatusCheck('University')) {
            $get_master_id = SmFeesMaster::join('sm_fees_assigns', 'sm_fees_assigns.fees_master_id', '=', 'sm_fees_masters.id')
                ->where('sm_fees_masters.fees_type_id', $smFeesPayment->fees_type_id)
                ->where('sm_fees_assigns.student_id', $smFeesPayment->student_id)->first();
            $fees_assign = SmFeesAssign::where('fees_master_id', $get_master_id->fees_master_id)->where('student_id', $smFeesPayment->student_id)->where('school_id', Auth::user()->school_id)->first();
    ...
    public function feesPaymentStripe($fees_type, $student_id, $amount, $assign_id, $record_id)
    {
        ...
        return view('backEnd.studentPanel.stripe_payment', ['stripe_info' => $stripe_info, 'fees_type' => $fees_type, 'student_id' => $student_id, 'amount' => $amount, 'assign_id' => $assign_id, 'record_id' => $record_id]);
    }
    ...
    public function feesPaymentStripeStore(Request $request)
    {
        ...
        if (moduleStatusCheck('University')) {
            ...
            $installment->payment_date = $smFeesPayment->payment_date;
            $installment->save();
            ...
            $smFeesPayment->fees_type_id = $request->fees_type;
            $smFeesPayment->academic_id = getAcademicId();
        }
    ...
    public function feesGenerateModalChild(Request $request, $amount, $student_id, $type, $assign_id, $record_id)
    {
        ...
        foreach ($discounts as $discount) {
            $fees_payment = SmFeesPayment::where('record_id', $record_id)->where('active_status', 1)->select('fees_discount_id')->where('fees_discount_id', $discount->id)->where('school_id', Auth::user()->school_id)->first();
            if (isset($fees_payment->fees_discount_id)) {
                $applied_discount[] = $fees_payment->fees_discount_id;
            }
        }
    ...
    {
        if($request->payment_mode == 'PayPal' || $request->payment_mode == 'Stripe' || $request->payment_mode == 'Paystack' || $request->payment_mode == 'CcAveune' || $request->payment_mode == 'PhonePay' || $request->payment_mode =='MomoPay'){
            ...
            try{
                if(directFees()){
                    ...
                    if($request->payment_mode == 'MomoPay') {

                        $momo_trans_id =(string) Str::uuid();
                        session()->put('momo_trans_id',$momo_trans_id);
                        $momo_pay['invoice_id'] = $request->installment_id;
                        $momo_pay['amount'] = $request->amount;
                        $momo_pay['student_id'] = $request->student_id;
                        $momo_pay['type'] = "installment";
                        $momo_pay['trans_id'] = $momo_trans_id;
                        $momo_pay['transaction_id'] =$new_subPayment->id;
                        try{
                            $payInfo = MomoPaymentInformation::create($momo_pay);
                            DB::commit();
                            return redirect()->route('mompay.pay',$payInfo->id);
    ...
    public function feesGenerateModalChildView($id, $type_id)
    {
        ...
        return view('backEnd.studentPanel.view_bank_payment', ['fees_payments' => $fees_payments]);
    }
    ...
    public function feesGenerateModalBankView($sid, $ft_id)
    {
        ...
        return view('backEnd.studentPanel.view_bank_payment', ['fees_payments' => $fees_payments, 'amount' => $amount]);
    }
    ...
    public function directFeesGenerateModalChild(Request $request, $amount, $installment_id, $record_id)
    {
            ...
            $data['Paystack'] = SmPaymentMethhod::where('method', 'Paystack')
                                ->where('school_id', Auth::user()->school_id)
                                ->first('active_status');
             $data['MomoPay'] = SmPaymentMethhod::where('method', 'MomoPay')
                                ->where('school_id', Auth::user()->school_id)
                                ->first('active_status');
        ...
        if (moduleStatusCheck('SslCommerz')) {
            $data['SslCommerz'] = SmPaymentMethhod::where('method', 'SslCommerz')
                ->where('school_id', Auth::user()->school_id)
                ->first('active_status');
        }
        ...
        $method['SslCommerz'] = SmPaymentGatewaySetting::where('gateway_name', 'SslCommerz')->where('school_id', Auth::user()->school_id)->first();
    ...
    public function directFeesPaymentStripe($installment_id)
    {
        ...
        return view('backEnd.feesCollection.directFees.stripePaymentModal', ['stripe_info' => $stripe_info, 'installment' => $installment]);
    ...
    public function directFeesTotalPaymentSubmit(Request $request)
    {
        ...
        if ($request->payment_mode === 'Stripe' || $request->payment_mode === 'Paystack' || $request->payment_mode === 'PayPal' || $request->payment_mode === 'CcAveune' || $request->payment_mode === 'PhonePay') {
            ...
            if ($request->payment_mode === 'CcAveune') {
                ...
                return view('ccaveune::redirecformPage', ['encrypted_data' => $encrypted_data, 'access_code' => $access_code]);
            }
        ...
        if ($request->payment_mode === 'bank' && $request->bank_id== '') {
            Toastr::error('Bank Field Required', 'Failed');
