...
class SmStudentPanelController extends Controller
{
    ...
    public function studentProfile(Request $request, $id = null)
    {
        try {
            ...
            $exam_terms = SmExamType::with('examSettings')->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)
                ->whereHas('examSetups', function ($query) use ($student_detail): void {
                    $query->where('class_id', $student_detail->class_id)
                        ->where('section_id', $student_detail->section_id);
                })
                ->get();
    ...
    public function studentAssignment()
    {
        ...
        if (Auth()->user()->role_id !== 1) {
            if ($user->role_id === 2) {
                ...
                SmNotification::clearUnreadCache($user->student->id, 2);
                SmNotification::clearUnreadCache($user->id, 2);
            }
        }
    ...
    {
        $student_detail = Auth::user()->student->load('studentRecords.teacherEvaluation');
        $records = $student_detail->studentRecords;
