...
class SmStudentIdCardController extends Controller
{
    public function index()
    {
        try {
            ...
            return view('backEnd.admin.idCard.student_id_card_list', ['id_cards' => $id_cards]);
    ...
    public function create_id_card()
    {
        try {
            $id_cards = SmStudentIdCard::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
            $roles = InfixRole::select('*')->where('id', '!=', 1)->where('is_saas', 0)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();

            return view('backEnd.admin.idCard.student_id_card', ['id_cards' => $id_cards, 'roles' => $roles]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->file('logo') ) {
                ...
                $file = $request->file('logo');
                $fileNameLogo = fileUpload($file, 'public/uploads/studentIdCard/');
            }
            ...
            if ($request->file('signature') ) {
                ...
                $file = $request->file('signature');
                $fileNameSignature = fileUpload($file, 'public/uploads/studentIdCard/');
            }
            ...
            if ($request->file('background_img') ) {
                ...
                $file = $request->file('background_img');
                $fileNameBackground = fileUpload($file, 'public/uploads/studentIdCard/');
            }
            ...
            if ($request->file('profile_image') ) {
                ...
                $file = $request->file('profile_image');
                $fileNameProfileImage = fileUpload($file, 'public/uploads/studentIdCard/');
            }

            $smStudentIdCard = new SmStudentIdCard();
            $smStudentIdCard->title = $request->title;
            $smStudentIdCard->logo = $fileNameLogo;
            $smStudentIdCard->school_id = Auth::user()->school_id;
            $smStudentIdCard->academic_id = getAcademicId();

            $smStudentIdCard->signature = $fileNameSignature;
            $smStudentIdCard->background_img = $fileNameBackground;
            $smStudentIdCard->profile_image = $fileNameProfileImage;
            if (in_array(2, $request->applicable_user) || in_array(3, $request->applicable_user)) {
                $smStudentIdCard->role_id = json_encode($request->applicable_user);
            } else {
                $smStudentIdCard->role_id = json_encode($request->role);
            }

            $smStudentIdCard->page_layout_style = $request->page_layout_style;
            $smStudentIdCard->user_photo_style = $request->user_photo_style;
            $smStudentIdCard->user_photo_width = $request->user_photo_width;
            $smStudentIdCard->user_photo_height = $request->user_photo_height;
            $smStudentIdCard->pl_width = $request->pl_width;
            $smStudentIdCard->pl_height = $request->pl_height;
            $smStudentIdCard->t_space = $request->t_space;
            $smStudentIdCard->b_space = $request->b_space;
            $smStudentIdCard->l_space = $request->l_space;
            $smStudentIdCard->r_space = $request->r_space;
            $smStudentIdCard->admission_no = $request->admission_no;
            $smStudentIdCard->student_name = $request->student_name;
            $smStudentIdCard->class = $request->class;
            $smStudentIdCard->father_name = $request->father_name;
            $smStudentIdCard->mother_name = $request->mother_name;
            $smStudentIdCard->student_address = $request->student_address;
            $smStudentIdCard->dob = $request->dob;
            $smStudentIdCard->blood = $request->blood;
            $smStudentIdCard->photo = $request->photo;
            $smStudentIdCard->signature_status = $request->signature_status;
            if (in_array(3, $request->applicable_user)) {
                $smStudentIdCard->phone_number = $request->phone_number;
            }

            $result = $smStudentIdCard->save();
            if ($result) {
    ...
    public function edit($id)
    {
        try {
            ...
            return view('backEnd.admin.idCard.student_id_card', ['id_cards' => $id_cards, 'id_card' => $id_card, 'roles' => $roles]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function generateIdCard()
    {
        ...
        try {
            ...
            return view('backEnd.admin.idCard.generate_id_card', ['id_cards' => $id_cards, 'roles' => $roles]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function generateIdCardSearch(Request $request)
    {
        ...
        try {
            ...
            return view('backEnd.admin.idCard.generate_id_card_old', ['id_cards' => $id_cards, 'class_id' => $class_id, 'classes' => $classes, 'students' => $students, 'card_id' => $card_id]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function generateIdCardPrint($s_id, $c_id)
    {
        ...
        try {
            ...
            $s_ids = explode('-', $s_id);
            $students = [];
            foreach ($s_ids as $_id) {
                $students[] = SmStudent::find($_id);
            }
