...
class SmStudentPromoteController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $classes = branchWise(SmClass::where('active_status', 1)->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)->get());
            $exams = branchWise(SmExamType::where('active_status', 1)->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)->get());
    ...
    public function studentCurrentSearch(Request $request)
    {
        ...
        try {
            ...
            $students = branchWise(SmStudent::query()->with('class', 'section', 'studentRecords'));
            ...
            $sessions = SmAcademicYear::where('active_status', 1)
                ->where('school_id', $auth->school_id)
                ->get();
            $currrent_academic_class = branchWise(SmClass::where('active_status', 1)
                ->where('academic_id', $request->current_session)
                ->where('school_id', $auth->school_id)
                ->get());

            $classes = branchWise(SmClass::with(['classSection' => function ($query) {
                $query->select(['section_id', 'class_id', 'id']);
            ...
            if ($next_class) {
                $next_sections = branchWise(SmClassSection::with('sectionWithoutGlobal')->where('class_id', '=', $next_class->id)->where('academic_id', $request->promote_session)
                    ->where('school_id', Auth::user()->school_id)->withoutGlobalScope(StatusAcademicSchoolScope::class)->get());
            }
            $search_current_class = SmClass::withoutGlobalScopes()->findOrFail($request->current_class);
            $search_current_section = SmSection::withoutGlobalScopes()->find($request->current_section);
            if (shiftEnable()) {
                    ...
                    if (gv($student_data, 'student') && gv($student_data, 'class') && gv($student_data, 'section')) {
                        ...
                        if (! $exit_record) {
                            ...
                            if (moduleStatusCheck('Branch')) {
                                $student_promote->branch_id = $request->branch_id;
                            }
                    ...
                    if (gv($student_data, 'student') && is_null(gv($student_data, 'section'))) {
                        ...
                        if ($student_record) {
                            ...
                            if (moduleStatusCheck('University')) {
                                ...
                                $graduate->session_id = SmSession::where('id', $student_record->session_id)
                                    ->where('school_id', Auth::user()->school_id)
                                    ->exists()
                                    ? $student_record->session_id
                                    : null;
                                if (shiftEnable()) {
    ...
    public function studentSearchWithExam(Request $request)
    {
        ...
        try {
            ...
            if ($request->branch_id) {
                $student_ids->where('branch_id', $request->branch_id);
            }
            ...
            $current_shift = $request->shift ?? null;
            ...
            $classes = branchWise(SmClass::withOutGlobalScope(StatusAcademicSchoolScope::class)->with('classSection')->where('active_status', 1)
                ->where('academic_id', $request->promote_session)
                ->where('school_id', Auth::user()->school_id)
                ->get());
            ...
            $next_sections = collect();
            if ($next_class) {
                $next_sections = branchWise(SmClassSection::with('sectionWithoutGlobal')
                    ->where('class_id', $next_class->id)
                    ->where('academic_id', $request->promote_session)
                    ->where('school_id', Auth::user()->school_id)
                    ->withoutGlobalScope(StatusAcademicSchoolScope::class)
                    ->get());
            }
            $search_current_class = SmClass::withOutGlobalScope(GlobalAcademicScope::class)->findOrFail($request->current_class);
            $search_current_section = SmSection::withOutGlobalScope(GlobalAcademicScope::class)->find($request->current_section);
            $search_current_academic_year = SmAcademicYear::find($request->current_session);
            $search_promote_academic_year = SmAcademicYear::find($request->promote_session);
            $search_current_shift = Shift::find($request->shift) ?? null;
            $search_current_branch = '';
            if (moduleStatusCheck('Branch')) {
                $search_current_branch = Branch::find($request->branch_id) ?? null;
            }
            if (shiftEnable()) {
            ...
            return view('backEnd.studentInformation.student_promote_with_exam', compact('next_class', 'next_sections', 'sessions', 'classes', 'students', 'current_session', 'current_class', 'current_section', 'promote_session', 'search_current_class', 'search_current_section', 'search_current_academic_year', 'search_promote_academic_year', 'sections', 'exams', 'exam_id', 'search_exams', 'current_shift', 'search_current_shift', 'branch_id', 'search_current_branch'));
        } catch (Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
