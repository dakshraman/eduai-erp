...
class SmTeacherController extends Controller
{
    ...
    public function uploadContentEdit($id)
    {
        ...
        if (teacherAccess()) {
            $uploadContents = SmTeacherUploadContent::with('classes', 'Sections')->where(function ($q): void {
                $q->where('created_by', Auth::user()->id)->orWhere('available_for_admin', 1);
        ...
        return view('backEnd.teacher.uploadContentList', ['editData' => $editData, 'contentTypes' => $contentTypes, 'classes' => $classes, 'sections' => $sections, 'uploadContents' => $uploadContents]);
    }
