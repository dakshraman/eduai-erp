...
use App\Models\SmContactMessage;
use App\Models\SmContactPage;
use App\Models\SmCourse;
use App\Models\SmCoursePage;
...
class SmFrontendController extends Controller
{
    ...
    {
        try {
            $setting = SmGeneralSettings::where('school_id', app('school')->id)->first() ?? SmGeneralSettings::where('school_id', 1)->first();

            if ($setting && $setting->website_btn === 0) {
                if (auth()->check()) {
            ...
            $url = explode('/', (string) ($setting ? $setting->website_url : ''));
            ...
            $home_data = is_array($home_data) ? $home_data : [];

            if (!$setting || $setting->website_url == '') {
                return view('frontEnd.home.light_home')->with(array_merge($data, $home_data));
            }

            if (!empty($url) && isset($url[max(array_keys($url))]) && $url[max(array_keys($url))] === 'home') {
                return view('frontEnd.home.light_home')->with(array_merge($data, $home_data));
            }
