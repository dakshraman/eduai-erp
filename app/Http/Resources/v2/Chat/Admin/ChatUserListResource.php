...
class ChatUserListResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        $blocked = BlockUser::where('block_by', auth()->id())
            ->where('block_to', (int) $this->id)
            ->exists();
        ...
        return [
            ...
            'image' => $this->avatar_url ? (string) assetPath($this->avatar_url) : null,
            'blocked' => $blocked,
            'active_status' => (string) $status,
            'status_color' => (string) $color,
            'last_message' => (string) @$conversation->first()->message,
            'last_message_time' => date('Y-m-d H:i:s', strtotime(@$conversation->first()->created_at)),
