...
class GoogleFCMTokenService
{
    ...
    protected function loadCredentials()
    {
        ...
        try {
            ...
            throw new \RuntimeException('The Firebase service-account JSON is invalid.', 0, $exception);
        }
        ...
        if ($privateKey === '' || @openssl_pkey_get_private($privateKey) === false) {
            throw new \RuntimeException('The Firebase private key is missing or invalid. Upload a fresh service-account JSON file.');
        }

        if (empty($credentials['client_email']) || empty($credentials['project_id'])) {
            throw new \RuntimeException('The Firebase service-account JSON is missing client_email or project_id.');
        }
