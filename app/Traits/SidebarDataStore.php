...
{
    public function defaultSidebarStore($role_id = null)
    {
    ...
    public function resetSidebarStore($role_name = 'staff')
    {
    ...
    public function permissions($role_name = 'staff')
    {
        ...
        if ($role_name === 'staff') {
            $perInfo = $perInfo->where(function ($q) {
                $q->where('is_admin', 1)->orWhere('is_teacher', 1);
            });
        }
    ...
    public function storeSidebar($sidebar, $key, $parent_id, $role_id)
    {
        ...
        Sidebar::updateOrCreate([
            ...
            'position' => $sidebar->position ?? ($key + 1),
            'level' => $sidebar->type,
            'parent' => $parent_id,
        ]);
    ...
    public function modulePermissionSidebar($role_id = null)
    {
    ...
    public function parentId($sidebar, $role_name = 'staff')
    {
    ...
    public function allActivePaidModules()
    {
        $activeModules = [];
        $key = 'paid_modules_school_' . auth()->user()->school_id;
        $modules = Cache::rememberForever($key, function () {
    ...
    public function paidModuleRoutes($role_name = 'staff')
    {
        // ->when($role_name == 'student',function($q){
        //         $q->where('is_student',1);
        //     })->when($role_name == 'parent',function($q){
    ...
    public function deActiveForPgsql()
    {
        if (db_engine() !== 'mysql') {
    ...
    public function deActiveForSaas()
    {
        if (moduleStatusCheck('Saas')) {
            $list = ['update-system', 'utility', 'manage-adons', 'backup-settings', 'utility', 'language-list'];
            Permission::whereIn('route', $list)
                ->where('school_id', auth()->user()->school_id)
                ->update(['is_menu' => 0, 'menu_status' => 0, 'status' => 0, 'is_saas' => 1]);
            $saasSettingsRoutes = SaasSettings::where('saas_status', 1)->pluck('route')->toArray();
            if ($saasSettingsRoutes) {
                Permission::whereIn('route', $saasSettingsRoutes)
                    ->where('school_id', auth()->user()->school_id)
                    ->update(['is_menu' => 1, 'menu_status' => 1, 'status' => 1, 'is_saas' => 0]);
            }
        }
