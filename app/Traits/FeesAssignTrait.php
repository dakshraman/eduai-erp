...
{
    public function assignSubjectFees($student_record, $subject_id, $semester_label_id, $fees_group_id = null, $withOutSubject = null): void
    {
        try {
            ...
            if ($fees_master) {
                ...
                if (! $exist) {
                    $studentRecord = StudentRecord::find($student_record);
                    $smFeesAssign = new SmFeesAssign();
                    $smFeesAssign->fees_amount = $fees_master->amount;
                    $smFeesAssign->fees_master_id = $fees_master->id;
                    $smFeesAssign->student_id = $studentRecord->student_id;
                    $smFeesAssign->record_id = $studentRecord->id;
                    $smFeesAssign->un_academic_id = getAcademicId();
                    $smFeesAssign->school_id = auth()->user()->school_id;
                    $smFeesAssign->un_semester_label_id = $semester_label_id;
                    $smFeesAssign->save();
                    ...
                    if (count($installments) > 0) {
                        foreach ($installments as $installment) {
                            ...
                            if ($checkExist) {
                                ...
                                $assignInstallment->assign_ids = json_encode([$smFeesAssign->id]);
                            }
    ...
    public function adjustCreditWithFees($record_id): void
    {
        $already_paid = 0;
        ...
        if ($have_credit) {
            ...
            if ($balace_amount > 0) {
                foreach ($assigned_ins as $assigned_in) {
                    if ($after_paid <= 0) {
                        break;
                    }

                    $installment_due = $assigned_in->amount - ($assigned_in->discount_amount + $assigned_in->paid_amount);
                    if ($installment_due && $after_paid > 0) {
                        ...
                        $assigned_in = UnFeesInstallmentAssign::find($assigned_in->id);
                        $assigned_in->payment_date = $newformat;
                        $assigned_in->payment_mode = $payment_mode_name;
                        $assigned_in->note = null;
                        $assigned_in->slip = null;
                        $assigned_in->active_status = 1;
                        $assigned_in->save();
                        $payable_amount = discountFeesAmount($assigned_in->id);
                        $sub_payment = $assigned_in->payments->sum('paid_amount');
                        ...
                        $new_subPayment->un_fees_installment_assign_id = $assigned_in->id;
                        ...
                        $new_subPayment->fees_type_id = $assigned_in->fees_type_id;
                        ...
                        $assigned_in->active_status = ($sub_payment + $paid_amount) === $payable_amount ? 1 : 2;
                        $assigned_in->paid_amount = $sub_payment + $paid_amount;
                        $assigned_in->save();
            ...
            foreach ($feesCredits as $feeCredit) {
                if ($feeCredit->amount && $feesCreditSum) {
                    $feeCredit->amount -= $already_paid;
    ...
    public function assignDiscount($discount_id, $record_id): void
    {
        ...
        if ($fees_discount && $installments) {
            ...
            if ($extra_credit > 0) {
                $feesInstallmentCredit = new FeesInstallmentCredit();
                $feesInstallmentCredit->student_id = $feesInstallment->student_id;
                $feesInstallmentCredit->student_record_id = $record_id;
                $feesInstallmentCredit->school_id = $feesInstallment->school_id;
                $feesInstallmentCredit->amount = $fees_discount->amount;
                $feesInstallmentCredit->save();
            }
        }
    }

    public function feesMasterUnAssign($record_id, $semester_label_id, $fees_group_id): void
    {
        StudentRecord::find($record_id);
        ...
        if ($installments && ($selectedAssignFees && is_null($have_payemts))) {
