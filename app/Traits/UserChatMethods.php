...
trait UserChatMethods
{
    ...
    {
        if ((Request::is('chat/*') || Request::is('api/chat/*')) && $this->role_id) {
    ...
    public function canChatWith(int $userId): bool
    {
        if ($userId === (int) auth()->id()) {
            return false;
        }

        $blocked = BlockUser::where(function ($query) use ($userId): void {
            $query->where('block_by', auth()->id())->where('block_to', $userId);
        })->orWhere(function ($query) use ($userId): void {
            $query->where('block_by', $userId)->where('block_to', auth()->id());
        })->exists();
        ...
        return Invitation::where('status', 1)
            ->where(function ($query) use ($userId): void {
                $query->where(function ($nested) use ($userId): void {
                    $nested->where('to', auth()->id())->where('from', $userId);
                })->orWhere(function ($nested) use ($userId): void {
                    $nested->where('from', auth()->id())->where('to', $userId);
                });
            })->exists();
    }
    ...
    {
        return Invitation::where(function ($query): void {
    ...
    {
        return $conversation = Conversation::with('forwardFrom', 'reply', 'fromUser', 'toUser')->where(function ($query) use ($userId): void {
    ...
    {
        return Conversation::with('forwardFrom', 'reply', 'fromUser', 'toUser')->where(function ($query) use ($userId): void {
            $query->where('from_id', $userId);
            $query->where('to_id', auth()->id());
    ...
    public function userSpecificConversationForLoadMore($userId, $ids)
    {
        ...
        return $conversation->filter(function ($value, $key) use ($ids): bool {
