...
class PaypalPayment
{
    ...
    public function __construct()
    {
        ...
        $this->_api_context->setTestMode(mb_strtolower($paypalDetails->gateway_mode) !== 'live');

    }

    public function handle(array $data)
    {
        $response = $this->_api_context->purchase([
        ...
        if ($data['type'] === 'Wallet') {
            ...
            $walletTransaction->payment_method = $data['payment_method'];
    ...
    public function successCallback()
    {
        ...
        try {
            ...
            $transaction = $this->_api_context->completePurchase([
            ...
            if ($response->isSuccessful() && $response->getData()['state'] === 'approved') {
                ...
                if (Session::get('payment_type') === 'Fees' && ! is_null(Session::get('fees_payment_id'))) {

                    $transcation = FmFeesTransaction::find(Session::get('fees_payment_id'));
                    $feesExtendedController = new FeesExtendedController();
                    $feesExtendedController->addFeesAmount(Session::get('fees_payment_id'), null);
                ...
                if (Session::get('payment_type') === 'Lms') {
                ...
                if (Session::get('payment_type') === 'direct_fees' && Session::get('sub_payment_id')) {
    ...
    public function cancelCallback()
    {
        ...
        if (Session::get('payment_type') === 'Fees') {
