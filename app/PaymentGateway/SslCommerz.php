...
class SslCommerz
{
    ...
    public function getPaymentData($config, $data)
    {
        ...
        $post_data['cart'] = json_encode([
            ['product' => $data['type'].' Payment', 'amount' => $data['amount']],
        ]);
    ...
    private function studentOldFeesPaymentData($config, $data)
    {
        ...
        // CART PARAMETERS
        $post_data['cart'] = json_encode([
            ['product' => $data['type'].' Course Purchase', 'amount' => $data['amount']],
        ]);
