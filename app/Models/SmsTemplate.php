...
class SmsTemplate extends Model
{
    ...
    protected static function booted(): void
    {
        static::creating(function (self $template): void {
            $template->purpose ??= $template->subject ?? 'general';
        });
    }

    public static function smsTempleteToBody($body, array $data)
    {
        $user = null;
        if (@$data['user_email']) {
            $user = User::where('email', $data['user_email'])->first();
        } elseif (@$data['receiver_email']) {
        ...
        $body = str_replace('[attendance_type]', @$data['attendance_type'], $body);

        return str_replace('[expired_time]', @$data['expired_time'], $body);
    }

    public static function emailTempleteToBody($body, array $data)
    {
        ...
        $body = str_replace('[attendance_type]',@$data['attendance_type'],$body);
        $body = str_replace('[otp_code]',@$data['otp_code'],$body);
        return str_replace('[status]', @$data['status'], $body);
    }
}
