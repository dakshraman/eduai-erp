...
class PageBuilderController extends Controller
{
    ...
    public function build($pageId, Request $request)
    {
        ...
        if (file_exists(resource_path($this->theme_pb_path))) {
            $componentDirectories = array_diff(scandir(resource_path($this->theme_pb_path)), ['..', '.']);
        }
        foreach ($componentDirectories as $directory) {
            if ($directory !== 'header-content' && $directory !== 'footer-content' && $directory !== 'footer-widget' && $directory !== 'footer-copyright') {
    ...
    public function renderPage($pageSlug)
    {
        ...
        $page = Page::where('school_id', app('school')->id)->select('id', 'name', 'title', 'description', 'slug', 'settings', 'status', 'home_page')
            ->whereSlug($pageSlug)
        ...
        if ($setting->website_url && $setting->website_url != '/') {

            if ($page->home_page && url()->current() != url($setting->website_url)) {
    ...
    public function footer(Request $request)
    {
        ...
        foreach ($componentDirectories as $directory) {
            if ($directory === 'footer-content' || $directory === 'footer-copyright' || $directory === 'footer-widget') {
                if (file_exists(resource_path($this->theme_pb_path.'/'.$directory.'/settings.php'))) {
                    $currentSettings = include resource_path($this->theme_pb_path.'/'.$directory.'/settings.php');
