...
class CreateSmStudentRegistrationFieldsTable extends Migration
{
    ...
    {
        Schema::create('sm_student_registration_fields', function (Blueprint $blueprint): void {
            $blueprint->id();
            $blueprint->string('field_name')->nullable();
            $blueprint->string('label_name')->nullable();
            $blueprint->tinyInteger('is_show')->nullable()->default(1);
            $blueprint->tinyInteger('active_status')->nullable()->default(1);
            $blueprint->tinyInteger('is_required')->nullable()->default(0);
            $blueprint->tinyInteger('student_edit')->nullable()->default(0);
            $blueprint->tinyInteger('parent_edit')->nullable()->default(0);
            $blueprint->tinyInteger('staff_edit')->nullable()->default(0);
            $blueprint->tinyInteger('type')->nullable()->comment('1=student,2=staff');
            $blueprint->tinyInteger('is_system_required')->nullable()->default('0');
        ...
        try {
            ...
            foreach ($all_schools as $school) {
                ...
                foreach ($currencies as $currency) {
                    if (! App\Models\SmCurrency::where('name', $currency[1])->where('school_id', $school->id)->first()) {
                        $store = new App\Models\SmCurrency();
                ...
                if (! App\Models\SmBackgroundSetting::where('title', 'Dashboard Background')->where('school_id', $school->id)->first()) {
                    $b = new App\Models\SmBackgroundSetting();
                ...
                if (! App\Models\SmBackgroundSetting::where('title', 'Login Background')->where('school_id', $school->id)->first()) {
                    $b = new App\Models\SmBackgroundSetting();
                    $b->school_id = $school->id;
            ...
            foreach ($all_schools as $all_school) {
                SmStudentRegistrationField::where('school_id', $all_school->id)->whereIn('field_name', $required_fields)->update(['is_required' => 1, 'is_system_required' => 1]);
            }
        ...
        $infixModuleInfo = new InfixModuleInfo();
        $infixModuleInfo->id = 951;
        $infixModuleInfo->module_id = 3;
        $infixModuleInfo->parent_id = 61;
        $infixModuleInfo->type = '2';
        $infixModuleInfo->is_saas = 0;
        $infixModuleInfo->route = 'Student Settings';
        $infixModuleInfo->lang_name = 'student_settings';
        $infixModuleInfo->icon_class = 'student_settings';
        $infixModuleInfo->active_status = 1;
        $infixModuleInfo->created_by = 1;
        $infixModuleInfo->updated_by = 1;
        $infixModuleInfo->school_id = 1;
        $infixModuleInfo->save();
