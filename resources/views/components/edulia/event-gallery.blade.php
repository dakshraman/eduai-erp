...
@php
    $events_count = App\Models\SmEvent::where('school_id', app()->bound('school') ? app('school')->id : 1)->count();
@endphp
@if ($events->isEmpty())
            ...
            <div class="events_item">
                @if (file_exists(public_path($event->uplad_image_file)))
                    <div class="events_item_img">
                        <img src="{{assetPath($event->uplad_image_file)}}" alt="{{$event->event_title}}">
                    </div>
                @endif
                <div class="events_item_inner">
                    ...
                    @if ($event->event_title)
                        <a href="{{route('frontend.event-details', $event->id)}}"
                           class="events_item_inner_title event_short_title">
                            {{$event->event_title}}
                        </a>
                    @endif
                    <a href="{{route('frontend.event-details', $event->id)}}">
        ...
        <div class="row text-center">
            <div class="col-md-12">
                <div class="load_more section_padding_top">
                    <a href="#" class="site_btn load_more_event_btn"
                       data-skip="{{$count}}">{{ __('edulia.load_more') }}</a>
                </div>
            </div>
        </div>
    @endif
