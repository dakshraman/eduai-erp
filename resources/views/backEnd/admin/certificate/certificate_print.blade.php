...
<head>
    ...
    <style>
        ...
        .bgImage {
            ...
            background-image: url({{assetPath($certificate->file)}});
...
@foreach($users as $user)
    @if (moduleStatusCheck('Lms'))
        <div class="DivBody" style="margin-bottom: {{@$gridGap}}px">
            @php
                $body = App\Models\SmStudentCertificate::certificateBody($certificate->body, 'Lms', $user->id);  #passing here student_id
            @endphp

            <img class="position_bg" src="{{assetPath($certificate->file)}}" style="height: 100vh; width: 100% !important">
    ...
    @else
        <div class="certificate_box_wrapper"
             style="background-image: url('{{ isset($certificate)? assetPath($certificate->background_image): ''}}'); margin-bottom: {{$gridGap}}px;">
            @php
                $body = App\Models\SmStudentCertificate::certificateBody($certificate->body, @$certificate->role, $user->user_id);
            @endphp
            {!!$body!!}
        </div>
        @endif
