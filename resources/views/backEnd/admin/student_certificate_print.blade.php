...
<head>
    ...
    <link rel="stylesheet" href="{{ assetPath('modules/Certificate/Resources/assets/css/editor.css') }}">
    <link rel="stylesheet" href="{{ assetPath('modules/Certificate/Resources/assets/css/paper/normalize.min.css') }}">
    <link rel="stylesheet" href="{{ assetPath('modules/Certificate/Resources/assets/css/paper/paper.css') }}">
    <link rel="stylesheet" href="{{ assetPath('public/backEnd/vendors/css/bootstrap.min.css') }}"/>
    <style>
        @font-face {
            font-family: 'Pinyon Script';
            src: url('{{ assetPath('public/backEnd/fonts/pinyonscript-regular-webfont.woff') }}') format('woff2'),
            url('{{ assetPath('public/backEnd/fonts/pinyonscript-regular-webfont.woff') }}') format('woff');
            font-weight: normal;
            font-style: normal;
    ...
    @php
        ...
        $certificateImage = @$certificate->file && file_exists($certificate->file)
            ? assetPath($certificate->file)
            : null;
    @endphp
...
<div class="certificate setup_height_width">
    @foreach ($students as $key => $student)
        <div class="DivBody setup_height_width">
            @if ($certificateImage)
                <img class="position_bg setup_height_width" src="{{ $certificateImage }}">
            @endif
                    ...
                    <div style="display: flex; justify-content: center;">
                        ...
                        <div style="flex: 8;">
                                ...
                                <div style="flex: 6;">
                                    @if($certificate->student_photo == 1)
                                        <img class="student-meta-img img-100"
                                             src="{{ file_exists(@$student->student->student_photo) ? assetPath($student->student->student_photo) : assetPath('public/uploads/staff/demo/staff.jpg') }}"
                                             alt="">
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="edu_summernote" style="text-align: center; margin-top: 7%;">
                                {{ isset($student['student_id']) ? App\Models\SmStudentCertificate::certificateBody($certificate->body, 2, $student['student_id']) : '' }}
                            </div>
...
<script src="{{ assetPath('https://infix_edu.test/public/backEnd/vendors/js/jquery-3.2.1.min.js') }}"></script>
