@php
    ...
    $section_id = is_array($section_id) ? ($section_id[0] ?? null) : $section_id;
    $subject_id = $selected && isset($selected['subject_id']) ? $selected['subject_id'] : null;
    $subject_id = is_array($subject_id) ? ($subject_id[0] ?? null) : $subject_id;
    $student_id = $selected && isset($selected['student_id']) ? $selected['student_id'] : null;
    $selectedBranchId = moduleStatusCheck('Branch') ? old('branch_id', $branch_id ?? userBranch()) : null;

    if ($academic_year) {
    $classes = isset($classes) ? $classes : (classes($academic_year) ?? null);
    ...
    $students = $class_id && $section_id ? students($class_id, $section_id) : null;
    }

    if ($selectedBranchId) {
    $classes = isset($classes) ? branchWiseApplyFilter($classes, 'branch_id', $selectedBranchId) : null;
    $sections = isset($sections) ? branchWiseApplyFilter($sections, 'branch_id', $selectedBranchId) : null;
    $subjects = isset($subjects) ? branchWiseApplyFilter($subjects, 'branch_id', $selectedBranchId) : null;
    $students = isset($students) ? branchWiseApplyFilter($students, 'branch_id', $selectedBranchId) : null;
    }
    $visiable = $visiable ?? [];
            ...
            <select
                ...
                @isset($sessions)
                    @foreach ($sessions as $session)
                        ...
                        <option value="{{ $session->id }}"
                                {{ old($academic_name, $academic_year ?? getAcademicId()) == $session->id ? 'selected' : '' }}>
    ...
    @if (in_array('shift', $visiable))
        <div class="{{ $div . ' ' . $mt }}">
            <div class="primary_input " id="common_select_shifts_div">
                ...
                <select class="primary_select form-control{{ $errors->has('shift') ? ' is-invalid' : '' }}"
                        name="{{ $shift_name }}"
                        id="common_select_shifts">
                    <option data-display="@lang('common.select_shift') {{ in_array('shift', $required) ? '*' : '' }}"
                            value="">
                    ...
                    @foreach (shifts() as $shift)
                        ...
                        <option value="{{ $shift->id }}"
                                {{ old('shift', $shift_id ?? '') == $shift->id ? 'selected' : '' }}>
                            {{ $shift->name }}
                        </option>
                    @endforeach
                </select>
...
@if (in_array('class', $visiable))
    @php
        $selectedClassId = old($class_name, $class_id ?? null);
        $classOptions = isset($classes) ? collect($classes) : collect();
        $selectedClass = null;

        if ($selectedClassId && ! $classOptions->contains('id', (int) $selectedClassId)) {
            $selectedClassQuery = App\Models\SmClass::withoutGlobalScopes()->where('id', $selectedClassId);
            $selectedClass = $selectedBranchId
                ? branchWiseApplyFilter($selectedClassQuery, 'branch_id', $selectedBranchId)->first()
                : $selectedClassQuery->first();
        }
    @endphp
    <div class="{{ $div . ' ' . $mt }}" id="common_select_classes_div">
        <div class="primary_input mb-25">
            ...
            <select class="primary_select form-control{{ $errors->has('class_id') ? ' is-invalid' : '' }}"
                ...
                <option data-display="@lang('common.select_class') {{ in_array('class', $required) ? '*' : '' }}"
                        value="">
...
@if (in_array('section', $visiable))
    <div class="{{ $div . ' ' . $mt }}" id="common_select_sections_div">
        ...
        <select class="primary_select form-control{{ $errors->has('section_id') ? ' is-invalid' : '' }} select_section"
            ...
            <option data-display="@lang('common.select_section') {{ in_array('section', $required) ? '*' : '' }}"
                    value="">
            ...
            @if(old($class_name))
                @php
                    $sections = collect();
                    if (old($class_name)) {
                        $sections = \App\Models\SmClassSection::where('class_id', old($class_name))
                            ->where('school_id', Auth::user()->school_id)
                            ->get();

                        if ($selectedBranchId) {
                            $sections = branchWiseApplyFilter($sections, 'branch_id', $selectedBranchId);
                        }
                    }
                @endphp
                @isset($sections)
                    @foreach ($sections as $section)
                        <option value="{{ $section->section_id }}"
                                {{ old($section_name, $section_id ?? '') == $section->section_id ? 'selected' : '' }}>
        ...
        <div class="pull-right loader loader_style" id="common_select_sections_loader"
             style="margin-top: -30px;padding-right: 21px;">
            <img src="{{ assetPath('public/backEnd/img/demo_wait.gif') }}" alt="" style="width: 28px;height:28px;">
        </div>
...
@if (in_array('subject', $visiable))
    <div class="{{ $div . ' ' . $mt }}" id="common_select_subject_div">
        ...
        <select class="primary_select form-control{{ $errors->has('subject_id') ? ' is-invalid' : '' }} select_subject"
            ...
            <option data-display="@lang('common.select_subject') {{ in_array('subject', $required) ? ' *' : '' }}"
                    value="">
            ...
            @isset($subjects)
                @foreach ($subjects as $subject)
                    ...
                    <option value="{{ $subject->subject_id }}"
                            {{ old($subject_name, $subject_id ?? '') == $subject->subject_id ? 'selected' : '' }}>
                        {{ $subject->subject->subject_name }}
                    </option>
                @endforeach
            @endisset
        </select>
        <div class="pull-right loader loader_style" id="common_select_subject_loader"
             style="margin-top: -30px;padding-right: 21px;">
            <img src="{{ assetPath('public/backEnd/img/demo_wait.gif') }}" alt="" style="width: 28px;height:28px;">
        </div>
...
@if (in_array('student', $visiable))
    @php
        $selectedStudentId = old('student', $student_id ?? null);
        $studentOptions = isset($students) ? collect($students) : collect();
        $selectedStudent = null;

        if ($selectedStudentId && ! $studentOptions->contains('id', (int) $selectedStudentId)) {
            $selectedStudentQuery = App\Models\SmStudent::withoutGlobalScopes()->where('id', $selectedStudentId);
            $selectedStudent = $selectedBranchId
                ? branchWiseApplyFilter($selectedStudentQuery, 'branch_id', $selectedBranchId)->first()
                : $selectedStudentQuery->first();
        }
    @endphp
    ...
    <div class="{{ $div . ' ' . $mt }}" id="common_select_student_div">
        ...
        <select class="primary_select form-control{{ $errors->has('student') ? ' is-invalid' : '' }}"
            ...
            <option data-display="@lang('reports.select_student') {{ in_array('student', $required) ? '*' : '' }}"
                    value="">
    ...
    <script>
        $(document).ready(function () {
                        ...
                        if (data.length) {
                            $.each(data, function (i, className) {
                                $("#common_select_classes").append(
                                    $("<option>", {
                                        value: className.id,
                                            text: className.class_name || className.name || '',
                                    })
                                );
                            });
                        }
            ...
            $("#common_select_classes").on("change", function () {
                ...
                var i = 0;
                var shift_id = null;
                if ($('#common_select_shifts').length) {
                    shift_id = $('#common_select_shifts').find(":selected").val();
                }
                var formData = {
                    id: $(this).val(),
                    shift_id: shift_id,
                    branch_id: $('select[name="branch_id"]').length ? $('select[name="branch_id"]').val() : '',
                };
            ...
            $("#common_select_sections").on("change", function () {
                ...
                var formData = {
                    ...
                    academic_year: $("#common_academic_years").length ? $("#common_academic_years").val() : '',
                    branch_id: $('select[name="branch_id"]').length ? $('select[name="branch_id"]').val() : '',
                    shift_id: $('#common_select_shifts').length ? $('#common_select_shifts').val() : '',
                };
            ...
            function getSubject(class_id, section_id) {
                var url = $("#url").val();
                var i = 0;
                var academic_id = $("#common_academic_years").length ? $("#common_academic_years").val() : '';
                var branch_id = $('select[name="branch_id"]').length ? $('select[name="branch_id"]').val() : '';
                var shift_id = $('#common_select_shifts').length ? $('#common_select_shifts').val() : '';
            ...
            // Handle Branch Change - Load Shifts and Classes by Branch
            $(document).on('change', '.for-data-get-shift-class', function() {
                var url = $("#url").val();
                var branch_id = $(this).val();
                var academic_id = $("#common_academic_years").val();

                if (!branch_id) return;

                // Load shifts by branch
                $.ajax({
                    type: "GET",
                    data: {
                        branch_id: branch_id,
