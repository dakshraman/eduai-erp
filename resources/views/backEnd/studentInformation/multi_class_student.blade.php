...
@section('mainContent')
    ...
    <section class="admin-visitor-area up_admin_visitor">
        <div class="container-fluid p-0">
            <div class="row">
                <div class="col-lg-12">
                    {{ html()->form('GET', route('student.multi-class-student'))->attributes([
                        ...
                        <div class="row">
                            ...
                            @else
                                @include('backEnd.common.search_criteria', [
                                    'div' => 'col-lg-3',
                                    'visiable' => ['academic', 'shift','class', 'section', 'student'],
                                    'class_name' => 'class_id',
                            ...
                            <div class="row">
                                @foreach ($students as $student)
                                    <div class="col-xl-6 col-12 mb-20 d-flex">
                                        {{ html()->form('POST', route('multi-record-store'))->attributes([
                                                'class' => 'w-100 d-flex',
                                                'id' => 'form_' . $student->id,
                                            ])->open() }}
                                        ...
                                        <div class="student_rec_card">
                                            ...
                                            <input type="hidden" name="session"
                                                value="{{ $academic_year ?? getAcademicId() }}">
                                            @if (moduleStatusCheck('Branch'))
                                            ...
                                            @else
                                                <input type="hidden" id="div_button"
                                                value="{{ generalSetting()->multiple_roll == 1 ? 'col-sm-4 col-6' : 'col-sm-4 col-6' }}">
                                            @endif
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.addMore', function() {
                ...
                let student_card = $(this).closest('.student_rec_card');
                let div_count = parseInt(student_card.find('#div_count').val());
                let div_button = student_card.find('#div_button').val();

                var div = `<div class="row mb-4 align-items-end student-record-input" id="div_id_${student_id}${div_count}">
                            @if(shiftEnable())
                             <div class="${div_button}">
                                <div class="primary_input">
                                    <select class="primary_select shiftSelectShift form-control{{ $errors->has('shift') ? ' is-invalid' : '' }}"
                                        name="new_record[${div_count}][shift][]" >
                                        <option data-display="@lang('common.select_shift') *" value="">
                                            @lang('admin.shift') *</option>
                            ...
                            <div class="col-sm-2 col-6 mt-4 mt-sm-0">
                                ...
                                <label for="is_default_${div_count}">@lang('student.student_information')</label>
                            </div>
                ...
                student_card.find('#div_count').val(div_count + 1);
            ...
            $(document).on("change", ".classSelectAcademicYear", function() {
                ...
                var formData = {
                    id: $(this).val(),
                    academic_id: $('input[name="session"]').val() || '',
                };
            ...
            $(document).on("change", ".shiftSelectShift", function() {
                var url = $("#url").val();
                var i = 0;
                var formData = {
                    id: $(this).val(),
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        target_select.empty().append(
                            $("<option>", {
                                value: '',
                                text: window.jsLang('select_class') + shift_required,
                            })
                        );
