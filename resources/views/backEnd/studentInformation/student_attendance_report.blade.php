        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="white-box">
                    ...
                    {{ html()->form('POST', route('student_attendance_report_search'))->attributes([
        ...
        <div class="white-box mt-40">
            <div class="row">
                <div class="col-lg-12 no-gutters d-flex align-items-center justify-content-between">
                    ...
                    @else
                    <a href="{{route('student-attendance-print', [$class_id, $section_id, $month, $year, shiftEnable() ? $shift_id : null])}}"
                        class="primary-btn small fix-gr-bg pull-right" target="_blank"><i class="ti-printer">
                        </i>@lang('common.print')</a>
                    @endif
                </div>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="lateday d-flex">
                        ...
                        <div class="mr-3">@lang('student.holiday'): <span class="text-dark">H</span></div>
                        <div class="mr-3">@lang('student.leave'): <span class="text-primary">Le</span></div>
                        <div class="mr-3">@lang('student.weekend'): <span class="text-muted">W</span></div>
                        ...
                        <div class="table-responsive">
                            <table id="table_id1" style="margin-bottom:25px" class="display school-table table-responsive"
                                ...
                                <tbody>
                                    ...
                                    @endphp
                                    <tr>
                                        <td>{{ $values[0]->studentInfo->full_name }}</td>
                                        <td>{{ $values[0]->studentInfo->admission_no }}</td>
                                        ...
                                        <td>
                                            @php
                                                ...
                                                echo $total_attendance > 0 ? number_format(($total_present / $total_attendance) * 100, 2).'%' : '-';
                                            @endphp
                                        </td>
                                            ...
                                            <td class="{{ ($i <= 18) ? 'all' : 'none' }}">
                                                @foreach($values as $value)
                                                    @if(strtotime($value->attendance_date) == strtotime($date))
                                                        @php
                                                            $attendanceType = $value->attendance_type;
                                                            $notes = $value->notes;
                                                        @endphp
                                                        {{ $attendanceType }} 
                                                        @if($notes) <i class="fas fa-info-circle" data-toggle="tooltip" data-placement="top" title="" data-original-title="{{$notes}}"></i> @endif
                                                    @endif
                                                @endforeach

                                                @if(is_null($attendanceType) && in_array($dayName, $weekendDayNames))
                                                    W
                                                @endif
                                            </td>
                                        @endfor
                    ...
                    <input type="hidden" id="total-attendance"
                        value="{{$total_grand_present.'-'.$total_absent.'-'.$total_late.'-'.$total_halfday.'-'.$total_holiday.'-'.$total_leave}}">
                </div>
            </div>
        </div>
    </div>
