...
<style>
    ...
    .attendanceP:checked ~ label,
    .attendanceP:checked ~ label:before {
        color: #28a745 !important;
        border-color: #28a745 !important;
    }

    .attendanceP:checked ~ label:after {
        background: #28a745 !important;
    }

    .attendanceA:checked ~ label,
    .attendanceA:checked ~ label:before {
        color: #dc3545 !important;
        border-color: #dc3545 !important;
    }

    .attendanceA:checked ~ label:after {
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="white-box">
                    ...
                    {{ html()->form('POST', route('student-search'))->attributes([
        ...
        <div class="row mt-40">
            <div class="col-lg-12">
                <div class="white-box">
                        ...
                        <div class="col-lg-12 no-gutters">
                            <div class="main-title">
                                ...
                                <h3 class="mb-15">@lang('student.student_attendance') | <small>
                                        ...
                                        @lang('common.date')
                                        @else
                                        @lang('common.class')  : {{$search_info['class_name']}}, @lang('common.section')  : {{$search_info['section_name']}}, @if(shiftEnable()) @lang('common.shift')  : {{$search_info['shift_name']}}, @endif @lang('common.date')
                                        @endif   : {{dateConvert($search_info['date'])}}</small>
                                </h3>
                                @endisset
                            </div>
                        </div>
                    ...
                    {{ html()->form('POST', route('student-attendance-store'))->class('form-horizontal')->open() }}
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{$student->studentDetail->admission_no}}
                                                    ...
                                                    @if(shiftEnable())
                                                    <input type="hidden" name="attendance[{{$student->id}}][shift]"
                                                        value="{{$student->shift_id}}">
                                                    @endif
                                                    @if(moduleStatusCheck('Branch'))
                                                    <input type="hidden" name="attendance[{{$student->id}}][branch_id]"
                                                ...
                                                <td>
                                                    <div class="d-flex radio-btn-flex">
                                                        ...
                                                        <div>
                                                            <input type="radio"
                                                                name="attendance[{{ $student->id }}][attendance_type]"
                                                                id="attendanceLe{{ $student->id }}" value="Le"
                                                                class="common-radio attendance_type"
                                                                {{ (
                                                                    ($student->studentDetail->DateWiseAttendances?->attendance_type === 'Le')
                                                                    || (!empty($student->is_on_leave))
                                                                ) ? 'checked' : '' }}>
                                                            <label class="text-nowrap" for="attendanceLe{{ $student->id }}">@lang('student.leave')</label>
                                                        </div>
                                                ...
                                                <td>
                                                    <div class="primary_input">
                                                        <input class="primary_input_field form-control note_{{ $student->id }}"
                                                            ...
                                                            value="{{ 
                                                                $student->studentDetail->DateWiseAttendances?->notes 
                                                                ?? ($student->leave_reason ?? '') 
                                                            }}">
                                                        <label class="primary_input_label" for="">
                                                            @lang('student.add_note_here')
                                                        </label>
                                                    </div>
                                                </td>
                                            </tr>
