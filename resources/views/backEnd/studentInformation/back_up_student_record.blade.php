                                    ...
                                    <table id="table_id" class="table Crm_table_active3" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                            @foreach ($studentRecords as $record)
                                                <tr>
                                                    @if(moduleStatusCheck('Branch'))
                                                    <td>{{ $record->branch ? $record->branch?->branch_name : '' }}</td>
                                                    @endif
                                                    ...
                                                    <td>{{ $record->class != '' ? $record->class->class_name : '' }}
                                                        ...
                                                        @if(shiftEnable()) {{ $record->shift ? '[' . $record->shift->name . ']' : '' }} @endif
                                                    </td>
                                                    @if (generalSetting()->with_guardian)
                                                    ...
                                                    <td>
                                                        ...
                                                        userPermission('disable_student_delete')
                                                                    ? '<a onclick="deleteId(' .
                                                                        $record->id .
                                                                        ');" class="dropdown-item delete-student-record-permanently" href="#" data-toggle="modal" data-target="#deleteStudentModal" data-id="' .
                                                                        $record->id .
                                                                        '" data-name="' .
                                                                        e($record->studentDetail->full_name) .
                                                                        '" data-admission="' .
                                                                        e($record->studentDetail->admission_no) .
                                                                        '" data-class-section="' .
                                                                        e(($record->class ? $record->class->class_name : '') . ($record->section ? ' (' . $record->section->section_name . ')' : '')) .
                                                                        '">' .
                ...
                <div class="modal-body">
                    <div class="text-center">
                        <h4 class="text-danger">
                            You are going to permanently remove this student record.
                            Removed data CANNOT be restored! Are you ABSOLUTELY Sure!
                        </h4>
                        <p class="mt-3 mb-0">
                            <strong>@lang('student.name'):</strong> <span id="delete_student_name"></span><br>
                            <strong>@lang('student.admission_no'):</strong> <span id="delete_student_admission"></span><br>
                            <strong>@lang('admin.class_Sec'):</strong> <span id="delete_student_class_section"></span>
                        </p>
                    </div>

                    <div class="mt-40 d-flex justify-content-between">
                        <button type="button" class="primary-btn tr-bg" data-dismiss="modal">@lang('common.cancel')</button>
                        {{ html()->form('POST', route('delete-student-record-permanently'))->attributes([
...
@push('script')
    <script>
        $(document).on('click', '.delete-student-record-permanently', function() {
            $('#student_delete_i').val($(this).data('id'));
            $('#delete_student_name').text($(this).data('name') || '');
            $('#delete_student_admission').text($(this).data('admission') || '');
            $('#delete_student_class_section').text($(this).data('class-section') || '');
        });
    </script>
@endpush
