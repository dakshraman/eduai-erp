        ...
        <div class="row">
            <div class="col-lg-12">  
                <div class="white-box">
                    ...
                    {{ html()->form('GET', route('subject-attendance-search'))->attributes([
            ...
            <div class="col-lg-12 ">
                <div class=" white-box mb-40">
                    <div class="row"> 
                        ...
                        @if(moduleStatusCheck('Branch'))                        
                            <div class="col-lg-3">
                                <strong> @lang('common.branch'): </strong> 
                        ...
                        @else  
                            ...
                            <div class="col-lg-3">
                                <strong> @lang('common.section'): </strong> {{$search_info['section_name']}} @if(shiftEnable()) [{{$search_info['shift_name']}}] @endif
                            </div>
                            {{-- <div class="col-lg-3">
                        ...
                        <div class="col-lg-6  col-md-6 no-gutters text-md-left mark-holiday ">
                            @if($attendance_type != "H")
                                <form action="{{route('student-subject-holiday-store')}}" method="POST">
                                    ...
                                    @if(moduleStatusCheck('Branch'))
                                    <input type="hidden" name="branch_id" value="{{$input['branch_id']}}">
                                    @endif
                                    @if(moduleStatusCheck('University'))
                ...
                {{ html()->form('POST', route('subject-attendance-store'))->attribute('class', 'form-horizontal')->open() }}
                                            ...
                                            {{-- student means student record data--}}
                                                <tr>
                                                    ...
                                                    <td>{{$student->studentDetail->admission_no}}
                                                        ...
                                                        @if(moduleStatusCheck('Branch'))
                                                        <input type="hidden" name="attendance[{{$student->id}}][branch_id]" value="{{$student->branch_id}}">
                                                        @endif
                                                        @if(moduleStatusCheck('University'))
                                                    ...
                                                    <td>
                                                        <div class="d-flex radio-btn-flex">
                                                            ...
                                                            <div class="mr-20">
                                                                <input type="radio" name="attendance[{{$student->id}}][attendance_type]" id="attendanceLe{{$student->id}}" value="Le" class="common-radio subject_attendance_type" {{ (
                                                                    ($student->studentDetail->DateWiseAttendances?->attendance_type === 'Le')
                                                                    || (!empty($student->is_on_leave))
                                                                ) ? 'checked' : '' }}>
                                                                <label for="attendanceH{{$student->id}}">@lang('student.leave')</label>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="primary_input">
                                                            <textarea class="primary_input_field form-control note_{{$student->id}}" cols="0" rows="2" name="attendance[{{$student->id}}][note]">{{ 
                                                                $student->studentDetail->DateSubjectWiseAttendances?->notes 
