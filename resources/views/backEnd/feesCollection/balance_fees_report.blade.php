    ...
    <section class="admin-visitor-area up_admin_visitor">
        <div class="container-fluid p-0">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="white-box">
                        {{ html()->form('POST', route('balance_fees_searches'))->attributes([
                ...
                <div class="row mt-40">
                    <div class="col-lg-12">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <x-table>
                                    <table class="table balance-custom-table school-table-data" cellspacing="0"
                                        width="100%">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        @php
                                                            ...
                                                            echo currency_format($value['totalFees']);
                                                        @endphp
                                                    </td>
                                                    <td>
                                                        @php
                                                            $grand_discount += $value['totalDiscount'];
                                                            echo currency_format($value['totalDiscount']);
                                                        @endphp
                                                    </td>
                                                    <td>
                                                        @php
                                                            $grand_fine += $value['totalFine'];
                                                            echo currency_format($value['totalFine']);
                                                        @endphp
                                                    </td>
                                                    <td>
                                                        @php
                                                            $grand_deposit += $value['totalDeposit'];
                                                            echo currency_format($value['totalDeposit']);
                                                        @endphp
                                                    </td>
                                                    <td>
                                                        @php
                                        ...
                                        <tfoot>
                                            ...
                                            <th>{{ currency_format($grand_discount) }}</th>
                                            <th>{{ currency_format($grand_fine) }}</th>
                                            <th>{{ currency_format($grand_deposit) }}</th>
                                            <th>{{ currency_format($grand_balance) }}</th>
                                        </tfoot>
                                    </table>
                                </x-table>
                            </div>
                ...
                <div class="row mt-40">
                    <div class="col-lg-12">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <x-table>
                                    <table class="school-table-data display school-table balance-custom-table"
                                        ...
                                        <tbody>
                                            @foreach ($student_records as $record)
                                                <tr>
                                                    ...
                                                    <td>{{ currency_format(@$record->feesInstallments->sum('amount')) }}</td>
                                                    <td>{{ currency_format(@$record->feesInstallments->sum('discount_amount')) }}</td>
                                                    <td>{{ currency_format(@$record->feesInstallments->sum('paid_amount')) }}</td>
                                                    <td>{{ currency_format(@$record->feesInstallments->sum('amount') - (@$record->feesInstallments->sum('paid_amount') - @$record->feesInstallments->sum('discount_amount'))) }}
                                                    </td>

                                                    @php
                                                        $grand_total += @$record->feesInstallments->sum('amount');
                                                        $grand_discount += @$record->feesInstallments->sum(
                                                            'discount_amount',
                                                        );
                                                        $grand_deposit += @$record->feesInstallments->sum(
                                                            'paid_amount',
                                                        );
                                        ...
                                        <tfoot>
                                            ...
                                            <th>{{ currency_format(@$grand_discount) }}</th>
                                            <th>{{ currency_format(@$grand_deposit) }}</th>
                                            <th>{{ currency_format($grand_total - $grand_deposit) }}</th>
                                        </tfoot>
                                    </table>
                                </x-table>
                            </div>
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                ...
                                <x-table>
                                    <table class="school-table-data table balance-custom-table" cellspacing="0"
                                        width="100%">
                                            ...
                                            <tbody>
                                                @foreach ($records as $record)
                                                    <tr>
                                                        ...
                                                        <td>{{ currency_format(@$record->directFeesInstallments->sum('amount')) }}</td>
                                                        <td>{{ currency_format(@$record->directFeesInstallments->sum('discount_amount')) }}
                                                        </td>
                                                        <td>{{ currency_format(@$record->directFeesInstallments->sum('paid_amount')) }}</td>
                                                        <td>{{ currency_format(@$record->directFeesInstallments->sum('amount') - (@$record->directFeesInstallments->sum('paid_amount') - @$record->directFeesInstallments->sum('discount_amount'))) }}
                                                        </td>

                                                        @php
                                                            $grand_total += @$record->directFeesInstallments->sum(
                                                                'amount',
                                                            );
                                                            $grand_discount += @$record->directFeesInstallments->sum(
                                                                'discount_amount',
                                                            );
                                                            $grand_deposit += @$record->directFeesInstallments->sum(
                                                                'paid_amount',
                                                            );
                                                            $grand_balance += $grand_total - $grand_deposit;
                                                        @endphp
