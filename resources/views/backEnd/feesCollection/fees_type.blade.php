            ...
            <div class="row">
                <div class="col-lg-3">
                    <div class="row">
                        <div class="col-lg-12">
                            @if (isset($fees_type))
                                {{ html()->form('POST', route('fees_type_update'))->attributes([
                            ...
                            @else
                                @if (userPermission('fees_type_store'))
                                    {{ html()->form('POST', route('fees_type_store'))->attributes([
                                            'class' => 'form-horizontal',
                                            'files' => true,
                                            'enctype' => 'multipart/form-data',
                                        ])->open() }}
                            ...
                            <div class="white-box">
                                ...
                                <div class="add-visitor">
                                    @if (moduleStatusCheck('Branch'))
                                        <div class="row">
                                            @include('branch::components.branch_select', [
                                                ...
                                                'mb' => 'mb-15',
                                                'branch_id' => isset($fees_type) ? $fees_type->branch_id : '',
                                            ])
                                        </div>
                                    @endif
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="primary_input">
                                                ...
                                                <input
                                                    ...
                                                    value="{{ isset($fees_type) ? $fees_type->name : old('name') }}">
                                                <input type="hidden" name="id"
                                                    value="{{ isset($fees_type) ? $fees_type->id : '' }}">
                                    ...
                                    <div class="row mt-25">
                                        <div class="col-lg-12">
                                            <select
                                                class="primary_select  form-control{{ $errors->has('fees_group') || session()->has('message-exist') ? ' is-invalid' : '' }}"
                                                name="fees_group" id="fees_group" {{ isset($fees_master) ? 'disabled' : '' }}>
                                                <option data-display="@lang('fees.fees_group') *" value="">
                                                    @lang('fees.fees_group') *</option>
                                                @foreach ($fees_groups as $fees_group)
                                                    @if (isset($fees_type))
                                                        <option
                                                            value="{{ $fees_group->id }}"{{ $fees_group->id == $fees_type->fees_group_id ? 'selected' : '' }}>
                                                            {{ $fees_group->name }} </option>
                                                    @else
                                                        <option value="{{ $fees_group->id }}"
                                                            {{ old('fees_group') != '' ? (old('fees_group') == $fees_group->id ? 'selected' : '') : '' }}>
                                                            {{ $fees_group->name }}</option>
                                                    @endif
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="row mt-25">
                                        <div class="col-lg-12">
                                            <div class="primary_input">
                                                <label class="primary_input_label" for="">@lang('fees.description')
                                                    <span></span></label>
                                                <textarea class="primary_input_field form-control" cols="0" rows="4" name="description">{{ isset($fees_type) ? $fees_type->description : old('description') }}</textarea>
                                    ...
                                    <table id="table_id" class="table" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                            @foreach ($fees_types as $fees_type)
                                                <tr>
                                                    @if (moduleStatusCheck('Branch'))
                                                    <td>{{ @$fees_type->branch?->branch_name }}</td>
                                                    @endif
                                                        ...
                                                        <x-drop-down>
                                                            @if (userPermission('fees_type_edit'))
                                                                <a class="dropdown-item"
                                                                    href="{{ route('fees_type_edit', [$fees_type->id]) }}">@lang('common.edit')</a>
                                                            @endif
                                                            @if (userPermission('fees_type_delete'))
                                                                @if (!@$fees_type->un_semester_label_id)
                                                                    <a class="dropdown-item" data-toggle="modal"
                                                                        data-target="#deleteFeesTypeModal{{ @$fees_type->id }}"
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="mt-40 d-flex justify-content-between">
                                            ...
                                            <a href="{{ route('fees_type_delete', [$fees_type->id]) }}"><button
