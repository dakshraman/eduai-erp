...
<table class="school-table school-table-style" cellspacing="0" width="100%">
    ...
    @php
        $discount_amount = App\Models\SmFeesAssign::discountSum($fees_assigned->student_id, $fees_assigned->feesGroupMaster->feesTypes->id, 'discount_amount');
        $total_discount += $discount_amount;
        $student_id = $fees_assigned->student_id;
    @endphp
    @php
        $paid = App\Models\SmFeesAssign::discountSum($fees_assigned->student_id, $fees_assigned->feesGroupMaster->feesTypes->id, 'amount');
        $total_grand_paid += $paid;
    @endphp
    @php
        $fine = App\Models\SmFeesAssign::discountSum($fees_assigned->student_id, $fees_assigned->feesGroupMaster->feesTypes->id, 'fine');
        $total_fine += $fine;
    @endphp
    ...
    @php
        $payments = App\Models\SmFeesAssign::feesPayment($fees_assigned->feesGroupMaster->feesTypes->id, $fees_assigned->student_id);
        $i = 0;
    @endphp
    ...
    @foreach($payments as $payment)
        <tr align="center">
            ...
            <td></td>
            <td></td>
            <td class="text-right"><img src="{{assetPath('public/backEnd/img/table-arrow.png')}}"></td>
