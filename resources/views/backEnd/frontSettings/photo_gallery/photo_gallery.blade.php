...
@section('mainContent')
                ...
                <div class="row">
                    <div class="col-lg-12">
                        @if (isset($add_photo_gallery))
                            {{ html()->form('POST', route('photo-gallery-update'))->attributes([
                        ...
                        @else
                            @if (userPermission('photo-gallery-store'))
                                {{ html()->form('POST', route('photo-gallery-store'))->attributes([
                                        'class' => 'form-horizontal',
                                        'files' => true,
                                        'enctype' => 'multipart/form-data',
                                    ])->open() }}
                        ...
                        <div class="white-box">
                            ...
                            <div class="add-visitor">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="primary_input">
                                            <label class="primary_input_label" for="">@lang('front_settings.description') <span
                                                    class="text-danger"> *</span></label>
                                            <textarea class="primary_input_field form-control{{ $errors->has('description') ? ' is-invalid' : '' }} newsSummerNote"
                                                cols="0" rows="4" name="description" id="address">{{ isset($add_photo_gallery) ? @$add_photo_gallery->description : old('description') }}</textarea>
                                ...
                                <div class="row mt-20">
                                    <div class="col-lg-12">
                                        <img class="previewImageSize {{ @$add_photo_gallery->feature_image ? '' : 'd-none' }}"
                                            src="{{ @$add_photo_gallery->feature_image ? assetPath($add_photo_gallery->feature_image) : '' }}"
                                            alt="" id="photoGalleryImageShow" height="100%" width="100%">
                                    </div>
                                </div>
                                <div class="row mt-40 align-items-center">
                                    ...
                                    <div class="col-lg-2 col-3 text-right">
                                        <a href="javascript:void(0)" class="primary-btn icon-only fix-gr-bg" id="addRowBtn">
                                        ...
                                        @if (config('app.app_sync'))
                                            <span class="d-inline-block" tabindex="0" data-toggle="tooltip"
                                                title="Disabled For Demo "> <button
                                                    ...
                                                    type="button">@lang('common.add')</button></span>
                                        @else
                                    ...
                                    <tbody>
                                        @foreach ($photoGalleries as $key => $value)
                                            ...
                                            <tr e_id="{{ $value->id }}">
                                                ...
                                                <td>
                                                    <x-drop-down>
                                                        <a class="dropdown-item view-gallery" data-id='{{ $value->id }}'  data-url="{{ route('photo-gallery-view-modal',$value->id) }}"
    ...
    <div class="modal fade admin-query" id="photogalleryModal">
        ...
        $(document).on('click','.view-gallery',function(){
            ...
            $.ajax({
                ...
                if(response.status == 1){                           
                    $('#photo_gallery_list').html(response.view);
                    $('#photogalleryModal').modal('show');
                }else{
                    toastr.error('Photo not found','Error');
                }
            });
        });
