                ...
                <ul class="nav nav-tabs tabs_scroll_nav" role="tablist">
                    @foreach($records as $key => $record) 
                        <li class="nav-item mb-0">
                            <a class="nav-link mb-0 @if($key== 0) active @endif " href="#tab{{$key}}" role="tab" data-toggle="tab">{{$record->class->class_name}} ({{$record->section->section_name}}) @if(shiftEnable() && $record->shift) [{{$record->shift->name}}] @endif </a>
                        </li>
                    @endforeach
                </ul>
                <!-- Tab panes -->
                                        ...
                                        @foreach($record->OnlineExam  as $online_exam)
                                            ...
                                            @if(!in_array(@$online_exam->id, @$marks_assigned))
                                                <tr>
                                                    ...
                                                    <td data-sort="{{strtotime(@$online_exam->start_date)}}">
                                                        <br>
                                                        {{ __("common.time") }} : {{date('d M Y h:i A', strtotime($online_exam->start_date.' '.@$online_exam->start_time)).' - '.date('d M Y h:i A', strtotime($online_exam->end_date.' '.@$online_exam->end_time))}}
                                                    </td>
                                                    @php
                                                        if (!is_null($online_exam->end_time)) {
                                                            $start = Carbon::parse($online_exam->start_time);
                                                            $end = Carbon::parse($online_exam->end_time);
                                                            $totalDuration = $end->diffInMinutes($start, false); 
                                                    ...
                                                    // date_default_timezone_set("Asia/Dhaka");
                                                        $startTime = strtotime(date("Y-m-d h:i", strtotime($online_exam->start_date . ' ' . $online_exam->start_time)));
                                                        $endTime = strtotime(date("Y-m-d h:i", strtotime($online_exam->end_date . ' ' . $online_exam->end_time)));
                                                        $now = date('Y-m-d h:i');
                                                        $now =  strtotime("now");
                                                    ...
                                                    @endphp
                                                        ...
                                                        @elseif($now >= $endTime)
                                                            <span class="btn primary-btn small  fix-gr-bg"
                                                                style="background:#dc3545">{{ __("exam.Closed") }}</span>
                                                        @else    
                                                        <span class="btn primary-btn small  fix-gr-bg"
                                                                style="background:#dc3545">{{ __("exam.Closed") }}</span>
                                                        @endif
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mt-40 d-flex justify-content-between">
                    ...
                    {{ html()->form('POST', route('online-exam-delete'))->attribute('enctype', 'multipart/form-data')->open() }}
