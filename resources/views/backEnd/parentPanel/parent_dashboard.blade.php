                                        ...
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <h1 class="gradient-color2">
                                                @if (generalSetting()->fees_status == 0)
                                                    ...
                                                    {{ currency_format($balance_fees) }}
                                                @elseif (isset($feesDue))
                                                    {{-- {{ $currency->currency_symbol }} --}}
                                                    {{ currency_format($feesDue) }}
                                                @endif
                                            </h1>
                                        </div>
                                    </div>
                                                                ...
                                                                <table id="default_table"
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        @foreach ($sm_weekends as $sm_weekend)
                                                                            @php
                                                                                if (moduleStatusCheck('University')) {
                                                                                    $studentClassRoutine = \App\Models\SmWeekend::universityStudentClassRoutine($record->un_semester_label_id, $record->un_section_id, $sm_weekend->id);
                                                                                } else {
                                                                                    $studentClassRoutine = \App\Models\SmWeekend::studentClassRoutineFromRecord($record->class_id, $record->section_id, $sm_weekend->id, $record->shift_id);
                                                                                }
                                                                            @endphp
                ...
                @if (userPermission('parent_attendance'))
                    <div class="row mt-40">
                        @php
                            ...
                            $year = $now->year;
                            $month = $now->month;
                            $days = date('t', strtotime($now->year . '-' . $now->month . '-01')); 
                                    ...
                                    @foreach ($children->studentRecords as $record)
                                        @if ($record->Exam)
                                            @foreach ($record->Exam->unique(function ($item) {
                                                ...
                                                <li class="nav-item">
                                                    <a class="nav-link @if ($key == 0) active @endif"
                                                       id="home-tab{{ $children->id . $exam->id }}"
                                    ...
                                    @foreach ($children->studentRecords as $record)
                                        @if ($record->Exam)
                                            @foreach ($record->Exam->unique(function ($item) {
                                                ...
                                                @php
                                                    $exam_routines = App\Models\SmExamSchedule::getAllExams($exam->class_id, $exam->section_id, $exam->exam_type_id, $exam->shift_id);
                                                @endphp
