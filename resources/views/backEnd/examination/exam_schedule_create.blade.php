            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="white-box">
                        ...
                        {{ html()->form('POST', route('exam_schedule_create_store'))->attributes([
                            ...
                            @else
                                ...
                                <div class="@if(shiftEnable()) col-lg-3 @else col-lg-4 @endif mt-30-md">
                                    ...
                                    <select
                                            class="primary_select form-control{{ $errors->has('exam_type') ? ' is-invalid' : '' }}"
                                            name="exam_type">
                                        <option data-display="@lang('exam.select_exam') *" value="">
                                            @lang('exam.select_exam') *
                                        </option>
                                        @foreach ($exam_types as $exam)
                                            <option value="{{ $exam->id }}"
                                                    {{ old('exam_type', @$exam_id ?? '') == $exam->id ? 'selected' : '' }}>
                                                {{ $exam->title }}
                                            </option>
                                        @endforeach
                                    </select>
        ...
        @if (isset($assign_subjects))
            <section class="mt-20">
                <div class="container-fluid p-0">
                    ...
                    {{ html()->form('POST', route('exam_schedule_store'))->attributes([
                            'class' => 'form-horizontal',
                            'files' => true,
                            'enctype' => 'multipart/form-data',
                            'id' => 'exam_schedule_store',
                        ])->open() }}
                    @if (!moduleStatusCheck('University'))
                            ...
                            <table class="table school-table-style" cellspacing="0" width="100%">
                                ...
                                @foreach ($assign_subjects as $assign_subject)
                                    <tr>
                                        ...
                                        <td>{{ @$assign_subject->class != '' ? @$assign_subject->class->class_name : '' }}
                                            ({{ @$assign_subject->section != '' ? @$assign_subject->section->section_name : '' }}
                                            )
                                        </td>
                                        @foreach ($exam_periods as $exam_period)
                                            @php
                                                $assigned_routine = App\Models\SmExamSchedule::assignedRoutine(
                                                    $class_id,
                                                    $assign_subject->section_id,
                                                    $exam_id,
                                            ...
                                            <td>
                                                @if (@$assigned_routine == '')
                                                    @if (@$assigned_routine_subject == '')
                                                        @if (userPermission('add-exam-routine-modal'))
                                                            <div class="col-lg-6">
                                                                <a href="{{ route('add-exam-routine-modal', [$assign_subject->subject_id, $exam_period->id, $class_id, $assign_subject->section_id, $exam_id, $section_id_all]) }}"
                                                                   class="primary-btn small tr-bg icon-only mr-10 modalLink"
                                                                   data-modal-size="modal-md"
                                                                   title="@lang('exam.create_exam_routine')">
                                                ...
                                                @else
                                                    <div class="col-lg-6">
                                                        ...
                                                        <a href="{{ route('edit-exam-routine-modal', [$assign_subject->subject_id, $exam_period->id, $class_id, $assign_subject->section_id, $exam_id, $assigned_routine->id, $section_id_all]) }}"
                                                           class="modalLink" data-modal-size="modal-md"
                                                           title="@lang('common.edit_exam_routine')">
                                                            <span class="ti-pencil-alt" id="addClassRoutine"></span>
                                                        </a>
                                                        <a href="{{ route('delete-exam-routine-modal', [$assigned_routine->id, $section_id_all]) }}"
                                                           class="modalLink" data-modal-size="modal-md"
                                                           title="@lang('common.delete_exam_routine')">
