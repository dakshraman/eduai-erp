...
@php
    ...
    $class=App\Models\SmClass::find(@$class_id);
    
    if($section_id==0){
        $section='All Sections';
    }else{
        $section=App\Models\SmSection::find(@$section_id);
        ...
        <tr>
            ...
            <td>
                <h3 style="font-size:22px !important"
                    class="text-white"> {{isset(generalSetting()->school_name)?generalSetting()->school_name:'EduAI'}} </h3>
                <p style="font-size:18px !important"
                   class="text-white mb-0"> {{isset(generalSetting()->address)?generalSetting()->address:'School Address'}} </p>
    ...
    <table class="table table-bordered table-striped" cellspacing="0" width="100%">
        ...
        <tr>
            ...
            @foreach($exam_periods as $exam_period)
                <th style="text-align:center" nowrap>{{ @$exam_period->period}}
                    <br>{{date('h:i A', strtotime(@$exam_period->start_time))}}
                    <br>To<br>{{ date('h:i A', strtotime(@$exam_period->end_time))}}</th>
            @endforeach
        </tr>
        @foreach($assign_subjects as $assign_subject)
            <tr>
                ...
                @foreach($exam_periods as $exam_period)
                    @php
                        $assigned_routine = App\Models\SmExamSchedule::assignedRoutine($class_id, $assign_subject->section_id, $exam_id, $assign_subject->subject_id, $exam_period->id);
