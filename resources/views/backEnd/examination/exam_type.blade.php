            ...
            <div class="row">
                ...
                <div class="col-lg-4 col-xl-3">
                    <div class="row">
                        <div class="col-lg-12">
                            @if (isset($exam_type_edit))
                                {{ html()->form('POST', route('exam_type_update'))->attributes([
                            ...
                            @else
                                @if (userPermission('exam_type_store'))
                                    {{ html()->form('POST', route('exam_type_store'))->attributes([
                                            'class' => 'form-horizontal',
                                            'files' => true,
                                            'enctype' => 'multipart/form-data',
                                        ])->open() }}
                                    ...
                                    @if (moduleStatusCheck('Branch'))
                                        <div class="row">
                                            @include('branch::components.branch_select', [
                                                ...
                                                'mb' => 'mb-15',
                                                'branch_id' => isset($exam_type_edit) ? $exam_type_edit->branch_id : '',
                                            ])
                                        </div>        
                                    @endif
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="primary_input">
                                                ...
                                                <input
                                                    class="primary_input_field form-control{{ $errors->has('exam_type_title') ? ' is-invalid' : '' }}"
                                                    type="text" name="exam_type_title" autocomplete="off"
                                                    value="{{ isset($exam_type_edit) ? old('exam_type_title', $exam_type_edit->title) : old('exam_type_title') }}">
                                                <input type="hidden" name="id"
                                                    value="{{ isset($exam_type_edit) ? $exam_type_edit->id : old('id') }}">
                                        ...
                                        <div class="col-lg-12">
                                            ...
                                            <div class="primary_input">
                                                <input type="checkbox" id="averageExam"
                                                    class="common-checkbox exam-checkbox" name="is_average" value="yes"
                                                    {{ isset($exam_type_edit) ? (old('is_average', $exam_type_edit->is_average ? 'yes' : '') == 'yes' ? 'checked' : '') : (old('is_average') == 'yes' ? 'checked' : '') }}>
                                    ...
                                    <div class="row mt-20 averagePercentage d-none">
                                        <div class="col-lg-12">
                                            <div class="primary_input">
                                                ...
                                                <input
                                                    class="primary_input_field form-control{{ $errors->has('average_mark') ? ' is-invalid' : '' }}"
                                                    type="number" step="0.01" min="0" name="average_mark" autocomplete="off"
                                                    value="{{ isset($exam_type_edit) ? old('average_mark', $exam_type_edit->average_mark) : old('average_mark') }}">
                                                @error('average_mark')
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($exams_types as $exams_type)
                                                <tr>
                                                    <td>{{ ++$i }}</td>
                                                    @if (moduleStatusCheck('Branch'))
                                                    <td>{{ @$exams_type->branch?->branch_name }}</td>
                                                    @endif
                                                    <td>{{ @$exams_type->title }}</td>
                                                    <td>{{ $exams_type->is_average == 1 ? __('common.yes') : __('common.no') }}
                                                    </td>
                                                        ...
                                                        <x-drop-down>
                                                            ...
                                                            @if (userPermission('exam_type_edit'))
                                                                <a class="dropdown-item"
                                                                    href="{{ route('exam_type_edit', [$exams_type->id]) }}">@lang('common.edit')</a>
                                                            @endif
                                                            @if (userPermission('exam_type_delete'))
                                                                <a class="dropdown-item" data-toggle="modal"
                                                                    data-target="#deleteSubjectModal{{ @$exams_type->id }}"
                                                                    href="#">@lang('common.delete')</a>
                                                            @endif
                            </div>
                        </div>
                        <a class="primary-btn small tr-bg" style="white-space: nowrap"
                            href="{{ route('exam-marks-setup', $exams_type->id) }}">
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="mt-40 d-flex justify-content-between">
                                            ...
                                            <a href="{{ route('exam_type_delete', [@$exams_type->id]) }}"
                                                class="text-light">
