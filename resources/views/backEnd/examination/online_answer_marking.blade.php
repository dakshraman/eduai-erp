        ...
        <div class="container-fluid p-0">
            <div class="row">
                ...
                <div class="col-lg-12">
                    ...
                    {{ html()->form('POST', route('online_exam_marks_store'))->attributes([
                                ...
                                <div class="">
                                    <div class="row  pl-10">
                                        <div class="col-lg-7 mt-20">
                                            ...
                                            <h4><strong>@lang('common.subjects')
                                                    : </strong>{{@$take_online_exam->onlineExam!=""?@$take_online_exam->onlineExam->subject->subject_name:""}}
                                            </h4>
                                            <h4><strong>@lang('exam.total_marks'): {{@$total_mark}} </strong></h4>
                                            <h4><strong>@lang('exam.minimum_percentage')
                                                    : {{@$online_exam_info->percentage}} </strong></h4>

                                            <p><strong>@lang('exam.instruction')
                                                    : </strong>{{@$take_online_exam->onlineExam->instruction}}</p>
                                        </div>
                                        <div class="col-lg-5 mt-20">
                                            ...
                                            <p class="mb-2">
                                                <strong>@lang('common.date') & @lang('common.time')
                                                    : </strong>{{@$take_online_exam->onlineExam !=""?dateConvert(@$take_online_exam->onlineExam->date):""}} {{ convertTime(@$take_online_exam->onlineExam->start_time)}}
                                                <strong>-</strong> {{@$take_online_exam->onlineExam!=""?  dateConvert(@$take_online_exam->onlineExam->end_date_time):""}} {{@$take_online_exam->onlineExam!=""?  convertTime(@$take_online_exam->onlineExam->end_time):""}}
                                            </p>
                            ...
                            <div class="question_table">
                                <table id="table_id" class="table" cellspacing="0" width="100%">
                                    ...
                                    @foreach($answered_questions as $question)
                                        @php
                                            $student_answer=App\Models\OnlineExamStudentAnswerMarking::StudentGivenAnswer($online_exam_info->id,$question->question_bank_id,$s_id);
                                            if ($question->questionBank->type=='MI') {
                                                    $submited_answer=\App\Models\OnlineExamStudentAnswerMarking::StudentImageAnswer($online_exam_info->id,$question->question_bank_id,$s_id);
                                                    $submited_answer_status=\App\Models\OnlineExamStudentAnswerMarking::StudentImageAnswerStatus($online_exam_info->id,$question->question_bank_id,$s_id);
                                                }
                                        @endphp
                                        ...
                                        <tr>
                                            <td width="80%">
                                                ...
                                                @if(@$question->questionBank->type == "MI")
                                                    <div class="qustion_banner_img">
                                                        <img src="{{assetPath($question->questionBank->question_image)}}"
                                                             alt="">
                                                    </div>
                                                @endif
                                                @if(@$question->questionBank->type == "M")
                                                    <input type="hidden" name="options_{{@$question->question_bank_id}}"
                                                           value="{{@$student_answer->user_answer}}">
                                                    @php
                                                        ...
                                                        @foreach($question->questionBank->questionMu as $multiple_option)
                                                            <div class="mt-20 mr-20">
                                                                <input type="radio" disabled
                                                                       id="answer{{@$multiple_option->id}}"
                                                                       class="common-checkbox"
                                                                       value="{{@$multiple_option->id}}" {{@$student_answer->user_answer == $multiple_option->id? 'checked': ''}}>
                                                ...
                                                @elseif(@$question->questionBank->type == "MI")
                                                    ...
                                                    <div class="quiestion_group">
                                                        ...
                                                        @endphp
                                                        @foreach($question->questionBank->questionMu as $multiple_option)
                                                            <div class="single_question {{$question_type_class}} {{isset($submited_answer)? in_array($multiple_option->id,$submited_answer) ? 'active' :'' : '' }}"
                                                                 style="background-image: url({{assetPath($multiple_option->title)}})"
                                                                ...
                                                                <div class="img_ovelay">
                                                                    <input data-question="{{@$question->question_bank_id}}"
                                                                           type="{{@$question->questionBank->answer_type}}"
                                                                           data-option="{{@$multiple_option->id}}"
                                                                           id="answer{{@$multiple_option->id}}"
                                                ...
                                                @elseif($question->questionBank->type == "T")
                                                    <input type="hidden" value="{{@$student_answer->user_answer}}"
                                                    ...
                                                    <div class="d-flex align-items-center justify-content-center radio-btn-flex mt-20">
                                                        <div class="mr-30">
                                                            <input type="radio" disabled
                                                                   id="true_{{@$question->question_bank_id}}" value="T"
                                                                   class="common-checkbox common-radio relationButton" {{@$student_answer->user_answer == "T"? 'checked': ''}}>
                                                            <label for="true_{{@$question->question_bank_id}}">@lang('exam.true')</label>
                                                        </div>
                                                        <div class="mr-30">
                                                            <input type="radio" disabled
                                                                   id="false_{{@$question->question_bank_id}}" value="F"
                                                                   class="common-checkbox common-radio relationButton" {{@$student_answer->user_answer == "F"? 'checked': ''}}>
                                                ...
                                                @else
                                                    ...
                                                    <div class="primary_input mt-20">
                                                        <textarea readonly
                                                                  class="primary_input_field form-control mt-10 has-content"
                                                                  cols="0" rows="5"
                                                                  name="suitable_words_{{@$question->question_bank_id}}">{{@$student_answer->user_answer}}</textarea>
                                                        <label class="primary_input_label"
                                                               for="">@lang('exam.suitable_words')</label>
                                                ...
                                                <div class="mt-20">
                                                    @if($question->questionBank->type == "M")
                                                        ...
                                                        @endphp
                                                        <h4 class="text-center">[@lang('exam.currect_answer')
                                                            : {{$currect_multiple}}]</h4>
                                                    @elseif(@$question->questionBank->type == "MI")
                                                        @php
                                                            $ques_bank_multiples = $question->questionBank->questionMu;
                                                        ...
                                                        <div class="quiestion_group">
                                                            ...
                                                            <div class="single_question "
                                                                 style="background-image: url({{assetPath($ques_bank_multiple->title)}})">
                                                    ...
                                                    @elseif(@$question->questionBank->type == "T")
                                                        <h4 class="text-center">[@lang('exam.currect_answer')
                                                            : {{@$question->questionBank->trueFalse == "T"? 'True': 'False'}}
                                                            ]</h4>
                                                    @else
                                                        <h4 class="text-center">[@lang('exam.currect_answer')
                                                            : {{@$question->questionBank->suitable_words}}]</h4>
                                                    @endif
                                                </div>
                                            </td>
                                            ...
                                            <td width="10%" class="text-right">
                                                <div class="mt-20 float-right">
                                                    <input type="radio"
                                                           id="marks_{{@$question->questionBank!=""?@$question->questionBank->id:""}}_incorrect"
                                                           class="common-checkbox common-radio relationButton"
                                            ...
                                            <td width="10%" class="text-right">
                                                <div class="mt-20">
                                                    <input type="radio"
                                                           id="marks_{{@$question->questionBank !=""?@$question->questionBank->id:""}}"
                                                           class="common-checkbox common-radio relationButton"
                                                           name="marks[]{{@$question->questionBank->id}}"
                                                           value="{{@$question->questionBank!=""?@$question->questionBank->id:""}}" {{@$correct}}>
