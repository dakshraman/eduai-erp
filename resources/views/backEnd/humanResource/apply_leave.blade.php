                                        ...
                                        <table class="table school-table-style" cellspacing="0" width="100%">
                                            ...
                                            @foreach ($my_leaves as $my_leave)
                                                @php
                                                    ...
                                                    $approved_leaves = App\Models\SmLeaveRequest::approvedLeave(
                                                        $my_leave->id,
                                                    );
                                                    $remaining_days = $my_leave->days - $approved_leaves;
                                                    $extra_days =
                                                        $remaining_days < 0
                                                            ? $approved_leaves - $my_leave->days
                                                            : 0;
                                                @endphp
            ...
            <div class="row">
                ...
                <div class="col-lg-3">
                    <div class="row">
                        <div class="col-lg-12">
                            @if (isset($apply_leave))
                                {{ html()->form('PUT', route('apply-leave-update', $apply_leave->id))->attributes([
                            ...
                            @else
                                @if (userPermission('apply-leave-store') || Auth::user()->role_id == 2)
                                    {{ html()->form('POST', route('apply-leave-store'))->attributes([
                                            'class' => 'form-horizontal',
                                            'files' => true,
                                            'enctype' => 'multipart/form-data',
                                        ])->open() }}
                                            ...
                                            <select
                                                ...
                                                @foreach ($leave_types as $leave_type)
                                                    <option value="{{ $leave_type->id }}"
                                                            {{ old('leave_define_id', isset($apply_leave) ? $apply_leave->leave_define_id : null) == $leave_type->id ? 'selected' : '' }}>
