...
<head>
    ...
    @if( ! is_null(schoolConfig() ))
        <link rel="icon" href="{{assetPath(schoolConfig()->favicon)}}" type="image/png"/>
    @else
    ...
    <link rel="stylesheet" href="{{ assetPath('public/backEnd/vendors/css/fnt.min.css') }}" />
...
@if(userRtlLtl() ==1)
    <link rel="stylesheet" href="{{ assetPath('public/backEnd/assets/css/rtl/bootstrap.rtl.min.css') }}" />
    <link rel="stylesheet" href="{{ assetPath('public/backEnd/assets/css/global_rtl.css') }}">
@else
    <link rel="stylesheet" href="{{ assetPath('public/backEnd/vendors/css/bootstrap.min.css') }}" />
@endif
<link rel="stylesheet" href="{{ assetPath('public/backEnd/assets/css/style.css')}}" />
...
<link rel="stylesheet" href="{{ assetPath('public/backEnd/vendors/select2/css/select2.min.css')}}" />
@endif 
...
@endif
    ...
    <script>
        ...
        window._translations = {!! cache('translations_'.app()->getLocale()) ?? '{}' !!};
    </script>
</head>
@php
...
<body class="admin"  style="{!! $css !!}">
        ...
        @if($chat_method == 'pusher')
            <input type="hidden" id="pusher_app_key" value="{{ $generalSetting->pusher_app_key }}">
            <input type="hidden" id="pusher_app_cluster" value="{{ $generalSetting->pusher_app_cluster }}">
        @endif
