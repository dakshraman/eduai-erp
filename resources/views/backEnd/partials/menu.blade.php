...
<div class="w-full" id="main-nav-for-chat">
    <div class="grid grid-cols-12">
        <div class="col-span-12 p-0">
            <div class="header_iner flex justify-between items-center">
                <div class="small_logo_crm lg:hidden">
                    <a href="#">
                        @if (!is_null($generalSetting->logo))
                            <img src="{{ assetPath($generalSetting->logo) }}" alt="logo">
                        @else
                            <img src="{{ assetPath('public/uploads/settings/logo.png') }}" alt="logo">
                        @endif
                    </a>
                </div>
                <div id="sidebarCollapse" class="sidebar_icon  lg:hidden">
                    <i class="ti-menu"></i>
                </div>
                    ...
                    <div class="search_inner">
                        <form action="#" onsubmit="return false;" autocomplete="off">
                            <div class="search_field">
                                <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 primary_input_field input-left-icon"
                                    placeholder="Search" id="search" aria-label="Search menu" role="combobox"
                                    aria-controls="livesearch" aria-autocomplete="list" onkeyup="showResult(this.value)">
                            </div>
                ...
                <div class="header_right flex justify-between items-center">
                    ...
                    @if (moduleStatusCheck('Branch'))
                        @include('branch::components.inc.menu_btn')
                    @endif
                    ...
                    <div class="dropdown d-sm-none" x-data="{ open: false }">
                        <button class="" type="button" id="mobileSelectorDropdown" @click="open = !open" @click.outside="open = false">
                        ...
                        <div class="dropdown-menu p-3" :class="{ 'block': open, 'hidden': !open }" aria-labelledby="dropdownMenuButton">
                    ...
                    <ul class="header_notification_warp flex items-center">

                        @if(!app()->environment('local') && Route::has('chat.index'))
                            @if (!app('general_settings')->get('chatting_method')  || app('general_settings')->get('chatting_method') == 'log')
                                <li x-data="jqueryChatNotification({
                                    unreads: {{ json_encode($notifications_for_chat ?? 0) }},
                                    ...
                                    asset_type: {{ json_encode(assetPath('public/')) }},
                                    ...
                                    <a class="pulse theme_color bell_notification_clicker" @click="open_modal = !open_modal" href="#">
                                        ...
                                        <span class="chat_badge notification_count" x-text="count_unread"></span>
                                        <span class="pulse-ring notification_count_pulse"></span>
                                    </a>
                                    <div class="Menu_NOtification_Wrap" :class="{'active': open_modal}">
                                        <div class="notification_Header">
                                            <h4>@lang('chat::chat.notification')</h4>
                                            <audio id="sound" :src="asset_type+'/chat/audio/notification.mp3'" muted></audio>
                                        </div>
                                        <div class="Notification_body">
                                            <template x-for="unread in unreads" :key="unread.id">
                                                <div class="single_notify flex items-center">
                                                    <div class="notify_thumb">
                                                        <a href="#"><img :src="asset_type + '/chat/images/spondon-icon.png'" alt=""></a>
                                                    </div>
                                                    <div class="notify_content">
                                                        <div x-if="unread.type == 'Modules\\Chat\\Notifications\\InvitationNotification'">
                                                            <a :href="getNotificationUrl(unread)">
                                                                <h5 x-text="getNotificationTitle(unread)"></h5>
                                                            </a>
                                                            <p x-html="getNotificationMessage(unread)"></p>
                                                        </div>
                                                        <div x-else-if="unread.type == 'Modules\\Chat\\Notifications\\GroupCreationNotification'">
                                                            <a :href="getNotificationUrl(unread)">
                                                                <h5 x-text="getNotificationTitle(unread)"></h5>
                                                            </a>
                                                            <p>@lang('chat::chat.you_are_invited_in_new_group')!</p>
                                                        </div>
                                                        <div x-else-if="unread.type == 'Modules\\Chat\\Notifications\\GroupMessageNotification'">
                                                            <a :href="getNotificationUrl(unread)">
                                                                <h5 x-text="getNotificationTitle(unread)"></h5>
                                                            </a>
                                                            <p>@lang('chat::chat.new_message_in_this_group')!</p>
                                                        </div>
                                                        <div x-else>
                                                            <a :href="getNotificationUrl(unread)">
                                                                <h5 x-text="getNotificationTitle(unread)"></h5>
                                        ...
                                        <div class="nofity_footer">
                                            <div class="submit_button text-center pt_20">
                                                <a :href="mark_all_as_read_url" class="primary-btn radius_30px text_white fix-gr-bg">@lang('chat::chat.mark_all_as_read')</a>
                                            </div>
                                        </div>
                                    </div>
                                </li>
                            @else
                                <li x-data="chatNotification({
                                    unreads: {{ json_encode($notifications_for_chat ?? 0) }},
                                    user_id: {{ json_encode(auth()->id()) }},
                                    redirect_url: {{ json_encode(route('chat.index')) }},
                                    asset_type: {{ json_encode(assetPath('public/')) }},
                        ...
                        <li class="scroll_notification_list">
                            <a class="pulse theme_color bell_notification_clicker show_notifications" href="#">
                                <!-- bell   -->
                                <img src="{{assetPath('public/backEnd/assets/img/icons/notification.svg')}}" alt="">
                    ...
                    <div class="redirect_links">
                        <div class="select_style flex">
                            @if (generalSetting()->website_btn == 1)
                                <a target="_blank" class=" mr-10 tab_hide"
                                    href="{{ url('/') }}"><img src="{{assetPath('public/backEnd/assets/img/icons/globe.svg')}}" alt=""></a>
                            @endif
                            ...
                            @if (generalSetting()->report_btn == 1)
                                @if (Auth::user()->role_id == $coltroller_role)
                                    <a class="mr-10 tab_hide"
                                        href="{{ route('student_report') }}"><img src="{{assetPath('public/backEnd/assets/img/icons/report.svg')}}" alt=""></a>
                                @endif
                            @endif
                    ...
                    <div class="profile_info">
                        ...
                        <div class="user_avatar_div">
                            <img id="profile_pic"
                                src="{{ @profile() && file_exists(@profile()) ? assetPath(profile()) : assetPath('public/backEnd/assets/img/avatar.png') }}"
                                alt="">
                        </div>
                            ...
                            <div class="profile_info_details">
                                ...
                                @if (Auth::user()->role_id == '2' && Auth::user()->is_saas == 0)
                                    <a href="{{ route('student-profile') }}">
                                        <img src="{{assetPath('public/backEnd/assets/img/icons/profile.svg')}}" class="mr-1" alt="">
                                ...
                                @elseif(Auth::user()->role_id != '3' && Auth::user()->is_saas == 0 && Auth::user()->staff)
                                    <a href="{{ route('viewStaff', Auth::user()->staff->id) }}">
                                        <img src="{{assetPath('public/backEnd/assets/img/icons/profile.svg')}}" alt="">
                                        @lang('common.view_profile')
                                        {{-- <span class="ti-user"></span> --}}
                                    </a>
                                @endif
                                @if (!config('app.app_sync') && auth()->user()->role_id != 1)
                                    @if (auth()->user()->staff && auth()->user()->staff->parent_id && auth()->user()->role_id == 3)
                                        <a href="{{ route('viewAsRole') }}">
                                            ...
                                            <img src="{{assetPath('public/backEnd/assets/img/icons/key.svg')}}" alt="" class="mr-1">
                                ...
                                @if (moduleStatusCheck('Saas') == true &&
                                    ...
                                    <a href="{{ route('viewAsSuperadmin') }}">
                                        <img src="{{assetPath('public/backEnd/assets/img/icons/key.svg')}}" alt="">
                                ...
                                <a href="{{ route('updatePassowrd') }}">
                                    <img src="{{assetPath('public/backEnd/assets/img/icons/password.svg')}}" alt="">
                                ...
                                <a href="{{ Auth::user()->role_id == 2 ? route('student-logout') : route('logout') }}"
                                    ...
                                    <img src="{{assetPath('public/backEnd/assets/img/icons/logout.svg')}}" alt="">
                                    @lang('common.logout')
...
<script>
    ...
    @if(moduleStatusCheck('Branch'))
    $(document).ready(function() {
        $('#branchChange').on('change', function() {
            var branchId = $(this).val();

            $.ajax({
                url: "{{ route('branch.change-branch') }}",
                type: "POST",
                data: {
                    branch_id: branchId,
                    _token: "{{ csrf_token() }}"
