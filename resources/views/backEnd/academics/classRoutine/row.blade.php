<tr class="0" id="row_{{ $row }}"> 
    ...
    <td class="border-top-0">
        <div class="primary_input mt-10 d-flex justify-content-center">
            <input type="checkbox" id="isBreak[{{ $row }}]" class="common-checkbox is_break_checkbox" data-row_id="{{ $row }}" value="1"   name="routine[{{ $row }}][is_break]"  {{isset($routine)? ($routine->is_break == 1 ? 'checked':''):''}}    >
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">  
        ...
        <div class="modal-body">
            <input type='checkbox' id="all_days_{{ $row }}" class=' all_days' data-row_id="{{ $row }}" name='all_days[]' value='0'>
            <label for='all_days_{{ $row }}'>{{__('common.select_all')}}</label>
            <div class='row p-0'>

                @foreach($sm_weekends as $sm_weekend)
                <div class="col-lg-4 pr-0">
                    <input type="checkbox" class=" day-checkbox day_{{ $row }}" value="{{ $sm_weekend->id }}" data-row_id="{{ $row }}" id="day_{{ $loop->index .'_'. $row }}"
