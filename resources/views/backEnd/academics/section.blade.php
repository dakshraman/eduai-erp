...
@section('mainContent')
    @php
        $isUniversitySection = request()->routeIs('university.section.*');
        $sectionIndexRoute = $isUniversitySection ? 'university.section.index' : 'section';
    @endphp
                ...
                @if (userPermission('section_store'))
                    <div class="row">
                        <div class="offset-lg-10 col-lg-2 text-right col-md-12 mb-20">
                            <a href="{{ route($sectionIndexRoute) }}" class="primary-btn small fix-gr-bg">
            ...
            <div class="row">
                <div class="col-lg-4 col-xl-3">
                    <div class="row">
                        <div class="col-lg-12">
                            @if (isset($section))
                                {{ html()->form('POST', route('section_update'))->attribute('class', 'form-horizontal')->attribute('files', true)->open() }}
                            @else
                                @if (userPermission('section_store'))
                                    {{ html()->form('POST', route('section_store'))->attribute('class', 'form-horizontal')->attribute('files', true)->open() }}
                                @endif
                            @endif
                            <div class="white-box">
                                    ...
                                    <div class="row">
                                        @if (moduleStatusCheck('Branch'))
                                            @include('branch::components.branch_select', [
                                                ...
                                                'mb' => 'mb-15',
                                                'branch_id' => isset($section) ? $section->branch_id : '',
                                            ])
                                        @endif
                                        ...
                                        <tbody>
                                            @foreach ($sections as $section)
                                                <tr>
                                                    ...
                                                    <td class="w-15">
                                                        <div class="d-flex align-item-center justify-content-start flex-wrap">
                                                            @if(userPermission('section_edit'))
                                                                <a class="primary-btn small fix-gr-bg icon-only ml-10  l-h-30" href="{{ route('section_edit', [$section->id]) }}">
                                                                    <i class="fa fa-edit"></i>
                                                                </a>
                                                            @endif
                                                            @if(userPermission('section_delete'))
                                                ...
                                                <div class="modal fade admin-query"
                                                    id="deleteSectionModal{{ @$section->id }}">
