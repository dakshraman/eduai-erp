...
<section class="admin-visitor-area up_st_admin_visitor">
    <div class="container-fluid p-0">
        ...
        @endif
        <div class="row">
            @if (userPermission('class_store') || isset($sectionId))
            <div class="col-lg-3">
                <div class="row">
                    <div class="col-lg-12">
                        @if (isset($sectionId))
                        {{ html()->form('POST', route('class_update'))->attribute('class',
                        'form-horizontal')->attribute('files', true)->open() }}
                        @else
                        @if (userPermission('class_store'))
                        {{ html()->form('POST', route('class_store'))->attribute('class',
                        'form-horizontal')->attribute('files', true)->open() }}
                        @endif
                        @endif
                        <div class="white-box">
                            ...
                            <div class="add-visitor">
                                <div class="row">
                                    ...
                                    'mb' => 'mb-15',
                                    'branch_id' => isset($classById) ? $classById->branch_id : '',
                                    ])
                                    @endif
                                    ...
                                    <div class="col-lg-12">
                                        <div class="primary_input">
                                            ...
                                            <input
                                                ...
                                                value="{{ isset($classById) ? @$classById->class_name : old('name') }}">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="primary_input">
                                            ...
                                            <input
                                                ...
                                                value="{{ isset($classById) ? @$classById->pass_mark : old('pass_mark') }}">
                                ...
                                <div class="row mt-25">
                                    <div class="col-lg-12">
                                        <label class="primary_input_label" for="">@lang('common.shift')<span
                                                class="text-danger"> *</span></label>
                                        @foreach(shifts() as $shift)
                                        <div class="">
                                            @if(isset($shiftId))
                                            <input type="checkbox" id="shift{{@$shift->id}}"
                                                class="common-checkbox form-control{{ @$errors->has('shift') ? ' is-invalid' : '' }}"
                                                name="shift[]" value="{{@$shift->id}}" {{in_array(@$shift->id,
                                            @$shiftId)? 'checked': ''}}>
                                            <label for="shift{{@$shift->id}}"> {{@$shift->shift_name}}</label>
                                            @else
                                            <input type="checkbox" id="shift{{@$shift->id}}"
                                                class="common-checkbox form-control{{ @$errors->has('shift') ? ' is-invalid' : '' }}"
                                                name="shift[]" value="{{@$shift->id}}">
                                            <label for="shift{{@$shift->id}}"> {{@$shift->shift_name}}</label>
                                            @endif
                                        </div>
                                        @endforeach
                                    ...
                                    <div class="col-lg-12">
                                        ...
                                        <div class="">
                                            ...
                                            <input type="checkbox" id="section{{ @$section->id }}"
                                                ...
                                                name="section[]" value="{{ @$section->id }}" {{ in_array(@$section->id,
                                            @$sectionId) ? 'checked' : '' }}>
            ...
            @endif
            <div class="@if (userPermission('class_store') || isset($sectionId)) col-lg-9 @else col-lg-12 @endif">
                <div class="white-box">
                    <div class="row">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @if(shiftEnable())
                                                @if (!empty($shift_wise_class_section))
                                                @foreach ($shift_wise_class_section as $shiftId => $sectionsInShift)
                                                @php
                                                ...
                                                {{ $shiftName }}:
                                                @endif
                                                {!! $sectionsInShift->filter(function($section) {
                                                ...
                                                @endif
                                                @else
                                                @if (!empty($class->groupclassSections))
                                                {!! $class->groupclassSections->filter(function($section) {
                                                return optional($section->sectionName)->id !== null;
                                            ...
                                            @endif
                                            <td>
                                                <a href="{{ route('sorting_student_list', [$class->id]) }}">{{
                                                    $class->records_count }}</a>
                                            </td>
                                            ...
                                            <td valign="top">
                                                ...
                                                <div class="d-flex align-item-center justify-content-start flex-wrap">
                                                    @if(userPermission('class_edit'))
                                                    <a class="primary-btn small fix-gr-bg icon-only ml-10  l-h-30"
                                                        href="{{ route('class_edit', [$class->id]) }}">
                                                        <i class="fa fa-edit"></i>
                                                    </a>
                                                    @endif
                                                    @if(userPermission('class_delete'))
                                                    <a class="primary-btn small fix-gr-bg icon-only ml-10 l-h-30"
