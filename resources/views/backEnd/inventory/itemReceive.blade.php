 ...
 @section('mainContent')
     ...
     <section class="admin-visitor-area">
         <div class="container-fluid p-0">
             @if (isset($editData))
            {{ html()->form('PUT', route('item-list-update', $editData->id))->attributes([
             ...
             @else
                ...
                {{ html()->form('POST', route('save-item-receive-data'))->attributes([
                    'class' => 'form-horizontal',
                    'files' => true,
                    'enctype' => 'multipart/form-data',
                                                 ...
                                                 <select
                                                     ...
                                                     @if (isset($expense_head))
                                                         @foreach ($expense_head as $key => $value)
                                                             <option value="{{ $value->id }}" {{ old('expense_head_id') == $value->id ? 'selected' : '' }}>{{ $value->head }}
                                                             </option>
                                                         @endforeach
                                                     @endif
                                                 </select>
                                                 ...
                                                 <select
                                                     ...
                                                     @foreach ($paymentMethhods as $key => $value)
                                                         <option data-string="{{ $value->method }}"
                                                             value="{{ $value->id }}" {{ old('payment_method') == $value->id ? 'selected' : '' }}>{{ $value->method }}</option>
                                                     @endforeach
                                                 </select>
                                                 ...
                                                 <select
                                                     ...
                                                     @if (isset($account_id))
                                                         @foreach ($account_id as $key => $value)
                                                             <option value="{{ $value->id }}" {{ old('bank_id') == $value->id ? 'selected' : '' }}>
                                                 ...
                                                 <select
                                                     ...
                                                     @if (isset($itemStores))
                                                         @foreach ($itemStores as $key => $value)
                                                             <option value="{{ $value->id }}" {{ old('store_id') == $value->id ? 'selected' : '' }}
                                                                 @if (isset($editData)) @if ($editData->category_name == $value->id)
                                                        @lang('inventory.selected') @endif
                                                                 @endif
                                         ...
                                         <div class="col-lg-12 mb-15">
                                             <div class="primary_input">
                                                 ...
                                                 <input
                                                     ...
                                                     value="{{ isset($editData) ? $editData->reference_no : old('reference_no') }}">
                                         ...
                                         <div class="col-lg-12 mb-20">
                                             <div class="primary_input">
                                                 ...
                                                 <textarea class="primary_input_field form-control" cols="0" rows="4" name="description"
                                                     id="description">{{ isset($editData) ? $editData->description : old('description') }}</textarea>
                                                   ...
                                                   <td class="border-top-0">
                                                       <div class="primary_input">
                                                           <input oninput="numberCheckWithDot(this)"
                                                               ...
                                                               type="number" step="{{inputStep()}}" id="unit_price1"
                                                               name="unit_price[]" autocomplete="off"
                                                               value="{{ isset($editData) ? decimal_only($editData->unit_price) : '' }}"
                                                   ...
                                                   <td class="border-top-0">
                                                       <div class="primary_input">
                                                           <input oninput="numberCheckWithDot(this)"
                                                               ...
                                                               type="number" step="{{inputStep()}}" name="total[]" id="total1" autocomplete="off"
                                                               value="{{ isset($editData) ? decimal_only($editData->sub_total) : decimal_only(0) }}" readonly>
                                           ...
                                           <tfoot>
                                               <tr>
                                                   ...
                                                   <th class="border-top-0">
                                                       <input type="number" class="primary_input_field form-control"
                                                           readonly="" step="{{inputStep()}}" id="subTotalQuantity" name="subTotalQuantity"
                                                           placeholder="{{decimal_only(0)}}" />
                                                   ...
                                                   <th class="border-top-0">
                                                       <input type="number" class="primary_input_field form-control{{ $errors->has('subTotal') ? ' is-invalid' : '' }}"
                                                           id="subTotal" step="{{inputStep()}}" name="subTotal" placeholder="{{decimal_only(0)}}" readonly="" />
                                        ...
                                        <div class="col-lg-4 mt-30-md">
                                            <div class="col-lg-12">
                                                <div class="primary_input md_mb_20">
                                                    ...
                                                    <input class="primary_input_field" type="number" step="{{inputStep()}}"
                                                        value="{{decimal_only(0)}}" name="totalPaid" id="totalPaid"
                                                        onkeyup="paidAmount();">
                                        ...
                                        <div class="col-lg-4 mt-30-md">
                                            <div class="col-lg-12">
                                                <div class="primary_input md_mb_20">
                                                    ...
                                                    <input class="primary_input_field" type="number" step="{{inputStep()}}" value="{{decimal_only(0)}}"
 ...
 <script>
    const decimal_digits = {{ generalSetting()->currencyDetail->decimal_digit ?? 0 }};
</script>
@push('script')
    <script>
        (function() {
            ...
            function money(value) {
                return toNumber(value).toFixed(decimal_digits);
            }

            function itemOptions() {
                return $('#productName1').html();
            }
            ...
            function recalculate() {
                ...
                $('#productTable tbody > tr[id^="row"]').each(function() {
                    const id = this.id.replace('row', '');
                    const price = toNumber($('#unit_price' + id).val());
                    const quantity = toNumber($('#quantity' + id).val());
                    const total = price * quantity;

                    $('#total' + id).val(money(total));
                    $('#totalValue' + id).val(money(total));
                ...
                $('#subTotalQuantity').val(money(quantityTotal));
                $('#subTotalQuantityValue').val(money(quantityTotal));
                $('#subTotal').val(money(subTotal));
                $('#subTotalValue').val(money(subTotal));

                if ($('#full_paid').is(':checked')) {
                    $('#totalPaid').val(money(subTotal));
                }

                const paid = Math.min(toNumber($('#totalPaid').val()), subTotal);
                $('#totalPaidValue').val(money(paid));
                $('#totalDue').val(money(subTotal - paid));
                $('#totalDueValue').val(money(subTotal - paid));
            }

            window.getTotal = recalculate;
            window.getTotalByPrice = recalculate;
            window.paidAmount = recalculate;
            ...
            window.addRow = function() {
                ...
                const row = `
                    <tr id="row${rowIndex}">
                        <td class="border-top-0">
                            <div class="primary_input">
                                <select class="primary_select form-control" name="item_id[]" id="productName${rowIndex}">
                                    ${itemOptions()}
                                </select>
                            </div>
                        </td>
                        <td class="border-top-0">
                            <div class="primary_input">
                                <input oninput="numberCheckWithDot(this)" class="primary_input_field form-control"
                                    type="number" step="{{ inputStep() }}" id="unit_price${rowIndex}" name="unit_price[]"
                                    autocomplete="off">
                            </div>
                        </td>
                        <td class="border-top-0">
                            <div class="primary_input">
                                <input oninput="numberCheckWithDot(this)" class="primary_input_field form-control"
                                    type="text" id="quantity${rowIndex}" name="quantity[]" autocomplete="off">
                            </div>
                        </td>
                        <td class="border-top-0">
                            <div class="primary_input">
                                <input oninput="numberCheckWithDot(this)" class="primary_input_field form-control"
                                    type="number" step="{{ inputStep() }}" name="total[]" id="total${rowIndex}"
                                    autocomplete="off" value="{{ decimal_only(0) }}" readonly>
                            </div>
                            <input type="hidden" name="totalValue[]" id="totalValue${rowIndex}" class="form-control">
                        </td>
                        <td>
                            <button class="primary-btn icon-only fix-gr-bg delete-item-receive-row" type="button">
                                <span class="ti-trash"></span>
                            </button>
                        </td>
                    </tr>`;

                $('#productTable tbody > tr[id^="row"]:last').after(row);
                if ($.fn.niceSelect) {
                    $('#productName' + rowIndex).niceSelect();
                }
            };

            $(document).on('keyup change', '#item-receive-form input[name="unit_price[]"], #item-receive-form input[name="quantity[]"], #item-receive-form select[name="item_id[]"], #totalPaid, #full_paid', recalculate);
            $(document).on('click', '.delete-item-receive-row', function() {
                if ($('#productTable tbody > tr[id^="row"]').length > 1) {
