        ...
        <div class="white-box mt-40">
            <div class="row">
                ...
                <div class="col-lg-8 pull-right">
                    @if(moduleStatusCheck('University'))
                    <a href="{{route('university.academics.classRoutinePrint', [$un_semester_label_id, $un_section_id, moduleStatusCheck('Branch') ? $branch_id : null])}}" class="primary-btn small fix-gr-bg pull-right" target="_blank"><i class="ti-printer"> </i> @lang('reports.print')</a>
                    @else
                    <a href="{{route('classRoutinePrint', [$class_id, $section_id, shiftEnable() ? ($shift_id ?: 'null') : 'null', moduleStatusCheck('Branch') ? ($branch_id ?: 'null') : 'null'])}}" class="primary-btn small fix-gr-bg pull-right" target="_blank"><i class="ti-printer"> </i> @lang('reports.print')</a>
                    @endif 
                </div>
            </div>
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <table id="default_table" class="table " cellspacing="0" width="100%">
                        @php
                            $routineFilter = function ($query) use ($class_id, $section_id, $shift_id, $branch_id) {
                                return $query->where('class_id', $class_id)
                                    ->where('section_id', $section_id)
                                    ->when(shiftEnable() && ! empty($shift_id), function ($query) use ($shift_id) {
                                        $query->where('shift_id', $shift_id);
                                    })
                                    ->when(moduleStatusCheck('Branch') && ! empty($branch_id), function ($query) use ($branch_id) {
                        ...
                        <thead>
                            ...
                            @php
                            $count = $routineFilter($sm_weekend->classRoutine())->count();
                            @endphp
                        ...
                        @php
                            ...
                            $i = 0;
                        @endphp
                        @foreach($routineFilter($sm_weekend->classRoutine())->get() as $routine)
