        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="white-box">
                    ...
                    {{ html()->form('POST', route('previous-student-record'))->attributes([
                        'class' => 'form-horizontal',
                        'files' => true,
                        'enctype' => 'multipart/form-data',
                        'id' => 'admissionNoSearch',
                    ])->open() }}
                    <div class="row align-items-end">
                        ...
                        <div class="col-lg-6 col-xl-6 col-md-6 mt-30-md md_mb_20 d-none" id="selectRecordDiv">
                        ...
                        <div class="col-lg-12 mt-20 text-left">
    ...
    <script>
        $(document).on('submit', '#admissionNoSearch', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response == 'not_valid' || response[0] == 'not_valid') {
                        ...
                        $.each(response, function(i, item) {
                            if (item.length) {
                                hasRecord = true;
                                $("#selectRecordDiv").removeClass('d-none');
                                $("#record").find("option").remove();
                                ...
                                $.each(item, function(i, record) {
                                    ...
                                    if(i == 0){
                                        $("#record").html('<option value="">@lang("common.select_record")</option>')
                                    }
                                    $("#record").append(
                                        $("<option>", {
                                            value: record.id,
                                ...
                                $("#record").niceSelect('update');
                                if (item.length === 1) {
                                    $("#record").val(item[0].id).niceSelect('update').trigger('change');
                                }
                            } else {
                                $("#selectRecordDiv").addClass('d-none');
                        ...
                        if (!hasRecord) {
                            toastr.warning("No previous result found", "Warning", {
                                timeOut: 5000,
                            });
                        }
                    }
                ...
                error: function(xhr) {
                    let message = xhr.responseJSON && xhr.responseJSON.errors
                        ? xhr.responseJSON.errors.admission_number
                        : "No Data Found";
                    $('.admission_number_error').html(message);
                },
                complete: function() {
                    i--;
                    if (i <= 0) {
        ...
        $(document).on('change', '#record', function(e) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    let message = xhr.responseJSON && xhr.responseJSON.html
