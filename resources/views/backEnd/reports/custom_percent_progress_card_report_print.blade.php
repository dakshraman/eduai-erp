...
<head>
    ...
    <style>
        ...
        .logo_img {
            display: flex;
            align-items: center;
            background: url({{assetPath('public/backEnd/img/report-admit-bg.png')}}) no-repeat center;
...
<div class="invoice">
    <div class="invoice_wrapper">
                        ...
                        <td style="padding: 0">
                            <div class="{{(resultPrintStatus('header'))? "logo_img": "logo_img custom_result_print"}}">
                                <div class="thumb_logo">
                                    <img src="{{assetPath(generalSetting()->logo)}}"
                                ...
                                @if(resultPrintStatus('image'))
                                    <div class="profile_thumb">
                                        <img src="{{ file_exists(@$student_detail->studentDetail->student_photo) ? assetPath($student_detail->studentDetail->student_photo) : assetPath('public/uploads/staff/demo/staff.jpg') }}"
        ...
        <div class="{{(resultPrintStatus('body'))? "meritTableBody": "meritTableBodyCustomReport"}}">
                    ...
                    <tr class="text-center" style='text-align:center'>
                        @if ($optional_subject_setup!='' && $student_optional_subject!='')
                            @if ($student_optional_subject->subject_id==$data->subject->id)
                                <td>{{$data->subject !=""?$data->subject->subject_name:""}} (@lang('common.optional'))
                                </td>
                            @else
            ...
            <table style="max-width: 400px; margin: 30px auto;"
                ...
                <tr>
                    ...
                    @if ($optional_subject_setup!='' && $student_optional_subject!='')
                        <td colspan="{{$colspan / $col_for_result + 7}}"
                            style="padding:10px; font-weight:bold">{{number_format($total_marks,2)}} @lang('reports.out_of') {{$all_exam_type_full_mark}} </td>
                    @else
                ...
                <div style="margin-top:auto; margin-bottom:20px; display: flex; justify-content: {{$examReportSignature->count() == 1 ? 'flex-end' : 'space-between'}}; flex-wrap: wrap; align-items: center;">
                    @foreach ($examReportSignature as $signature)
                        <div style="text-align: right; display: flex; align-items: center; justify-content: center; margin-right: 24px; flex-direction: column">
                            <div>
                                <img src="{{assetPath($signature->signature)}}" width="150px" height="40px"
                                     alt="{{$signature->title}}">
                            </div>
