                                ...
                                <select class="primary_select form-control{{ $errors->has('exam') ? ' is-invalid' : '' }}"
                                    ...
                                    @foreach($exams as $exam)
                                        <option value="{{$exam->id}}"
                                                {{isset($InputExamId)? ($InputExamId == $exam->id? 'selected':''):''}}>{{$exam->title}}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('exam'))
            ...
            <div class="container-fluid p-0">
                <div class="row justify-content-end">
                    <div class="col-lg-8 pull-right mt-2 pt-2">
                        <a href="{{route('merit-list/print', [$InputExamId, $InputClassId, $InputSectionId, $shift->id ?? null])}}"
                                                    ...
                                                    <div class="table-responsive">
                                                        <table class="w-100 mt-30 mb-20 table table-bordered meritList">
                                                            ...
                                                            @foreach($allresult_data as $key => $row)
                                                                @php
                                                                    $student_detail = App\Models\SmStudent::where('id', $row->student_id)->first();
                                                                    $optional_subject = \App\Models\SmOptionalSubjectAssign::where('student_id', $student_detail->id)
                                                                ...
                                                                @foreach($markslist as $mark)
                                                                    @php
                                                                        $is_optional = App\Models\SmOptionalSubjectAssign::is_optional_subject($row->student_id, $get_subject_id[$count]);
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td>{{ $key + 1 }}</td>
                                                                    <td>{{ $current_total_marks }}</td>
                                                                    <!-- Use current student's total marks -->
