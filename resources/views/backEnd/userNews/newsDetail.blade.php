...
@push('css')
    <link rel="stylesheet" href="{{ assetPath('/') }}/Modules/News/public/assets/css/style.css" />
                        ...
                        <div class="row justify-content-center">
                            <div class="col-lg-9 col-md-10 col-sm-12 course__details mb-3">
                                <div class="text-center">
                                    <img class="img-fluid full-width-cover-image" src="{{assetPath($news->image)}}" alt="">
                                </div>
                                            ...
                                            @foreach ($news->newsComments as $comment)
                                                        ...
                                                        <div class="thumb">
                                                            {{-- @if ($comment->user->role_id == 2)
                                                                <img class="img-fluid rounded-circle"
                                                                    src="{{ assetPath($comment->user->student->student_photo) }}"
                                                                    alt="{{ @$comment->user->first_name }}">
                                                            @elseif($comment->user->role_id == 3)
                                                                <img class="img-fluid rounded-circle"
                                                                    src="{{ assetPath($comment->user->parent->guardians_photo) }}"
                                                                    alt="{{ @$comment->user->parent->first_name }}">
                                                            @else --}}
                                                                <img class="img-fluid rounded-circle"
                                                                    src="{{ @profile() && file_exists(@profile()) ? asset(profile()) : assetPath('public/backEnd/assets/img/avatar.png')  }}"
                                                ...
                                                <div class="modal fade" id="editCommentModal{{$comment->id}}" tabindex="-1" role="dialog" aria-labelledby="editModalCommentLabel{{$comment->id}}" aria-hidden="true">
                                                    <div class="modal-dialog modal-lg" role="document">
                                                        ...
                                                        {{ html()->form('POST', route('user-news-comment.update'))->attributes([
                                                            ...
                                                            <div class="d-flex align-items-center gap-10">
                                                                <div class="thumb">
                                                                    {{-- @if ($reply->user->role_id == 2)
                                                                        <img class="img-fluid rounded-circle" src="{{ assetPath($reply->user->student->student_photo) }}" alt="{{ @$reply->user->first_name }}">
                                                                    @elseif($reply->user->role_id == 3)
                                                                        <img class="img-fluid rounded-circle" src="{{ assetPath($reply->user->parent->guardians_photo) }}" alt="{{ @$reply->user->parent->first_name }}">
                                                                    @else --}}
                                                                        <img class="img-fluid rounded-circle" src=" {{ @profile() && file_exists(@profile()) ? asset(profile()) : assetPath('public/backEnd/assets/img/avatar.png') }}">
                                                                    {{-- @endif --}}
                                                                </div>
                                                                <div class="d-flex">
                                                                    <div>
                                                            ...
                                                            <div class="modal fade" id="editReplyModal{{$reply->id}}" tabindex="-1" role="dialog" aria-labelledby="editModalReplyLabel{{$reply->id}}" aria-hidden="true">
                                                                <div class="modal-dialog modal-lg" role="document">
                                                                    <div class="modal-content">
                                                                        ...
                                                                        <div class="modal-body">
                                                                        {{ html()->form('POST', route('user-news-comment-reply.update'))->attributes([
                                                                            'class' => 'form-horizontal',
                                                                            'files' => true,
                                                                            'enctype' => 'multipart/form-data',
                                                                        ])->open() }}
