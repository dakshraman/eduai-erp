...
@section('mainContent')
    ...
    <section class="admin-visitor-area up_st_admin_visitor">
        <div class="container-fluid p-0">
            @if (isset($room_type))
                @if (userPermission('add-room-type'))
                    <div class="row">
            ...
            <div class="row">
                ...
                <div class="col-lg-3">
                    <div class="row">
                        <div class="col-lg-12">
                            @if (isset($room_type))
                                {{ html()->form('PUT', route('room-type-update', $room_type->id))->attributes([
                            ...
                            @else
                            @if (userPermission('add-room-type'))
                                {{ html()->form('POST', route('room-type-store'))->attributes([
                                        'class' => 'form-horizontal',
                                        'enctype' => 'multipart/form-data',
                                        'files' => true,
                                    ])->open() }}
                                    ...
                                    <div class="row">
                                        @if (moduleStatusCheck('Branch'))
                                            @include('branch::components.branch_select', [
                                                ...
                                                'mb' => 'mb-15',
                                                'branch_id' => isset($room_type) ? $room_type->branch_id : '',
                                            ])
                                        @endif
                                        <div class="col-lg-12">
                                            <div class="primary_input">
                                                <label class="primary_input_label" for="">@lang('dormitory.room_type') <span
                                                        class="text-danger"> *</span></label>
                                                <input
                                                    class="primary_input_field form-control{{ $errors->has('type') ? ' is-invalid' : '' }}"
                                                    type="text" name="type" autocomplete="off"
                                                    value="{{ isset($room_type) ? $room_type->type : old('type') }}">
                                                <input type="hidden" name="id"
                                                    value="{{ isset($room_type) ? $room_type->id : '' }}">
                                        ...
                                        <tbody>
                                            @foreach ($room_types as $key => $room_type)
                                                <tr>
                                                    <td>{{ $key + 1 }}</td>
                                                    @if (moduleStatusCheck('Branch'))
                                                    <td>{{ @$room_type->branch?->branch_name }}</td>
                                                    @endif
                                                    <td>{{ @$room_type->type }}</td>
                                                    <td>{{ @$room_type->description }}</td>
                                                    <td>
                                                        <x-drop-down>
                                                            @if (userPermission('edit-room-type'))
                                                            <a class="dropdown-item"
                                                                href="{{ route('room-type-edit', [$room_type->id]) }}">
                                                                @lang('common.edit')</a>
                                                            @endif
                                                            @if (userPermission('delete-room-type'))

                                                            <a class="dropdown-item" data-toggle="modal"
                                                                data-target="#deleteRoomTypeModal{{ @$room_type->id }}"
                                                ...
                                                <div class="modal fade admin-query"
                                                    id="deleteRoomTypeModal{{ @$room_type->id }}">
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                ...
                                                                <div class="mt-40 d-flex justify-content-between">
                                                                    ...
                                                                    {{ html()->form('DELETE', route('room-type-delete', $room_type->id))->attribute('enctype', 'multipart/form-data')->open() }}
