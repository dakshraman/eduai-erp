@php
    ...
    if (moduleStatusCheck('ParentRegistration')) {
        $reg_setting = registrationSetting();
...
<head>
    ...
    @if ($setting->site_title == 1)
        <link rel="stylesheet" href="{{ assetPath('public/backEnd/') }}/css/rtl/bootstrap.min.css" />
    @else
    ...
    @if (isset($ttl_rtl) && $ttl_rtl == 1)
        ...
        <link rel="stylesheet" href="{{ assetPath('public/backEnd/assets/css/rtl/style.css') }}" />
    @endif
...
<script src="{{ assetPath('public/whatsapp-support/scripts.js') }}"></script>
