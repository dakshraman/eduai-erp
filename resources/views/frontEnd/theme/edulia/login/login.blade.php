...
@php
    ...
    if (empty($login_background)) {
        $css = 'background: url(' . assetPath('public/backEnd/img/edulia-login-bg.jpg') . ') no-repeat center; background-size: cover; ';
    } else {
        if (!empty($login_background->image)) {
            $css = 'background: url("' . assetPath($login_background->image) . '") no-repeat center; background-size: cover';
        } else {
...
<head>
    ...
    <link rel="icon" href="{{ assetPath($gs->favicon) }}" type="image/png"/>
    ...
    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="{{ assetPath('public/theme/edulia/css/bootstrap.min.css') }}">

    <!-- Fontawesome CSS -->
    <link rel="stylesheet" href="{{ assetPath('public/theme/edulia/css/fontawesome.all.min.css') }}">

    @if(userRtlLtl() ==1)
        <link rel="stylesheet" href="{{ assetPath('public/theme/edulia/css/style_rtl.css') }}">
    @else
        <link rel="stylesheet" href="{{ assetPath('public/theme/edulia/css/style.css') }}">
    @endif
    <style>
        .row_gap_24 {
            row-gap: 24px;
...
<section class="login" style="{{ $css }}">
    <div class="login_wrapper">
        ...
        <div class="login_wrapper_login_content">
            <div class="login_wrapper_logo text-center"><img src="{{ assetPath($gs->logo) }}"
                                                             alt=""></div>
            ...
            <div class="saas_school_top_five_link_show mt-4">
                <div class="row row-gap-10 justify-content-center">
                    ...
                    @foreach ($schools as $school)
                        <div class="col-4 col-sm-4 col-md-3">
                            <a target="_blank" href="//{{ $school->domain . '.' . config('app.short_url') }}/home"
                               class="link_to_school">{{ Str::limit($school->school_name, 20, '...') }}</a>
                        </div>
                    @endforeach
                </div>
            </div>
...
<!-- jQuery JS -->
<script src="{{ assetPath('public/theme/edulia/js/jquery.min.js') }}"></script>
...
<!-- Main Script JS -->
<script src="{{ assetPath('public/theme/edulia/js/script.js') }}"></script>
