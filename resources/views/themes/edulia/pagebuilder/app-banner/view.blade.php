    ...
    <div class="row">
        @if (!empty(pagesetting('app_banner_image')))
            <div class="col-xxl-4 col-4 col-sm-5 download_app_container">
                <img src="{{ assetPath(pagesetting('app_banner_image')[0]['thumbnail']) }}" class="download_app_image"
                    alt="{{ __('edulia.Image') }}">
            </div>
        @endif
                        ...
                        @foreach (pagesetting('app_banner_items') as $item)
                            <li>
                                @if (!empty($item['item_icon']))
                                    <img src="{{ assetPath($item['item_icon'][0]['thumbnail']) }}"
                                        alt="{{ __('edulia.Image') }}">
                                @endif
                                {{ $item['item_heading'] }}
                            </li>
                    ...
                    @foreach (pagesetting('app_download_items') as $item)
                        <a href="{{ $item['item_link'] }}" target="_blank">
                            @if (!empty($item['item_image']))
                                <img src="{{ assetPath($item['item_image'][0]['thumbnail']) }}" alt="{{ __('edulia.Image') }}">
                            @endif
                        </a>
                    @endforeach
                </div>
