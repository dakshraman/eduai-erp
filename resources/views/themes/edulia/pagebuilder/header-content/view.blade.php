...
<link rel="stylesheet" href="{{assetPath('public/theme/' . activeTheme() . '/packages/zeynep/zeynep.min.css')}}">
<link rel="stylesheet" href="{{assetPath('public/theme/' . activeTheme() . '/themify/themify-icons.min.css')}}">
@endpushonce
@php
                ...
                <div class="text-end">
                    <nav class="heading_sub_right">
                            ...
                            @if (!empty(pagesetting('header-right-menus')))
                                @foreach (pagesetting('header-right-menus') as $icon)
                                    <li class='social-links-list'>
                                        <a href="{{ gv($icon, 'header-right-icon-url') }}" target='_blank'
                                            class='social-links-list-link'>
                        ...
                        <ul>
                                ...
                                @if (pagesetting('header-parent-registration-is-show-menu') == 1)
                                    <li>
                                        <a href="{{ route('parentregistration/registration', $reg_setting->url) }}"
                                            target="{{ pagesetting('header-parent-registration-redirect-menu') == '1' ? '_self' : '_blank' }}">
                            ...
                            @if (moduleStatusCheck('Partner'))
                                <li>
                                    <a href="{{ route('partner.registration') }}">
                                        <i class="far fa-users"></i>
                                        <!-- {{ __('partner.partner') }} --> Partner Registration
                                    </a>
                                </li>
                            @endif
                            ...
                            @if (moduleStatusCheck('Saas') && session('domain') == 'school')
                                @if (pagesetting('header-school-is-show-menu') == 1)
                                    <li>
                                        <a href="{{ route('institution-register-new') }}"
                                            target="{{ pagesetting('header-school-redirect-menu') == '1' ? '_self' : '_blank' }}">
    ...
    <div class="heading_mobile">
        ...
        <div class='text-center'>
            <a href='{{url('/')}}' class="heading_logo">
                <img src="{{ pagesetting('header_menu_image') ? assetPath(pagesetting('header_menu_image')[0]['thumbnail']) : defaultLogo($generalSetting->logo) }}"
                    alt="">
            </a>
        </div>
                ...
                @if (!empty(pagesetting('header_menu_search')) && pagesetting('header_menu_search') == 1)
                    <div class="col-md-3 text-end mobile-none">
                        <form action='#' methods='GET' class="heading_main_search mb-0">
                            <div class="input-control">
                                <input type="search" class="input-control-input"
                                    placeholder='{{pagesetting('header_menu_search_placeholder')}}' required>
...
<script src="{{ assetPath('public/theme/' . activeTheme() . '/packages/zeynep/zeynep.min.js') }}"></script>
<script>
