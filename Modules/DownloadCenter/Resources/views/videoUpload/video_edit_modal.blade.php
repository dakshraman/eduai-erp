<div class="container-fluid">
    {{ html()->form('POST', route('download-center.video-list-update'))
        ->attributes(['class' => 'form-horizontal', 'files' => true, 'enctype' => 'multipart/form-data'])
        ->open() }}
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            @if (moduleStatusCheck('Branch'))
                <div class="row">
                    @include('branch::components.branch_select', [
                        ...
                        'mb' => 'mb-15',
                        'branch_id' => $video->branch_id ?? old('branch_id'),
                    ])
                </div>
            @endif
            ...
            <div class="row">
                @if(moduleStatusCheck('University'))
                    @include('university::common.session_faculty_depart_academic_semester_level',[
                        'div' => 'col-lg-6',
                        'hide' => ['USUB','USEC'],
                        'editData' => $video
                    ])
                @else   
                    @if(shiftEnable())
                        @includeIf('backEnd.shift.shift_include', [
                            ...
                            'required' => true,
                            'id' => 'edit_shift',
                            'editData' => $video->shift_id ?? old('shift'),
                        ]) 
                    @endif
                        ...
                        <select class="primary_select form-control{{ $errors->has('section_id') ? ' is-invalid' : '' }} select_section"
                            ...
                            @foreach ($sections ?? [] as $section)
                                <option value="{{ $section->id }}"
                                    {{ $video->section_id ?? '' == $section->id ? 'selected' : '' }}>
                ...
                <div class="col-lg-6">
                    <div class="primary_input">
                        <label class="primary_input_label" for="">@lang('downloadCenter.title')<span class="text-danger">*</span></label>
                        <input class="primary_input_field read-only-input form-control{{ $errors->has('title') ? ' is-invalid' : '' }}"
                               type="text" name="title" id="title" value="{{ $video->title ?? old('title') }}">
                        <span class="text-danger" id="titleError"></span>
                        <span class="text-danger">{{ $errors->first('title') }}</span>
                    </div>
                </div>

                <div class="{{ moduleStatusCheck('University') ? 'col-lg-12 mt-15':'col-lg-6' }}">
                    <div class="primary_input">
                        <label class="primary_input_label" for="">@lang('downloadCenter.video_link')<span class="text-danger">*</span></label>
                        <input class="primary_input_field read-only-input form-control{{ $errors->has('video_link') ? ' is-invalid' : '' }}"
                               type="text" name="video_link" id="video_link" value="{{ $video->youtube_link ?? old('video_link') }}">
        ...
        <div class="col-lg-12 mt-25">
            <div class="primary_input">
                ...
                <textarea class="primary_input_field form-control" name="description" id="description" cols="0" rows="3">{{ $video->description ?? old('description') }}</textarea>
            </div>
        </div>
        ...
        <div class="col-lg-12 text-center mt-25">
            <div class="d-flex justify-content-between">
                <button type="button" class="primary-btn tr-bg" data-dismiss="modal">@lang('downloadCenter.cancel')</button>
                <button class="primary-btn fix-gr-bg submit" id="save_button_query" type="submit">@lang('downloadCenter.save')</button>
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        $(".select_shift").on("change", function () {
            var url = $("#url").val();
            var i = 0;
            ...
            $.ajax({
                ...
                success: function (data) {
                    $("#common_select_class_video").empty().append(
                        $("<option>", {
                            value: '',
                    ...
                    if (data.length) {
                        $.each(data, function (i, className) {
                            $("#common_select_class_video").append(
                                $("<option>", {
                                    value: className.id,
                    ...
                    $('#common_select_class_video').niceSelect('update');
