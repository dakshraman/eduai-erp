...
{
    private function resolveYoutubeFileName(string $youtubeLink): string
    {
        try {
            return getYoutubeName($youtubeLink) ?: $youtubeLink;
    ...
    public function contentList()
    {
        try {
            $editContent = null;
            $contents = branchWise(Content::with('contentType', 'user')->get());
            $contentTpyes = branchWise(ContentType::all());
            $roles = InfixRole::select('*')->where('is_saas', 0)->where('id', '!=', 1)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
            $classes = branchWise(SmClass::get());

            return view('downloadcenter::contentList.contentList', ['contentTpyes' => $contentTpyes, 'contents' => $contents, 'editContent' => $editContent, 'roles' => $roles, 'classes' => $classes]);
    ...
    public function contentListSave(Request $request)
    {
        ...
        $validator = Validator::make($input, [
            ...
            'youtube_link' => 'nullable|required_without:content_file|url',
        ...
        try {
            ...
            if ($request->youtube_link) {
                $newContent->file_name = $this->resolveYoutubeFileName($request->youtube_link);
    ...
    public function contentListSearch(Request $request)
    {
        ...
        try {
            $contents = branchWise(Content::where('file_name', 'LIKE', '%'.$request->name.'%')->with('contentType', 'user')->get());
            $contentTpyes = branchWise(ContentType::get());
            $editContent = null;
            $roles = InfixRole::select('*')->where('is_saas', 0)->where('id', '!=', 1)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
            ...
            return view('downloadcenter::contentList.contentList', ['contentTpyes' => $contentTpyes, 'contents' => $contents, 'editContent' => $editContent, 'roles' => $roles, 'classes' => $classes, 'name' => $name]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
