...
class ContentShareListController extends Controller
{
    public function contentShareList()
    {
        try {
            if (auth()->user()->role_id === 2) {
                ...
                $allSharedContents = branchWise(ContentShareList::get());
                $sharedContents = [];
                foreach ($allSharedContents as $allSharedContent) {
                    ...
                    if (
                        ...
                        (! shiftEnable() || $allSharedContent->shift_id === $student->studentRecord->shift_id) &&
                ...
                $sharedContents = branchWise(ContentShareList::with('user')->get());
            }
    ...
    public function contentShareListSave(ContentShareRequest $contentShareRequest)
    {
        try {
            ...
            if ($contentShareRequest->selectTab === 'C') {
                ...
                $contentShareList->shift_id = $contentShareRequest->shift;
                if ($contentShareRequest->section_ids) {
                    $contentShareList->section_ids = $contentShareRequest->section_ids;
                }
            }
            if (moduleStatusCheck('Branch')) {
                $contentShareList->branch_id = $contentShareRequest->branch_id;
            }
    ...
    public function contentViewLinkModal($id)
    {
        try {
            ...
            $individuals = ($viewContent->ind_user_ids) ? branchWise(User::whereIn('id', $viewContent->ind_user_ids)->get()) : null;
            $classSections = ($viewContent->class_id) ? branchWise(SmClassSection::where('class_id', $viewContent->class_id)
    ...
    public function contentShareLink($url)
    {
        try {
            ...
            $contents = branchWise(Content::whereIn('id', $sharedContent->content_ids)->get());
