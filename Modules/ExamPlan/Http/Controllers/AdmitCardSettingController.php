...
class AdmitCardSettingController extends Controller
{
    ...
    public function setting()
    {
        ...
        return view('examplan::setting.admitCardSetting', ['setting' => $setting]);
    }
    ...
    public function admitcard()
    {
        try {
            $exams = branchWise(SmExamType::where('active_status', 1)
                ...
                ->get());
            $classes = branchWise(SmClass::where('academic_id', getAcademicId())->where('school_id', auth()->user()->school_id)->get());

            return view('examplan::admitCard', ['exams' => $exams, 'classes' => $classes]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Error');
    ...
    public function universityAdmitCardSearch($request)
    {
        ...
        try {
            ...
            if ($request->branch_id && moduleStatusCheck('Branch')) {
                branchWiseApplyFilter($exam, 'branch_id', $request->branch_id);
            }
            ...
            $old_admit_ids = [];
            foreach ($old_admits as $old_admit) {
                $old_admit_ids[] = $old_admit->student_record_id;
            }

            $active_status = 1;
            if ($exam_routine->isNotEmpty()) {
                ...
                if ($request->branch_id && moduleStatusCheck('Branch')) {
                    branchWiseApplyFilter($student_records, 'branch_id', $request->branch_id);
                }

                $records = $student_records->get();

                return view('examplan::admitCard', ['records' => $records, 'exam_id' => $exam_id, 'class_id' => $class_id, 'old_admit_ids' => $old_admit_ids, 'branch_id' => $branch_id]);
            }
    ...
    public function admitcardSearch(Request $request)
    {
        ...
        try {
            ...
            $branch_id = moduleStatusCheck('Branch') ? $request->branch_id : '';
            $exam->where('school_id', auth()->user()->school_id)->where('academic_id', getAcademicId());
            if ($request->exam) {
            ...
            if ($request->shift) {
                $exam->where('shift_id', $request->shift);
            }
            ...
            if ($exam_routine->isNotEmpty()) {
                ...
                $exams = branchWise(SmExamType::where('active_status', 1)
                    ...
                    ->where('school_id', Auth::user()->school_id)
                    ->get());
                $classes = branchWise(SmClass::where('academic_id', getAcademicId())
                    ->where('school_id', auth()->user()->school_id)
                    ->get());

                return view('examplan::admitCard', ['exams' => $exams, 'classes' => $classes, 'records' => $records, 'exam_id' => $exam_id, 'class_id' => $class_id, 'old_admit_ids' => $old_admit_ids, 'shift_id' => $shift_id, 'section_id' => $section_id, 'branch_id' => $branch_id]);
            }
    ...
    public function UniversityAdmitCardGenerate($request)
    {
        try {
            $student_records = [];
            $studentRecord = null;
            ...
            if ($request->data) {
                ...
                foreach ($student_records as $student_record) {
                    $admit_card = AdmitCard::where('exam_type_id', $request->exam_type_id)->where('student_record_id', $student_record)->first();

                    $studentRecord = StudentRecord::find($student_record);
                    if (! $admit_card) {
                        $new_admit = new AdmitCard();
                        $new_admit->student_record_id = $student_record;
                ...
                if (empty($studentRecord)) {
                    Toastr::error('Student not selected', 'Error');
                ...
                if ($setting->admit_layout === 2) {
                    return view('university::exam_plan.admitcardPrint_2', ['setting' => $setting, 'assign_subjects' => $assign_subjects, 'exam_routines' => $exam_routines, 'admitcards' => $admitcards]);
                }

                if ($setting->admit_layout === 1) {
                    return view('university::exam_plan.admitcardPrint', ['setting' => $setting, 'assign_subjects' => $assign_subjects, 'exam_routines' => $exam_routines, 'admitcards' => $admitcards]);
                }
            }
            ...
            if ($request->data) {
                ...
                foreach ($student_records as $student_record) {
                    $admit_card = AdmitCard::where('exam_type_id', $request->exam_type_id)
                        ->where('student_record_id', $student_record)
                        ->first();
                    $studentRecord = StudentRecord::find($student_record);
                    ...
                    if (! $admit_card) {
                        ...
                        $data['shift_id'] = $new_admit->studentRecord ? $new_admit->studentRecord->shift_id : '';

                        $user_id = StudentRecord::find($student_record)->studentDetail->user_id;
                ...
                foreach ($unique_user_ids as $unique_user_id) {
                    $this->sent_notifications('Exam_Admit_Card', [$unique_user_id], $data, ['Student', 'Parent']);
                }
                if (empty($studentRecord)) {
                    Toastr::error('Student not selected', 'Error');
                ...
                $exam_routines = SmExamSchedule::where('class_id', $studentRecord->class_id)
                    ->when(shiftEnable(), function ($query) use ($studentRecord) {
                        return $query->where('shift_id', $studentRecord->shift_id);
                    })
                    ->where('section_id', $studentRecord->section_id)
                    ->where('exam_term_id', $request->exam_type_id)->orderBy('date', 'ASC')->get();

                if ($setting->admit_layout === 2) {
                    return view('examplan::admitcardPrint_2', ['setting' => $setting, 'assign_subjects' => $assign_subjects, 'exam_routines' => $exam_routines, 'admitcards' => $admitcards]);
                }

                if ($setting->admit_layout === 1) {
                    return view('examplan::admitcardPrint', ['setting' => $setting, 'assign_subjects' => $assign_subjects, 'exam_routines' => $exam_routines, 'admitcards' => $admitcards]);
                }
            }
