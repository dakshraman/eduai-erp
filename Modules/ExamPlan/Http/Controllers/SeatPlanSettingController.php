...
class SeatPlanSettingController extends Controller
{
    ...
    public function universitySeatPlanSearch($request)
    {
        ...
        try {
            ...
            $exam = branchWise(SmExamSchedule::query());
    ...
    public function seatplanSearch(Request $request)
    {
        ...
        try {
            ...
            if ($exam_routine->isNotEmpty()) {
                ...
                return view('examplan::seatPlan', compact('exams', 'classes', 'records', 'exam_id', 'class_id', 'seat_plan_ids', 'shift_id', 'section_id', 'branch_id'));
            }
            Toastr::warning('Exam shedule is not ready', 'warning');
    ...
    public function universitySeatPlanGenerate($request)
    {
        try {
            $user = Auth::user();
            ...
            if ($request->data) {
                ...
                if (empty($student_records)) {
                    ...
                    return redirect()->to('examplan/seatplan');
                }
                ...
                foreach ($student_records as $record) {
                    ...
                    if (! $seat_plan) {
                        ...
                        $new_seat->created_by = $user->id;
                        $new_seat->school_id = $user->school_id;
                        $new_seat->un_academic_id = $academic_id;
                        $new_seat->save();
                ...
                $seat_plans = SeatPlan::with([
                    'studentRecord' => function ($query) {
                        $query->select(['student_id', 'id', 'un_section_id', 'un_department_id']);
                    },
                    'studentRecord.studentDetail' => function ($query) {
                        $query->select(['id', 'roll_no', 'admission_no', 'student_photo', 'full_name']);
                    },
                    'studentRecord.unSection' => function ($query) {
                        $query->select(['id', 'section_name']);
                    },
                    'studentRecord.unDepartment' => function ($query) {
                    ...
                    ->whereIn('student_record_id', $student_records)
                    ->get();

                $setting = SeatPlanSetting::where('school_id', $user->school_id)->where('un_academic_id', $academic_id)->first();
                if (! $setting) {
                    $oldSetting = SeatPlanSetting::where('school_id', $user->school_id)->latest()->first();
                    $setting = $oldSetting->replicate();
                    $setting->un_academic_id = $academic_id;
                    $setting->save();
                }
            ...
            if ($request->data) {
                ...
                foreach ($student_records as $record) {
                    ...
                    if (! $seat_plan) {
                        ...
                        $data['shift_id'] = $new_seat->studentRecord ? $new_seat->studentRecord->shift_id : '';
