                    ...
                    <div class="seat-left" @if(!$setting->student_photo) style="max-width: 100%; padding-right: 0px;" @endif>
                        ...
                        <ul>
                            @if(moduleStatusCheck('Branch'))
                                <li>
                                    {{ $seat_plan->studentRecord->branch?->branch_name }}
                                </li>
                            @endif
                            @if($setting->class_section)
                                @if (moduleStatusCheck('University'))
                                    <li>
                                        {{ $seat_plan->studentRecord->unDepartment->name }}
                                        ({{ $seat_plan->studentRecord->unSection->section_name }})
                                    </li>
                                @else    
                                    <li>{{@$seat_plan->studentRecord->class->class_name}}
                                        ({{@$seat_plan->studentRecord->section->section_name}})
                                        @if(shiftEnable() && @$seat_plan->studentRecord->shift)
                                        [{{@$seat_plan->studentRecord->shift->name}}]
                                        @endif
                                    </li>
                                @endif
                            @endif
                        ...
                        <div class="seat-right">
                            <div class="seat-img">
                                @if($seat_plan->studentRecord->studentDetail->student_photo)
                                    <img src="{{assetPath(@$seat_plan->studentRecord->studentDetail->student_photo)}}"
                                         alt="{{@$seat_plan->studentRecord->studentDetail->full_name}}">
                                @else
                                    @if($seat_plan->studentRecord->studentDetail->gender->base_setup_name == 'Male')
                                         <img src="{{assetPath('modules/ExamPlan/Public/images/profile.jpg')}}"   alt="{{@$seat_plan->studentRecord->studentDetail->full_name}}">
                                    @else    
                                        <img src="{{assetPath('public/uploads/staff/demo/father.png')}}"   alt="{{@$seat_plan->studentRecord->studentDetail->full_name}}">
                                    @endif                          
                                @endif
                            </div>
                        </div>
...
@if($setting->student_photo)
            @if($seat_plan->studentRecord->studentDetail->student_photo)
                <img src="{{assetPath(@$seat_plan->studentRecord->studentDetail->student_photo)}}" alt="{{@$seat_plan->studentRecord->studentDetail->full_name}}">
