...
<head>
    ...
    <!-- Main css -->
    <link rel="stylesheet" href="{{assetPath('public/exam_plan/assets/css/style.min.css')}}">
                ...
                <tr>
                    <td class="flex flex-wrap gap-2 items-center">
                        <div class="logo w-110">
                            <img src="{{ assetPath(generalSetting()->logo)}}" class="w-full"
                                 alt="{{ assetPath(generalSetting()->school_name)}}">
                        </div>
                        <div class="content">
            ...
            <div class="user border p-10 flex">
                    ...
                    <tr class="border-bottom">
                        ...
                        <td class="text-start" width="25%" style="border-left: 1px solid #1a1818;">
                            @if(moduleStatusCheck('Branch'))
                                @lang('common.branch') - {{@$admitcard->studentRecord->branch->branch_name}}
                            @endif
                        ...
                        <td class="text-start" width="25%" style="border-left: 1px solid #1a1818;">
                            @if($setting->class_section)
                                <p class="fs-16 fw-bold text-black">@lang('student.section') : <span
                                            class="fs-14 fw-bold text-red">{{@$admitcard->studentRecord->section->section_name}} 
                                            @if(shiftEnable()) {{isset($admitcard->studentRecord->shift) ? ' ['. $admitcard->studentRecord->shift->name.']' : ''}} @endif</span>
                                </p>
                            @endif
                        </td>
                    </tr>
                ...
                @if($setting->student_photo)
                    <div class="profile flex items-center justify-center border">
                            <img src="{{assetPath(@$admitcard->studentRecord->studentDetail->student_photo != '' ? @$admitcard->studentRecord->studentDetail->student_photo : 'public/uploads/staff/demo/staff.jpg')}}"
                                    alt="{{assetPath(@$admitcard->studentRecord->studentDetail->full_name)}}">
                    </div>
                @endif
            </div>
            <div class="h-10"></div>
                ...
                @if($setting->principal_signature)
                    ...
                    <div class="singnature_img">
                        @if($setting->principal_signature_photo)
                            <img src="{{assetPath($setting->principal_signature_photo)}}">
                        @endif
                    </div>
