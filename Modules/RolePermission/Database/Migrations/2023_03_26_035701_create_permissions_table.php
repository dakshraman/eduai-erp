...
class CreatePermissionsTable extends Migration
{
    ...
    public function up()
    {
        ...
        // Skip data seeding in test environment to speed up migrations
        if (app()->environment('testing')) {
            return;
        }

        // this file will be first
        $adminPermissionList = include base_path('./resources/var/permission/without_student_parent_positions.php');
        foreach ($adminPermissionList as $item) {
            storePermissionData($item);
        }
        // first file end
        $studentPermissionList = include base_path('./resources/var/permission/student_permissions.php');
        foreach ($studentPermissionList as $item) {
            storePermissionData($item);
        }
        $parentPermissionList = include base_path('./resources/var/permission/parent_permissions.php');
