...
class GroupChatController extends Controller
{
    ...
    public function create()
    {
        ...
        return response()->json(['users' => $users, 'myGroups' => $myGroups]);
    }
    ...
    public function show(Group $group)
    {
        ...
        $myGroups = $this->groupService->getAllGroup();
        $users = $this->invitationService->getAllConnectedUsers();
        $remainingUsers = $users->filter(function ($value, $key) use ($group): bool {
        ...
        $single_threads = $only_threads->sortByDesc('created_at')->take(20)->map(function ($value, $key): GroupMessageRecipient {
        ...
        return response()->json(['group' => $group, 'users' => $users, 'myGroups' => $myGroups, 'remainingUsers' => $remainingUsers, 'myRole' => $myRole, 'single_threads' => $single_threads]);
    }
    ...
    public function addPeople(Request $request): bool
    {
        $group = Group::find($request->group_id);
        GroupUser::create([
            'group_id' => $group->id,
    ...
    public function removePeople(Request $request): bool
    {
    ...
    public function loadMore(Request $request)
    {
        ...
        $ids = $request->ids;
        $newThreads = $group->threads->filter(function ($value, $key) use ($ids): bool {
