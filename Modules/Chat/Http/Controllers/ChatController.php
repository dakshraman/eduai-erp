...
class ChatController extends Controller
{
    ...
    public function index($id = null, $notification_id = null)
    {
        try {
            ...
            if ($users->isEmpty()) {
                ...
                $activeUser->blocked = in_array($activeUser->id, $this->invitationService->getBlockUsers());
            }

            $groups = $this->groupService->getAllGroup();

            return view('chat::index', ['users' => $users, 'activeUser' => $activeUser, 'messages' => $messages, 'groups' => $groups]);
    ...
    public function store(Request $request)
    {
        ...
        $limit = ((int) app('general_settings')->get('chat_file_limit') * 1024) > 0 ? (int) app('general_settings')->get('chat_file_limit') * 1024 : 204800;
        ...
        if (! $recipient || ! auth()->user()->canChatWith((int) $recipient->id)) {
            return response()->json([
                'success' => false,
        ...
        [$img_name, $original_name, $type] = $this->fileHandle($request);
    ...
    public function show($id)
    {
        ...
        return view('chat::show', ['users' => $users, 'activeUser' => $activeUser, 'messages' => $messages]);
    }
    ...
    public function files($type, $id)
    {
        ...
        return view('chat::files', ['groups' => $groups, 'users' => $users, 'messages' => $messages, 'name' => $name, 'type' => $type, 'group' => $group]);
    }
    ...
    public function newNotificationCheck(Request $request)
    {
        ...
        $array = json_decode($request->notification_ids);
    ...
    public function fileHandle(Request $request): array
    {
        ...
        return [$img_name, $original_name, $type];
    }
    ...
    public function new()
    {
        ...
        return view('chat::new-chat', ['users' => $users, 'groups' => $groups]);
    }
