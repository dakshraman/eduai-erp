...
class InvitationRepository
{
    ...
    public function invitationUpdate($type, $id): bool
    {
    ...
    public function getAllConnectedUsers()
    {
        ...
        foreach ($users as $user) {
            $last = $user->userSpecificConversation($user->id)->first();
            if ($last) {
                $user->custom_order = $last->id;
                $user->last_message = $last->message_type === 0 ? $last->message : 'Attachment';
            } else {
                $user->last_message = '';
            }
        }
    ...
    public function blocksResult(): array
    {
