...
{
    use RestartOctan,SidebarDataStore;
    ...
    public function sectionEditForm(Request $request, $id)
    {
        ...
        $data['editPermissionSection'] = SmMenu::where('id', $id)
            ->where('school_id', auth()->user()->school_id)
            ->first();

        if (! $data['editPermissionSection']) {
            Toastr::error(__('common.Operation failed'), 'Failed');

            return redirect()->route('menumanage.index', ['role_name' => $role_name]);
        }
        ...
        $view = $role_id && empty($request->role_name) ? 'menumanage::role_index' : 'menumanage::index';
    ...
    public function sectionUpdate(Request $request)
    {
        ...
        $section = SmMenu::where('id', $request->id)
            ->where('school_id', auth()->user()->school_id)
            ->first();
        ...
        $catch_for = 'sidebar_'.$request->role_name.'_school_'.auth()->user()->school_id;
        Cache::forget($catch_for);
    ...
    public function removeSection(Request $request)
    {
        ...
        if ($request->id !== 1) {
            $role_id = $this->getRoleId($request->role_name);
            $menu = SmMenu::with(['childs' => function ($q) use ($role_id) {
                $q->where('role_id', $role_id);
            }])
                ->where('id', $request->id)
            ...
            if (! empty($menu->childs) && $menu->childs->count() > 0) {
    ...
    public function menuRemove(Request $request)
    {
        ...
        Cache::forget($catch_for);
        $data = $request->all();
        $menu = SmMenu::where('id', $data['id'])->with('childs')->first();

        if ($menu) {
            if (count($menu->childs)) {
                foreach ($menu->childs as $child) {
                    DB::table('sm_menus')->where('id', $child->id)->update(['menu_status' => 0]);
                }
            }
    ...
    public function sortSection(Request $request): void
    {
        ...
        $catch_for = 'sidebar_'.$role_name.'_school_'.auth()->user()->school_id;
        Cache::forget(sidebar_cache_key($catch_for));
        $this->reloadOctane();
    }
