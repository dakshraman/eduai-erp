...
@endsection
@section('mainContent')
    <link rel="stylesheet" href="{{ assetPath('modules/rolepermission/css/style.css') }}">
    <style type="text/css">
    ...
    <div class="erp_role_permission_area ">
        ...
        {{ html()->form('POST', route('menumanage.store.menu'))->attributes([
                                ...
                                <div class="permission_header d-flex align-items-center justify-content-between">
                                    <div>
                                        <input type="hidden" name="all_modules_id[]"
                                            value="{{ $module_info->infix_module_id }}">
                                        <input type="checkbox" name="module_id[]"
                                            value="{{ $module_info->infix_module_id }}" id="Main_Module_{{ $key }}"
                                            class="common-radio permission-checkAll main_module_id_{{ $module_info->infix_module_id }}"
                                            {{ $check_sidebar != '' ? (in_array($module_info->infix_module_id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                        <label
                                            for="Main_Module_{{ $key }}">{{ __('menumanage::menuManage.' . $module_info->name) }}</label>
                                    </div>
                                    <div class="arrow collapsed" data-toggle="collapse"
                                        data-target="#Role{{ $module_info->infix_module_id }}"></div>
                                </div>
                                            ...
                                            @foreach ($subModule as $row2)
                                                ...
                                                @else
                                                    <li>
                                                        ...
                                                        <div class="submodule">
                                                            <input type="hidden" name="all_modules_id[]"
                                                                value="{{ $row2->infix_module_id }}">
                                                            <input id="Sub_Module_{{ $row2->infix_module_id }}"
                                                                name="module_id[]" value="{{ $row2->infix_module_id }}"
                                                                class="infix_csk common-radio module_id_{{ $module_info->infix_module_id }} module_link"
                                                                type="checkbox"
                                                                {{ $check_sidebar != '' ? (in_array($row2->infix_module_id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                                            <label
                                                                for="Sub_Module_{{ $row2->infix_module_id }}">{{ __('menumanage::menuManage.' . $row2->name) }}</label>
                                                            <br>
                                                        </div>
                                                        ...
                                                        <ul class="option row3">
                                                            ...
                                                            $childModule = DB::table('sidebar_news')
                                                                ...
                                                                ->where('route', '!=', '')
                                                                ->where('parent_id', $row2->infix_module_id)
                                                                ->orderby('id', 'ASC')
                                                                ->get();
                                                            ?>
                                                            @foreach ($childModule as $row3)
                                                                <li>
                                                                    <div class="module_link_option_div"
                                                                        id="{{ $row2->infix_module_id }}">
                                                                        <input type="hidden" name="all_modules_id[]"
                                                                            value="{{ $row3->infix_module_id }}">
                                                                        <input id="Option_{{ $row3->id }}"
                                                                            name="module_id[]"
                                                                            value="{{ $row3->infix_module_id }}"
                                                                            class="infix_csk common-radio module_id_{{ $module_info->infix_module_id }} module_option_{{ $module_info->infix_module_id }}_{{ $row2->infix_module_id }} module_link_option"
                                                                            type="checkbox"
                                                                            {{ !empty($check_sidebar) ? (in_array($row3->infix_module_id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                                                        <label class="nowrap"
                                                ...
                                                <li>
                                                    ...
                                                    <div class="submodule">
                                                        ...
                                                        <input id="Sub_Module_{{ $row2->id }}" name="module_id[]"
                                                            value="{{ $row2->id }}"
                                                            class="infix_csk common-radio module_id_{{ $module_info->id }} module_link"
                                                            type="checkbox"
                                                            ...
                                                            <li>
                                                                <div class="module_link_option_div"
                                                                    ...
                                                                    <input id="Option_{{ $row3->id }}"
                                                                        ...
                                                                        class="infix_csk common-radio module_id_{{ $module_info->id }} module_option_{{ $module_info->id }}_{{ $row2->id }} module_link_option"
                                                                        type="checkbox"
