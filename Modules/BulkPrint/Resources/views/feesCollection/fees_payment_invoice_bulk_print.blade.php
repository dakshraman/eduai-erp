...
<head>
    ...
    <link rel="stylesheet" href="{{ assetPath('backEnd/css/report/bootstrap.min.css')}}">
                                    ...
                                    <li>
                                        <p>
                                            @if(moduleStatusCheck('University'))
                                                ...
                                                :  {{ @$student->unFaculty->name .'('. @$student->unDepartment->name .')' }}
                                            @else
                                                @lang('common.class'): {{@$student->class->class_name}}
                                            @endif
                    ...
                    @foreach($fees_assigneds as $fees_assigned)
                        @php
                            ...
                            $discount_amount = $fees_assigned->applied_discount;
                              ...
                              $paid = \App\Models\SmFeesAssign::feesPayment($fees_assigned->feesGroupMaster->feesTypes->id,$fees_assigned->student_id, $student->id)->sum('amount');
                              $total_grand_paid += $paid;
                              //Sum of total fine for single fees type
                            $fine = \App\Models\SmFeesAssign::feesPayment($fees_assigned->feesGroupMaster->feesTypes->id,$fees_assigned->student_id, $student->id)->sum('fine');
                            $total_fine += $fine;
