...
use App\Models\SmHrPayrollEarnDeduc;
...
class BulkPrintController extends Controller
{
    ...
    {
        try {
            $id_cards = branchWise(SmStudentIdCard::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get());
            ...
            return view('bulkprint::admin.generate_id_card', ['id_cards' => $id_cards, 'roles' => $roles]);
        }
    ...
    public function studentidBulkPrintSearch(Request $request)
    {
        ...
        try {
            if ($request->role == 2) {
                $s_students = branchWise(SmStudent::query()->with('parents', 'bloodGroup'));
                if (moduleStatusCheck('Branch')) {
                    $s_students->where('branch_id', $branch_id);
                }
                $s_students = $s_students->status()->get();
            }
            elseif ($request->role == 3) {
                $studentGuardian = branchWise(SmStudent::query()->where('school_id', Auth::user()->school_id));
                if (moduleStatusCheck('Branch')) {
                    $studentGuardian->where('branch_id', $branch_id);
                }
                $studentGuardian->get('parent_id');
                $s_students = branchWise(SmParent::query()->whereIn('id', $studentGuardian));
                if (moduleStatusCheck('Branch')) {
                    $s_students->where('branch_id', $branch_id);
                }
                ...
                $s_students = branchWise(SmStaff::query()->where('role_id', $request->role));
                if (moduleStatusCheck('Branch')) {
                    $s_students->where('branch_id', $branch_id);
                }
                $s_students = $s_students->status()->get();
            ...
            $gridGap = $request->grid_gap ?? 15;

            return view('bulkprint::admin.id_card_bulk_print', ['id_card' => $id_card, 's_students' => $s_students, 'role_id' => $role_id, 'gridGap' => $gridGap, 'branch_id' => $branch_id]);
        }
    ...
    public function ajaxIdCard(Request $request)
    {
        try {
            ...
            $id_cards = branchWise(SmStudentIdCard::where('active_status', 1)->get());
            $idCards = [];
    ...
    public function ajaxRoleIdCard(Request $request)
    {
        try {
            ...
            foreach ($role_ids as $role_id) {
                $d['id'] = Role::find($role_id)->id;
                $d['name'] = Role::find($role_id)->name;
    ...
    {
        try {
            $id_cards = branchWise(SmStudentIdCard::where('active_status', 1)->where('role_id', '!=', '["2"]')->where('school_id', Auth::user()->school_id));
            if (moduleStatusCheck('Branch')) {
                $id_cards = $id_cards->get(['id', 'title', 'branch_id']);
            }
            else {
                $id_cards = $id_cards->get(['id', 'title']);
            }

            $roles = Role::where('school_id', Auth::user()->school_id)->whereNotIn('id', [1, 2, 3])->get();

            return view('bulkprint::admin.staff_generate_id_card', ['id_cards' => $id_cards, 'roles' => $roles]);
        }
        catch (Exception $exception) {
    ...
    public function staffidBulkPrintSearch(Request $request)
    {
        try {
            ...
            if ($request->role == 2) {
                ...
                if ($request->section) {
                    $s_students->where('section_id', $request->section_id);
                }
    ...
    public function certificateBulkPrint()
    {
        try {
            ...
            $certificates = branchWise(SmStudentCertificate::get());

            return view('bulkprint::admin.generate_certificate_bulk', ['roles' => $roles, 'classes' => $classes, 'certificates' => $certificates]);
        }
        catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function certificateBulkPrintSearch(Request $request)
    {
        try {
            ...
            if (moduleStatusCheck('University')) {
                ...
                $student_ids = StudentRecord::query()
                    ->when(!empty($request->certificateBulkClass), function ($query) use ($request) {
            ...
            $data['students'] = SmStudent::whereIn('id', $student_ids)
                ->where('school_id', auth()->user()->school_id)
                ->when(moduleStatusCheck('Branch') && $request->filled('branch_id'), function ($query) use ($request): void {
                    $query->where(function ($branchQuery) use ($request): void {
            ...
            $data['certificates'] = branchWise(SmStudentCertificate::get());
            $data['type'] = 'school';
            $data['gridGap'] = $request->grid_gap;
            $data['branch_id'] = $request->branch_id;
            if (! $data['certificate']) {
                Toastr::error('Certificate Not Found', 'Failed');

                return redirect()->back()->withInput();
            }

            if ($data['users']->isNotEmpty()) {
                return view('bulkprint::admin.generate_certificate_bulk', $data);
            }
            else {
    ...
    public function bulkGenerateCertificatePrint(Request $request, $user_id, $certificate_id)
    {
        try {
            $userIds = collect(explode('-', (string) $user_id))
                ->filter()
                ->unique()
                ->values()
                ->all();

            $students = SmStudent::whereIn('user_id', $userIds)
                ->where('school_id', auth()->user()->school_id)
                ->get();
            $certificate = SmStudentCertificate::find($certificate_id);

            if (! $certificate || $students->isEmpty()) {
                Toastr::error('Certificate or student not found', 'Failed');

                return redirect()->back();
            }

            return view('bulkprint::admin.student_certificate_bulk_print', [
                'students' => $students,
    ...
    public function settings()
    {
        ...
        return view('bulkprint::feesCollection.invoice_settings', ['invoiceSettings' => $invoiceSettings]);
    }
    ...
    public function feeVoucherPrint()
    {
        try {
            ...
            return view('bulkprint::feesCollection.fees_bulk_print', ['classes' => $classes]);
        }
        catch (Exception $exception) {
    ...
    public function feeVoucherPrintSearch(Request $request)
    {
        try {
            ...
            $invoiceSettings = InvoiceSetting::where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->firstOrNew();
    ...
    public function payrollBulkPrint()
    {
        ...
        try {
            $roles = InfixRole::where('is_saas', 0)->where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 10)->where(function ($q): void {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->orderBy('name', 'asc')
                ->get();

            return view('bulkprint::humanResource.payroll.payroll_bulk_print', ['roles' => $roles]);
        }
        catch (Exception $exception) {
    ...
    public function payrollBulkPrintSearch(Request $request)
    {
        try {
            ...
            $staff_ids = branchWise(SmStaff::query());
            if ($request->role_id) {
            ...
            return view('bulkprint::humanResource.payroll.payroll_bulk_print_invoice', ['payrollDetails' => $payrollDetails, 'payrollEarnDetails' => $payrollEarnDetails, 'payrollDedcDetails' => $payrollDedcDetails, 'schoolDetails' => $schoolDetails]);
        }
        catch (Exception $exception) {
    ...
    {
        $courses = branchWise(Course::get());

        return view('bulkprint::admin.lmsCertificate', ['courses' => $courses]);
    }
    ...
    public function lmsCertificateBulkPrintSeacrh(Request $request)
    {
        try {
            ...
            $users = branchWise(SmStudent::whereIn('user_id', $studenId)->get());
            $certificate = SmStudentCertificate::find($courses->certificate_id);
            $gridGap = $request->grid_gap;
            $type = 'school';

            return view('backEnd.admin.certificate.certificate_print', ['users' => $users, 'certificate' => $certificate, 'gridGap' => $gridGap, 'type' => $type]);
        }
        catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    {
        try {
            $classes = branchWise(SmClass::where('school_id', auth()->user()->school_id)
                ->where('academic_id', getAcademicId())
                ->get());

            return view('bulkprint::feesInvoice.feesInvoiceBulk', ['classes' => $classes]);
        }
        catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    {
        try {
            $invoices = FmFeesInvoice::when($request->class, function ($query) use ($request): void {
                $query->where('class_id', $request->class);
            })
                ->when($request->shift, function ($query) use ($request): void {
                ...
                ->when($request->student, function ($query) use ($request): void {
                $query->whereHas('recordDetail', function ($q) use ($request) {
                ...
                ->when($request->un_session_id, function ($query) use ($request): void {
                $query->whereHas('recordDetail', function ($q) use ($request): void {
            ...
            if ($invoiceSettings->invoice_type === 'slip') {
                return view('bulkprint::feesInvoice.feesInvoiceBulkPrintSlip', ['invoices' => $invoices, 'invoiceSettings' => $invoiceSettings]);
            }

            return view('bulkprint::feesInvoice.feesInvoiceBulkPrint', ['banks' => $banks, 'invoices' => $invoices]);
    ...
    public function feesInvoiceBulkPrintSettings()
    {
        ...
        return view('bulkprint::feesInvoice.feesInvoiceSettings', ['feesInvoiceSettings' => $feesInvoiceSettings]);
    }
