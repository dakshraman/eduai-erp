...
@section('mainContent')
    @push('css')
        <link rel="stylesheet" href="{{ assetPath('modules/fees/feesStyle.css') }}" />
        <style>
            .ti-calendar:before {
    ...
    <section class="admin-visitor-area up_st_admin_visitor">
        <div class="container-fluid p-0">
            @if (isset($invoiceInfo))
                {{ html()->form('POST', route('fees.fees-invoice-update'))->attributes(['class' => 'form-horizontal'])->open() }}
                <input type="hidden" name="id" class="editValue" value="{{ $invoiceInfo->id }}">
            @else
                {{ html()->form('POST', route('fees.fees-invoice-store'))->attributes(['class' => 'form-horizontal'])->open() }}
            @endif
            <div class="row">
                <div class="col-lg-3">
                    <div class="row">
                            ...
                            <div class="white-box">
                                ...
                                <div class="add-visitor">
                                    ...
                                    @if (moduleStatusCheck('Branch'))
                                        <div class="row">
                                            @include('branch::components.branch_select', [
                                                ...
                                                'mb' => 'mb-15',
                                                'branch_id' => old('branch_id', isset($invoiceInfo) ? $invoiceInfo->branch_id : userBranch()),
                                            ])
                                        </div>
                                    @endif
                                    ...
                                    @else
                                        ...
                                        <div class="row">
                                            <div class="col-lg-12 mt-30-md" id="id-card-div">
                                                <label for="class"> {{ __('common.class') }} <span
                                                ...
                                                <select
                                                    ...
                                                    @foreach ($classes as $class)
                                                        <option value="{{ $class->id }}"
                                                            {{ old('class') == $class->id ? 'selected' : (isset($invoiceInfo) && $invoiceInfo->class_id == $class->id ? 'selected' : '') }}>
                                                            {{ @$class->class_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="row mt-15">
                                            <div class="col-lg-12" id="selectStudentDiv">
                                                ...
                                                @php
                                                    ...
                                                    if(old('class')){
                                                        $selectedBranchId = old('branch_id', moduleStatusCheck('Branch') ? userBranch() : null);
                                                        $studentQuery = App\Models\StudentRecord::where('class_id', old('class'))
                                                            ->where('is_promote', 0)
                                                            ->where('school_id', Auth::user()->school_id)
                                                            ->where('academic_id', getAcademicId());

                                                        if (moduleStatusCheck('Branch') && $selectedBranchId) {
                                                            $studentQuery = branchWiseApplyFilter($studentQuery, 'branch_id', $selectedBranchId);
                                                        }
                                                ...
                                                <select
                                                    ...
                                                    @if (isset($invoiceInfo) || $students)
                                                        @foreach ($students as $student)

                                                            <option value="{{ $student->id }}"
                                                                {{ old('student') == $student->id ? 'selected' : (isset($invoiceInfo) && $student->id == $invoiceInfo->record_id ? 'selected' : '') }}>
                                                                {{ $student->studentDetail->full_name }}
                                                                ({{ $student->section->section_name }} - {{ $student->roll_no }})
                                                            </option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                                <div class="pull-right loader" id="selectStudentLoader"
                                                    ...
                                                    <img src="{{ assetPath('modules/Fees/Resources/assets/img/pre-loader.gif') }}"
                                    ...
                                    <div class="row mt-15">
                                        ...
                                        <div class="col-lg-12">
                                            <div class="primary_input">
                                                <label class="primary_input_label" for="">@lang('fees.create_date') <span
                                                        class="text-danger"> *</span></label>
                                                        ...
                                                        <div class="col">
                                                            <div class="">
                                                                <input
                                                                    ...
                                                                    value="{{ isset($invoiceInfo) ? date('m/d/Y', strtotime($invoiceInfo->create_date)) : date('m/d/Y') }}">
                                                            </div>
                                                        </div>
                                    ...
                                    <div class="row mt-15">
                                        ...
                                        <div class="col-lg-12">
                                            <div class="primary_input">
                                                <label class="primary_input_label" for="">@lang('fees.due_date') <span
                                                        class="text-danger"> *</span></label>
                                                <div class="primary_datepicker_input">
                                                    <div class="no-gutters input-right-icon">
                                                        <div class="col">
                                        ...
                                        <div class="col-lg-12">
                                            <select
                                                ...
                                                @foreach ($feesGroups as $feesGroup)
                                                    <option value="grp{{ $feesGroup->id }}">{{ $feesGroup->name }}
                                                    </option>
                                                @endforeach
                                    ...
                                    <table class="table school-table-style fees_invoice_type_table" cellspacing="0"
                                            ...
                                            @if (isset($invoiceInfo))
                                                @foreach ($invoiceDetails as $key => $invoiceDetail)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <div class="primary_input">
                                                                <input
                                                                    ...
                                                                    value="{{ isset($invoiceDetail) ? decimal_only($invoiceDetail->amount) : old('amount') }}" step="{{ inputStep() }}">
                                                        ...
                                                        <td>
                                                            <div class="primary_input">
                                                                <input
                                                                    ...
                                                                    value="{{ isset($invoiceDetail) ? decimal_only($invoiceDetail->weaver) : old('weaver') }}" step="{{ inputStep() }}">
                                                        ...
                                                        <td class="subTotal">
                                                            {{ isset($invoiceDetail) ? ($invoiceDetail->amount > 0 ? decimal_only($invoiceDetail->sub_total) : decimal_only(0)) : '' }}
                                                        </td>
                                                        <input type="hidden" name="sub_total[]" class="inputSubTotal"
                                                            value="{{ isset($invoiceDetail) ? ($invoiceDetail->amount > 0 ? decimal_only($invoiceDetail->sub_total) : decimal_only(0)) : '' }}" step="{{ inputStep() }}">
                                                        @if (!isset($invoiceDetail))
                                                            <td>
                                                                <input
                                                                    ...
                                                                    value="{{ isset($invoiceDetail) ? decimal_only($invoiceDetail->paid_amount) : old('paid_amount') }}" step="{{ inputStep() }}">
                                                            </td>
                                                        @endif
                                                        <td>
                                                            <button class="primary-btn icon-only fix-gr-bg"
                                                                ...
                                                                type="button" title="@lang('common.edit_note')">
                                                                        ...
                                                                        <div class="modal-body">
                                                                            <div class="primary_input">
                                                                                <input
                                                                                    ...
                                                                                    type="text" name="note[]"
                                                                                    autocomplete="off"
                                                                                    value="{{ isset($invoiceDetail) ? $invoiceDetail->note : '' }}">
                                                                                <label class="primary_input_label"
                                                                                    for="">@lang('common.note')</label>
                                                                            ...
                                                                            <div
                                                                                ...
                                                                                <button type="button"
                                                                                    ...
                                                                                    data-dismiss="modal">@lang('common.update')</button>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            ...
                                                            <input type="hidden" class="fees"
                                                                value="grp{{ $invoiceDetail->feesType->fees_group_id }}">
                                                            <input type="hidden" class="fees"
                                                                value="typ{{ $invoiceDetail->fees_type }}">
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            @endif
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                @if (!isset($invoiceInfo))
                                                    <td class="showPaidAmount">{{currency_format(0)}}</td>
                                                @endif
                                                <td></td>
...
@push('script')
    <script type="text/javascript" src="{{ assetPath('modules/fees/js/app.js') }}?v=2"></script>
    <script>
    ...
    <script>
        $(document).on('input', '.amount', function() {
            ...
            if (isNaN(v) || v <= 0) {
                ...
                $(this).closest('tr').find('.inputSubTotal').val('0');
                $(this).closest('tr').find('.weaver').val('0');
            }
        });
    </script>
    <script>
        $(document).ready(function() {
            $('.amount').each(function() {
                ...
                if (isNaN(amount) || amount <= 0) {
                    ...
                    tr.find('.subTotal').text((0).toFixed(decimal_digits));
                    tr.find('.inputSubTotal').val((0).toFixed(decimal_digits));
