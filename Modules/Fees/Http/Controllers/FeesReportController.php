...
class FeesReportController extends Controller
{
    ...
    public function feesReportDatatable(Request $request)
    {
        try {
            ...
            if (! empty($branchId)) {
                $fees_dues->whereHas('recordDetail', function ($query) use ($branchId): void {
                    branchWiseApplyFilter($query, 'branch_id', $branchId);
                });
            }
            ...
            if ($dateFrom && $dateTo) {
                if ($request->type === 'payment') {
                    $fees_dues->whereIn('id', FmFeesTransaction::where('school_id', $schoolId)
                        ->where('academic_id', $academicId)
            ...
            return DataTables::of($filtered)
                ...
                ->addColumn('balance', function ($row) {
                    return currency_format($row->Tsubtotal - $row->Tpaidamount + $row->Tfine);
                })
    ...
    {
        $data['branch_id'] = moduleStatusCheck('Branch') ? userBranch() : null;

        $select =['id', 'class_name'];
        if (moduleStatusCheck('University')) {
            $select = ['id', 'class_name', 'branch_id'];
        }

        $data['classes'] = branchWise(SmClass::where('school_id', auth()->user()->school_id)
    ...
    {
        $data = [];
        $branchId = moduleStatusCheck('Branch') ? ($request->branch_id ?? userBranch()) : null;
        ...
        $classes = SmClass::where('school_id', auth()->user()->school_id)
            ...
            ->select($select)
            ->get();
        $data['classes'] = $branchId ? branchWiseApplyFilter($classes, 'branch_id', $branchId) : $classes;
        $date_from = null;
        $date_to = null;
