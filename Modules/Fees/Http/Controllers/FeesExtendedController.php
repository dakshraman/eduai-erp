...
class FeesExtendedController extends Controller
{
    public function invStore($request)
    {
        ...
        if (moduleStatusCheck('Branch')) {
            $fmFeesInvoice->branch_id = $request->branch_id ?? null;
        }
        ...
        foreach ($request->feesType as $key => $type) {
            ...
            if ($request->paid_amount[$key] ?? 0 > 0) { // Check this. When submit data without paid amount
