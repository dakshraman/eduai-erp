...
class StudentFeesController extends Controller
{
    ...
    public function studentFeesList()
    {
        ...
        return view('fees::student.feesInfo', ['student_id' => $student_id, 'records' => $records]);
    }
    ...
    public function studentAddFeesPayment($id)
    {
        try {
            ...
            return view('fees::student.studentAddPayment', ['classes' => $classes, 'feesGroups' => $feesGroups, 'feesTypes' => $feesTypes, 'paymentMethods' => $paymentMethods, 'bankAccounts' => $bankAccounts, 'invoiceInfo' => $invoiceInfo, 'invoiceDetails' => $invoiceDetails, 'stripe_info' => $stripe_info, 'razorpay_info' => $razorpay_info]);
        ...
        try {
            ...
            if ($request->payment_method === 'Wallet') {
                ...
                if ($user->wallet_balance === 0) {
                    Toastr::warning('Insufficient Balance', 'Warning');
                ...
                if ($data['payment_method'] === 'RazorPay') {
                    ...
                    if (moduleStatusCheck('SslCommerz')) {
                        $data['type'] = 'Fees';
                        $ssl_commerz = new SslCommerz();
                        $ssl_response = $ssl_commerz->handle($data);
                        if ($ssl_response['success'] === true) {
                            return redirect()->to($ssl_response['url']);
                        }
