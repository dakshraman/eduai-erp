...
{
    use NotificationSend, RestartOctan;

    public function feesGroup()
    {
        $feesGroups = branchWise(FmFeesGroup::where('school_id', Auth::user()->school_id)
    ...
    public function feesGroupStore(Request $request)
    {
        ...
        try {
            ...
            if (moduleStatusCheck('Branch')) {
                $fmFeesGroup->branch_id = $request->branch_id;
            }
            $fmFeesGroup->save();
    ...
    public function feesGroupUpdate(Request $request)
    {
        ...
        try {
            ...
            if (moduleStatusCheck('Branch')) {
                $feesGroup->branch_id = $request->branch_id;
            }
            $feesGroup->save();
    ...
    public function feesType()
    {
        ...
        $feesTypes = branchWise(FmFeesType::where('type', 'fees')
            ->where('school_id', Auth::user()->school_id)
            ->where('academic_id', getAcademicId())
            ->get());
    ...
    public function feesTypeStore(Request $request)
    {
        ...
        try {
            ...
            if (moduleStatusCheck('Branch')) {
                $fmFeesType->branch_id = $request->branch_id;
            }
            $fmFeesType->save();
    ...
    public function feesTypeUpdate(Request $request)
    {
        ...
        try {
            ...
            if (moduleStatusCheck('Branch')) {
                $feesType->branch_id = $request->branch_id;
            }
            $feesType->save();
    ...
    public function feesInvoiceStore(Request $request)
    {
        ...
        try {
            ...
            if ($request->student !== 'all_student') {
                ...
                if ($request->groups) {
                    ...
                    if (! $request->singleInvoice) {
                        $student = StudentRecord::where('id', $request->student)->first();
                        if ($student) {
                        ...
                        $feesCarry = feesCarryForward($student_id, $feesType, $amount, $sub_total);
                ...
                $allStudents = branchWise(StudentRecord::with(['studentDetail' => function ($q) {
                    return $q->where('active_status', 1);
                }, 'studentDetail.parents'])
                    ->whereHas('studentDetail', function ($q) {
                        return $q->where('active_status', 1);
                    })
                    ->when($request->shift && shiftEnable(), function ($query) use ($request) {
            ...
            Toastr::success('Fees Invoice Generated Successful', 'Success');
    ...
    public function feesInvoiceEdit($id)
    {
        try {
            ...
            $students = StudentRecord::where('id', $invoiceInfo->record_id)
                ...
                ->when($invoiceInfo->branch_id, function ($query) use ($invoiceInfo) {
                    $query->where('branch_id', $invoiceInfo->branch_id);
                })
    ...
    public function feesInvoiceUpdate(Request $request)
    {
        ...
        try {
            ...
            if (moduleStatusCheck('Branch')) {
                $storeFeesInvoice->branch_id = $request->branch_id ?? null;
            }
    ...
    public function feesPaymentStore(Request $request)
    {
        ...
        try {
            ...
            if ($request->add_wallet > 0) {
                ...
                $user->wallet_balance = $walletBalance + (float) round($request->add_wallet, getDecimalDigit());
                $user->update();
                $walletTransaction = new WalletTransaction();
                $walletTransaction->amount = (float) round($request->add_wallet, getDecimalDigit());
                ...
                $compact['previous_balance'] = $user->wallet_balance - round($request->add_wallet, getDecimalDigit());
            ...
            $fmFeesTransaction->invoice_number = ''; // Temp or let's use a generator if it has one? wait, I will just set it to 'TEMP'
            ...
            foreach ($request->fees_type as $key => $type) {
                ...
                $storeFeesInvoiceChield->weaver = (float) round($request->weaver[$key], getDecimalDigit());
                $storeFeesInvoiceChield->due_amount = (float) round($request->due[$key], getDecimalDigit());
                $storeFeesInvoiceChield->paid_amount += (float) round($request->paid_amount[$key], getDecimalDigit()) - (float) round($request->extraAmount[$key], getDecimalDigit());
                $storeFeesInvoiceChield->fine += (float) round($request->fine[$key], getDecimalDigit());
                $storeFeesInvoiceChield->update();
                $storeWeaver = new FmFeesWeaver();
                $storeWeaver->fees_invoice_id = $request->invoice_id;
                $storeWeaver->fees_type = $type;
                $storeWeaver->student_id = $student->id;
                $storeWeaver->weaver = (float) round($request->weaver[$key], getDecimalDigit());
                ...
                if ($request->paid_amount[$key] > 0) {
                    ...
                    $storeTransactionChield->weaver = (float) round($request->weaver[$key], getDecimalDigit());
                    $storeTransactionChield->fine = (float) round($request->fine[$key], getDecimalDigit());
                    $storeTransactionChield->paid_amount = (float) round($request->paid_amount[$key], getDecimalDigit());
                ...
                $add_income->amount = (float) round($request->paid_amount[$key], getDecimalDigit());
                ...
                if ($request->payment_method === 'Bank') {
                    ...
                    $bank = SmBankAccount::where('id', $request->bank)
                        ->where('school_id', Auth::user()->school_id)
                        ->first();
                    $after_balance = $bank->current_balance + (float) round($request->paid_amount[$key], getDecimalDigit());

                    $bank_statement = new SmBankStatement();
                    $bank_statement->amount = (float) round($request->paid_amount[$key], getDecimalDigit());
    ...
    public function bankPayment()
    {
        ...
        $feesPayments = branchWise(FmFeesTransaction::with('feeStudentInfo', 'transcationDetails', 'transcationDetails.transcationFeesType')
    ...
    public function searchBankPayment(BankFeesPayment $bankFeesPayment)
    {
        try {
            ...
            $branch_id = $bankFeesPayment->branch_id ?? null;
            $payment_date = $bankFeesPayment->payment_date;
            $approve_status = $bankFeesPayment->approve_status;
            ...
            $student_ids = StudentRecord::when($bankFeesPayment->class, function ($query) use ($bankFeesPayment): void {
                ...
                ->when($bankFeesPayment->branch_id, function ($query) use ($bankFeesPayment) {
                    $query->where('branch_id', $bankFeesPayment->branch_id);
                })
                ->where('school_id', auth()->user()->school_id)
                ->pluck('student_id')
                ->unique();

            $feesPayments = branchWise(FmFeesTransaction::when($bankFeesPayment->approve_status, function ($query) use ($bankFeesPayment): void {
                ...
                ->when($bankFeesPayment->branch_id, function ($query) use ($bankFeesPayment) {
                    $query->whereHas('recordDetail', function ($q) use ($bankFeesPayment) {
                        return $q->where('branch_id', $bankFeesPayment->branch_id);
                    });
                })
            ...
            return view('fees::bankPayment', ['classes' => $classes, 'shift_id' => $shift_id, 'feesPayments' => $feesPayments, 'class_id' => $class_id, 'section_id' => $section_id, 'class' => $class, 'branch_id' => $branch_id, 'payment_date' => $payment_date, 'approve_status' => $approve_status]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function feesInvoiceDatatable()
    {
        ...
        $studentInvoices = branchWise(FmFeesInvoice::where('type', $fees_type)
        ...
        if (isset($studentInvoices)) {
            ...
            return DataTables::of($studentInvoices)
                ...
                ->addColumn('amount', function ($row) {
                    return currency_format($row->Tamount);
                })
                ->addColumn('weaver', function ($row) {
                    return currency_format($row->Tweaver);
                })
                ->addColumn('fine', function ($row) {
                    return currency_format($row->Tfine);
                })
                ->addColumn('paid_amount', function ($row) {
                    return currency_format($row->Tpaidamount);
                })
                ->addColumn('balance', function ($row) {
                    ...
                    return currency_format($amount + $fine - ($paid_amount + $weaver));
                })
