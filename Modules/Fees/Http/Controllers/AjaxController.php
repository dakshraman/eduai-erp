...
class AjaxController extends Controller
{
    ...
    private function applySelectedBranch($query, $branchId)
    {
        if (moduleStatusCheck('Branch') && $branchId) {
    ...
    public function ajaxSelectStudent(Request $request)
    {
        try {
            ...
            $query = StudentRecord::with('studentDetail', 'section')
                ->where('class_id', $request->classId)
                ->where('is_promote', 0)
                ->where('school_id', auth()->user()->school_id)
                ->where('academic_id', getAcademicId());

            $allStudents = $this->applySelectedBranch($query, $branchId)->get();
    ...
    public function ajaxSelectFeesType(Request $request)
    {
        try {
            ...
            if ($type === 'grp') {
                ...
                $query = FmFeesType::where('fees_group_id', $groupId)
                    ->where('type', 'fees')
                    ->where('school_id', auth()->user()->school_id)
                    ->where('academic_id', getAcademicId());

                $feesGroups = $this->applySelectedBranch($query, $branchId)->get();

                return view('fees::_allFeesType', ['feesGroups' => $feesGroups, 'editData' => $editData]);
            }

            $typeId = mb_substr($request->type, 3);
            $query = FmFeesType::where('id', $typeId)
                ->where('type', 'fees')
                ->where('school_id', auth()->user()->school_id)
                ->where('academic_id', getAcademicId());

            $feesType = $this->applySelectedBranch($query, $branchId)->first();

            return view('fees::_allFeesType', ['feesType' => $feesType, 'editData' => $editData]);
