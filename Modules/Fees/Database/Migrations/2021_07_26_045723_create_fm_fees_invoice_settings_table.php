...
class CreateFmFeesInvoiceSettingsTable extends Migration
{
    ...
    {
        Schema::create('fm_fees_invoice_settings', function (Blueprint $blueprint): void {
            $blueprint->id();
            $blueprint->text('invoice_positions')->nullable();
            $blueprint->string('uniq_id_start')->nullable();
            $blueprint->string('prefix')->nullable();
            $blueprint->integer('class_limit')->nullable();
            $blueprint->integer('section_limit')->nullable();
            $blueprint->integer('admission_limit')->nullable();
            $blueprint->string('weaver')->nullable();
        ...
        try {
            foreach ($feesInfixModuleInfos as $feeInfixModuleInfo) {
                $checkExist = InfixModuleInfo::find($feeInfixModuleInfo[0]);
                if ($checkExist) {
                    continue;
                }

                $feesInvoiceData = new InfixModuleInfo;
                $feesInvoiceData->id = $feeInfixModuleInfo[0];
                $feesInvoiceData->module_id = $feeInfixModuleInfo[1];
                $feesInvoiceData->parent_id = $feeInfixModuleInfo[2];
                $feesInvoiceData->type = $feeInfixModuleInfo[3];
                $feesInvoiceData->is_saas = $feeInfixModuleInfo[4];
                $feesInvoiceData->name = $feeInfixModuleInfo[5];
                $feesInvoiceData->route = $feeInfixModuleInfo[6];
                $feesInvoiceData->lang_name = $feeInfixModuleInfo[7];
                $feesInvoiceData->icon_class = $feeInfixModuleInfo[8];
                $feesInvoiceData->active_status = $feeInfixModuleInfo[9];
                $feesInvoiceData->created_by = $feeInfixModuleInfo[10];
                $feesInvoiceData->updated_by = $feeInfixModuleInfo[11];
                $feesInvoiceData->school_id = $feeInfixModuleInfo[12];
                $feesInvoiceData->created_at = $feeInfixModuleInfo[13];
                $feesInvoiceData->updated_at = $feeInfixModuleInfo[14];
