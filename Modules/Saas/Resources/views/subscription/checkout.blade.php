        ...
        <div class="col-lg-8 mx-auto">
            <div class="card mt-4">
                <div class="card-header">
                    <h4 class="mb-0">Checkout - {{ $plan->name }} Plan</h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="card bg-light border-0 mb-4">
                                <div class="card-body">
                                    ...
                                    <div class="d-flex justify-content-between mb-2">
                                        <span>Price:</span>
                                        <strong>${{ number_format($plan->price, 2) }}/month</strong>
                                    </div>
                                    <div class="d-flex justify-content-between mb-2">
                                        <span>Teachers:</span>
                                        <strong>{{ $plan->staff_quantity ?? '-' }}</strong>
                                    </div>
                                    <div class="d-flex justify-content-between mb-2">
                                        <span>Students:</span>
                                        <strong>{{ $plan->student_quantity ?? '-' }}</strong>
                                    </div>
                                    @if($plan->trial_days > 0)
                                    <div class="mt-3 p-2 bg-success bg-opacity-10 rounded">
                                        <small class="text-success"><i class="fas fa-gift"></i> {{ $plan->trial_days }}-day free trial included</small>
                                    </div>
                                    @endif
                                </div>
                            </div>
                        ...
                        <div class="col-md-6">
                            <form action="{{ route('subscription.process') }}" method="POST" id="checkoutForm">
                                @csrf
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="payment_type" class="form-select" id="paymentType">
                                        ...
                                        @if($plan->trial_days > 0)
                                        <option value="trial">Start Free Trial ({{ $plan->trial_days }} days)</option>
                                        @endif
                                    </select>
                                </div>
                                ...
                                <div id="paymentMethodSection">
                                    ...
                                    <div id="stripeFields">
                                        ...
                                        <div class="mb-3">
                                            ...
                                            <input type="email" name="cardholder_email" class="form-control" value="{{ Auth::user()->email ?? '' }}" required>
                                        </div>
                                        <div class="mb-3">
                                ...
                                <button type="submit" class="btn btn-primary btn-lg w-100 mt-3" id="submitBtn">
                                    <span id="buttonText">Pay ${{ number_format($plan->price, 2) }}</span>
