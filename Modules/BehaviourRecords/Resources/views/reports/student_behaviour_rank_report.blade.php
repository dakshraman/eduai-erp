        ...
        <div class="container-fluid p-0">
            <div class="row mt-20">
                <div class="col-lg-12 student-details up_admin_visitor">
                    <div class="row">
                        <div class="col-lg-12">
                            {{ html()->form('GET', route('behaviour_records.student_behaviour_rank_report_search'))->attributes([
                                ...
                                <div class="row">
                                    @if (moduleStatusCheck('Branch'))
                                        @include('branch::components.branch_select', [
                                            'grid_class' => 'col-lg-3',
                                            'mb' => '',
                                            'branch_id' => @$branch_id ? @$branch_id : old('branch_id'),
                                        ])
                                    @endif
                                    ...
                                    <div class="col-lg-3 {{ moduleStatusCheck('University') ? 'mt-50-md':'mt-30-md' }} " id="select_type_div">
                                        <label class="primary_input_label" for="">@lang('behaviourRecords.type')
                                            <span class="text-danger"> </span></label>

                                        <select class="primary_select" id="select_type" name="type">
                                            <option data-display="@lang('behaviourRecords.select_type')" value="">
                                                @lang('behaviourRecords.select_type')
                                            </option>
                                            <option value="lesser_than_or_equal"
                                                {{ old('type', @$type ?? '') == 'lesser_than_or_equal' ? 'selected' : '' }}>
                                                @lang('behaviourRecords.less_than_or_equal')
                                            </option>
                                            <option value="greater_than_or_equal"
                                                {{ old('type', @$type ?? '') == 'greater_than_or_equal' ? 'selected' : '' }}>
                                                @lang('behaviourRecords.great_than_or_equal')
                                            </option>
                                        </select>
                                    </div>
                                    <div class="col-lg-3 {{ moduleStatusCheck('University') ? 'mt-50-md':'mt-30-md' }}" id="point_div">
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    @foreach ($data->studentRecords as $studentRecord)
                                                                        {{ $studentRecord->unFaculty->name }}({{ $studentRecord->unDepartment->name }})
                                                                    @endforeach
                                                                </td>
                                                                @else    
                                                                <td>
                                                                    @foreach ($data->studentRecords as $studentRecord)
                                                                        {{ $studentRecord->class->class_name }}({{ $studentRecord->section->section_name }}) @if(shiftEnable())[{{ @$studentRecord->shift->name }}]@endif
                                                                    @endforeach
                                                                </td>
                                                                @endif
                                                                <td>{{ $data->gender->base_setup_name }}</td>
                                                                <td>{{ $data->mobile }}</td>
                                                                <td>{{ $data->incidents->sum('point')}}</td>
                                                                <td>
                                                                    <x-drop-down>
