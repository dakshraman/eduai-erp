...
class WalletController extends Controller
{
    ...
    {
        $walletTotalAmounts = $this->getTotalWalletAmount('diposit', 'approve');
    ...
    {
        $walletTotalAmounts = $this->getTotalWalletAmount('diposit', 'reject');
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($walletRefunds)
                ...
                ->addColumn('file_view', function ($row) {
                    if ($row->file) {
                        return '<a class="dropdown-item file_'.$row->id.'" data-file="'.assetPath($row->file).'"  onclick="fileView('.$row->id.');" href="#">'.app('translator')->get('common.view').'</a>';
                    }
                })
    ...
    public function walletRefundRequestStore(Request $request)
    {
        ...
        try {
            ...
            $walletTransaction->amount = round($request->refund_amount, 5);
    ...
    public function walletRejectRefund(Request $request)
    {
        try {
            ...
            $compact['create_date'] = $status->created_by;
            $compact['current_balance'] = $user->wallet_balance;
            $compact['add_balance'] = round($request->amount, 5);
            $compact['reject_reason'] = $request->reject_note;
    ...
    public function walletReportSearch(Request $request)
    {
        ...
        try {
            ...
            return view('wallet::walletReport', ['walletReports' => $walletReports, 'date_from' => $date_from, 'date_to' => $date_to, 'type' => $type, 'status' => $status]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
                    ...
                    ->addColumn('showFile', function ($row): string {
                        ...
                        if ($row->file) {
                            ...
                            if ($pdf[1] === 'pdf') {
                                ...
                                $btn = '<a class="text-color showFile'.$row->id.' asset'.$row->id.'" onclick="showFile('.$row->id.');" data-asset="'.assetPath($row->file).'" data-file="'.url(@$row->file).'" data-toggle="modal" data-target="#showFile'.$row->id.'"  href="#">'.__('common.view').'</a>';
                            }
                        }
