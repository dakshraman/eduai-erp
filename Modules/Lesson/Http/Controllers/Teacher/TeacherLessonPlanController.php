...
class TeacherLessonPlanController extends Controller
{
    public function teacherLessonPlan(Request $request)
    {
        ...
        try {
            ...
            $teachers = SmStaff::where('active_status', 1)->where('user_id', $login_id)->where(function ($q): void {
            ...
            return view('lesson::teacher.teacherLessonPlan', ['dates' => $dates, 'this_week' => $this_week, 'class_times' => $class_times, 'teacher_id' => $teacher_id, 'sm_weekends' => $sm_weekends, 'teachers' => $teachers]);
    ...
    public function teacherLessonPlanOverview()
    {
        ...
        try {
            ...
            $login_id = Auth::user()->id;
            $teacher = SmStaff::where('active_status', 1)->where('user_id', $login_id)->where(function ($q): void {
                $q->where('role_id', 4)->orWhere('previous_role_id', 4);
            })->where('school_id', Auth::user()->school_id)->first();
            $teachers = $teacher->id;
            ...
            return view('lesson::teacher.teacher_lesson_plan_overview', ['lessonPlanDetail' => $lessonPlanDetail, 'lessons' => $lessons, 'topics' => $topics, 'classes' => $classes, 'teachers' => $teachers]);
        } catch (Exception$exception) {
    ...
    public function searchTeacherLessonPlanOverview(Request $request)
    {
        ...
        try {
            ...
            if (moduleStatusCheck('University')) {
                ...
                $total = LessonPlanner::lessonPlanner($request->teacher, $request->class, $request->section, $request->subject, $request->shift ?? null)->count();
                $completed_total = LessonPlanner::lessonPlanner($request->teacher, $request->class, $request->section, $request->subject, $request->shift ?? null)->where('completed_status', 'completed')->count();
                $percentage = $total > 0 ? $completed_total / $total * 100 : 0;

                if ($request->teacher  && $request->class  && $request->section  && $request->subject ) {
                    $lessonPlanner = LessonPlanner::lessonPlanner($request->teacher, $request->class, $request->section, $request->subject, $request->shift ?? null)->distinct('lesson_detail_id')->get();
                    $alllessonPlanner = LessonPlanner::lessonPlanner($request->teacher, $request->class, $request->section, $request->subject, $request->shift ?? null)->get();
            ...
            return view('lesson::teacher.teacher_lesson_plan_overview', ['total' => $total, 'completed_total' => $completed_total, 'alllessonPlanner' => $alllessonPlanner, 'lessonPlanner' => $lessonPlanner, 'classes' => $classes, 'teachers' => $teachers, 'percentage' => $percentage, 'class_id' => $class_id, 'section_id' => $section_id, 'shift_id' => $shift_id, 'subject_id' => $subject_id]);
