...
class LessonPlanController extends Controller
{
    ...
    public function lessonPlannerLesson($day, $teacher_id, $routine_id, $lesson_date)
    {
        ...
        try {
            $routine = SmClassRoutineUpdate::where('id', $routine_id)
                ...
                ->where('school_id', Auth::user()->school_id)
                ->first();
            $branch_id = $routine->branch_id ?? null;
            $shift_id = $routine->shift_id ?? null;
            ...
            return view('lesson::lessonPlan.add_new_lesson_planner_form', ['teacher_id' => $teacher_id, 'lesson_date' => $lesson_date, 'day' => $day, 'class_id' => $class_id, 'section_id' => $section_id, 'subject_id' => $subject_id, 'teacher_detail' => $teacher_detail, 'lessons' => $lessons, 'routine_id' => $routine_id, 'routine' => $routine, 'branch_id' => $branch_id, 'shift_id' => $shift_id]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function addNewLessonPlan(Request $request)
    {
        ...
        try {
            ...
            if (shiftEnable()) {
                $lessonPlanner->shift_id = $request->shift_id;
            }
            if (moduleStatusCheck('Branch')) {
                $lessonPlanner->branch_id = $request->branch_id;
            }
        ...
        try {
            if (moduleStatusCheck('University')) {
                ...
                $lesson_ids = SmLessonTopic::when($un_session_id, function ($q) use ($un_session_id) {
                    ...
                    ->when(moduleStatusCheck('Branch') && $request->filled('branch_id'), function ($q) use ($request): void {
                        $q->where(function ($branchQuery) use ($request): void {
            ...
            $query = SmLessonTopic::when($class_id, function ($q) use ($class_id): void {
            ...
            if (moduleStatusCheck('Branch')) {
                $query->when($branch_id, function ($q) use ($branch_id): void {
                    $q->where(function ($branchQuery) use ($branch_id): void {
                        $branchQuery->where('branch_id', $branch_id)
                            ->orWhereNull('branch_id')
                            ->orWhere('branch_id', 0);
                    });
                });
            }
            $lesson_ids = $query->get()->pluck('lesson_id')->toArray();
            ...
            return view('lesson::lessonPlan.manage_lesson', ['lesson_id' => $lesson_id, 'classes' => $classes, 'lessons' => $lessons, 'topics_detail' => $topics_detail, 'class_id' => $class_id, 'shift_id' => $shift_id, 'section_id' => $section_id, 'subject_id' => $subject_id, 'selected' => $selected, 'branch_id' => $branch_id]);
    ...
    public function manageLessonPlanner()
    {
        ...
        try {
            ...
            $lessons = branchWise(SmLesson::get());
            $topics = branchWise(SmLessonTopic::get());
    ...
    public function searchLessonPlan(Request $request)
    {
        ...
        try {
            ...
            $sections = [];
            if ($class_id) {
                $sectionIds = branchWise(SmClassSection::where('class_id', '=', $class_id)->get());
            ...
            return view('lesson::lessonPlan.manage_lesson_planner', ['total' => $total, 'completed_total' => $completed_total, 'alllessonPlanner' => $alllessonPlanner, 'lessonPlanner' => $lessonPlanner, 'classes' => $classes, 'teachers' => $teachers, 'percentage' => $percentage, 'subjects' => $subjects, 'sections' => $sections, 'class_id' => $class_id, 'section_id' => $section_id, 'subject_id' => $subject_id, 'shift_id' => $shift_id, 'teacher_id' => $teacher_id, 'selected' => $selected,
                'un_session_id' => $un_session_id, 'un_faculty_id' => $un_faculty_id, 'un_department_id' => $un_department_id, 'un_academic_id' => $un_academic_id, 'un_semester_id' => $un_semester_id, 'un_semester_label_id' => $un_semester_label_id, 'un_section_id' => $un_section_id, 'un_subject_id' => $un_subject_id, 'branch_id' => $branch_id]);
        } catch (Exception $exception) {
            Toastr::error('Operation Failed', 'Failed');
    ...
    public function ajaxSelectSubject(Request $request)
    {
        ...
        try {
            ...
            $subject_all = SmAssignSubject::with('subject')
                ->when(moduleStatusCheck('Branch'), fn ($query): object => branchWise($query))
                ...
                ->when($request->teacher, function ($query) use ($request): void {
                    $query->where('teacher_id', $request->teacher);
                })
                ->when(! $request->teacher && ! $is_admin && ! teacherAccess(), function ($query) use ($staff_info): void {
                    if ($staff_info) {
                        $query->where('teacher_id', $staff_info->id);
                    }
                })
                ->when($request->academic_id, function ($query) use ($request): void {
    ...
    public function loadDefault()
    {
        ...
        $data['teachers'] = branchWise(SmStaff::where('active_status', 1)
