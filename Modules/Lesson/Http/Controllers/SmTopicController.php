...
class SmTopicController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $smLessonTopic = new SmLessonTopic;
            $smLessonTopic->class_id = $request->class;
            $smLessonTopic->section_id = $request->section;
            $smLessonTopic->shift_id = shiftEnable() ? $request->shift : null;
            $smLessonTopic->subject_id = $request->subject;
            $smLessonTopic->lesson_id = $request->lesson;
            $smLessonTopic->created_at = YearCheck::getYear().'-'.date('m-d h:i:s');
            $smLessonTopic->school_id = Auth::user()->school_id;
            if (moduleStatusCheck('University')) {
                $common = App::make(UnCommonRepositoryInterface::class);
                $common->storeUniversityData($smLessonTopic, $request);
            } else {
                $smLessonTopic->academic_id = getAcademicId();
            }
            if (moduleStatusCheck('Branch')) {
                $smLessonTopic->branch_id = $request->branch_id;
                $smLessonTopic->lesson_id = $request->lesson;
            }

            $smLessonTopic->save();
            $smTopic_id = $smLessonTopic->id;
            $topics = $request->topic ?? [];
            $length = count($topics);
            for ($i = 0; $i < $length; $i++) {
                $topic_title = $topics[$i];
    ...
    public function edit($id)
    {
        try {
            ...
            $data['lessons'] = branchWise(SmLesson::where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get());
            $data['topicDetails'] = branchWise(SmLessonTopicDetail::where('topic_id', $data['topic']->id)->where('academic_id', getAcademicId())
                ->where('school_id', Auth::user()->school_id)->get());
            if (shiftEnable()) {
                $data['shift_id'] = $data['topic']->shift_id;
            }
            $data['class_id'] = $data['topic']->class_id;
            $data['section_id'] = $data['topic']->section_id;
            if (moduleStatusCheck('University')) {
    ...
    {
        $relations =['lesson', 'class', 'section', 'subject', 'shift'];
        if (moduleStatusCheck('Branch')) {
            $relations = array_merge($relations, ['branch']);
        }
        // if ($request->ajax()) {
        if (Auth::user()->role_id === 4) {
            ...
            if ($request->filled('branch_id')) {
                $subjects = $subjects->where(function ($query) use ($request): void {
                    $query->where('branch_id', $request->branch_id)
                        ->orWhereNull('branch_id')
                        ->orWhere('branch_id', 0);
                });
            } else {
                $subjects = branchWise($subjects);
            }
            $subjects = $subjects->pluck('subject_id');

            $topics = branchWise(SmLessonTopic::with($relations)
                ->whereIn('subject_id', $subjects)
                ->where('sm_lesson_topics.academic_id', getAcademicId())
                ->where('sm_lesson_topics.school_id', Auth::user()->school_id)
                ->when($request->filled('branch_id'), function ($query) use ($request): void {
                    $query->where(function ($q) use ($request): void {
                        $q->where('sm_lesson_topics.branch_id', $request->branch_id)
                            ->orWhereNull('sm_lesson_topics.branch_id')
                            ->orWhere('sm_lesson_topics.branch_id', 0);
                    });
                }));
        } else {
            $topics = branchWise(SmLessonTopic::with($relations)
        ...
        if ($request->has('order')) {
            ...
            switch ($orderColumn) {
                case 'branch.branch_name':
                    $topics = $topics
                        ->leftJoin('branches', 'sm_lesson_topics.branch_id', '=', 'branches.id')
                        ->orderBy('branches.branch_name', $orderDirection)
                        ->select('sm_lesson_topics.*');
                    break;
    ...
    public function loadTopic()
    {
        ...
        if (Auth::user()->role_id === 4) {
            $subjects = branchWise(SmAssignSubject::select('subject_id')->where('teacher_id', $teacher_info->id)->get());
            $data['topics'] = branchWise(SmLessonTopic::with(['lesson', 'class', 'section', 'subject'])
                ->whereIn('subject_id', $subjects)
                ->where('academic_id', getAcademicId())
            ...
            $data['topics'] = branchWise(SmLessonTopic::with(['lesson', 'class', 'section', 'subject'])->where('academic_id', getAcademicId())->where('school_id', $auth->school_id)->get());
        }

        if (! teacherAccess()) {
            $data['classes'] = branchWise(SmClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', $auth->school_id)->get());
        } else {
            $select = moduleStatusCheck('Branch')?['sm_classes.id', 'class_name', 'branch_id'] : [ 'sm_classes.id', 'class_name'];

            $data['classes'] = branchWise(SmAssignSubject::where('teacher_id', $teacher_info->id)
                ->join('sm_classes', 'sm_classes.id', 'sm_assign_subjects.class_id')
                ->where('sm_assign_subjects.active_status', 1)
                ->where('sm_assign_subjects.school_id', $auth->school_id)
                ->where('sm_assign_subjects.academic_id', getAcademicId())
                ->select($select)
                ->groupBy('sm_classes.id')
                ->get());
        }
        $selectSubject =moduleStatusCheck('Branch')?['id', 'subject_name', 'branch_id'] : ['id', 'subject_name'];
        $selectSection =moduleStatusCheck('Branch')?['id', 'section_name', 'branch_id'] : ['id', 'section_name'];
        $data['subjects'] = branchWise(SmSubject::select($selectSubject)->get());
        $data['sections'] = branchWise(SmSection::select($selectSection)->get());
