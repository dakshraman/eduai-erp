...
class SmLessonController extends Controller
{
    ...
    public function storeLesson(Request $request)
    {
        ...
        try {
            $sections = SmAssignSubject::where('class_id', $request->class)
                ...
                ->when(moduleStatusCheck('Branch'), function ($q) use ($request): void {
            ...
            if (moduleStatusCheck('University')) {
                if ($request->un_section_id) {
                    $sections = branchWise(UnSubject::where('un_department_id', $request->un_department_id)
            ...
            if (! moduleStatusCheck('University') && $sections->isEmpty()) {
                DB::rollBack();
                Toastr::error('Assign subject not found for selected class, section and branch', 'Failed');
            ...
            foreach ($sections as $section) {
                foreach ($request->lesson as $lesson) {
                    ...
                    if (moduleStatusCheck('Branch')) {
                        $smLesson->branch_id = $request->branch_id;
                    }
                    if (moduleStatusCheck('University')) {
        ...
        try {
            ...
            if ($newLessonCount > 0) {
                ...
                for ($i = count($existingLessons); $i < $counter; $i++) {
                    ...
                    if (moduleStatusCheck('Branch')) {
                        $newLesson->branch_id = $request->branch_id;
                    }
    ...
    public function deleteLesson($id)
    {
        ...
        $lesson_detail = branchWise(SmLesson::where([
            ...
            ->when(moduleStatusCheck('Branch'), function ($q) use ($lesson) {
                return $q->where('branch_id', $lesson->branch_id);
            })
            ->get());
    ...
    public function loadLesson()
    {
        ...
        if ($user->role_id === 4 && $teacher_info) {
            $subjects = branchWise(SmAssignSubject::select('subject_id')
                ->where('teacher_id', $teacher_info->id)->get());
        }

        $data['subjects'] = branchWise(SmSubject::where('active_status', 1)
            ->where('academic_id', getAcademicId())
            ->where('school_id', $user->school_id)->get());
        $data['sections'] = branchWise(SmSection::where('active_status', 1)
            ->where('academic_id', getAcademicId())
            ->where('school_id', $user->school_id)->get());

        if ($user->role_id === 4) {
            if (moduleStatusCheck('University')) {
                $data['lessons'] = branchWise(SmLesson::query()->whereIn('un_subject_id', $subjects)->statusCheck()->groupBy(['class_id', 'section_id', 'un_subject_id']));
            } else {
                $data['lessons'] = branchWise(SmLesson::query()->whereIn('subject_id', $subjects)->statusCheck()->groupBy(['class_id', 'section_id', 'subject_id']));
            }
        } else {
            $data['lessons'] = branchWise(SmLesson::query()
                ->groupBy(['class_id', 'section_id', 'subject_id'])
                ->statusCheck());
        }

        if (moduleStatusCheck('University')) {
            $data['lessons'] = $data['lessons']->groupBy(['un_department_id', 'un_faculty_id', 'un_section_id', 'un_subject_id'])->get();
        } else {
            $query = $data['lessons'];
            ...
            if (moduleStatusCheck('Branch')) {
                $query->with('branch');
                $data['lessons'] = $query
                    ->select('class_id', 'section_id', 'subject_id', 'lesson_title', 'active_status', 'id', 'shift_id', 'branch_id')
//                    ->groupBy('class_id', 'section_id', 'subject_id', 'lesson_title', 'active_status', 'id', 'shift_id', 'branch_id')
                    ->get();
            } else {
                $data['lessons'] = $query
...
//                    ->groupBy('class_id', 'section_id', 'subject_id', 'lesson_title', 'active_status', 'id', 'shift_id')
        ...
        if (! teacherAccess()) {
            ...
            $select =moduleStatusCheck('Branch')?['id', 'class_name', 'branch_id'] : ['id', 'class_name'];

            $data['classes'] = branchWise(SmClass::where('active_status', 1)
                ->where('academic_id', getAcademicId())
                ->where('school_id', $user->school_id)->select($select)->get());
        } else {
            $select = moduleStatusCheck('Branch')?['sm_classes.id', 'class_name', 'branch_id'] : [ 'sm_classes.id', 'class_name'];
            $data['classes'] = $teacher_info ? branchWise(SmAssignSubject::where('teacher_id', $teacher_info->id)
    ...
    public function lessonListAjax(Request $request)
    {
        if (! $request->ajax()) {
            ...
            if (Auth::user()->role_id === 4) {
                $lessons = branchWise(SmLesson::with('lessons', 'class', 'section', 'subject')
