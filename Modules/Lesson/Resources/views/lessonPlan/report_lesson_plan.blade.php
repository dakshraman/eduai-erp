...
@section('mainContent')
    <link rel="stylesheet" href="{{ assetPath('modules/lesson/jquery-ui.css') }}">
    <link rel="stylesheet" href="{{ assetPath('modules/lesson/lesson_plan.css') }}">
    ...
    <section class="admin-visitor-area">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="white-box">
                    {{ html()->form('POST', route('serach-report-lesson-plan'))->attributes([
                            'class' => 'form-horizontal',
                            'files' => true,
                            'enctype' => 'multipart/form-data',
                            'id' => 'search_lesson_Plan',
                        ])->open() }}
                    <div class="row">
                        <input type="hidden" name="url" id="url" value="{{ URL::to('/') }}">
                        <div class="col-lg-3 mt-30-md">
                            <select
                                class="primary_select form-control{{ $errors->has('teahcer') ? ' is-invalid' : '' }} select_teacher"
                        ...
                        <div class="col-lg-3 mt-30-md" id="select_subject_div">
                                ...
                                @isset($subjects)
                                    @foreach ($subjects as $subject)
                                        <option value="{{ $subject->id }}"
                                            {{ isset($subject_id) ? ($subject_id == $subject->id ? 'selected' : '') : '' }}>
                            ...
                            <div class="pull-right loader" id="select_subject_loader"
                                ...
                                <img src="{{ assetPath('public/backEnd/img/demo_wait.gif') }}" alt=""
                                    ...
                                    @foreach ($lessonPlanner as $data)
                                        <tr>
                                                ...
                                                @php
                                                    $alllessonPlannerTopic = DB::table('lesson_planners')
                                                        ->join(
                                                            ...
                                                            'sm_lessons.id',
                                                            '=',
                                                            'lesson_planners.lesson_detail_id',
                                                        )
                                                        ->join(
                                                            'sm_lesson_topic_details',
                                                            'sm_lesson_topic_details.id',
                                                            '=',
                                                            'lesson_planners.topic_detail_id',
                                                        )
                                                        ->where('lesson_detail_id', $data->lesson_detail_id)
                                                        ->where('lesson_planners.active_status', 1)
                                                        ->select(
                                                            'lesson_planners.*',
                                                            'sm_lessons.lesson_title',
                                                            'sm_lesson_topic_details.topic_title',
                                                        )
                                                        ->get();
                                                @endphp
                                            ...
                                            <td>
                                                @foreach ($alllessonPlannerTopic as $allData)
                                                    @php
                                                        $topicdate = DB::table('lesson_planners')
                                                            ->where('id', $allData->id)
                                                            ->first();
                                                    @endphp
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {{ html()->form('POST', route('lessonPlan-complete-status'))->attributes([
                            'class' => 'form-horizontal',
                            'files' => true,
                            'enctype' => 'multipart/form-data',
                            'name' => 'myForm',
                    ...
                    <div class="form-group">
                        ...
                        <input
                            class="primary_input_field  primary_input_field date form-control form-control{{ $errors->has('complete_date') ? ' is-invalid' : '' }}"
                            id="complete_date" type="text" name="complete_date" value="{{ date('m/d/Y') }}">
                    </div>
