...
@push('css')
    <style>
        ...
        .primary-btn.tr-bg.small.icon_bordered_btn {
            background: transparent;
            border: 1px solid var(--gradient_3, #7c32ff) !important;
            font-weight: 600;
            font-size: 12px;
        }

        .primary-btn.tr-bg.small.icon_bordered_btn:hover {
            box-shadow: 0px 0px 10px 0px rgba(124, 50, 255, 0.3) !important;
        }
    </style>
@endpush
@section('mainContent')
                ...
                <div class="col-lg-12">
                    ...
                    @if (userPermission('add-new-lesson-plan'))
                        <div class="white-box">
                            ...
                            {{ html()->form('POST', route('lesson-planner'))->class('form-horizontal')->attribute('enctype', 'multipart/form-data')->attributes(['files' => true])->id('search_student')->open() }}
                            <div class="row">
                                ...
                                <div class="col-lg-12">
                                    ...
                                    <select
                                        ...
                                        @foreach ($teachers as $teacher)
                                            ...
                                            <option value="{{ @$teacher->id }}"
                                                    {{ old('teacher') == $teacher->id ? 'selected' : (@$teacher_id == $teacher->id ? 'selected' : '') }}>
                                                {{ @$teacher->full_name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="col-lg-12 col-md-12">
                            <div class="main-title">
                                ...
                                <h3 class="text-center "><a
                                            href="{{ url('/lesson/dicrease-week/' . $teacher_id . '/' . $dates[0]) }}">
                                        < Week {{ $week_number }} | <span class="yearColor">
                                                {{ date('Y', strtotime($dates[0])) }}
                                            </span><a
                                                href="{{ url('/lesson/change-week/' . $teacher_id . '/' . $dates[6]) }}">
                                            >
                                        </a></h3>
                                ...
                                <table class="table school-table-style" cellspacing="0" width="100%">
                                    ...
                                    <tr>
                                        ...
                                        @foreach ($sm_weekends as $key => $sm_weekend)
                                            @php
                                                $teacherClassRoutineById = App\Models\SmWeekend::teacherClassRoutineById(
                                                    $sm_weekend->id,
                                                    $teacher_id,
                                                );
                                            @endphp
                                            ...
                                            @php
                                                if (!in_array($routine->id, $used)) {
                                                    if (moduleStatusCheck('University')) {
                                                        $tr[$i][$sm_weekend->name][$loop->index][
                                                        ...
                                                        $tr[$i][$sm_weekend->name][$loop->index][
                                                            'subject_id'
                                                        ] = $routine->unSubject ? $routine->unSubject->id : null;
                                                    } else {
                                                        $tr[$i][$sm_weekend->name][$loop->index][
                                                            'subject'
                                                        ] = $routine->subject
                                                        ...
                                                        $tr[$i][$sm_weekend->name][$loop->index][
                                                            'subject_id'
                                                        ] = $routine->subject ? $routine->subject->id : null;
                                                    }

                                                    $tr[$i][$sm_weekend->name][$loop->index][
                                                        'class_name'
                                                    ] = $routine->class ? $routine->class->class_name : '';
                                                    $tr[$i][$sm_weekend->name][$loop->index][
                                                        'section_name'
                                                    ] = $routine->section ? $routine->section->section_name : '';
                                                    $tr[$i][$sm_weekend->name][$loop->index][
                                                        'class_room'
                                                    ] = $routine->classRoom ? $routine->classRoom->room_no : '';
                                                    $tr[$i][$sm_weekend->name][$loop->index][
                                                        'teacher'
                                                    ] = $routine->teacherDetail
                                                        ? $routine->teacherDetail->full_name
                                                        : '';
                                                    $tr[$i][$sm_weekend->name][$loop->index]['start_time'] =
                                                        $routine->start_time;
                                                    $tr[$i][$sm_weekend->name][$loop->index]['end_time'] =
                                                        $routine->end_time;

                                                    $tr[$i][$sm_weekend->name][$loop->index]['is_break'] =
                                                        $routine->is_break;
                                                    $used[] = $routine->id;

                                                    $tr[$i][$sm_weekend->name][$loop->index][
                                                        'class_id'
                                                    ] = $routine->class ? $routine->class->id : null;
                                                    $tr[$i][$sm_weekend->name][$loop->index][
                                                        'section_id'
                                                    ] = $routine->section ? $routine->section->id : null;

                                                    $tr[$i][$sm_weekend->name][$loop->index][
                                                        'class_room_id'
                                                    ] = $routine->classRoom ? $routine->classRoom->id : null;
                                                    $tr[$i][$sm_weekend->name][$loop->index][
                                                        'teacher_id'
                                                    ] = $routine->teacherDetail
                                                        ? $routine->teacherDetail->id
                                                        : null;
                                                    $tr[$i][$sm_weekend->name][$loop->index]['routine_id'] =
                                                        $routine->id;
                                                }
                                                        ...
                                                        @if ($classes && gv($classes, $i))
                                                            ...
                                                            @else
                                                                <span
                                                                        ...
                                                                        <br> </span>
                                                                <span class=""> <strong>
                                                                            {{ $classes[$i]['subject'] }} </strong>
                                                                        ({{ $classes[$i]['subject_code'] }})
                                                                        <br> </span>
                                                                @if ($classes[$i]['class_room'])
                                                                    <span class=""> <strong>@lang('common.room')
                                                                                :</strong> {{ $classes[$i]['class_room'] }}
                                                                            <br> </span>
                                                                @endif
                                                                @if ($classes[$i]['class_name'])
                                                                    <span class="">
                                                                            {{ $classes[$i]['class_name'] }} @if ($classes[$i]['section_name'])
                                                                            (
                                                                ...
                                                                @php
                                                                    ...
                                                                    if (moduleStatusCheck('University')) {
                                                                        $un_semester_label_id =
                                                                            $classes[$i]['un_semester_label_id'];
                                                                        ...
                                                                        @if (userPermission('view-lesson-planner-lesson'))
                                                                            <div class="col-2 text-left">
                                                                                <a href="{{ route('view-lesson-planner-lesson', [$lessonPlan->id]) }}"
                                                                                   class="primary-btn small tr-bg icon-only modalLink  icon_bordered_btn"
                                                                        ...
                                                                        @if (userPermission('edit-lesson-planner-lesson'))
                                                                            <div class="col-2 text-left">
                                                                                <a href="{{ route('edit-lesson-planner-lesson', [$lessonPlan->id]) }}"
                                                                                   class="primary-btn small tr-bg icon-only mr-10 modalLink icon_bordered_btn"
                                                                                   data-modal-size="modal-lg"
                                                                ...
                                                                @else
                                                                    ...
                                                                    <div class="col-lg-6 text-left px-0">
