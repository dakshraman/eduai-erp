...
@endsection
@section('mainContent')
    <link rel="stylesheet" href="{{ assetPath('modules/lesson/css/jquery-ui.css') }}">
@section('script')
    <script type="text/javascript" src="{{ assetPath('modules/lesson/js/app.js') }}"></script>
    <script type="text/javascript" href="{{ assetPath('modules/lesson/js/developer.js') }}"></script>
@stop
...
<section class="admin-visitor-area up_st_admin_visitor">
    <div class="container-fluid p-0">
        ...
        @endif
        @if (isset($lesson))
            {{ html()->form('PUT', route('exam-update', $lesson->id))->attribute('class', 'form-horizontal')->attribute('enctype', 'multipart/form-data')->attribute('files', true)->open() }}
        @else
            @if (userPermission('lesson.create-store'))
                {{ html()->form('POST', route('lesson.storelesson'))->attribute('class', 'form-horizontal')->open() }}
            @endif
        @endif
        <div class="row">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($lessons as $lesson)
                                            <tr>
                                                ...
                                                @if (moduleStatusCheck('Branch'))
                                                    <td>{{ $lesson->branch?->branch_name }}</td>
                                                @endif
                                                @if (moduleStatusCheck('University'))
                                                ...
                                                @else
                                                    ...
                                                    @if(shiftEnable())
                                                    <td>{{ $lesson->shift != '' ? $lesson->shift->name : '' }}
                                                    </td>
                                                    @endif
                                                @endif
                                                ...
                                                <td>
                                                    ...
                                                    @endphp
                                                    {{ $lesson_title->pluck('lesson_title')->implode(', ') }}
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            <div class="mt-40 d-flex justify-content-between">
                                                                ...
                                                                {{ html()->form('DELETE', route('lesson-delete', $lesson->id))->attribute('enctype', 'multipart/form-data')->open() }}
